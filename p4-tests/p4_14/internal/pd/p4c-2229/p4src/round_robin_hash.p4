 ...
 * a simple round-robin hash calculation via an incrementing register.
 *
 * Since the maximum register width that we can use is 32 bits, we will limit
 * ourselves to the basic fair hashing. In the real life it might make sense 
 * to mix the register output with the hash, calculated over the IPv4/IPv6 
 * fields.
 *
...
#if HASH_WIDTH > 32
#error "round_robin.p4 module only supports hash widths up to 32 bits"
#endif
...
blackbox stateful_alu packet_num_inc {
    reg: packet_num;
    ...
    output_dst                : hash_meta.packet_num;
}

action packet_num_inc() {
    packet_num_inc.execute_stateful_alu(0);
}

table packet_num_inc {
    actions          { packet_num_inc; }
    default_action : packet_num_inc();
    size           : 1;
}
