...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".qos_hit_e2e_mirror") action qos_hit_e2e_mirror(bit<8> qid, bit<32> mirror_id) {
        clone(CloneType.E2E, (bit<32>)mirror_id);
    }
    @name(".egress_qos") table egress_qos {
        ...
        key = {
            hdr.vlan_tag_.isValid(): exact;
            hdr.vlan_tag_.vid      : exact;
        }
        size = 8192;
    }
    apply {
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".do_deflect_on_drop") action do_deflect_on_drop() {
        hdr.ig_intr_md_for_tm.deflect_on_drop = 1w1;
    }
    @name(".dmac_miss") action dmac_miss(bit<16> flood_mc_index) {
        hdr.ig_intr_md_for_tm.mcast_grp_a = flood_mc_index;
    }
    @name(".dmac_unicast_hit") action dmac_unicast_hit(bit<9> egress_port) {
        hdr.ig_intr_md_for_tm.ucast_egress_port = egress_port;
    }
    @name(".dmac_multicast_hit") action dmac_multicast_hit(bit<16> mc_index) {
        hdr.ig_intr_md_for_tm.mcast_grp_a = mc_index;
        hdr.ig_intr_md_for_tm.enable_mcast_cutthru = 1w1;
    }
    @name(".dmac_uc_mc_hit") action dmac_uc_mc_hit(bit<9> egress_port, bit<16> mc_index) {
        hdr.ig_intr_md_for_tm.ucast_egress_port = egress_port;
        hdr.ig_intr_md_for_tm.mcast_grp_a = mc_index;
        hdr.ig_intr_md_for_tm.enable_mcast_cutthru = 1w1;
    }
    @name(".qos_miss") action qos_miss() {
        hdr.ig_intr_md_for_tm.ingress_cos = 3w0;
        hdr.ig_intr_md_for_tm.qid = 5w0;
    }
    @name(".qos_hit_eg_bypass_i2e_mirror") action qos_hit_eg_bypass_i2e_mirror(bit<5> qid, bit<32> mirror_id) {
        hdr.ig_intr_md_for_tm.ingress_cos = 3w1;
        hdr.ig_intr_md_for_tm.qid = qid;
        bypass_egress();
        clone(CloneType.I2E, (bit<32>)mirror_id);
    }
    ...
    @name(".qos_hit_no_eg_bypass_no_mirror") action qos_hit_no_eg_bypass_no_mirror(bit<5> qid, bit<2> color) {
        hdr.ig_intr_md_for_tm.ingress_cos = 3w1;
        hdr.ig_intr_md_for_tm.qid = qid;
        hdr.ig_intr_md_for_tm.packet_color = color;
    }
