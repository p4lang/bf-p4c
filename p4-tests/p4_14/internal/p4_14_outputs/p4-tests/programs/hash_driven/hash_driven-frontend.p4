...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".cnt_action") action cnt_action() {
        hash<bit<4>, bit<4>, tuple<bit<4>>, bit<5>>(temp_3, HashAlgorithm.identity, 4w0, { hdr.ipv4.version }, 5w16);
    ...
    @name(".meter_action") action meter_action(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac) {
        ...
        hash<bit<8>, bit<8>, tuple<bit<8>>, bit<9>>(temp_4, HashAlgorithm.identity, 8w0, { hdr.ipv4.protocol }, 9w256);
