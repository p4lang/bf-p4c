...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".meter_or_action") action meter_or_action(bit<9> egress_port, bit<8> idx) {
        {
            bit<3> temp;
            meter_1.execute_meter((bit<32>)idx, temp);
    ...
    @name(".cnt_action") action cnt_action() {
        {
            bit<4> temp_0;
            hash(temp_0, HashAlgorithm.identity, 4w0, { hdr.ipv4.version }, 5w16);
    ...
    @name(".meter_action") action meter_action(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac) {
        ...
        {
            bit<8> temp_1;
            hash(temp_1, HashAlgorithm.identity, 8w0, { hdr.ipv4.protocol }, 9w256);
            meter_0.execute_meter((bit<32>)temp_1, hdr.ipv4.diffserv);
        }
    }
