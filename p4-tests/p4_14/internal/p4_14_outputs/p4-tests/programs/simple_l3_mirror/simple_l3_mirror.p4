...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".parse_cpu_ethernet") state parse_cpu_ethernet {
        packet.extract(hdr.cpu_ethernet);
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".mirror_dest") table mirror_dest {
        actions = {
            send_ether;
            send_to_cpu;
            send_rspan;
        ...
        key = {
            meta.mirror_meta.mirror_dest: exact;
        }
    }
    apply {
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".acl_mirror") action acl_mirror(bit<32> mirror_dest) {
        ...
        clone3(CloneType.I2E, (bit<32>)mirror_dest, { meta.mirror_meta.ingress_port, meta.mirror_meta.mirror_dest, meta.mirror_meta.mirror_type });
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.cpu_ethernet);
        packet.emit(hdr.to_cpu);
