...
#define PARSE_VXLAN_UDP \
     UDP_PORT_VXLAN mask 0xffff: parse_vxlan;
...
parser parse_ipv4 {
   //return select(ig_intr_md.ingress_port, current(8,8)){ //ipv4.diffserv
   return select(ig_intr_md.ingress_port){
       //inport_diffserv: parse_intl45_ipv4;
...
header gre_t gre;
    ...
    !defined(DTEL_WATCH_INNER_ENABLE)
parser parse_gre {
    extract(gre);
...
#endif // DTEL_REPORT_ENABLE && !DTEL_WATCH_INNER_ENABLE
