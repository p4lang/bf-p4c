...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @ternary(1) @ignore_table_dependency("int_sink_local_report") @name(".tunnel_encap_process_outer") table _tunnel_encap_process_outer_0 {
        actions = {
            _nop_17();
            _fabric_rewrite();
            _ipv4_vxlan_rewrite();
    ...
    @name(".egress_mirror_and_drop") action _egress_mirror_and_drop(bit<8> reason_code, bit<8> platform_id) {
        mark_to_drop();
    }
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".gtpu_replace") action _gtpu_replace(bit<48> dmac, bit<48> smac, bit<32> destip, bit<32> srcip, bit<16> destport, bit<16> srcport) {
        hdr.ethernet.dstAddr = dmac;
        hdr.ethernet.srcAddr = smac;
    ...
    @name(".gtpu_replace") action _gtpu_replace_2(bit<48> dmac, bit<48> smac, bit<32> destip, bit<32> srcip, bit<16> destport, bit<16> srcport) {
        hdr.ethernet.dstAddr = dmac;
        hdr.ethernet.srcAddr = smac;
        hdr.ipv4.srcAddr = srcip;
        hdr.ipv4.dstAddr = destip;
    ...
    @name(".gtpu_forward") table _gtpu_forward_0 {
        actions = {
            _nop_22();
            _gtpu_replace();
    ...
    @name(".gtpu_reforward") table _gtpu_reforward_0 {
        actions = {
            _nop_23();
            _gtpu_replace_2();
    ...
    @name(".set_tunnel_lookup_flag") action _set_tunnel_lookup_flag_0() {
        meta.tunnel_metadata.tunnel_lookup = 1w1;
    }
    @name(".set_tunnel_vni_and_lookup_flag") action _set_tunnel_vni_and_lookup_flag_0(bit<24> tunnel_vni) {
    ...
    @name(".ipv4_dest_vtep") table _ipv4_dest_vtep {
        actions = {
            _nop_28();
            _set_tunnel_lookup_flag_0();
            _set_tunnel_vni_and_lookup_flag_0();
    ...
    @name(".compute_lkp_non_ip_hash") action _compute_lkp_non_ip_hash() {
        hash<bit<16>, bit<16>, tuple_4, bit<32>>(meta.hash_metadata.hash1, HashAlgorithm.crc16, 16w0, { meta.ingress_metadata.ifindex, meta.l2_metadata.lkp_mac_sa, meta.l2_metadata.lkp_mac_da, meta.l2_metadata.lkp_mac_type }, 32w65536);
    }
    ...
    apply {
        ...
        _gtpu_reforward_0.apply();
        _gtpu_forward_0.apply();
...
control verifyChecksum(inout headers hdr, inout metadata meta) {
    apply {
        verify_checksum<tuple_5, bit<16>>(true, { hdr.inner_ipv4.version, hdr.inner_ipv4.ihl, hdr.inner_ipv4.diffserv, hdr.inner_ipv4.totalLen, hdr.inner_ipv4.identification, hdr.inner_ipv4.flags, hdr.inner_ipv4.fragOffset, hdr.inner_ipv4.ttl, hdr.inner_ipv4.protocol, hdr.inner_ipv4.srcAddr, hdr.inner_ipv4.dstAddr }, hdr.inner_ipv4.hdrChecksum, HashAlgorithm.csum16);
        verify_checksum<tuple_5, bit<16>>(true, { hdr.ipv4.version, hdr.ipv4.ihl, hdr.ipv4.diffserv, hdr.ipv4.totalLen, hdr.ipv4.identification, hdr.ipv4.flags, hdr.ipv4.fragOffset, hdr.ipv4.ttl, hdr.ipv4.protocol, hdr.ipv4.srcAddr, hdr.ipv4.dstAddr }, hdr.ipv4.hdrChecksum, HashAlgorithm.csum16);
    }
}

control computeChecksum(inout headers hdr, inout metadata meta) {
    apply {
        update_checksum<tuple_5, bit<16>>(true, { hdr.inner_ipv4.version, hdr.inner_ipv4.ihl, hdr.inner_ipv4.diffserv, hdr.inner_ipv4.totalLen, hdr.inner_ipv4.identification, hdr.inner_ipv4.flags, hdr.inner_ipv4.fragOffset, hdr.inner_ipv4.ttl, hdr.inner_ipv4.protocol, hdr.inner_ipv4.srcAddr, hdr.inner_ipv4.dstAddr }, hdr.inner_ipv4.hdrChecksum, HashAlgorithm.csum16);
        update_checksum<tuple_5, bit<16>>(true, { hdr.ipv4.version, hdr.ipv4.ihl, hdr.ipv4.diffserv, hdr.ipv4.totalLen, hdr.ipv4.identification, hdr.ipv4.flags, hdr.ipv4.fragOffset, hdr.ipv4.ttl, hdr.ipv4.protocol, hdr.ipv4.srcAddr, hdr.ipv4.dstAddr }, hdr.ipv4.hdrChecksum, HashAlgorithm.csum16);
    }
}
