...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".meter_3") @pre_color(hdr.ipv4.diffserv) direct_meter<bit<8>>(MeterType.bytes) meter_3;
    @name(".meter_0") meter(32w500, MeterType.bytes) meter_0;
    @meter_pre_color_aware_per_flow_enable(1) @name(".meter_2") meter(32w500, MeterType.bytes) meter_2;
    @name(".meter_lpf") Lpf<bit<32>, bit<32>>(500) meter_lpf;
    @name(".meter_lpf_direct") DirectLpf<bit<32>>() meter_lpf_direct;
    @name(".meter_lpf_tcam") Lpf<bit<32>, bit<32>>(500) meter_lpf_tcam;
    @name(".meter_lpf_tcam_direct") DirectLpf<bit<32>>() meter_lpf_tcam_direct;
    @name(".count_color") action count_color(bit<32> color_idx) {
        colorCntr.count((bit<32>)color_idx);
    }
    ...
    @name(".next_hop_ipv4_lpf") action next_hop_ipv4_lpf(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac, bit<32> lpf_idx) {
        ...
        hdr.ipv4.srcAddr = meter_lpf.execute(hdr.ipv4.srcAddr, lpf_idx);
    }
    @name(".nop") action nop() {
    }
    ...
    @name(".meter_action") action meter_action(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac, bit<32> idx) {
        ...
        meter_0.execute_meter((bit<32>)idx, hdr.ipv4.diffserv);
    }
    ...
    @name(".meter_action_color_unaware") action meter_action_color_unaware(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac, bit<32> idx) {
        next_hop_ipv4(egress_port, srcmac, dstmac);
        meter_2.execute_meter((bit<32>)idx, hdr.ipv4.diffserv);
    }
    ...
    @name(".next_hop_ipv4") action next_hop_ipv4_0(bit<9> egress_port, bit<48> srcmac, bit<48> dstmac) {
        meter_3.read(hdr.ipv4.diffserv);
        hop(hdr.ipv4.ttl, egress_port);
        hdr.ethernet.srcAddr = srcmac;
        hdr.ethernet.dstAddr = dstmac;
    }
