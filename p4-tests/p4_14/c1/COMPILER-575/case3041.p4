...
parser JimFalls {
   ...
   set_metadata(Flourtown.Wheeler, 1);
...
parser Lansdale {
   set_metadata(Lisle.Maxwelton, 1);
   return ingress;
}
parser Salamonia {
   extract( Linville );
   ...
   set_metadata(Flourtown.Wheeler, 2);
...
parser Crumstown {
   set_metadata( Lisle.Maxwelton, 3 );
   extract(Plato);
   extract(Wyndmoor);
   return ingress;
}
...
parser Bigspring {
   extract( Gakona );
   set_metadata(CruzBay.Euren, Gakona.PellLake);
   set_metadata(CruzBay.Joplin, Gakona.Sweeny);
   set_metadata(CruzBay.Daykin, Gakona.Sabetha);
   ...
   set_metadata(Flourtown.Balmville, 1);
...
parser Piney {
   set_metadata(Lisle.Bellamy, 1);
   return ingress;
}
parser ElVerano {
   extract( Hiseville );
   set_metadata(Dorset.Despard, Hiseville.AvonLake);
   set_metadata(Dorset.Sasakwa, Hiseville.Glynn);
   set_metadata(Dorset.Bagdad, Hiseville.Bevier);
   ...
   set_metadata(Flourtown.Balmville, 2);
...
parser Greenbelt {
   ...
   set_metadata(Lisle.Bellamy, 3);
   return ingress;
}
parser Temvik {
   extract( Tamms );
   set_metadata( Lisle.Suffern, Tamms.Louviers );
   set_metadata( Lisle.Welch, Tamms.Felton );
   set_metadata( Lisle.Natalia, Tamms.Duquoin );
   set_metadata( Lisle.Odenton, Tamms.Bellmead );
   set_metadata( Lisle.Ganado, Tamms.Triplett );
...
@pragma pa_no_init ingress Shivwits.Moose
...
action Mesita() {
   ...
   modify_field( Lisle.CeeVee, Flourtown.Balmville );
   ...
   modify_field( Lisle.Maxwelton, Lisle.Bellamy );
}
action Kisatchie() {
   ...
   modify_field( Lisle.CeeVee, Flourtown.Wheeler );
...
control Radom {
   if ( Lisle.Grannis == 0 and Nondalton.Buncombe == 1 ) {
      if ( ( Nondalton.Fairchild == 1 ) and ( ( Lisle.CeeVee & 1 ) == 1 ) ) {
         apply( Bellville ) {
            Sultana {
               apply(Coamo);
            }
         }
      } else if ( ( Nondalton.Welcome == 1 ) and ( ( Lisle.CeeVee & 2 ) == 2 ) ) {
         apply( Tonasket ) {
            Sultana {
               apply( Frontenac );
            }
...
action Corbin() {
   modify_field( Shivwits.Moose, Atlas.ElkRidge );
}
...
action Alston() {
   ...
   bit_xor( Atlas.Greenlawn, Lisle.Maxwelton, 1 );
}
action Wyandanch() {
   modify_field( Atlas.Havana, Lisle.Aplin );
...
action Novinger( Weehawken, Tillicum, Tanana, Amonate, Newtown, Bowlus, Heaton, Hiland, Risco ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, Heaton );
...
action Balfour( Gillette, Frontier, Asher, Riverland, Mather, Kalkaska, Glenolden, Frankston, Roachdale ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, Glenolden );
...
action Linden( Lindsay, Globe, Panacea, Buckeye, Inkom, Nevis, DuBois, Laton, Aldrich ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, DuBois );
...
action BealCity( Rumson, Bayville, Bennet, Portville, Sully, Weiser, Palco, LaPalma, Mankato ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, Palco );
...
action Corvallis( Kranzburg, Abernant, Manteo, Wymore, Philbrook, Pittsboro, Ambrose, Yulee, Campton ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, Ambrose );
...
action Steprock( Pierson, Offerle, Wimberley, Newburgh, Cutler, Moweaqua, Caguas, Alsen, Williams ) {
   ...
   bit_and( Wenden.Quamba, Lisle.Seattle, Caguas );
...
action Newtonia( LaMarque, Groesbeck, RedLevel, Mellott, Flippen, Traverse, Lepanto, FlyingH, Power ) {
   ...
   bit_and( Coolin.Quamba, Lisle.Seattle, Lepanto );
...
action Bernice( Lenoir, Odebolt, Mattoon, Lutts, Bodcaw, Ardara, Langtry, Ballville, Barnsdall ) {
   ...
   bit_and( Coolin.Quamba, Lisle.Seattle, Langtry );
...
action Yerington( Francisco, Cadott, Onycha, Desdemona, LaUnion, DoeRun, Spivey, Penitas, PineHill ) {
   ...
   bit_and( Coolin.Quamba, Lisle.Seattle, Spivey );
...
action Malaga( Hewitt, Emlenton, Ranburne, Petrey, Cogar, Browndell, Yemassee, Natalbany, Gasport ) {
   ...
   bit_and( Coolin.Quamba, Lisle.Seattle, Yemassee );
...
action Navarro( August, Watters, Gerster, Suttle, Klukwan, Compton, Leoma, Tchula, Antoine ) {
   ...
   bit_and( Coolin.Quamba, Lisle.Seattle, Leoma );
