...
/*
 * The following tables are needed to make sure the 
 * fields are not optimized out
 */
action set_egress_spec(port) {
    //modify_field(standard_metadata.egress_spec, port);
    modify_field(ig_intr_md_for_tm.ucast_egress_port, port);
}

table use_8bit_fields {
    reads {
        m.field_8_01  :  exact;
        m.field_8_02  :  exact;
        m.field_8_03  :  exact;
...
control ingress {
    ...
    /*
     * Ensure that all fields are referenced 
     */
    apply(use_8bit_fields);
    apply(use_16bit_fields_1);
    apply(use_16bit_fields_2);
    apply(use_32bit_fields_1);
    apply(use_32bit_fields_2);
}
