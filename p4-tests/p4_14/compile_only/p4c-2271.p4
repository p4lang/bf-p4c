...
# 1 "<command-line>"
# 1 "pktgen9.p4"
// Hide sections of code to isolate compiler bugs
// #define HIDE_SEQ 1
// #define HIDE_LAT 1
// #define HIDE_TSTAMP 1
// #define HIDE_UDF 1
// #define HIDE_INSTRUM 1
// #define HIDE_STATS 1
...
header_type metadata_t
{
    fields
    {
        ...
        txpgid : 10; // the pgid to put into egress instrumentation
        pipe_port : 4;
        port_pgid_index : 15;
...
#ifdef FIX_SALU_PROBLEM
@pragma pa_container_size ingress meta.seq_delta 32
#endif
...
// ipv6
header tcp_t outer_tcpv6;
header udp_t outer_udpv6;
...
parser parse_outer_tcpv6
{
    extract(outer_tcpv6);
    return parse_big_sig_ig_instrum;
}

parser parse_outer_udpv6
{
    extract(outer_udpv6);
    return parse_big_sig_ig_instrum;
}
...
action do_get_fp_port(fp_port, pipe_port)
{
...
action do_remap_stream(stream_id)
{
...
action do_calib_rx_tstamp(offset) {
  add(meta.rx_tstamp_calibrated,instrum.tstamp, offset);
}
...
blackbox stateful_alu store_first_tstamp_salu
{
    ...
    update_lo_1_value: instrum.tstamp; // store current pkt instrum tstamp
}
...
/*
* Store the timestamp, assimed called only once per flow
*/
register rx_last_stamp_reg
{
    width: 64;
    instance_count: 16384;
}
...
// Store curr tstamp 
blackbox stateful_alu store_last_tstamp_salu
{
    reg: rx_last_stamp_reg; // 32-bit reg stores prev latency
    condition_lo: instrum.tstamp > register_lo;
    update_lo_1_predicate: condition_lo;
    update_lo_1_value: instrum.tstamp; // store current pkt instrum tstamp
    update_lo_2_predicate: not condition_lo;
    update_lo_2_value: instrum.tstamp; // store current pkt instrum tstamp
...
{
    store_last_tstamp_salu.execute_stateful_alu(meta.pgid_pipe_port_index);
}
...
action rx_latency_calc()
{
    ...
    subtract(meta.latency, ig_intr_md_from_parser_aux.ingress_global_tstamp, meta.rx_tstamp_calibrated);
}
...
{
    apply(rx_store_last_tstamp_tbl);
...
header_type burst_pkt_cntr_t
{
    fields
    {
        max : 32; // burst size for stream
        drop : 1; // drop flag for stream
    }
}
...
action do_load_burst_pkt_cntr_counter_params(max)
{
    modify_field(burst_pkt_cntr.max,max);
...
blackbox stateful_alu burst_pkt_cntr
{
    ...
    condition_lo : register_lo < burst_pkt_cntr.max;
    ...
    update_hi_2_predicate : not condition_lo;
    update_hi_2_value : 1;
    output_value : alu_hi; // return next counter value
...
@pragma field_list_field_slice ig_intr_md_from_parser_aux.ingress_global_tstamp 31 0
...
@pragma field_list_field_slice ig_intr_md_from_parser_aux.ingress_global_tstamp 47 32
...
control ingress
{
    ...
    */
    ...
    apply(ig_remap_stream_tbl);
    process_ig_pgid_statsB();
    process_load_udf_cntr_params_ingress();
...
{
    // Egress Timestamp 
    modify_field(instrum.tstamp, eg_intr_md_from_parser_aux.egress_global_tstamp);
...
action do_calib_tx_tstamp(offset) {
  add_to_field(instrum.tstamp, offset);
}
...
control egress
{
    ...
    if ((meta.is_pktgen == 1 ) and ((eg_intr_md.egress_port & 128) == 0))
    {
        ...
        process_load_udf2_cntr_params();
        process_compute_conditional_cntr_udf();
        process_compute_inner_cntr_udf();
        process_incr_burst_pkt_cntr();
        process_udf_vlist_mac_ip();
        process_intermediate_inner_cntr_udf();
        ...
        process_intermediate_outer_cntr_udf();
        process_update_cntr_udf();
        process_update_cntr_udf_nested();

        if(burst_pkt_cntr.drop == 1)
        {
            process_burst_drops();
        }
        else
