...
header_type int_digest_header_t {
    fields {
        digest : 32;
    }
}
...
action fib_hit_myip(nexthop_index) {
    modify_field(l3_metadata.fib_hit, 1);
    modify_field(l3_metadata.fib_nexthop, nexthop_index);
    modify_field(l3_metadata.fib_nexthop_type, 0);
...
action set_tunnel_rewrite_details_with_dmac(outer_bd, smac_idx, dmac_idx, sip_index, dip_index) {
...
action set_tunnel_rewrite_details(smac_idx, sip_index, dip_index) {
    modify_field(tunnel_metadata.tunnel_smac_index, smac_idx);
    set_ip_index(sip_index, dip_index);
}
...
action nat_update_l4_checksum() {
    ...
    modify_field(nat_metadata.l4_len, 0);
...
action update_udp_checksum() {
    modify_field(nat_metadata.update_udp_checksum, 1);
}

action update_tcp_checksum() {
    modify_field(nat_metadata.update_tcp_checksum, 1);
}

action update_inner_udp_checksum() {
    modify_field(nat_metadata.update_inner_udp_checksum, 1);
}

action update_inner_tcp_checksum() {
    modify_field(nat_metadata.update_inner_tcp_checksum, 1);
}

table update_l4_checksum {
    reads {
        nat_metadata.update_checksum : ternary;
        tunnel_metadata.egress_tunnel_type : ternary;
        udp.valid : ternary;
        tcp.valid : ternary;
        inner_udp.valid : ternary;
        inner_tcp.valid : ternary;
        udp.checksum : ternary;
        inner_udp.checksum : ternary;
    }
    actions {
        nop;
        update_udp_checksum;
...
action set_pool_id(id) {
    modify_field(l4l_metadata.pool_id, id);
}

@pragma idletime_precision 2
@pragma proxy_hash_width 32
table l4l_conn_table {
    reads {
        l3_metadata.vrf : exact;
        ipv4_metadata.lkp_ipv4_sa : exact;
        ipv4_metadata.lkp_ipv4_da : exact;
...
action set_no_update(id) {
    modify_field(l4l_metadata.update_status, 0);
    set_pool_id(id);
}

action do_caching(id) {
    modify_field(l4l_metadata.update_status, 1);
    set_pool_id(id);
}

action use_cache(id, old_id) {
    modify_field(l4l_metadata.update_status, 2);
    set_pool_id(id);
    modify_field(l4l_metadata.old_id, old_id);
}

table l4l_listener_table {
    reads {
        l3_metadata.vrf : exact;
        ipv4_metadata.lkp_ipv4_da : exact;
        l3_metadata.lkp_ip_proto : exact;
...
action use_old_id() {
    modify_field(l4l_metadata.pool_id, l4l_metadata.old_id);
}

table l4l_use_old_id {
    actions {
        use_old_id;
    }
    default_action: use_old_id;
    size : 1;
}

field_list l4l_learn_digest {
    l3_metadata.vrf;
    ...
    tcp.dstPort;
    l4l_metadata.pool_id;
}

action generate_conn_digest() {
    generate_digest(0, l4l_learn_digest);
}

table l4l_learn_conn {
    actions {
        generate_conn_digest;
    }
    default_action: generate_conn_digest;
    size : 1;
}

control process_l4l_get_pool {

    apply(l4l_conn_table) {
        on_miss {
            apply(l4l_listener_table);

            if (l4l_metadata.update_status == 1
                    or l4l_metadata.update_status == 2) {
                process_l4l_conn_cache();
            }
            if (l4l_metadata.update_status == 2
                    and l4l_metadata.cache_status == 0) {

                apply(l4l_use_old_id);
            }
            else if (nat_metadata.nat_rewrite_index == 0 and (l4l_metadata.pool_id != 0)) {
...
action set_l4l_ipv4_member(member_ip, member_port, nat_rewrite_index) {
    modify_field(ipv4_metadata.lkp_ipv4_da, member_ip);
    modify_field(l3_metadata.lkp_l4_dport, member_port);
    modify_field(nat_metadata.nat_rewrite_index, nat_rewrite_index);
    modify_field(nat_metadata.ingress_nat_mode, 3);
}

field_list client_hash_fields {
    ipv4_metadata.lkp_ipv4_sa;
    l3_metadata.lkp_l4_sport;
}

field_list_calculation member_selection_hash {
    input {
        client_hash_fields;
    }
    algorithm : crc32;
...
action_selector member_selector {
    selection_key : member_selection_hash;
    selection_mode : fair;
}

action_profile l4l_selector_action_profile {
    actions {
        nop;
        set_l4l_ipv4_member;
    }
    size : 16384;
    dynamic_action_selection : member_selector;
}


@pragma selector_max_group_size 512
@pragma selector_num_max_groups 1024
table l4l_pool_table {
    reads {
        l3_metadata.vrf : exact;
        ipv4_metadata.lkp_ipv4_da : exact;
        l3_metadata.lkp_ip_proto : exact;
        l3_metadata.lkp_l4_dport : exact;
        l4l_metadata.pool_id : exact;
    }
    action_profile: l4l_selector_action_profile;
    size : 16384;
}

control process_l4l_get_member {

    if (nat_metadata.nat_rewrite_index == 0 and (l4l_metadata.pool_id != 0)) {

        apply(l4l_pool_table);
    }
...
field_list lkp_ipv4_src_fields {
    ipv4_metadata.lkp_ipv4_sa;
    l3_metadata.lkp_l4_sport;
}

field_list_calculation l4l_filter_hash1 {
    input { lkp_ipv4_src_fields; }
...
field_list_calculation l4l_filter_hash2 {
    input { lkp_ipv4_src_fields; }
...
field_list_calculation l4l_filter_hash3 {
    input { lkp_ipv4_src_fields; }
...
field_list_calculation l4l_filter_hash4 {
    input { lkp_ipv4_src_fields; }
...
register l4l_filter1{
    width : 1;
    instance_count : 16384;
}
register l4l_filter2{
    width : 1;
    instance_count : 16384;
}
register l4l_filter3{
    width : 1;
    instance_count : 16384;
}
...
blackbox stateful_alu l4l_filter_set_alu1{
    reg: l4l_filter1;
    update_lo_1_value: set_bit;
}
blackbox stateful_alu l4l_filter_set_alu2{
    reg: l4l_filter2;
    update_lo_1_value: set_bit;
}
blackbox stateful_alu l4l_filter_set_alu3{
    reg: l4l_filter3;
    update_lo_1_value: set_bit;
}


action set_l4l_filter1() {

    l4l_filter_set_alu1.execute_stateful_alu_from_hash(l4l_filter_hash1);
...
action set_l4l_filter2() {

    l4l_filter_set_alu2.execute_stateful_alu_from_hash(l4l_filter_hash2);
...
action set_l4l_filter3() {

    l4l_filter_set_alu3.execute_stateful_alu_from_hash(l4l_filter_hash3);
...
table l4l_set_bloom_filter1 {
    actions { set_l4l_filter1; }
    default_action: set_l4l_filter1;
    size: 1;
}
table l4l_set_bloom_filter2 {
    actions { set_l4l_filter2; }
    default_action: set_l4l_filter2;
    size: 1;
}
table l4l_set_bloom_filter3 {
    actions { set_l4l_filter3; }
    default_action: set_l4l_filter3;
    size: 1;
}


blackbox stateful_alu l4l_filter_read_alu1{
    reg: l4l_filter1;
    update_lo_1_value: read_bitc;
    output_value: alu_lo;
    output_dst: l4l_metadata.cache_status;
    reduction_or_group: l4l_or_group;
}
blackbox stateful_alu l4l_filter_read_alu2{
    reg: l4l_filter2;
    update_lo_1_value: read_bitc;
    output_value: alu_lo;
    output_dst: l4l_metadata.cache_status;
...
blackbox stateful_alu l4l_filter_read_alu3{
    reg: l4l_filter3;
    update_lo_1_value: read_bitc;
    output_value: alu_lo;
    output_dst: l4l_metadata.cache_status;
...
table l4l_read_bloom_filter1 {
    actions { read_l4l_filter1; }
    default_action: read_l4l_filter1;
    size: 1;
}
table l4l_read_bloom_filter2 {
    actions { read_l4l_filter2; }
    default_action: read_l4l_filter2;
    size: 1;
}
table l4l_read_bloom_filter3 {
    actions { read_l4l_filter3; }
    default_action: read_l4l_filter3;
    size: 1;
}

control process_l4l_conn_cache {
    if (l4l_metadata.update_status == 1) {
        apply(l4l_set_bloom_filter1);
        apply(l4l_set_bloom_filter2);
        apply(l4l_set_bloom_filter3);
    }
    else if (l4l_metadata.update_status == 2) {
        apply(l4l_read_bloom_filter1);
        apply(l4l_read_bloom_filter2);
...
control process_rewrite {
    if ((egress_metadata.routed != 0) and
...
control process_telemetry_local_report {
...
action compute_flow_hash_outer() {
    modify_field_with_hash_based_offset(
        ...
        telemetry_flow_hash_outer_calc, 4294967296);
}

action compute_flow_hash_inner() {
    modify_field_with_hash_based_offset(
        telemetry_md.flow_hash, 0,
        telemetry_flow_hash_inner_calc, 4294967296);
}
...
# 203 "/home/jklee/workspace/l4l/submodules/switch/p4src/switch.p4"
        ...
        process_l4l_get_pool();
        process_l4l_get_member();
...
# 387 "/home/jklee/workspace/l4l/submodules/switch/p4src/switch.p4"
        ...
        if ((eg_intr_md.deflection_flag == 0) and
            ...
            if (not (eg_intr_md_from_parser_aux.clone_src != 0)) {
                ...
                process_telemetry_local_report();
            }
