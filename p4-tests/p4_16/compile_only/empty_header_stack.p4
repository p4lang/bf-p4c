...
parser iPrsr(packet_in packet,
  ...
  /*
  Checksum<bit<16>>(HashAlgorithm_t.CSUM16) check_1;
  Checksum<bit<16>>(HashAlgorithm_t.CSUM16) check_2;
  Checksum<bit<16>>(HashAlgorithm_t.CSUM16) check_3;
  Checksum<bit<16>>(HashAlgorithm_t.CSUM16) check_4;
  Checksum<bit<16>>(HashAlgorithm_t.CSUM16) check_5;
  */
  ...
  state start {
    ...
    //check_1.add(hdr.ethernet);
    //check_2.add(hdr.ethernet);
    //check_3.add(hdr.ethernet);
    //check_4.add(hdr.ethernet);
    //check_5.add(hdr.ethernet);
  ...
  state more_parsing {
    packet.extract(hdr.junk[0]);
    //packet.extract(hdr.junk_0);
    transition select( packet.lookahead<bit<8>>() ) {
      0xFE : accept;
      default : more_parsing_1;
    }
  }
  state more_parsing_1 {
    packet.extract(hdr.junk[1]);
    //packet.extract(hdr.junk_1);
    transition select( packet.lookahead<bit<8>>() ) {
      0xFE : accept;
      //default : more_parsing_2;
      default : accept;
    }
  }
}
