...
const bit<8> PPS_BYTES_PER_STAGE= 4;
const bit<8> PPS_BITS_PER_STAGE = PPS_BYTES_PER_STAGE * 8;
const bit<8> PPS_STAGE_CNT = 10;
const bit<8> PPS_PARSE_CNT = 11;
const bit<16> PPS_TABLE_SIZE = 1024;

header pps_found_match_h {
    bit<8> match;
    // bool match;
}

header pps_data_h {
    bit<32> data;// 1 byte * 10
}

struct pps_metadata_t {
    bit<16> pps_payload_pos;
    bit<8> pps_parse_depth;
    bit<16> payload_rest;
    // pps_data_h[4] found_match;
}

struct metadata_t {
    pps_metadata_t pps_md;
}
...
struct header_t {
    ...
    pps_data_h[PPS_PARSE_CNT] pps_data;
    pps_found_match_h[PPS_BYTES_PER_STAGE] found_match;
}

# 1 "pps_ctrl_manual.p4" 1
// struct
//     pps_data_t[PPS_PARSE_CNT] pps_data;

parser PpsParser(packet_in pkt,
                 inout header_t hdr,
                 inout pps_metadata_t pps_md) {

    // ParserCounter<bit<8>>() pps_data_cnt;

    bit<16> tmp_payload_rest;

    state start {
        // pps_md.found_match[0].setValid();
        // pps_md.found_match[1].setValid();
        // pps_md.found_match[2].setValid();
        // pps_md.found_match[3].setValid();
        // pps_data_cnt.set(0);

        transition parse_payload_length;
    }

    state parse_payload_length {
        transition select(pps_md.payload_rest) {
            0 : accept;
            default : extract_payload;
        }
    }


    state extract_payload {
        pkt.extract(hdr.pps_data.next);
        hdr.found_match.next = {8w0};
        // pps_data_cnt.increment(1);
        // pps_md.pps_payload_pos = pps_md.pps_payload_pos + 4;
        tmp_payload_rest = pps_md.payload_rest |-| (bit<16>) PPS_BYTES_PER_STAGE;
        // pps_md.payload_rest = pps_md.payload_rest - (int<16>) PPS_BYTES_PER_STAGE;
...
control PpsTable(inout header_t hdr,
                 inout pps_metadata_t pps_md)
                (bit<8> stage,
                 bit<8> byte_ofs,
                 bit<8> match_width_byte,
                 bit<16> pps_table_size) {
    ...
    apply {
        // pps_md.found_match[stage].match = (bit<8>) pps_stage_offset.apply().hit;
        if (pps_stage_offset.apply().hit) {
            hdr.found_match[stage*match_width_byte+byte_ofs].match = 1;
        } else {
            hdr.found_match[stage*match_width_byte+byte_ofs].match = 0;
        }
    }
}
...
control PpsManual(
    ...
    PpsTable(stage = 0,
             byte_ofs = 0,
             match_width_byte = PPS_BYTES_PER_STAGE,
             pps_table_size = PPS_TABLE_SIZE) pps_stage0_b0;
    // PpsTable(stage = 0,
    //          byte_ofs = 1,
    //          match_width_byte = PPS_BYTES_PER_STAGE,
    //          pps_table_size = PPS_TABLE_SIZE) pps_stage0_b1;
    // PpsTable(stage = 0,
    //          byte_ofs = 2,
    //          match_width_byte = PPS_BYTES_PER_STAGE,
    //          pps_table_size = PPS_TABLE_SIZE) pps_stage0_b2;
    // PpsTable(stage = 0,
    //          byte_ofs = 3,
    //          match_width_byte = PPS_BYTES_PER_STAGE,
    //          pps_table_size = PPS_TABLE_SIZE) pps_stage0_b3;

    apply {
        pps_stage0_b0.apply(hdr = hdr,
                            pps_md = ig_md.pps_md);
        hdr.pps_data.pop_front(1);
        // pps_stage0_b1.apply(hdr);
        // pps_stage0_b2.apply(hdr);
        // pps_stage0_b3.apply(hdr);
    }
}
# 70 "scanner.p4" 2
...
parser SwitchIngressParser(
    ...
    PpsParser() pps_parser;

    state start {
        // ig_md.pps_md = {0,0,0,0};
    ...
    state parse_udp {
        ...
        ig_md.pps_md.payload_rest = hdr.udp.hdr_length;
    ...
    state parse_pps {
        pps_parser.apply(pkt, hdr, ig_md.pps_md);
        transition accept;
    }
}
