...
#define switch_bd_width              8  // derek: was 16
#define switch_nexthop_width        16
#define switch_outer_nexthop_width  16
#define switch_smac_index_width     16
#define switch_cpu_reason_width      8  // derek: was 16
*/
...
typedef bit<8> switch_nexthop_t;
typedef bit<8> switch_outer_nexthop_t;
...
#if __TARGET_TOFINO__ == 3
    bit<5> _pad1; // 7  \ 16 total
#else
    bit<7> _pad1; // 7  \ 16 total
#endif
    switch_port_t port; // 9 or 11  /
    switch_bd_t bd; // 10
...
//  bit<16> hash;
 ...
 bool hdr_3_ethernet_isValid;
 bool hdr_3_ipv4_isValid;
...
struct nsh_metadata_t {
 ...
 bool sf1_active; // ingress / egress
    ...
    bit<8> sfc; // ingress only (for sfp sel)
 bit<16> hash_1; // ingress only (for sfp sel)
    bool l2_fwd_en; // ingress only

    bit<8> dsap; // egress only (for egress sf)
...
const bit<32> OUTER_ECMP_GROUP_TABLE_SIZE = 4096; // derek: unused
...
control IngressIpv4Acl(
 ...
 table acl {
  key = {
   lkp.ip_src_addr[31:0] : ternary; lkp.ip_dst_addr[31:0] : ternary; lkp.ip_proto : ternary; lkp.ip_tos : ternary; lkp.ip_flags : ternary; lkp.tcp_flags : ternary; lkp.tunnel_type : ternary; lkp.tunnel_id : ternary;
...
control VlanXlate(
 ...
 action set_vlan_tagged(vlan_id_t vid, bit<3> pcp) {
  ...
  hdr.vlan_tag[0].pcp = pcp; // derek: added this here...barefoot set it.
...
parser NpbIngressParser(
 ...
 bit<8> protocol_inner;
    ...
    state start {
        ...
        ig_md.tunnel_3.hdr_3_ethernet_isValid = false;
        ig_md.tunnel_3.hdr_3_ipv4_isValid = false;
        ig_md.tunnel_3.hdr_3_ipv6_isValid = false;
...
// #endif // PTP_ENABLE
    ...
    state parse_transport_vlan {
        ...
        // ig_md.lkp_0.tunnel_id = (bit<switch_tunnel_id_width>)pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;

        // ./npb/pipe/npb.bfa:2037: error: Ran out of constant output slots
        // ig_md.lkp_0.tunnel_id[15:0] = pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;
        // ig_md.lkp_0.tunnel_id[15:0] = pkt.lookahead<bit<16>>();        
     ...
     pkt.extract(hdr.transport.vlan_tag[0]);
        ...
        // first aren't byte aligned: 77:ingress::ig_md.lkp_1.tunnel_id<32> ^0 ^bit[0..15] meta solitary mocha
        // ig_md.lkp_1.tunnel_id = (bit<switch_tunnel_id_width>)hdr.transport.vlan_tag[0].vid;
        // ig_md.lkp_1.tunnel_id[11:0] = hdr.transport.vlan_tag[0].vid;
        // ig_md.lkp_1.tunnel_id[15:0] = (bit<16>)hdr.transport.vlan_tag[0].vid;        

        // error: Inferred incompatible container alignments for field ingress::ig_md.lkp_1.tunnel_id:
        // alignment = 5 != alignment = 0 (little endian)
        // ig_md.lkp_1.tunnel_id[15:0] = (hdr.transport.vlan_tag[0].pcp ++ hdr.transport.vlan_tag[0].cfi ++
        // hdr.transport.vlan_tag[0].vid);
...
// #endif // PTP_ENABLE
    ...
    state parse_outer_br {
     pkt.extract(hdr.outer.e_tag);
        ...
        ig_md.lkp_1.pcp_0 = hdr.outer.e_tag.pcp; // todo: should we populate pcp here?        
    ...
    state parse_outer_vlan_0 {
        ...
        // [--Werror=type-error] error: cast: Cannot unify T function<T>() to bit<32>
        // ig_md.lkp_1.tunnel_id = (bit<switch_tunnel_id_width>)pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;

        // ./npb/pipe/npb.bfa:2037: error: Ran out of constant output slots
        // ig_md.lkp_1.tunnel_id[15:0] = pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;
        // ig_md.lkp_1.tunnel_id[15:0] = pkt.lookahead<bit<16>>();
     ...
     pkt.extract(hdr.outer.vlan_tag[0]);
        ...
        // error: Field is extracted in the parser into multiple containers, but the container slices after the
        // first aren't byte aligned: 77:ingress::ig_md.lkp_1.tunnel_id<32> ^0 ^bit[0..15] meta solitary mocha
        // ig_md.lkp_1.tunnel_id = (bit<switch_tunnel_id_width>)hdr.outer.vlan_tag[0].vid;
        // ig_md.lkp_1.tunnel_id[11:0] = hdr.outer.vlan_tag[0].vid;
        // ig_md.lkp_1.tunnel_id[15:0] = (bit<16>)hdr.outer.vlan_tag[0].vid;        

        // error: Inferred incompatible container alignments for field ingress::ig_md.lkp_1.tunnel_id:
        // alignment = 5 != alignment = 0 (little endian)
        // ig_md.lkp_1.tunnel_id[15:0] = (hdr.outer.vlan_tag[0].pcp ++ hdr.outer.vlan_tag[0].cfi ++ hdr.outer.vlan_tag[0].vid);


        ig_md.lkp_1.mac_type = hdr.outer.vlan_tag[0].ether_type;
        ig_md.lkp_1.pcp_0 = hdr.outer.vlan_tag[0].pcp;
    ...
    state parse_outer_vlan_1 {
     pkt.extract(hdr.outer.vlan_tag[1]);
        ig_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_VLAN;
        //ig_md.lkp_1.tunnel_id[23:12] = hdr.outer.vlan_tag[1].vid;
        //ig_md.lkp_1.tunnel_id = (bit<switch_tunnel_id_width>)hdr.outer.vlan_tag[1].vid;

        ig_md.lkp_1.mac_type = hdr.outer.vlan_tag[1].ether_type;
        ig_md.lkp_1.pcp_1 = hdr.outer.vlan_tag[1].pcp;
    ...
    state parse_inner_vlan {
        ...
        // [--Werror=type-error] error: cast: Cannot unify T function<T>() to bit<32>
        // ig_md.lkp_2.tunnel_id = (bit<switch_tunnel_id_width>)pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;
        ...
        // ig_md.lkp_2.tunnel_id = (bit<switch_tunnel_id_width>)hdr.inner.vlan_tag[0].vid;
    ...
    state parse_inner_gre {
        ...
        transition select(
            ...
            (0,0,0,0,0x0800): parse_inner_inner_ipv4;
            (0,0,0,0,0x86dd): parse_inner_inner_ipv6;
            default: parse_udf;
        }
    }
    ...
    state parse_inner_inner_ipv4 {
        ig_md.tunnel_3.hdr_3_ipv4_isValid = true;
  transition parse_udf;
    }
    state parse_inner_inner_ipv6 {
        ig_md.tunnel_3.hdr_3_ipv6_isValid = true;
  transition parse_udf;
    }
...
parser NpbEgressParser(
 ...
 // bit<8>  inner_protocol;
    ...
    state start {
        ...
        eg_md.tunnel_3.hdr_3_ethernet_isValid = false;
        eg_md.tunnel_3.hdr_3_ipv4_isValid = false;
        eg_md.tunnel_3.hdr_3_ipv6_isValid = false;
  ...
  // ----- extract dedup bridged metadata -----
    ...
    state parse_outer_br_scope0 {
     pkt.extract(hdr.outer.e_tag);
        ...
        eg_md.lkp_1.pcp_0 = hdr.outer.e_tag.pcp;
    ...
    state parse_outer_vlan_0_scope0 {
     pkt.extract(hdr.outer.vlan_tag[0]);
        ...
        eg_md.lkp_1.pcp_0 = hdr.outer.vlan_tag[0].pcp;
    ...
    state parse_outer_vlan_1_scope0 {
     pkt.extract(hdr.outer.vlan_tag[1]);
        ...
        eg_md.lkp_1.pcp_1 = hdr.outer.vlan_tag[1].pcp;
        eg_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_VLAN;
        //eg_md.lkp_1.tunnel_id[23:12] = hdr.outer.vlan_tag[1].vid;
        //eg_md.lkp_1.tunnel_id = (bit<switch_tunnel_id_width>)hdr.outer.vlan_tag[1].vid;
    ...
    state parse_outer_ipv4_scope0 {
        ...
        transition select(
            ...
            (5, 3w2 &&& 3w5, 0, 1): parse_outer_icmp_igmp_overload;
            (5, 3w2 &&& 3w5, 0, 2): parse_outer_icmp_igmp_overload;
            ...
            (5, 3w2 &&& 3w5, 0, 0x32): parse_outer_esp_overload_scope0;
            default: accept;
        }
    }
    ...
    state parse_inner_vlan_scope1 {
        ...
        eg_md.lkp_1.pcp_0 = hdr.inner.vlan_tag[0].pcp;
        eg_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_VLAN;
    ...
    state parse_inner_ipv4_scope0 {
        pkt.extract(hdr.inner.ipv4);
           ...
           (5, 3w2 &&& 3w5, 0, 4): parse_inner_inner_ipv4;
           (5, 3w2 &&& 3w5, 0, 41): parse_inner_inner_ipv6;
           default : accept;
       }
    }
    ...
    state parse_inner_ipv4_scope1 {
        ...
        transition select(
            ...
            hdr.inner.ipv4.frag_offset,
            hdr.inner.ipv4.protocol) {
            (5, 3w2 &&& 3w5, 0, 1): parse_inner_icmp_igmp_overload;
            (5, 3w2 &&& 3w5, 0, 2): parse_inner_icmp_igmp_overload;
            ...
            (5, 3w2 &&& 3w5, 0, 0x32): parse_inner_esp_overload_scope1;
            (5, 3w2 &&& 3w5, 0, 4): parse_inner_inner_ipv4;
            (5, 3w2 &&& 3w5, 0, 41): parse_inner_inner_ipv6;
            default : accept;
       }
    ...
    state extract_inner_gtp_c_scope0 {
        eg_md.tunnel_2.type = SWITCH_TUNNEL_TYPE_GTPC;
     transition accept;
    }
    ...
    state extract_inner_gtp_u_scope0 {
        pkt.extract(hdr.inner.gtp_v1_base);
        eg_md.tunnel_2.type = SWITCH_TUNNEL_TYPE_GTPU;
        transition select(pkt.lookahead<bit<4>>()) {
            4: parse_inner_inner_ipv4;
            6: parse_inner_inner_ipv6;
            default: accept;
    ...
    state parse_inner_inner_ipv4 {
        eg_md.tunnel_3.hdr_3_ipv4_isValid = true;
  transition accept;
    }
    state parse_inner_inner_ipv6 {
        eg_md.tunnel_3.hdr_3_ipv6_isValid = true;
  transition accept;
    }
...
control TunnelDecapFixEthertype(
 ...
 action fix_l2_vlan_tag1(bit<16> ether_type) {
  hdr_1.vlan_tag[1].ether_type = ether_type;
 }

 action fix_l2_vlan_tag0(bit<16> ether_type) {
  hdr_1.vlan_tag[0].ether_type = ether_type;
 }

 action fix_l2_vn_tag(bit<16> ether_type) {
  hdr_1.vn_tag.ether_type = ether_type;
 }

 action fix_l2_e_tag(bit<16> ether_type) {
  hdr_1.e_tag.ether_type = ether_type;
 }

 action fix_l2_eth(bit<16> ether_type) {
  hdr_1.ethernet.ether_type = ether_type;
 ...
 table fix_l2 {
  ...
  const entries = {
   ...
   (false, false, false, _, _, _, _, true, _, _, _, _ ) : fix_l2_eth (0x0800); // do   check for eth true 
   (false, false, false, _, _, _, _, false, _, _, _, _ ) : fix_l2_eth (0x86dd); // do   check for eth true

   // 0 terminates (rm  hdr1's l2, since hdr2 has one)
   (false, false, true, _, _, _, _, _, _, _, _, _ ) : NoAction ();

   // 1 terminate  (fix hdr1's l2 etype, based on hdr2's ip type)
   (true, false, _, _, _, _, true, _, false, true, _, _ ) : fix_l2_vlan_tag1(0x0800);
   (true, false, _, _, _, _, true, _, false, false, _, _ ) : fix_l2_vlan_tag1(0x86dd);
   (true, false, _, _, _, true, false, _, false, true, _, _ ) : fix_l2_vlan_tag0(0x0800);
   (true, false, _, _, _, true, false, _, false, false, _, _ ) : fix_l2_vlan_tag0(0x86dd);
   (true, false, _, _, true, false, false, _, false, true, _, _ ) : fix_l2_vn_tag (0x0800);
   (true, false, _, _, true, false, false, _, false, false, _, _ ) : fix_l2_vn_tag (0x86dd);
   (true, false, _, true, false, false, false, _, false, true, _, _ ) : fix_l2_e_tag (0x0800);
   (true, false, _, true, false, false, false, _, false, false, _, _ ) : fix_l2_e_tag (0x86dd);
   (true, false, _, false, false, false, false, _, false, true, _, _ ) : fix_l2_eth (0x0800); // don't check for eth true...it might not be
   (true, false, _, false, false, false, false, _, false, false, _, _ ) : fix_l2_eth (0x86dd); // don't check for eth true...it might not be

   // 1 terminate  (rm  hdr1's l2, since hdr2 has one)
   (true, false, _, _, _, _, _, _, true, _, _, _ ) : fix_l2_decap ();

   // 2 terminates (fix hdr1's l2 etype, based on hdr3's ip type)
   (true, true, _, _, _, _, true, _, _, _, false, true ) : fix_l2_vlan_tag1(0x0800);
   (true, true, _, _, _, _, true, _, _, _, false, false) : fix_l2_vlan_tag1(0x86dd);
   (true, true, _, _, _, true, false, _, _, _, false, true ) : fix_l2_vlan_tag0(0x0800);
   (true, true, _, _, _, true, false, _, _, _, false, false) : fix_l2_vlan_tag0(0x86dd);
   (true, true, _, _, true, false, false, _, _, _, false, true ) : fix_l2_vn_tag (0x0800);
   (true, true, _, _, true, false, false, _, _, _, false, false) : fix_l2_vn_tag (0x86dd);
   (true, true, _, true, false, false, false, _, _, _, false, true ) : fix_l2_e_tag (0x0800);
   (true, true, _, true, false, false, false, _, _, _, false, false) : fix_l2_e_tag (0x86dd);
   (true, true, _, false, false, false, false, _, _, _, false, true ) : fix_l2_eth (0x0800); // don't check for eth true...it might not be
   (true, true, _, false, false, false, false, _, _, _, false, false) : fix_l2_eth (0x86dd); // don't check for eth true...it might not be

   // 2 terminates (rm  hdr1's l2, since hdr3 has one)
   (true, true, _, _, _, _, _, _, _, _, true, _ ) : fix_l2_decap ();
  }
  const default_action = NoAction;
 }
...
//	TunnelDecapTransportIngress(switch_tunnel_mode_t.PIPE) tunnel_decap_transport_ingress;
 ...
 apply {
  ...
  // -----------------------------------------------------------------
  // Non-scoped signals, needed later for decaps
  // -----------------------------------------------------------------
  ig_md.tunnel_3.hdr_3_ipv4_isValid = true; // TODO: Need the parser to set this (v4 vs v6)!
  // -----------------------------------------------------------------
  // Set Initial Scope
  // -----------------------------------------------------------------
...
//		tunnel_encap_transport_ingress.apply(hdr_0, tunnel_0, hdr_1);
  ...
  TunnelDecapFixEthertype.apply(hdr_1, tunnel_1, hdr_2, tunnel_2, ig_md.tunnel_3.hdr_3_ipv4_isValid, ig_md.tunnel_3.hdr_3_ethernet_isValid);
...
control npb_egr_sf_proxy_hdr_strip (
 ...
 action hdr_strip_1__from_vlan_tag1__to_vlan_tag0() {
  hdr_1.vlan_tag[0].ether_type = hdr_1.vlan_tag[1].ether_type;
 ...
 action hdr_strip_2__from_vlan_tag1__to_vlan_tag0_a() {
  // tag1 to tag0
  hdr_1.vlan_tag[0].ether_type = hdr_1.vlan_tag[1].ether_type;
  hdr_1.vlan_tag[1].setInvalid();
  // vn to eth
 ...
 action hdr_strip_2__from_vlan_tag1__to_vlan_tag0_b() {
  // tag1 to tag0
  hdr_1.vlan_tag[0].ether_type = hdr_1.vlan_tag[1].ether_type;
  hdr_1.vlan_tag[1].setInvalid();
  // e  to eth
 ...
 table hdr_strip {
  ...
  actions = {
   NoAction();
   hdr_strip_1__from_vlan_tag1__to_vlan_tag0();
   ...
   hdr_strip_2__from_vlan_tag1__to_vlan_tag0_a();
   hdr_strip_2__from_vlan_tag1__to_vlan_tag0_b();
   hdr_strip_2__from_vlan_tag0__to_eth();
  }
  const entries = {

   // My notes, on a complete truth table for just two things (vlan and vn)
   // ================     ====================
   // Pkt {vlan, vn}       Enables {vlan, vn}
   // ================     ====================
   // (false, false)       (false, false)          --> no action            (nothing enabled + nothing in pkt)
   // (false, false)       (false, true )          --> no action            (nothing in pkt)
   // (false, false)       (true,  false)          --> no action            (nothing in pkt)
   // (false, false)       (true,  true )          --> no action            (nothing in pkt)
   // ----------------     --------------------
   // (false, true )       (false, false)          --> no action            (nothing enabled)
   // (false, true )       (false, true )          --> VN to ETH   : case 1
   // (false, true )       (true,  false)          --> no action            (pkt vs enble opposite)
   // (false, true )       (true,  true )          --> VN to ETH   : case 1
   // ----------------     --------------------
   // (true,  false)       (false, false)          --> no action            (nothing enabled)
   // (true,  false)       (false, true )          --> no action            (pkt vs enble opposite)
   // (true,  false)       (true,  false)          --> VLAN to ETH : case 2
   // (true,  false)       (true,  true )          --> VLAN to ETH : case 2
   // ----------------     --------------------
   // (true,  true )       (false, false)          --> no action            (nothing enabled)
   // (true,  true )       (false, true )          --> VN to ETH   : case 3
   // (true,  true )       (true,  false)          --> VLAN to VN  : case 4
   // (true,  true )       (true,  true )          --> VLAN to ETH : case 5 (double delete case)

   // =========================     ====================
   // Pkt                           Enables
   // =========================     ====================
   // case 1 (single vn/e delete, only vn/e          valid):
   (_, false, true, false, _, true, _ ): hdr_strip_1__from_vn_tag_____to_eth(); // pkt: 0   tags, vn --> vn   to eth
   (_, false, false, true, _, _, true ): hdr_strip_1__from_e_tag______to_eth(); // pkt: 0   tags, e  --> e    to eth
   // case 2 (single vlan delete, only vlan          valid):
   (true, true, false, false, true, _, _ ): hdr_strip_1__from_vlan_tag1__to_vlan_tag0(); // pkt: 2   tags, -  --> vlan to vlan
   (false, true, false, false, true, _, _ ): hdr_strip_1__from_vlan_tag0__to_eth(); // pkt: 1   tag,  -  --> vlan to eth
   // case 3 (single vn/e delete, both vlan and vn/e valid):
   (_, true, true, false, false, true, _ ): hdr_strip_1__from_vn_tag_____to_eth(); // pkt: 1-2 tags, vn --> vn   to eth
   (_, true, false, true, false, _, true ): hdr_strip_1__from_e_tag______to_eth(); // pkt: 1-2 tags, e  --> e    to eth
   // case 4 (single vlan delete, both vlan and vn/e valid):
   (true, true, true, false, true, false, _ ): hdr_strip_1__from_vlan_tag1__to_vlan_tag0(); // pkt: 2   tags, vn --> vlan to vlan
   (false, true, true, false, true, false, _ ): hdr_strip_1__from_vlan_tag0__to_vn_tag(); // pkt: 1   tag,  vn --> vlan to vn
   (true, true, false, true, true, _, false): hdr_strip_1__from_vlan_tag1__to_vlan_tag0(); // pkt: 2   tags, e  --> vlan to vlan
   (false, true, false, true, true, _, false): hdr_strip_1__from_vlan_tag0__to_e_tag(); // pkt: 1   tag,  e  --> vlan to e
   // case 5 (double delete cases):
   (true, true, true, false, true, true, _ ): hdr_strip_2__from_vlan_tag1__to_vlan_tag0_a(); // pkt: 2   tags, vn --> vlan to vlan, vn to eth (delete both vlan1 and vn)
   (false, true, true, false, true, true, _ ): hdr_strip_2__from_vlan_tag0__to_eth(); // pkt: 1   tag,  vn --> vlan to eth             (delete both vlan0 and vn)
   (true, true, false, true, true, _, true ): hdr_strip_2__from_vlan_tag1__to_vlan_tag0_b(); // pkt: 2   tags, e  --> vlan to vlan, e  to eth (delete both vlan1 and e )
   (false, true, false, true, true, _, true ): hdr_strip_2__from_vlan_tag0__to_eth(); // pkt: 1   tag,  e  --> vlan to eth             (delete both vlan0 and e )
  }
 }
...
control npb_egr_top (
 ...
 apply {
  ...
  // Non-scoped signals, needed later for decaps
  // -------------------------------------
  eg_md.tunnel_3.hdr_3_ipv4_isValid = true; // TODO: Need the parser to set this (v4 vs v6)!
  // -------------------------------------
...
//			eg_md.lkp_1.tunnel_inner_type = eg_md.tunnel_1.type;
  ...
  TunnelDecapFixEthertype.apply(hdr_1, tunnel_1, hdr_2, tunnel_2, eg_md.tunnel_3.hdr_3_ipv4_isValid, eg_md.tunnel_3.hdr_3_ethernet_isValid);
...
//		unicast.apply(hdr.transport, ig_md);
 ...
 // the new parser puts bridging in outer
  ...
  if (ig_md.lkp_1.ip_type == SWITCH_IP_TYPE_NONE)
   compute_non_ip_hash(ig_md.lkp_1, ig_md.hash);
  else
   compute_ip_hash(ig_md.lkp_1, ig_md.hash);

//		nexthop.apply(ig_md.lkp_1, ig_md, ig_md.hash[15:0]);
  nexthop.apply(ig_md.lkp_1, ig_md, ig_md.hash[(32/2-1):0]);
//		outer_fib.apply(ig_md, ig_md.hash[31:16]);
  outer_fib.apply(ig_md, ig_md.hash[32 -1:32/2]);
...
//			lag.apply(ig_md, ig_md.hash[31:16], ig_intr_md_for_tm.ucast_egress_port);
  ...
  tunnel_encap_transport_ingress.apply(hdr.transport, ig_md.tunnel_0);
