...
header MemcachedHdr_t {
    ...
    bit<8> extrasLen;
    bit<8> dataType;
    bit<16> vBucketIdOrStatus; // vbucket ID in request, status in response
    bit<32> totalBodyLen;
    bit<32> opaque;
    bit<64> CAS;
}
...
control MemcachedIngressDeparser(
    packet_out pkt,
    inout headers_t hdr,
    in metadata_t ig_md,
...
control MemcachedEgressDeparser(
    packet_out pkt,
    inout headers_t hdr,
    in metadata_t eg_md,
...
Pipeline(
    MemcachedIngressParser(),
    MemcachedIngress(),
    MemcachedIngressDeparser(),
    MemcachedEgressParser(),
    MemcachedEgress(),
    MemcachedEgressDeparser()) pipe0;
Switch(pipe0) main;
