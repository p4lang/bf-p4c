...
const ping_pong_t ping_pong_ghost_write = 1;
...
//-----------------------------------------------------------------------------
control GhostThreadSfcInit(in ghost_intrinsic_metadata_t g_intr_md,
                  out ghost_metadata_t g_md)
                  (const_t queue_idx_size) {

    action copy_intr_md() {
        g_md = {
            (ping_pong_t)g_intr_md.ping_pong,
            (pipe_id_t)g_intr_md.pipe_id,
...
//-----------------------------------------------------------------------------
control GhostThreadGhostWriteQueueBpUtilization(in ghost_metadata_t g_md) {
    int<32> qd_delta = 0;
...
//-----------------------------------------------------------------------------
// A set of registers to count the ghost messages per queue and total. Very
// helpful for monitoring and debugging purposes. This control writes the
// data from the Ghost Thread to the shared registers.
//
// @param g_md : The ghost meta data
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// A set of registers to count the ghost messages per queue and total. Very
// helpful for monitoring and debugging purposes. This control reads the
// counters from the Ingress Thread.
//
// @param sfc : The sfc meta data
//-----------------------------------------------------------------------------
control IngressThreadReadGhostMessageCounter(inout switch_sfc_ingress_metadata_t sfc) {
    ...
    RegisterAction<bit<32>, bit<1>, bit<32>>(sfc_ping_ghost_total_cnt_reg) ping_ghost_total_cnt_read = {
...
control IngressThreadSfcInit(in ghost_intrinsic_metadata_t g_intr_md,
    ...
    * ig_md.port
    * ig_md.qos.qid: map to q_idx together with port, match concurrently
    * ig_md.qos.tc: Use to activate/deactivate sfc trigger together with port, match sequentually
    *
...
control SwitchGhost(in ghost_intrinsic_metadata_t g_intr_md) {
    ghost_metadata_t g_md;

    GhostThreadSfcInit(SFC_QUEUE_IDX_SIZE) sfc_init;
    GhostThreadWriteGhostMessageCounter() write_ghost_msg_counter;
    GhostThreadGhostWriteQueueBpUtilization() write_queue_buffer_util;
