    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    action acl_mirror(MirrorId_t mirror_session) {
        ...
        meta.ing_port_mirror.header_info =
                                              (header_info_t)ING_PORT_MIRROR;
        meta.ing_port_mirror.pad0           = 0;
        meta.ing_port_mirror.ingress_port   = ig_intr_md.ingress_port;
        meta.ing_port_mirror.pad1           = 0;
    ...
    out egress_intrinsic_metadata_t  eg_intr_md)
{
    ...
    state start {
        ...
        meta.header_type = pkt.lookahead<inthdr_h>().header_type;
        meta.header_info = pkt.lookahead<inthdr_h>().header_info;
        transition select(meta.header_type, meta.header_info) {
            (HEADER_TYPE_BRIDGE,         _):
                           parse_bridge;
            (HEADER_TYPE_MIRROR_INGRESS, (bit<4>)ING_PORT_MIRROR):
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    action send_to_cpu() {
        ...
        hdr.to_cpu.setValid();
        hdr.to_cpu.mirror_type     =
            meta.ing_port_mirror.header_type ++
            meta.ing_port_mirror.header_info;
        hdr.to_cpu.mirror_session  =
                          (bit<16>)meta.ing_port_mirror.mirror_session;
        hdr.to_cpu.ingress_port    =
                          (bit<16>)meta.ing_port_mirror.ingress_port;
