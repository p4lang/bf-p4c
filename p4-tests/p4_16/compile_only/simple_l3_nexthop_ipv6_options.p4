    ...
    out ingress_intrinsic_metadata_t  ig_intr_md)
{
    ...
    state parse_ipv6_ext {
        ...
        transition select(hdr.ipv6_ext.hdr_ext_len) {
            0x00 &&& 0xF8 : parse_ipv6_options;
            8 : parse_ipv6_options;
            default: parse_ipv6_options_partial;
        }
    }

    state parse_ipv6_options {
        pkt.extract(hdr.ipv6_options, (bit<32>)hdr.ipv6_ext.hdr_ext_len*8+48);
        transition accept;
    }

    state parse_ipv6_options_partial {
        pkt.extract(hdr.ipv6_options, hdr.ipv6_options.minSizeInBits());
        transition accept;
    }
}
