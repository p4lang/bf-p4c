...
/* 
 * The maximum number of hash bits we can calculate per single table, action
 * or expression is 32. If you need more, you need to manually split the 
 * calculation. Thus, we'll calculate 32 bits, although we do not needs
 * that many (see HASH_WIDTH above).  
 */
typedef bit<32> selector_hash_t;
    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    Hash<selector_hash_t>(HashAlgorithm_t.CRC32) ipv4_hash_calc;
    Hash<selector_hash_t>(HashAlgorithm_t.CRC32) ipv6_hash_calc;
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            /* Calculate the hash for IPv4 packets */
            hash = ipv4_hash_calc.get({
                    hdr.ipv4.src_addr,
                    hdr.ipv4.dst_addr,
                    hdr.ipv4.protocol,
            ...
            /* Perform IPv4 destination lookup */
           ...
           /* Calculate the hash for IPv6 packets */
            hash = ipv6_hash_calc.get({
                    hdr.ipv6.src_addr,
                    hdr.ipv6.dst_addr,
                    hdr.ipv6.next_hdr,
                    meta.l4_lookup.word_1,
            ...
            /* Perform IPv6 destination lookup */
        ...
        /* 
         * This one-liner will do all the work, since the table nexthop
         * is implemented using ActionProfile() and ActionSelector() externs
         */
        nexthop.apply();
