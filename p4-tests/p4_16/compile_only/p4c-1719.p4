...
header bridge_h {
    ...
    @pa_container_size("ingress", "hdr.bridge.ingress_port",  16)
    @pa_container_size("egress",  "meta.bridge.ingress_port", 16)
    bit<7>    pad0;
    ...
    out ingress_intrinsic_metadata_t  ig_intr_md)
{
    ...
    state parse_ethernet {
        ...
        transition select(hdr.ethernet.ether_type) {
            ETHERTYPE_TPID &&& 0xEFFF: parse_vlan_tag;
            default: accept;
        }
    }
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    action add_vlan_tag() {
        ...
        hdr.ethernet.ether_type = ETHERTYPE_TPID;
        modify_vlan_tag();
    }
    ...
    table egr_vlan_encap {
        key = {
            hdr.vlan_tag.isValid() : exact;
            send_egress_tagged     : exact;
        }
        actions = {
            add_vlan_tag; remove_vlan_tag; modify_vlan_tag; NoAction;
        }
        const entries = {
            (false, false): NoAction();
            (false,  true): add_vlan_tag();
            (true,  false): remove_vlan_tag();
            (true,   true): modify_vlan_tag();
        }
        size = 4;
    }
