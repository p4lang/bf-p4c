    ...
    inout ingress_intrinsic_metadata_for_tm_t ig_tm_md)
{
  ...
  Hash<bit<16>>(HashAlgorithm_t.CRC16) hasher11;
#endif
...
#if __TARGET_TOFINO__ == 1
    /* -- Hasher used in the apply block (special handling in the normalize pass).
     *    An action with no key and containing a hash is supported only by Tofino1. */
    hdr.result.result11 = hasher11.get({
        hdr.test.field1,
        hdr.test.field2,
        hdr.test.field1 & hdr.test.field2,
        32w0xaa77aa77});
#else
    hdr.result.result11 = 0x8ca4;
#endif
