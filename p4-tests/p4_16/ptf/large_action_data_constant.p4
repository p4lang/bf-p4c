...
struct headers_t {
    ethernet_t ethernet;
    // we make sure that my_hdr.f2 is placed in a 16-bit PHV container: this way
    // it doesn't occupy the whole container and if we write a large constant to
    // it, the constant won't come from the instruction itself
...
control ingress(inout headers_t hdr, inout metadata_t meta,
    ...
    action set_f2_miss() {
        standard_metadata.egress_spec = standard_metadata.ingress_port;
        // using a smaller constant (e.g 1) works because the constant then comes
        // directly from the instruction.
        hdr.my_hdr.f2 = 199;
    }

    table t {
        key = { hdr.my_hdr.f1 : exact; }
        actions = { set_f2_hit; set_f2_miss; }
