...
//      hdr.outer.dtel.timestamp = (bit<32>) eg_md.ingress_timestamp;
    ...
    table config {
        key = {
            ...
            hdr.outer.ipv4.isValid() : ternary;
            hdr.outer.ipv6.isValid() : ternary;
#endif
#ifdef DTEL_IFA_EDGE
            hdr.outer.ipv4.tos[7:2] : ternary;
            hdr.outer.ipv6.tos[7:2] : ternary;
#endif
...
control EgressDtel(inout switch_header_t hdr,
    ...
    apply {
        ...
        if (eg_md.pkt_src == SWITCH_PKT_SRC_DEFLECTED && hdr.outer.dtel_drop_report.isValid())
            eg_md.port = hdr.outer.dtel_report.egress_port;
