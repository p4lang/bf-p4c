...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_nsh_chain_middle_add(self, self.target,
			#ingress
			[ig_port_1], ig_lag_ptr+1, 0,           spi, si-(popcount(sf_bitmask_0)),                          sf_bitmask_1, rmac, nexthop_ptr+1, bd, eg_lag_ptr+1, 0+1, 0+1, [1        ], 0, dsap,
			#tunnel
...
#		testutils.verify_no_other_packets(self, 0, 1)
		...
		src_pkt, exp_pkt = npb_simple_1lyr_udp(
			spi=spi, si=si-(popcount(sf_bitmask_0)), sap=sap, vpn=vpn, ttl=62, scope=0,
			dmac=dmac, smac=smac,
...
#		testutils.send_packet(self, ig_port_2, str(src_pkt))
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_0], spi, si,                                                   sf_bitmask_0, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, 1, [1        ],
			#tunnel
		...
		npb_nsh_chain_middle_del(self, self.target,
			#ingress
			[ig_port_1], spi, si-(popcount(sf_bitmask_0)),                          sf_bitmask_1, rmac, nexthop_ptr+1, eg_lag_ptr+1, 0+1, 0+1, 1, [1        ],
			#tunnel
		...
		npb_nsh_chain_end_del(self, self.target,
			#ingress
			[ig_port_2], spi, si-(popcount(sf_bitmask_0))-(popcount(sf_bitmask_1)), sf_bitmask_2, rmac, nexthop_ptr+2, eg_lag_ptr+2, 0+2, 0+2, 1, [eg_port_2]
			#egress
		)
