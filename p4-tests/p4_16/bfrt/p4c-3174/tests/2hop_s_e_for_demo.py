...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		ig_port_3 = swports[1] # derek changed from [4] so that this test can run on the hw, which only has 4 cpu ports total.
		eg_port_3 = swports[1] # derek changed from [4] so that this test can run on the hw, which only has 4 cpu ports total.
...
#		sf_bitmask_0            = 1 # Bit 0 = ingress, bit 1 = multicast, bit 2 = egress
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port_0], ig_lag_ptr+0, 0, sap, vpn, spi_0, si_0,                          sf_bitmask_0, rmac, nexthop_ptr+0, bd, eg_lag_ptr+0, 0+0, 0+0, [eg_port_3], 0, dsap,
			#tunnel
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port_1], ig_lag_ptr+1, 0, sap, vpn, spi_1, si_1,                          sf_bitmask_0, rmac, nexthop_ptr+1, bd, eg_lag_ptr+1, 0+1, 0+1, [eg_port_3], 0, dsap,
			#tunnel
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port_2], ig_lag_ptr+2, 0, sap, vpn, spi_2, si_2,                          sf_bitmask_0, rmac, nexthop_ptr+2, bd, eg_lag_ptr+2, 0+2, 0+2, [eg_port_3], 0, dsap,
			#tunnel
...
#		testutils.verify_packets(self, exp_pkt, [eg_port_3])
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_0], spi_0, si_0,                          sf_bitmask_0, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, 1, [eg_port_0],
			#tunnel
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_1], spi_1, si_1,                          sf_bitmask_0, rmac, nexthop_ptr+1, eg_lag_ptr+1, 0+1, 0+1, 1, [eg_port_1],
			#tunnel
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_2], spi_2, si_2,                          sf_bitmask_0, rmac, nexthop_ptr+2, eg_lag_ptr+2, 0+2, 0+2, 1, [eg_port_2],
			#tunnel
		...
		npb_nsh_chain_end_del(self, self.target,
			#ingress
			[ig_port_3], spi_0, si_0-(popcount(sf_bitmask_0)), sf_bitmask_1, rmac, nexthop_ptr+8, eg_lag_ptr+8, 0+8, 0+8, 1, [eg_port_3]
			#egress
		)

		npb_nsh_chain_end_del(self, self.target,
			#ingress
			[ig_port_3], spi_1, si_1-(popcount(sf_bitmask_0)), sf_bitmask_1, rmac, nexthop_ptr+9, eg_lag_ptr+9, 0+9, 0+9, 1, [eg_port_3]
			#egress
		)

		npb_nsh_chain_end_del(self, self.target,
			#ingress
			[ig_port_3], spi_2, si_2-(popcount(sf_bitmask_0)), sf_bitmask_1, rmac, nexthop_ptr+10, eg_lag_ptr+10, 0+10, 0+10, 1, [eg_port_3]
			#egress
		)
...
#		npb_npb_sf0_policy_l34_v4_del(self, self.target, sap=sap, l3_proto=6, l3_proto_mask=0xff, l4_src=443, l4_src_mask=0xffff) # hop 0 - reclassify to drop
		...
		npb_npb_sfp_sel_del(self, self.target, vpn, sfc_0, 0, 0+0, 1) # hop 0
		npb_npb_sfp_sel_del(self, self.target, vpn, sfc_1, 0, 0+1, 1) # hop 0 (port 80)
		npb_npb_sfp_sel_del(self, self.target, vpn, sfc_2, 0, 0+2, 1) # hop 0 (port 443)
