...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port_0], ig_lag_ptr+0, 0, sap, vpn, spi, si,                          sf_bitmask_0, rmac, nexthop_ptr+0, bd, eg_lag_ptr+0, 0+0, 0+0, [eg_port_0], 0, dsap,
			#tunnel
...
#		exp_pkt = testutils.simple_udp_packet(eth_dst=dmac)
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_0], spi, si,                          sf_bitmask_0, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, 1, [eg_port_0],
			#tunnel
		...
		npb_nsh_chain_end_del(self, self.target,
			#ingress
			[ig_port_1], spi, si-(popcount(sf_bitmask_0)), sf_bitmask_1, rmac, nexthop_ptr+1, eg_lag_ptr+1, 0+1, 0+1, 1, [eg_port_1]
			#egress
		)
