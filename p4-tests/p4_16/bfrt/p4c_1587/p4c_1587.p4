...
parser SwitchEgressParser(
    ...
    state parse_bridged_metadata {
        ...
        eg_md.flags.capture_ts = hdr.bridged_md.capture_ts;
...
control MirrorRewrite(inout switch_header_t hdr,
                      ...
                      inout switch_pkt_length_t pktLength) {
    bit<16> length;
    ...
    action rewrite_erspan_type3(
        ...
        add_ipv4_header(
            0, 8w64, 47, sip, dip);
        hdr.ipv4.total_len = pktLength + 16w24;
    ...
    action rewrite_erspan_type3_platform_specific(
        ...
        add_ipv4_header(
            0, 8w64, 47, sip, dip);
        hdr.ipv4.total_len = pktLength + 16w32;
        hdr.inner_ethernet = hdr.ethernet;
        add_ethernet_header(smac, dmac, 0x0800);
    }
    table rewrite {
    ...
    action adjust_length(bit<16> length_offset) {
        pktLength = pktLength + length_offset;
    }
    table pkt_length {
