...
// define to enable parser extraction of the outermost L2 underlay (versus dropping it)
...
// Generic Routing Encapsulation (GRE) -- RFC 1701/2784/2890
header gre_h {
    bit<1> C;
...
header nsh_extr_h {
 ...
 bit<8> ver; // word 0
 bit<1> scope; // word 0
 bit<7> reserved4; // word 0
 bit<16> ifindex; // word 0

 bit<16> vpn_id; // word 1
 bit<16> sfc_data; // word 1

 bit<8> ifclass; // word 2
 bit<24> reserved5; // word 2
...
//  bool                            terminate;              // set by sff
    ...
    bit<8> sf_bitmask; //  1 byte
    bit<8> flow_class; //  1 byte?

    // ---------------
    // pkt data
    // ---------------

 // ANY NSH DATA THAT MAY CHANGE IN THE *INGRESS* GOES HERE (TO KEEP MIRRORING WORKING).
 // NOTE: ANY NSH DATA THAT MAY CHNAGE IN THE *EGRESS*, DOESN'T NEED TO GO HERE
 // (SOME EXAMPLES INCLUDE HEADER VERSIONS, TTL, NEXT PROTO, ETC).
...
header snoop_ipv4_untagged_h {
    mac_addr_t enet_dst_addr;
    mac_addr_t enet_src_addr;
    bit<16> enet_ether_type;

    bit<4> ipv4_version;
    bit<4> ipv4_ihl; // lookahead<bit<120>>()[3:0]
    bit<8> ipv4_diffserv;
    bit<16> ipv4_total_len;
    bit<16> ipv4_identification;
    bit<3> ipv4_flags; // lookahead<bit<163>>()[2:0]
    bit<13> ipv4_frag_offset; // lookahead<bit<176>>()[12:0]
    bit<8> ipv4_ttl;
    bit<8> ipv4_protocol; // lookahead<bit<192>>()[7:0]

} // 14B + 10B = 24B


header snoop_ipv4_tagged_h {
    mac_addr_t enet_dst_addr;
    mac_addr_t enet_src_addr;
    bit<16> enet_ether_type;
    ...
    bit<4> ipv4_version;
    bit<4> ipv4_ihl; // lookahead<bit<152>>()[3:0]
    bit<8> ipv4_diffserv;
    bit<16> ipv4_total_len;
    bit<16> ipv4_identification;
    bit<3> ipv4_flags; // lookahead<bit<195>>()[2:0]
    bit<13> ipv4_frag_offset; // lookahead<bit<208>>()[12:0]
    bit<8> ipv4_ttl;
    bit<8> ipv4_protocol; // lookahead<bit<224>>()[7:0]
...
header snoop_gre_untagged_h {
    ...
    // bit<1> gre_C;       // lookahead<bit<161>>()[0:0]
    // bit<1> gre_R;
    // bit<1> gre_K;       // lookahead<bit<163>>()[0:0]
    // bit<1> gre_S;       // lookahead<bit<164>>()[0:0]
    bit<4> gre_CRKS; // lookahead<bit<164>>()[3:0]
    bit<1> gre_s;
    bit<3> gre_recurse;
    bit<5> gre_flags;
    bit<3> gre_version; // lookahead<bit<176>>()[2:0]
    bit<16> gre_proto; // lookahead<bit<192>>()[15:0]

} // 20B + 4B = 24B

header snoop_gre_tagged_h {
    bit<3> vlan_pcp;
    bit<1> vlan_cfi;
    vlan_id_t vlan_vid;
    bit<16> vlan_ether_type;
    ...
    // bit<1> gre_C;       // lookahead<bit<193>>()[0:0]
    // bit<1> gre_R;
    // bit<1> gre_K;       // lookahead<bit<195>>()[0:0]
    // bit<1> gre_S;       // lookahead<bit<196>>()[0:0]
    bit<4> gre_CRKS; // lookahead<bit<196>>()[3:0]
    bit<1> gre_s;
    bit<3> gre_recurse;
    bit<5> gre_flags;
    bit<3> gre_version; // lookahead<bit<208>>()[2:0]
...
// Extreme Networks - Added
// -------------------------------------
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_NSH = 3; // extreme added
...
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_ERSPAN = 7; // extreme added
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_GRE = 8; // extreme added
...
struct switch_ingress_flags_t {
    bool transport_ipv4_checksum_err;
...
struct switch_bridged_metadata_t {
    ...
    bit<8> nsh_extr_sf_bitmask; //  1 byte
    bit<8> nsh_extr_flow_class; //  1 byte?
    ...
    // ext: type 1 - word 0-3
 bit<1> nsh_extr_scope;
 bit<16> nsh_extr_ifindex;
    bit<16> nsh_extr_vpn_id; //  3 bytes
 bit<16> nsh_extr_sfc_data;
 bit<8> nsh_extr_ifclass;
...
struct switch_ingress_metadata_t {
    ...
    switch_tunnel_metadata_t tunnel_transport; // extreme added
...
struct switch_header_transport_t {
    ...
    ethernet_h ethernet;
    vlan_tag_h[2] vlan_tag; //todo: encap doc shows only single tag
    nsh_extr_h nsh_extr;

    ipv4_h ipv4;
    ipv4_option_h ipv4_option; // todo: no ipv4 options shown in encap doc
    ipv6_h ipv6; // todo: no v6 transport shown in encap doc
    udp_h udp; // todo: no longer supported
    tcp_h tcp; // todo: no tcp transport shown in encap doc

    vxlan_h vxlan; // todo: no longer supported
    gre_h gre;
    gre_extension_sequence_h gre_extension_sequence;
    nvgre_h nvgre; // todo: no nvgre transport shown in encap doc
    erspan_type2_h erspan_type2;
    erspan_type3_h erspan_type3; // todo: remove - not supported
}
...
const bit<32> NPB_ING_SFC_INTF_CLASS_TABLE_DEPTH = 2048;
const bit<32> NPB_ING_SFC_VPN_ID_CLASS_TABLE_DEPTH = 8192;
const bit<32> NPB_ING_SFC_FLOWCLASS_CLASS_TABLE_DEPTH = 4096;
const bit<32> NPB_ING_SFC_SFC_TABLE_DEPTH = 8192;
const bit<32> NPB_ING_SFC_SF_SEL_TABLE_DEPTH = 8192; // derek, what size to make this?

// sf #0 -- basic / advanced
const bit<32> NPB_ING_SF_0_BAS_ADV_POLICY_ACT_SEL_TABLE_DEPTH = 1024;
...
const bit<32> NPB_ING_SF_0_BAS_ADV_POLICY_L3_V4_TABLE_DEPTH = 1024;
const bit<32> NPB_ING_SF_0_BAS_ADV_POLICY_L3_V6_TABLE_DEPTH = 1024;
const bit<32> NPB_ING_SF_0_BAS_ADV_POLICY_L7_TABLE_DEPTH = 1024;

// sff -- forwards the packets to the sf's, then forwards to the packet along the chain.
const bit<32> NPB_ING_SFF_SCHD_TABLE_PART1_DEPTH = 2048;
const bit<32> NPB_ING_SFF_SCHD_TABLE_PART2_DEPTH = 8192; // for action_profile type table
...
const bit<32> NPB_EGR_SF_2_EGRESS_SFP_TRUNC_TABLE_DEPTH = 8;
...
action add_bridged_md(
    ...
    bridged_md.base = {
        ...
        ig_md.nsh_extr.sf_bitmask, //  1 byte
        ig_md.nsh_extr.flow_class, //  1 byte?
        ...
        // ext: type 1 - word 0-3
  ig_md.nsh_extr.scope,
  ig_md.nsh_extr.ifindex,
        ig_md.nsh_extr.vpn_id, //  3 bytes
  ig_md.nsh_extr.sfc_data,
  ig_md.nsh_extr.ifclass
...
//            disable_learning ? SWITCH_DIGEST_TYPE_INVALID : ig_intr_md_for_dprsr.digest_type;
 ...
 // -----------------
    ...
    action redirect (
        ...
        ig_md.nsh_extr.flow_class = flow_class;
    }
 ...
 // -----------------
    ...
    action redirect_sfc (
        ...
        ig_md.nsh_extr.sfc_data = sfc_data;
 ...
 // -----------------
    ...
    action redirect_trunc (
        ...
        ig_intr_md_for_dprsr.mtu_trunc_len = mtu_trunc_len;
 ...
 // -----------------
    ...
    table system_acl {
        key = {
            ig_md.port_lag_label : ternary;
            ig_md.bd_label : ternary;
//            ig_md.ifindex : ternary; // extreme removed (we include it below)
   ig_md.l4_port_label : ternary; // extreme added
...
//            ig_md.lkp.ip_ttl : ternary; // extreme removed
            ig_md.lkp.ip_proto : ternary;
//            ig_md.lkp.ip_frag : ternary; // extreme removed
            ig_md.lkp.ip_dst_addr : ternary;

            ig_md.lkp.l4_src_port : ternary;
            ig_md.lkp.l4_dst_port : ternary;
//            ig_md.lkp.arp_opcode : ternary; // extreme removed
   ...
   // ----- Extreme Added -----
            ...
            // metadata
            ig_md.nsh_extr.vpn_id : ternary;
            ig_md.nsh_extr.flow_class : ternary;
            length_bitmask : ternary;
            ig_md.nsh_extr.ifindex : ternary;
            ig_md.tunnel.type : ternary;
            ig_md.nsh_extr.ifclass : ternary;
        }

        actions = {
            NoAction;
            drop;
//            copy_to_cpu; // extreme removed
//            redirect_to_cpu; // extreme removed

            redirect(); // extreme added
            redirect_sfc(); // extreme added
            redirect_trunc(); // extreme added
            redirect_sfc_and_trunc(); // extreme added
        }
...
// MAC ACL
//-----------------------------------------------------------------------------
control EgressMacAcl(in switch_header_outer_t hdr,
                     inout switch_egress_metadata_t eg_md,
                     out switch_stats_index_t index)(
                     switch_uint32_t table_size=512) {
    table acl {
...
// IPv4 ACL
//-----------------------------------------------------------------------------
control EgressIpv4Acl(in switch_header_outer_t hdr,
                      in switch_lookup_fields_t lkp,
                      inout switch_egress_metadata_t eg_md,
                      out switch_stats_index_t index)(
                      switch_uint32_t table_size=512) {
...
// IPv6 ACL
//-----------------------------------------------------------------------------
control EgressIpv6Acl(in switch_header_outer_t hdr,
                      in switch_lookup_fields_t lkp,
                      inout switch_egress_metadata_t eg_md,
                      out switch_stats_index_t index)(
                      switch_uint32_t table_size=512) {
...
control EgressAcl(in switch_header_outer_t hdr,
                  in switch_lookup_fields_t lkp,
                  inout switch_egress_metadata_t eg_md)(
                  switch_uint32_t ipv4_table_size=512,
                  switch_uint32_t ipv6_table_size=512,
...
control NSHFixer(
 ...
 action set_type_2tag() {
  hdr.vlan_tag[1].ether_type = 0x894F;
 }

    table set_ether_type {
        key = {
   hdr.nsh_extr.isValid() : exact;
        ...
        const default_action = NoAction;
        const entries = {
            (true, false, false) : set_type_0tag();
            (true, true, false) : set_type_1tag();
            (true, true, true ) : set_type_2tag();
        }
    }
 ...
 action set_type_eth() {
  hdr.nsh_extr.next_proto = 0x3; // 1 = ipv4, 2 = ipv6, 3 = ethernet, 4 = nsh, 5 = mpls.
 }

 action set_type_v4() {
  hdr.nsh_extr.next_proto = 0x1; // 1 = ipv4, 2 = ipv6, 3 = ethernet, 4 = nsh, 5 = mpls.
 }

 action set_type_v6() {
  hdr.nsh_extr.next_proto = 0x2; // 1 = ipv4, 2 = ipv6, 3 = ethernet, 4 = nsh, 5 = mpls.
 }

    table set_nsh_type {
        key = {
//			hdr.nsh_extr.isValid()    : exact; // don't need this, as it doesn't matter if it's valid or not....
        ...
        const default_action = NoAction;
        const entries = {
            (false, false) : set_type_eth();
            (true, false) : set_type_v4();
            (true, true ) : set_type_v6();
        }
    }
...
control PktValidation(
        in switch_header_outer_t hdr,
        in switch_header_transport_t hdr_transport,
...
//-----------------------------------------------------------------------------
    ...
    action valid_unicast_pkt_e_tagged() {
        ...
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.e_tag.ether_type;
        lkp.pcp = hdr.e_tag.pcp;
    }

    action valid_multicast_pkt_e_tagged() {
        lkp.pkt_type = SWITCH_PKT_TYPE_MULTICAST;
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.e_tag.ether_type;
    ...
    action valid_broadcast_pkt_e_tagged() {
        lkp.pkt_type = SWITCH_PKT_TYPE_BROADCAST;
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.e_tag.ether_type;
    ...
    action valid_unicast_pkt_vn_tagged() {
        ...
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.vn_tag.ether_type;
        lkp.pcp = 0; // todo: acceptable?
    }

    action valid_multicast_pkt_vn_tagged() {
        lkp.pkt_type = SWITCH_PKT_TYPE_MULTICAST;
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.vn_tag.ether_type;
    ...
    action valid_broadcast_pkt_vn_tagged() {
        lkp.pkt_type = SWITCH_PKT_TYPE_BROADCAST;
        lkp.mac_src_addr = hdr.ethernet.src_addr;
        lkp.mac_dst_addr = hdr.ethernet.dst_addr;
        lkp.mac_type = hdr.vn_tag.ether_type;
    ...
    table validate_ethernet {
        ...
        actions = {
            ...
            valid_multicast_pkt_e_tagged;
            valid_broadcast_pkt_e_tagged;
            valid_unicast_pkt_vn_tagged;
            valid_multicast_pkt_vn_tagged;
            valid_broadcast_pkt_vn_tagged;
        }
...
//-----------------------------------------------------------------------------
  ...
  lkp.ip_len = hdr.ipv4.total_len; // extreme added
        malformed_pkt(reason);
    }
...
//-----------------------------------------------------------------------------
  ...
  lkp.ip_len = hdr.ipv6.payload_len; // extreme added
        malformed_pkt(reason);
    }
...
//-----------------------------------------------------------------------------
    ...
    table validate_other {
        ...
        actions = {
            ...
            set_tcp_ports;
            set_udp_ports;
            //set_sctp_ports; // causes us to blow out of phv
            set_icmp_type;
            set_igmp_type;
            set_arp_opcode;
        }
    ...
    table validate_nsh {
        ...
        key = {
            hdr_transport.nsh_extr.version : range;
            hdr_transport.nsh_extr.o : ternary;
            hdr_transport.nsh_extr.ttl : ternary;
            hdr_transport.nsh_extr.len : range;
            hdr_transport.nsh_extr.md_type : range;
            hdr_transport.nsh_extr.next_proto : range;
            hdr_transport.nsh_extr.si : ternary;
            //hdr_transport.nsh_extr.md_class : ternary;
            //hdr_transport.nsh_extr.type : ternary;
            //hdr_transport.nsh_extr.md_len : range;
            //hdr_transport.nsh_extr.xxx : ternary;
        }
...
*/
        ...
        if (hdr_transport.nsh_extr.isValid()) { // extreme added
            // --- PACKET HAS UNDERLAY ---
...
control InnerPktValidation(
        ...
        inout switch_lookup_fields_t lkp,
        inout switch_ingress_flags_t flags,
  inout bool flags_ipv4_checksum_err,
...
//            flags.inner_ipv4_checksum_err : ternary;
            flags_ipv4_checksum_err : ternary;
            hdr.ipv4.version : ternary;
            hdr.ipv4.ihl : ternary;
            hdr.ipv4.ttl : ternary;
        }
...
//-----------------------------------------------------------------------------
control IngressTunnel(in switch_header_transport_t hdr,
                      inout switch_ingress_metadata_t ig_md,
   ...
   bit<3> drop // extreme added
        ...
        tunnel.terminate = true;
  ig_intr_md_for_dprsr.drop_ctl = drop; // extreme added 
    }
...
//                            pkt_validation.apply(hdr, lkp, ig_md.flags, ig_md_flags_ipv4_checksum_err, ig_md.drop_reason);
                        }
...
// IP source and destination VTEP
//-----------------------------------------------------------------------------
control IngressTunnelInner(in switch_header_inner_t hdr,
                      inout switch_ingress_metadata_t ig_md,
                      inout bool ig_md_flags_ipv4_checksum_err,
                      inout switch_lookup_fields_t lkp,
                      inout switch_tunnel_metadata_t tunnel
...
/*
        ...
        ig_md.ipv6.multicast_snooping = mld_snooping_enable;
*/
//        ig_md.tunnel.terminate = true; // extreme modified
        tunnel.terminate = terminate;
    }

    table dst_vtep {
        key = {
            tunnel.type : exact;
            ig_md.port_lag_label : ternary; // extreme added
            tunnel.id : ternary; // extreme added
        }
    ...
    apply {
            ...
            switch(dst_vtep.apply().action_run) {
                dst_vtep_hit: {
                    vni_to_bd_mapping.apply();
                    pkt_validation.apply(hdr, ig_md.lkp, ig_md.flags, ig_md.flags.inner_ipv4_checksum_err, ig_md.drop_reason);
                }
                set_vni_properties: {
                    pkt_validation.apply(hdr, ig_md.lkp, ig_md.flags, ig_md.flags.inner_ipv4_checksum_err, ig_md.drop_reason);
                }
...
//
//-----------------------------------------------------------------------------
control TunnelDecap(inout switch_header_outer_t hdr,
                    inout switch_header_inner_t hdr_inner,
                    in switch_egress_metadata_t eg_md,
    ...
    action decap_inner_udp() {
        hdr.udp = hdr_inner.udp;
        hdr_inner.udp.setInvalid();
    }

    action decap_inner_tcp() {
        hdr.tcp = hdr_inner.tcp;
        hdr_inner.tcp.setInvalid();
        hdr.udp.setInvalid();
    }
    ...
    table decap_inner_l4 {
        key = { hdr_inner.udp.isValid() : exact; }
        actions = {
            decap_inner_udp;
            decap_inner_unknown;
        }
    ...
    action copy_ipv4_header() {
        hdr.ipv4.setValid();
        hdr.ipv4.version = hdr_inner.ipv4.version;
        hdr.ipv4.ihl = hdr_inner.ipv4.ihl;
        hdr.ipv4.total_len = hdr_inner.ipv4.total_len;
        hdr.ipv4.identification = hdr_inner.ipv4.identification;
        hdr.ipv4.flags = hdr_inner.ipv4.flags;
        hdr.ipv4.frag_offset = hdr_inner.ipv4.frag_offset;
        hdr.ipv4.protocol = hdr_inner.ipv4.protocol;
        // hdr.ipv4.hdr_checksum = hdr_inner.ipv4.hdr_checksum;
        hdr.ipv4.src_addr = hdr_inner.ipv4.src_addr;
        hdr.ipv4.dst_addr = hdr_inner.ipv4.dst_addr;

        if (mode == switch_tunnel_mode_t.UNIFORM) {
            // NoAction.
        } else if (mode == switch_tunnel_mode_t.PIPE) {
            hdr.ipv4.ttl = hdr_inner.ipv4.ttl;
            hdr.ipv4.diffserv = hdr_inner.ipv4.diffserv;
        }

        hdr_inner.ipv4.setInvalid();
    }

    action copy_ipv6_header() {
        hdr.ipv6.setValid();
        hdr.ipv6.version = hdr_inner.ipv6.version;
        hdr.ipv6.flow_label = hdr_inner.ipv6.flow_label;
        hdr.ipv6.payload_len = hdr_inner.ipv6.payload_len;
        hdr.ipv6.next_hdr = hdr_inner.ipv6.next_hdr;
        hdr.ipv6.src_addr = hdr_inner.ipv6.src_addr;
        hdr.ipv6.dst_addr = hdr_inner.ipv6.dst_addr;

        if (mode == switch_tunnel_mode_t.UNIFORM) {
            // NoAction.
        } else if (mode == switch_tunnel_mode_t.PIPE) {
            hdr.ipv6.hop_limit = hdr_inner.ipv6.hop_limit;
            hdr.ipv6.traffic_class = hdr_inner.ipv6.traffic_class;
        }

        hdr_inner.ipv6.setInvalid();
    }
    ...
    action decap_inner_ethernet_ipv4() {
        hdr.ethernet = hdr_inner.ethernet;
  hdr.vlan_tag[0] = hdr_inner.vlan_tag[0]; // extreme added
        copy_ipv4_header();
        hdr.ipv6.setInvalid();
        hdr_inner.ethernet.setInvalid();
        hdr_inner.vlan_tag[0].setInvalid(); // extreme added
        hdr.vlan_tag[1].setInvalid(); // extreme added
  ...
  hdr.vlan_tag[0] = hdr_inner.vlan_tag[0]; // extreme added
    ...
    table decap_inner_ip {
        key = {
            hdr_inner.ethernet.isValid() : exact;
            hdr_inner.ipv4.isValid() : exact;
            hdr_inner.ipv6.isValid() : exact;
        }
    ...
    apply {
        ...
        if (!(eg_md.bypass & SWITCH_EGRESS_BYPASS_REWRITE != 0) && tunnel.terminate)
...
//
//-----------------------------------------------------------------------------
control TunnelDecapTransport(inout switch_header_transport_t hdr,
    ...
    apply {
        ...
        if(tunnel.terminate == true) {

            // Just invalidate everything -- ingress deparser will then
            // take care of actually removing the header for us....

            hdr.ethernet.setInvalid(); // should we not decap this, since whatever we send out will always be ethernet?
            hdr.vlan_tag[0].setInvalid();
...
control TunnelRewrite(inout switch_header_transport_t hdr,
                      inout switch_egress_metadata_t eg_md,
                      in switch_tunnel_metadata_t tunnel
) (
...
//
//-----------------------------------------------------------------------------
control TunnelEncap(inout switch_header_transport_t hdr,
                    inout switch_header_outer_t hdr_outer,
                    inout switch_egress_metadata_t eg_md,
...
/*
        hdr_inner.ipv4.setValid();
        hdr_inner.ipv4.version = hdr.ipv4.version;
        hdr_inner.ipv4.ihl = hdr.ipv4.ihl;
        hdr_inner.ipv4.diffserv = hdr.ipv4.diffserv;
        hdr_inner.ipv4.total_len = hdr.ipv4.total_len;
        hdr_inner.ipv4.identification = hdr.ipv4.identification;
        hdr_inner.ipv4.flags = hdr.ipv4.flags;
        hdr_inner.ipv4.frag_offset = hdr.ipv4.frag_offset;
        hdr_inner.ipv4.ttl = hdr.ipv4.ttl;
        hdr_inner.ipv4.protocol = hdr.ipv4.protocol;
        // hdr_inner.ipv4.hdr_checksum = hdr.ipv4.hdr_checksum;
        hdr_inner.ipv4.src_addr = hdr.ipv4.src_addr;
        hdr_inner.ipv4.dst_addr = hdr.ipv4.dst_addr;
*/
        hdr.ipv4.setInvalid();
    }
    ...
    action copy_ipv6_header() {
/*
        hdr_inner.ipv6.version = hdr.ipv6.version;
        hdr_inner.ipv6.flow_label = hdr.ipv6.flow_label;
        hdr_inner.ipv6.payload_len = hdr.ipv6.payload_len;
        hdr_inner.ipv6.src_addr = hdr.ipv6.src_addr;
        hdr_inner.ipv6.dst_addr = hdr.ipv6.dst_addr;
        hdr_inner.ipv6.hop_limit = hdr.ipv6.hop_limit;
        hdr_inner.ipv6.traffic_class = hdr.ipv6.traffic_class;
*/
        hdr.ipv6.setInvalid();
    }
    ...
    action rewrite_inner_ipv4_udp() {
        payload_len = hdr_outer.ipv4.total_len;
        copy_ipv4_header();
//      hdr_inner.udp = hdr.udp;
        hdr.udp.setInvalid();
        ip_proto = 4;
    }
...
//      hdr_inner.tcp = hdr.tcp;
    ...
    table encap_outer {
        key = {
            hdr_outer.ipv4.isValid() : exact;
            hdr_outer.ipv6.isValid() : exact;
            hdr_outer.udp.isValid() : exact;
            // hdr_outer.tcp.isValid() : exact;
        }
...
//        hdr_inner.ethernet = hdr.ethernet;
  ...
  add_l2_header();
        ...
        // ----- tunnel -----
        add_gre_header(0x0800, 0);

        // ----- l2 -----
        hdr.ethernet.ether_type = 0x0800;

  hdr_outer.ethernet.setInvalid();
  hdr_outer.vlan_tag[0].setInvalid();
  hdr_outer.vlan_tag[1].setInvalid();
    }
 ...
 action rewrite_ipv4_erspan(switch_mirror_session_t session_id) {
  add_l2_header();
        ...
        hdr.ipv4.total_len = payload_len + 16w50; // 20 V4 + 4 GRE + 12 ERSPAN + 14 ETHERNET

        // ----- tunnel -----
        add_gre_header(0x22EB, 0);
  add_erspan_header(0, session_id);
...
//	IngressTunnelInner(IPV4_SRC_TUNNEL_TABLE_SIZE) tunnel;
 ...
 // =========================================================================
 // W/O NSH... Table #2/3: FlowType Classifier / SFC Classifier
 // =========================================================================

 // Note: this action should always closely follow the action in npb_ing_sf_npb_basic_adv_top.p4

 action ing_sfc_sfc_class_23_permit (
  bit<8> flow_class, // table 2

  bit<8> sfc, // table 3
  bit<16> sfc_data, // table 3

  bit<24> nsh_sph_spi, // table 3
  bit<8> nsh_sph_si, // table 3

  bit<8> sf_bitmask // table 3
 ) {
  // ----- change nsh -----
...
//		ig_md.nsh_extr.sfc_is_new             = 1;
  ig_md.nsh_extr.sfc = sfc; // table 3
  ig_md.nsh_extr.sfc_data = sfc_data; // table 3

  // base - word 0

  // base - word 1
  ig_md.nsh_extr.spi = nsh_sph_spi; // table 3
  ig_md.nsh_extr.si = nsh_sph_si; // table 3

  // ext type 1 - word 0-3

  // change metadata
  ig_md.nsh_extr.flow_class = flow_class; // table 2
  ig_md.nsh_extr.sf_bitmask = sf_bitmask; // table 3
 }
 ...
 table ing_sfc_sfc_class_23 {
  key = {
   ig_md.nsh_extr.vpn_id : ternary; // table 2
   ig_md.nsh_extr.ifclass : ternary; // table 3

   // l2
   ig_md.lkp.pkt_type : ternary; // table 2
   ig_md.lkp.mac_type : ternary; // table 2

   // l3
   ig_md.lkp.ip_type : ternary; // table 2
   ig_md.lkp.ip_proto : ternary; // table 2

   // l4
   ig_md.lkp.l4_src_port : ternary; // table 2
   ig_md.lkp.l4_dst_port : ternary; // table 2
  }

  actions = {
   NoAction;
   ing_sfc_sfc_class_23_permit;
   ing_sfc_sfc_class_miss;
  }

  const default_action = ing_sfc_sfc_class_miss(SWITCH_DROP_REASON_SFC_TABLE_MISS);
  size = NPB_ING_SFC_FLOWCLASS_CLASS_TABLE_DEPTH;
 }
 ...
 // =========================================================================
 // W/O NSH... Table #2: FlowType Classifier
 // =========================================================================

 action ing_sfc_flowclass_class_2_hit (
  bit<8> flow_class
 ) {
  // ext type 1 - word 0-3
  ig_md.nsh_extr.flow_class = flow_class;
 ...
 table ing_sfc_flowclass_class_2 {
  key = {
   // l2
   ig_md.lkp.pkt_type : ternary;
   ig_md.lkp.mac_type : ternary;
 ...
 // =========================================================================
 // W/O NSH... Table #3: SFC Classifier
 // =========================================================================

 action ing_sfc_sfc_class_3_permit (
  bit<8> sfc,
  bit<16> sfc_data,
...
//		ig_md.nsh_extr.sfc_is_new             = 1;
  ...
  ig_md.nsh_extr.sf_bitmask = sf_bitmask;
 ...
 table ing_sfc_sfc_class_3 {
  key = {
   ig_md.nsh_extr.vpn_id : exact;
   ig_md.nsh_extr.ifclass : exact;
   ig_md.nsh_extr.flow_class : exact;
  }

  actions = {
   NoAction;
   ing_sfc_sfc_class_3_permit;
   ing_sfc_sfc_class_miss;
  }

  const default_action = ing_sfc_sfc_class_miss(SWITCH_DROP_REASON_SFC_TABLE_MISS);
  size = NPB_ING_SFC_SFC_TABLE_DEPTH;
 }
...
//			hdr.nsh_extr.si  : exact;
 ...
 apply {
  ...
  // always terminate transport tunnels

  ig_md.tunnel_transport.terminate = true;
  ...
  if(hdr.nsh_extr.isValid()) {
   ...
   // base: word 1
   ig_md.nsh_extr.spi = hdr.nsh_extr.spi;
   ig_md.nsh_extr.si = hdr.nsh_extr.si;

   // ext: type 1 - word 1-3
   ig_md.nsh_extr.scope = hdr.nsh_extr.scope;
   ig_md.nsh_extr.ifindex = hdr.nsh_extr.ifindex; // may not need this line, is this ever modified after the initial sfc?
   ig_md.nsh_extr.vpn_id = hdr.nsh_extr.vpn_id;
   ig_md.nsh_extr.sfc_data = hdr.nsh_extr.sfc_data;
   ig_md.nsh_extr.ifclass = hdr.nsh_extr.ifclass; // may not need this line, is this ever modified after the initial sfc?
   ig_md.nsh_extr.timestamp = hdr.nsh_extr.timestamp; // may not need this line, is this ever modified after the initial sfc?
   ...
   // metadata
   ig_md.nsh_extr.valid = 1; // * see design note below
   ig_md.nsh_extr.sfc_is_new = 1; // * see design note below
   ...
   // ext: type 1 - word 1-3
   ig_md.nsh_extr.scope = 0;
   ig_md.nsh_extr.ifindex = (bit<16>) ig_md.port_lag_index; // interface
   ig_md.nsh_extr.vpn_id = ig_md.bd; // tenant
      ig_md.nsh_extr.ifclass = (bit<8>) ig_md.port_lag_label; // site
...
action ing_sf_policy_redirect (
 inout switch_ingress_metadata_t ig_md,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,

 inout bit<14> mtu_trunc_len_internal,
...
action ing_sf_policy_redirect_sfc (
 inout switch_ingress_metadata_t ig_md,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
...
action ing_sf_policy_redirect_trunc (
 inout switch_ingress_metadata_t ig_md,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
 ...
 mtu_trunc_len_internal = mtu_trunc_len;
}

// ---------------------------------

action ing_sf_policy_redirect_sfc_and_trunc (
 inout switch_ingress_metadata_t ig_md,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
...
// Note: These tables closely follow Barefoot's IngressAcl() tables....

control NpbIngSfPolicyL2(
 inout switch_ingress_metadata_t ig_md,
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,

 inout bit<14> mtu_trunc_len
) {
 table ing_sf_policy_l2 {
  key = {
            ig_md.port_lag_label : ternary;
            ig_md.bd_label : ternary;
...
//			ig_md.nsh_extr.flow_class           : ternary;
  ...
  actions = {
   NoAction;
   ing_sf_policy_drop(ig_md, ig_intr_md_for_dprsr, index);
   ing_sf_policy_permit(ig_md, index);
   ing_sf_policy_redirect(ig_md, index, nexthop, mtu_trunc_len);
   ing_sf_policy_redirect_sfc(ig_md, index, nexthop, mtu_trunc_len);
   ing_sf_policy_redirect_trunc(ig_md, index, nexthop, mtu_trunc_len);
   ing_sf_policy_redirect_sfc_and_trunc(ig_md, index, nexthop, mtu_trunc_len);
  }
 ...
 apply {
  ing_sf_policy_l2.apply();
 }
}

// =============================================================================

control NpbIngSfPolicyL34(
 inout switch_ingress_metadata_t ig_md,
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,

 in bit<16> length_bitmask_internal,
 inout bit<14> mtu_trunc_len
) {
 table ing_sf_policy_l34 {
  key = {
            ig_md.port_lag_label : ternary;
            ig_md.bd_label : ternary;
            ig_md.l4_port_label : ternary;
...
//			ig_md.lkp.mac_type                  : ternary;
   ...
   ig_md.nsh_extr.vpn_id : ternary;
//			ig_md.nsh_extr.flow_class           : ternary;
   length_bitmask_internal : ternary;
   ig_md.nsh_extr.ifindex : ternary;
   ig_md.tunnel.type : ternary;
   ig_md.nsh_extr.ifclass : ternary;
  }
 ...
 apply {
  ing_sf_policy_l34.apply();
 }
}

// =============================================================================

control NpbIngSfPolicyL34_v4(
 inout switch_ingress_metadata_t ig_md,
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
...
//			ig_md.nsh_extr.flow_class           : ternary;
 ...
 apply {
  ing_sf_policy_l34_v4.apply();
 }
}

// =============================================================================

control NpbIngSfPolicyL34_v6(
 inout switch_ingress_metadata_t ig_md,
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
...
//			ig_md.nsh_extr.flow_class           : ternary;
 ...
 apply {
  ing_sf_policy_l34_v6.apply();
 }
}

// =============================================================================

control NpbIngSfPolicyL7(
 inout switch_ingress_metadata_t ig_md,
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
...
//			ig_md.lkp.mac_type                  : ternary;
  ...
  const default_action = NoAction;
  size = NPB_ING_SF_0_BAS_ADV_POLICY_L7_TABLE_DEPTH;
 }

 // =========================================================================

 apply {
  ing_sf_policy_l7.apply();
 }
}

// =============================================================================

control npb_ing_sf_npb_basic_adv_acl (
 inout switch_header_transport_t hdr,
 inout switch_ingress_metadata_t ig_md,
 in ingress_intrinsic_metadata_t ig_intr_md,
 in ingress_intrinsic_metadata_from_parser_t ig_intr_md_from_prsr,
 ...
 Counter<bit<16>, switch_stats_index_t>(
  NPB_ING_SF_0_BAS_ADV_POLICY_L3_V4_TABLE_DEPTH + NPB_ING_SF_0_BAS_ADV_POLICY_L3_V6_TABLE_DEPTH + NPB_ING_SF_0_BAS_ADV_POLICY_L2_TABLE_DEPTH, CounterType_t.PACKETS_AND_BYTES) stats;
 ...
 apply {
  ...
  if(!(ig_md.bypass & SWITCH_INGRESS_BYPASS_ACL != 0)) {
   if (ig_md.lkp.ip_type == SWITCH_IP_TYPE_NONE || (mac_packet_class_enable && ig_md.flags.mac_pkt_class)) {

    NpbIngSfPolicyL2.apply(ig_md, ig_intr_md_for_dprsr, stats_index, nexthop, mtu_trunc_len);
   }
  }
  ...
  if (!(ig_md.bypass & SWITCH_INGRESS_BYPASS_ACL != 0) && (!mac_packet_class_enable || !ig_md.flags.mac_pkt_class)) {
   ...
   if (ig_md.lkp.ip_type == SWITCH_IP_TYPE_IPV6) {
    NpbIngSfPolicyL34_v6.apply(ig_md, ig_intr_md_for_dprsr, stats_index, nexthop, length_bitmask_internal, mtu_trunc_len);

   } else if (!mac_acl_enable || ig_md.lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
    NpbIngSfPolicyL34_v4.apply(ig_md, ig_intr_md_for_dprsr, stats_index, nexthop, length_bitmask_internal, mtu_trunc_len);
   }
  }
...
control npb_ing_sf_npb_basic_adv_system_acl (
 inout switch_header_transport_t hdr,
 inout switch_ingress_metadata_t ig_md,
 in ingress_intrinsic_metadata_t ig_intr_md,
 in ingress_intrinsic_metadata_from_parser_t ig_intr_md_from_prsr,
 ...
 table ing_sf_policy_l2347 {
  key = {
            ig_md.port_lag_label : ternary;
            ig_md.bd_label : ternary;
//            ig_md.ifindex                       : ternary;
            ig_md.l4_port_label : ternary; // derek added -- not in acl.p4, but seems like it should be here....
  ...
  actions = {
   NoAction;
   ing_sf_policy_redirect();
   ing_sf_policy_redirect_sfc();
   ing_sf_policy_redirect_trunc();
   ing_sf_policy_redirect_sfc_and_trunc();
   ing_sf_policy_drop();
  }
...
//      ig_intr_md_for_dprsr.drop_ctl = discard; // drop packet
    ...
    table ing_sf_action_sel {
        ...
        size = NPB_ING_SF_0_BAS_ADV_POLICY_ACT_SEL_TABLE_DEPTH;
    }
 ...
 bit<16> length_bitmask_internal;
 ...
 apply {
  if(ig_md.nsh_extr.sf_bitmask[0:0] == 1) {
   ...
   if(action_bitmask_internal[0:0] == 1) {
    ...
    npb_ing_sf_npb_basic_adv_acl.apply (
     ...
     length_bitmask_internal
    );

//				acl.apply(ig_md.lkp, ig_md, length_bitmask_internal);
...
control npb_ing_sff_top (
 ...
 action dmac_miss(
  ...
  ig_md.tunnel_transport.terminate = end_of_chain;
 }
...
/*
    ...
    table ing_sff_dec_si {
        key = { ig_md.nsh_extr.sf_bitmask[2:1] : exact; }
        actions = { new_si; }
        const entries = {
...
*/
    ...
    table ing_sff_dec_si {
        key = { ig_md.nsh_extr.sf_bitmask[2:0] : exact; }
        actions = { new_si; }
        const entries = {
            0 : new_si(0); // 0 bits set
...
// TODO: Modify these functions to include flowclass and vpn_id....
/*
...
//			if((ig_md_nsh_extr_si == 0) && (ig_md.tunnel_transport.terminate == false)) { // check for si of 0 (or underflow, since we used a saturating subtract)
//				ig_intr_md_for_dprsr.drop_ctl = 0x1; // drop packet
//			}
...
control MTU(in switch_header_outer_t hdr,
            in switch_egress_metadata_t eg_md,
...
control IngressUnicast(in switch_lookup_fields_t lkp,
 ...
 // | 1			| 1			| sfc/sff
    ...
    apply {
//      if (rmac.apply().hit) { // extreme removed
...
*/
     ...
     ig_md.tunnel
          ...
          // note: TODO - modify these hash functions to include flowclass and vpn_id (although I
...
parser ParserUnderlayL2(
    ...
    state start {
        ...
        ether_type = hdr.outer.ethernet.ether_type;
    ...
    state parse_vlan {
     pkt.extract(hdr.outer.vlan_tag.next);
        ether_type = hdr.outer.vlan_tag.last.ether_type;
...
parser NpbIngressParser(
    ...
    Checksum() transport_ipv4_checksum;
    ...
    state parse_port_metadata {
        ...
        transition snoop_layer2;
    }

    // todo: need to understand interaction w/ cpu better.
    // todo: can we assume no vlan-tags after cpu header?
    state parse_cpu {
        pkt.extract(hdr.outer.fabric);
        pkt.extract(hdr.outer.cpu);
        ig_md.bypass = hdr.outer.cpu.reason_code;
        ig_md.flags.capture_ts = (bool) hdr.outer.cpu.capture_ts;
        transition select(hdr.outer.cpu.ether_type) {
    ...
    // Determine if there's an NSH xport present.
    state snoop_layer2 {
        transition select(
                    pkt.lookahead<bit<112>>()[15:0], // pkt.lookahead<snoop_layer2_h>().enet_ether_type;
                    pkt.lookahead<bit<144>>()[15:0]) { // pkt.lookahead<snoop_layer2_h>().vlan_ether_type;

            (0x894F , _ ): parse_xport_nsh_untagged;
            (0x8100, 0x894F ): parse_xport_nsh_tagged;
            (0x0800, _ ): snoop_ipv4_untagged;
            (0x8100, 0x0800): snoop_ipv4_tagged;
            default: parse_outer_ethernet; // no xport present
        }
    }

    // Determine if there's a possible GRE transport present.
    // If options or frag, assume no transport present.
    state snoop_ipv4_untagged {
        transition select(
                pkt.lookahead<bit<120>>()[3:0], // pkt.lookahead<snoop_ipv4_untagged_h>().ipv4_ihl;
                pkt.lookahead<bit<163>>()[2:0], // pkt.lookahead<snoop_ipv4_untagged_h>().ipv4_flags;
                pkt.lookahead<bit<176>>()[12:0], // pkt.lookahead<snoop_ipv4_untagged_h>().ipv4_frag_offset;
                pkt.lookahead<bit<192>>()[7:0]) { // pkt.lookahead<snoop_ipv4_untagged_h>().ipv4_protocol;       

            (5, 3w2 &&& 3w5, 0, 47): extract_enet_possible_untagged_gre_xport;
            default: parse_outer_ethernet; // no xport present
        }
    }
    ...
    state snoop_ipv4_tagged {
        transition select(
                pkt.lookahead<bit<152>>()[3:0], // pkt.lookahead<snoop_ipv4_tagged_h>().ipv4_ihl;
                pkt.lookahead<bit<195>>()[2:0], // pkt.lookahead<snoop_ipv4_tagged_h>().ipv4_flags;
                pkt.lookahead<bit<208>>()[12:0], // pkt.lookahead<snoop_ipv4_tagged_h>().ipv4_frag_offset;
                pkt.lookahead<bit<224>>()[7:0]) { // pkt.lookahead<snoop_ipv4_tagged_h>().ipv4_protocol;       

            (5, 3w2 &&& 3w5, 0, 47): extract_enet_possible_tagged_gre_xport;
            default: parse_outer_ethernet; // no xport present
        }
    }

    // Extract ethernet into both xport and outer PHV locations so we can snoop deeper. 
    state extract_enet_possible_untagged_gre_xport {
        ...
        transition snoop_gre_untagged;

        // transition select(
        //         pkt.lookahead<bit<164>>()[3:0],     // pkt.lookahead<snoop_gre_untagged_h>().gre_CRKS;
        //         pkt.lookahead<bit<192>>()[15:0]) {  // pkt.lookahead<snoop_gre_untagged_h>().gre_proto;       
        //     (0, GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_untagged_erspan1;
        //     (2, GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_untagged_erspan2;
        //     default: invalidate_xport_untagged_gre; // no xport present
        // }
    }

    // Extract ethernet into both xport and outer PHV locations so we can snoop deeper. 
    state extract_enet_possible_tagged_gre_xport {
        //transition parse_outer_ethernet;
        pkt.extract(hdr.transport.ethernet);
        hdr.outer.ethernet.setValid();
        hdr.outer.ethernet.dst_addr = hdr.transport.ethernet.dst_addr;
        hdr.outer.ethernet.src_addr = hdr.transport.ethernet.src_addr;
        hdr.outer.ethernet.ether_type = hdr.transport.ethernet.ether_type;
        transition snoop_gre_tagged;
    }

    // Determine if there's a possible GRE ERSPAN transport present
    state snoop_gre_untagged {
        transition select(
                //pkt.lookahead<bit<161>>()[0:0],     // pkt.lookahead<snoop_gre_untagged_h>().gre_C;
                //pkt.lookahead<bit<163>>()[0:0],     // pkt.lookahead<snoop_gre_untagged_h>().gre_K;
                //pkt.lookahead<bit<164>>()[0:0],     // pkt.lookahead<snoop_gre_untagged_h>().gre_S;




                //pkt.lookahead<bit<176>>()[2:0],     // pkt.lookahead<snoop_gre_untagged_h>().gre_version;
                pkt.lookahead<bit<192>>()[15:0]) { // pkt.lookahead<snoop_gre_untagged_h>().gre_proto;       

            //(0,0,0,0,GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_untagged_erspan1;
            //(0,0,1,0,GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_untagged_erspan2;




            (0x88BE): extract_xport_untagged_erspan1;
            (0x88BE): extract_xport_untagged_erspan2;


            // (ETHERTYPE_IPV4): extract_ipv4_possible_untagged_gre_xport;
            // (ETHERTYPE_IPV6): extract_ipv4_possible_untagged_gre_xport;
            default: invalidate_xport_untagged_gre; // no xport present
        }
    }

    // Determine if there's a possible GRE ERSPAN transport present
    state snoop_gre_tagged {
        transition select(
                //pkt.lookahead<bit<193>>()[0:0],     // pkt.lookahead<snoop_gre_tagged_h>().gre_C;
                //pkt.lookahead<bit<195>>()[0:0],     // pkt.lookahead<snoop_gre_tagged_h>().gre_K;
                //pkt.lookahead<bit<196>>()[0:0],     // pkt.lookahead<snoop_gre_tagged_h>().gre_S;
                //pkt.lookahead<bit<196>>()[3:0],     // pkt.lookahead<snoop_gre_tagged_h>().gre_CRKS;
                //pkt.lookahead<bit<208>>()[2:0],     // pkt.lookahead<snoop_gre_tagged_h>().gre_version;
                pkt.lookahead<bit<224>>()[15:0]) { // pkt.lookahead<snoop_gre_tagged_h>().gre_proto;      

            //(0,0,0,0,GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_tagged_erspan1;
            //(0,0,1,0,GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_tagged_erspan2;
            //(0, GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_tagged_erspan1;
            //(2, GRE_PROTOCOLS_ERSPAN_TYPE_2): extract_xport_tagged_erspan2;
            (0x88BE): extract_xport_tagged_erspan1;
            (0x88BE): extract_xport_tagged_erspan2;
            // (ETHERTYPE_IPV4): extract_ipv4_possible_tagged_gre_xport;
            // (ETHERTYPE_IPV6): extract_ipv4_possible_tagged_gre_xport;
            default: invalidate_xport_tagged_gre; // no xport present
        }
    }

    // No xport present, invalidate xport enet and continue parsing on outer
    state invalidate_xport_untagged_gre {
        hdr.transport.ethernet.setInvalid();
        transition parse_outer_ipv4;
    }

    // No xport present, invalidate xport enet and continue parsing on outer
    state invalidate_xport_tagged_gre {
        hdr.transport.ethernet.setInvalid();
        transition parse_outer_vlan;
    }
    ...
    state parse_xport_nsh_untagged {



        pkt.extract<ethernet_h>(_); // drop it

     pkt.extract(hdr.transport.nsh_extr);
        ig_md.orig_pkt_had_nsh = 1;
        ig_md.tunnel_transport.type = SWITCH_TUNNEL_TYPE_NSH;
        transition parse_outer_ethernet;
    }

    state parse_xport_nsh_tagged {
    ...
    // Extract untagged ERSPAN 1 transport.
    state extract_xport_untagged_erspan1 {
        hdr.outer.ethernet.setInvalid(); // overwrite should take of this.
     pkt.extract(hdr.transport.ipv4);
        transport_ipv4_checksum.add(hdr.transport.ipv4);
        ig_md.flags.transport_ipv4_checksum_err = transport_ipv4_checksum.verify();
     pkt.extract(hdr.transport.gre);
     pkt.extract(hdr.transport.erspan_type2);
        //todo: do we need to differentiate between erspan 1&2 here?
        ig_md.tunnel_transport.type = SWITCH_TUNNEL_TYPE_ERSPAN;
  //ig_md.tunnel_transport.id =  (bit<32>)hdr.transport.erspan_type1.vlan;
        transition parse_outer_ethernet;
    }

    // Extract untagged ERSPAN 2 transport.
    state extract_xport_untagged_erspan2 {
        hdr.outer.ethernet.setInvalid(); // overwrite should take of this.
     pkt.extract(hdr.transport.ipv4);
        transport_ipv4_checksum.add(hdr.transport.ipv4);
        ig_md.flags.transport_ipv4_checksum_err = transport_ipv4_checksum.verify();
     pkt.extract(hdr.transport.gre);
     pkt.extract(hdr.transport.gre_extension_sequence);
     pkt.extract(hdr.transport.erspan_type2);
        //todo: do we need to differentiate between erspan 1&2 here?
        ig_md.tunnel_transport.type = SWITCH_TUNNEL_TYPE_ERSPAN;
  //ig_md.tunnel_transport.id =  (bit<32>)hdr.transport.erspan_type2.vlan;
        transition parse_outer_ethernet;
    }

    // Extract tagged ERSPAN 1 transport.
    state extract_xport_tagged_erspan1 {
        hdr.outer.ethernet.setInvalid(); // overwrite should take of this.
  ...
  //ig_md.tunnel_transport.id =  (bit<32>)hdr.transport.erspan_type1.vlan;
    ...
    // Extract tagged ERSPAN 2 transport.
    state extract_xport_tagged_erspan2 {
        hdr.outer.ethernet.setInvalid(); // overwrite should take of this.
     pkt.extract(hdr.transport.vlan_tag[0]);
     pkt.extract(hdr.transport.ipv4);
        transport_ipv4_checksum.add(hdr.transport.ipv4);
  ...
  //ig_md.tunnel_transport.id =  (bit<32>)hdr.transport.erspan_type1.vlan;
    ...
    //         IP_PROTOCOLS_IPV4: parse_outer_ipinip;
    //         IP_PROTOCOLS_IPV6: parse_outer_ipv6inip;
    ...
    state parse_outer_ipv4 {
        ...
        transition select(
            ...
            (5, 3w2 &&& 3w5, 0, 1): parse_outer_icmp;
            (5, 3w2 &&& 3w5, 0, 2): parse_outer_igmp;
            (5, 3w2 &&& 3w5, 0, 4): parse_outer_ipinip;
            (5, 3w2 &&& 3w5, 0, 41): parse_outer_ipv6inip;
            (5, 3w2 &&& 3w5, 0, 17): parse_outer_udp;
            (5, 3w2 &&& 3w5, 0, 6): parse_outer_tcp;
            (5, 3w2 &&& 3w5, 0, 0x84): parse_outer_sctp;
            (5, 3w2 &&& 3w5, 0, 0x32): parse_outer_esp;
            (5, 3w2 &&& 3w5, 0, 47): parse_outer_gre;
            default : accept;
        }
    }
    ...
    state parse_outer_vxlan {
        ...
        ig_md.tunnel.id = hdr.outer.vxlan.vni;
...
//#if defined(IPINIP) && defined(IPV6_TUNNEL_ENABLE)
  ...
  ig_md.tunnel.id = hdr.outer.nvgre.vsid; // extreme added
     transition parse_inner_ethernet;
    }
    ...
    state parse_outer_gtp_c {
        ...
        transition select(pkt.lookahead<bit<3>>(), pkt.lookahead<bit<5>>()) {
...
//        ig_md.tunnel.id = hdr.outer.gtp_v1_v2_teid.teid; // extreme added
     transition accept;
    }
...
//     }
    ...
    state parse_inner_ipv4 {
        ...
        transition select(
            ...
            (5, 3w2 &&& 3w5, 0, 17): parse_inner_udp;
            (5, 3w2 &&& 3w5, 0, 6): parse_inner_tcp;
            (5, 3w2 &&& 3w5, 0, 0x84): parse_inner_sctp;
            (5, 3w2 &&& 3w5, 0, 0x32): parse_inner_esp;
            (5, 3w2 &&& 3w5, 0, 47): parse_inner_gre;
            default : accept;
        }
    }
...
//#if defined(EGRESS_IP_ACL_ENABLE) || defined(EGRESS_MIRROR_ACL_ENABLE)
  ...
  eg_md.vrf = hdr.bridged_md.tunnel.vrf;
        eg_md.tunnel_transport.terminate = hdr.bridged_md.tunnel.terminate_nsh; // extreme added
  ...
  // ---------------
        ...
        eg_md.nsh_extr.sf_bitmask = hdr.bridged_md.base.nsh_extr_sf_bitmask; //  1 byte
        eg_md.nsh_extr.flow_class = hdr.bridged_md.base.nsh_extr_flow_class; //  1 byte?
  ...
  // ---------------
        ...
        // ext: type 1 - word 0-3
  eg_md.nsh_extr.scope = hdr.bridged_md.base.nsh_extr_scope;
  eg_md.nsh_extr.ifindex = hdr.bridged_md.base.nsh_extr_ifindex;
        eg_md.nsh_extr.vpn_id = hdr.bridged_md.base.nsh_extr_vpn_id; //  3 bytes
  eg_md.nsh_extr.sfc_data = hdr.bridged_md.base.nsh_extr_sfc_data;
  eg_md.nsh_extr.ifclass = hdr.bridged_md.base.nsh_extr_ifclass;



  eg_md.nsh_extr.timestamp = 0;
...
// Transport ethernet (on NSH) is always being dropped by ingress MAU now
// #ifdef PARSER_EXTRACT_L2_UNDERLAY
//         transition snoop_transport;
// #else
        //transition select(eg_md.nsh_extr.valid) {
        transition select(eg_md.orig_pkt_had_nsh) {
            0: parse_ethernet_ETH;
            1: parse_transport_nsh;
        }
        // transition select(eg_md.tunnel_transport.type) {
        //     SWITCH_TUNNEL_TYPE_NONE: parse_ethernet_ETH;
        //     SWITCH_TUNNEL_TYPE_NSH: parse_transport_nsh;
        // }
// #endif /* PARSER_EXTRACT_L2_UNDERLAY */
    }
...
// #ifdef PARSER_EXTRACT_L2_UNDERLAY
// 
//     // Snoop ahead here to determine type of transport
//     state snoop_transport {
//         transport_ethernet_snoop_h snoop_enet = pkt.lookahead<transport_ethernet_snoop_h>();
//         transition select(snoop_enet.ether_type1, snoop_enet.ether_type2) {
...
// #endif /* PARSER_EXTRACT_L2_UNDERLAY */
    ...
    state parse_ethernet_ETH {
        pkt.extract(hdr.outer.ethernet);
        transition select(hdr.outer.ethernet.ether_type) {
            0x893F : parse_br_ETH;
            0x8926 : parse_vn_ETH;
            0x8100 : parse_vlan_ETH;
            0x88A8 : parse_vlan_ETH;
...
*/
        ...
        pkt.emit(hdr.transport.nsh_extr);
        pkt.emit(hdr.transport.ipv4);
        pkt.emit(hdr.transport.ipv6); // todo: no v6 transport shown in encap doc
        pkt.emit(hdr.transport.gre);
        pkt.emit(hdr.transport.nvgre); // todo: no nvgre transport shown in encap doc
        //pkt.emit(hdr.transport.erspan_type1); // todo: terminated on ingress?
        //pkt.emit(hdr.transport.erspan_type2); // todo: terminated on ingress?
        pkt.emit(hdr.transport.erspan_type3); // todo: not supported - remove
        pkt.emit(hdr.transport.udp); // todo: no longer supported
        pkt.emit(hdr.transport.tcp); // todo: no tcp transport shown in encap doc
        pkt.emit(hdr.transport.vxlan); // todo: no longer supported
    ...
    apply {
        ...
        if (hdr.transport.ipv4_option.isValid()) {
            hdr.transport.ipv4.hdr_checksum = ipv4_checksum.update({
                ...
                hdr.transport.ipv4_option.type,
                hdr.transport.ipv4_option.length,
                hdr.transport.ipv4_option.value});
        } else if (hdr.transport.ipv4.isValid()) {
            hdr.transport.ipv4.hdr_checksum = ipv4_checksum.update({
                hdr.transport.ipv4.version,
                hdr.transport.ipv4.ihl,
                hdr.transport.ipv4.diffserv,
...
*/
        ...
        // ***** OUTER *****
        pkt.emit(hdr.outer.ethernet);
        pkt.emit(hdr.outer.fabric); // Egress Only.
        pkt.emit(hdr.outer.cpu); // Egress Only.
        pkt.emit(hdr.outer.timestamp); // Egress only.
...
//-----------------------------------------------------------------------------
control MirrorRewrite(inout switch_header_outer_t hdr,
                      inout switch_egress_metadata_t eg_md)(
                      switch_uint32_t table_size=1024) {
    bit<16> length;
...
//        implementation = bd_action_profile;
 ...
 // ----------------------------------------------
 // table: set peer link properties
 // ----------------------------------------------
...
control EgressPortMapping(
 ...
 // ----------------------------------------------
    ...
    action cpu_rewrite() {
        ...
        hdr.ethernet.ether_type = 0x9999;
    }
...
//      bit<FLOW_CLASS_WIDTH> flow_class
        ...
        eg_md.nsh_extr.sf_bitmask = sf_bitmask;
//      eg_md.nsh_extr.vpn_id                        = vpn_id;
//      eg_md.nsh_extr.flow_class                    = flow_class;
    }
...
control EgressQoS(inout switch_header_outer_t hdr,
                  in switch_port_t port,
                  inout switch_egress_metadata_t eg_md)(
                  switch_uint32_t table_size=1024) {
...
//-------------------------------------------------------------------------------------------------
control WRED(inout switch_header_outer_t hdr,
             in switch_egress_metadata_t eg_md,
             in egress_intrinsic_metadata_t eg_intr_md,
             out bool flag) {
...
//          1  : discard();
 ...
 apply {
  ...
  // Move whatever comes across as metadata into the header.

  // base: word 0
  // (nothing to do)

  // base: word 1
  hdr.nsh_extr.spi = eg_md.nsh_extr.spi;
  hdr.nsh_extr.si = eg_md.nsh_extr.si;

  // ext: type 1 - word 0-3
  hdr.nsh_extr.scope = eg_md.nsh_extr.scope; // word 0
  hdr.nsh_extr.ifindex = eg_md.nsh_extr.ifindex; // word 0
  hdr.nsh_extr.vpn_id = eg_md.nsh_extr.vpn_id; // word 1
  hdr.nsh_extr.sfc_data = eg_md.nsh_extr.sfc_data; // word 1
  hdr.nsh_extr.ifclass = eg_md.nsh_extr.ifclass; // word 2
  hdr.nsh_extr.timestamp = eg_md.nsh_extr.timestamp; // word 3

//		if((eg_md.nsh_extr.valid == 1) && (hdr.nsh_extr.isValid())) {
  if(hdr.nsh_extr.isValid()) {
   ...
   hdr.nsh_extr.ttl = 0x3f; // 63 is the rfc's recommended default value.
   hdr.nsh_extr.len = 0x6; // in 4-byte words (1 + 1 + 4).
   hdr.nsh_extr.reserved2 = 0x0;
   hdr.nsh_extr.md_type = 0x1; // 0 = reserved, 1 = fixed len, 2 = variable len.
   ...
   // ext: type 1 - word 0-3
   hdr.nsh_extr.ver = 0x2; // word 0
   hdr.nsh_extr.reserved4 = 0x0; // word 0

   hdr.nsh_extr.reserved5 = 0x0; // word 2
...
//			if((hdr.nsh_extr.si == 0) && (eg_md.tunnel_transport.terminate == false)) { // check for si of 0 (or underflow, since we used a saturating subtracts)
//				eg_intr_md_for_dprsr.drop_ctl = 0x1; // drop packet
//			}

   if(eg_md.tunnel_transport.terminate == false) { // check for si of 0 (or underflow, since we used a saturating subtracts)
    if((hdr.nsh_extr.ttl == 0) || (hdr.nsh_extr.si == 0)) {
     eg_intr_md_for_dprsr.drop_ctl = 0x1; // drop packet
    }
   }
...
control npb_egr_sf_multicast_top (
 ...
 apply {
  ...
  if(eg_md.nsh_extr.sf_bitmask[1:1] == 1) {
            ...
            hdr.nsh_extr.si = hdr.nsh_extr.si - 1; // decrement sp_index
...
//		eg_intr_md_for_dprsr.drop_ctl = discard; // drop packet
 ...
 apply {
  ...
  if(eg_md.nsh_extr.sf_bitmask[2:2] == 1) {

   // ==================================
   // Remove NSH
   // ==================================

//			hdr.nsh_extr.setInvalid(); // proxy removes the nsh
...
control npb_egr_sf_proxy_top_part2 (
 ...
 action egr_sf_trunc_hit(
  bit<14> mtu_trunc_len
 ) {

  eg_intr_md_for_dprsr.mtu_trunc_len = mtu_trunc_len;

 }

 // =====================================

 action egr_sf_trunc_miss(
 ) {
 }
 ...
 table egr_sf_trunc {
  ...
  const default_action = egr_sf_trunc_miss;
  size = NPB_EGR_SF_2_EGRESS_SFP_TRUNC_TABLE_DEPTH;
 }
...
control IngressHdrStackCounters(
    in switch_header_outer_t hdr,
    in switch_header_transport_t hdr_underlay,
    in switch_header_inner_t hdr_inner
) {
    ...
    table cntrs_inner_tbl {
        key = {
            hdr_inner.ethernet.isValid() : exact;
            hdr_inner.vlan_tag[0].isValid() : exact;
    ...
    apply {
        ...
        if(hdr_underlay.nsh_extr.isValid()) {
            cntrs_underlay_tunnel_addr = 1;
        }
        // else if(hdr_underlay.erspan_type1.isValid()) {  // GRE/ERSPAN
        //     cntrs_outer_tnl_addr = 2;
        // }
        // else if(hdr_underlay.erspan_type2.isValid()) {  // GRE/ERSPAN
        //     cntrs_outer_tnl_addr = 3;
        // }
        // else if(hdr_underlay.vxlan.isValid()) {
        ...
        else if(hdr.gtp_v2_base.isValid() && hdr.gtp_v1_v2_teid.isValid()) { //GTP-C
            cntrs_tunnel_addr = 5;
        }
        ...
        if(hdr_inner.ipv4.isValid()) {
            cntrs_inner_addr_l3 = 1;
        }
        else if(hdr_inner.ipv6.isValid()) {
        ...
        // if(hdr_inner.icmp.isValid()) {
        //     cntrs_inner_addr_l4 = 1;
        // }
        // else if(hdr_inner.igmp.isValid()) {
        //     cntrs_inner_addr_l4 = 2;
        // }
        // else if(hdr_inner.udp.isValid()) {
        //     cntrs_inner_addr_l4 = 3;
        // }
        // else if(hdr_inner.tcp.isValid()) {
        //     cntrs_inner_addr_l4 = 4;
        // }
        // else if(hdr_inner.sctp.isValid()) {
        //     cntrs_inner_addr_l4 = 5;
        // }
        // else if(hdr_inner.gre.isValid()) {
        //     cntrs_inner_addr_l4 = 6;
        // }
        // else if(hdr_inner.esp.isValid()) {
        //     cntrs_inner_addr_l4 = 7;
        // }
        // else {
...
control SwitchIngress(
 ...
 IngressTunnelInner(IPV4_SRC_TUNNEL_TABLE_SIZE) tunnel_inner;
...
//    IngressSystemAcl() system_acl;
    TunnelDecapTransport(switch_tunnel_mode_t.PIPE) tunnel_decap_transport;
...
*/
        ...
        IngressHdrStackCounters.apply(hdr.outer, hdr.transport, hdr.inner);
