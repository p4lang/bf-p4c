...
#				tunnel_encap_ptr, EgressTunnelType.NSH.value, tunnel_encap_nexthop_ptr, tunnel_encap_bd, dmac, tunnel_encap_smac_ptr, smac
			...
			npb_nsh_chain_start_end_add(self, self.target,ig_pipe,
				#ingress
				[ig_port_new], ig_lag_ptr, 0, sap, vpn, spi_new, si_new, sf_bitmask, rmac, nexthop_ptr, bd, eg_lag_ptr, 0, 0, [eg_port_new], False, 0, dsap
				#tunnel
#				tunnel_encap_ptr, EgressTunnelType.NSH.value, tunnel_encap_nexthop_ptr, tunnel_encap_bd, dmac, tunnel_encap_smac_ptr, smac
				#egress
			)
			...
			npb_npb_sf0_policy_l34_v4_add(self, self.target,ig_pipe, sap=sap, l3_proto=0x11, l3_proto_mask=0xff, flow_class=flow_class_acl, sfc_enable=1, sfc=sfc)

			# -----------------
			# Ingress SFP Sel
			# -----------------

			npb_npb_sfp_sel_add(self, self.target,ig_pipe, vpn, flow_class_sfp, sfc, 0, 0, [spi_new], [si_new], [sf_bitmask])
...
#			src_pkt =           Ether(b'\xd8\x84\x66\xf9\x3c\x00\x00\x00\x00\x00\x11\x22\x08\x00') # ethernet
			src_pkt =           Ether(b'\x33\x33\x33\x33\x33\x33\x00\x00\x00\x00\x11\x22\x08\x00') # ethernet
			src_pkt = src_pkt / IP   (b'\x45\x00\x00\x74\x00\x01\x00\x00\x40\x2f\x3e\x1f\x0a\x0a\x0a\x0a\x14\x14\x14\x14') # ipv4
			src_pkt = src_pkt / TCP  (b'') # gre + ipv4, really
...
#			exp_pkt = src_pkt
			...
			exp_pkt =           Ether(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00') # ethernet
			exp_pkt = exp_pkt / IP   (b'\x45\x00\x00\x5c\x00\x01\x00\x00\x40\x00\x64\x8c\x05\x05\x05\x05\x06\x06\x06\x06') # ipv4
			exp_pkt = exp_pkt / TCP  (b'') #
