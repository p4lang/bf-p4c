...
#from test_mau_tb_tbl_wrap_folded import *
...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.NONE.value,   tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)

			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.VXLAN.value,  tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.IPINIP.value, tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GRE.value,    tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GTPC.value,   tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GTPU.value,   tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)
			npb_npb_sf0_policy_l2_add(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.ERSPAN.value, tun_type_mask=0xf, flow_class=flow_class_acl, dedup_en=1)

#			npb_npb_sf2_policy_l2_add    (self, self.target, dsap=dsap, l2_etype=0x0800, l2_etype_mask=0xffff, dedup_en=1)
...
#			time.sleep(1)
			...
			# send 20 packets (0 through 19)
			for x in range(20):
				logger.info("Sending packet %d on port %d", x, ig_port)
				testutils.send_packet(self, ig_port, src_pkt)

				if(x < 16):
					# these packets should be received

					logger.info("Verify packet %d on port %d", x, eg_port)
