...
def npb_tunnel_rmac_add(self, target, ig_pipe, dst_addr):
	...
	if((p4_name == 'pgm_mp_npb_top') or (p4_name == 'pgm_fp_npb_dedup_dtel_vcpFw_top')):
...
def dtel_queue_report_check_quota_add(self, target, ig_pipe,
	...
	# set dtel queue report (register)
	dtel_queue_report_quotas_add(self, self.target, ig_pipe,
		index             = reg_index,
	...
	# set dtel queue report (table)
	dtel_queue_report_check_quota_table_update_add(self, self.target, ig_pipe,
	...
	# set dtel queue report (table)
	dtel_queue_report_check_quota_table_reset_add(self, self.target, ig_pipe,
		pkt_src           = PktSrc.BRIDGED.value,
		qalert            = 0,
		qid               = qid,
		port              = port,
...
def dtel_queue_report_check_quota_del(self, target, ig_pipe,
	...
	'''
	# set dtel queue report (register)
	dtel_queue_report_quotas_del(self, self.target, ig_pipe,
		index             = reg_index
	)
	'''
	# set dtel queue report (table)
	dtel_queue_report_check_quota_table_del(self, self.target, ig_pipe,
		pkt_src           = PktSrc.BRIDGED.value,
		qalert            = 1,
		qid               = qid,
		port              = port
