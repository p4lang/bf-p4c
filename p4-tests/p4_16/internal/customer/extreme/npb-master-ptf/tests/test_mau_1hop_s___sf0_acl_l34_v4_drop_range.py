...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			#Len Range
			npb_npb_sf0_len_rng_add(self, self.target, ig_pipe,    0x0, 0x1fff, 0) # <  8k
			npb_npb_sf0_len_rng_add(self, self.target, ig_pipe, 0x2000, 0xffff, 1) # >= 8k

			npb_npb_sf0_policy_l34_v4_add(self, self.target, ig_pipe, sap=sap, l3_len_rng=0x0000, l3_len_rng_mask=0xffff, flow_class=flow_class_acl, drop=0) # don't drop
			npb_npb_sf0_policy_l34_v4_add(self, self.target, ig_pipe, sap=sap, l3_len_rng=0x0001, l3_len_rng_mask=0xffff, flow_class=flow_class_acl, drop=1) # drop
