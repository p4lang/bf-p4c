	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			for i in range(0, upper_test_limit):
				#print i
				npb_npb_sf0_policy_l2_add(self, self.target,ig_pipe, sap=sap+i, vpn=vpn+i, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, flow_class=flow_class_acl, scope=1)
