...
// resources - counter, register, meter
    DirectRegister<pair>() r;
    ...
    // modification.
    DirectRegisterAction<pair, bit<32>>(r) reg_action = {
...
// resources end
    // Do nothing for direct action

////// Declaration section end


/////  Actions begin
    action tcp_sport_modify(bit<16> sPort, PortId_t port) {
    ...
    action tcp_dport_modify(bit<16> dPort, PortId_t port) {
    ...
    action ipsa_modify(ipv4_addr_t sa, PortId_t port
    ) {
      reg_action.execute();
    ...
    action ipda_modify(ipv4_addr_t da, PortId_t port) {
...
// Separate key fields based on exm, tern, lpm,
// range, valid, atcam, alpm
    ...
    // Do nothing for direct action
        ...
        size = 100000; // match_tbl table size
        registers = r;
