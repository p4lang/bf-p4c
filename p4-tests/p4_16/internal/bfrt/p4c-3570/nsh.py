...
#        PacketListField('Metadata', None, XIntField, count_from='Len')
        BitFieldLenField('Len', None, 7,
                         length_of='Metadata',
                         adjust=lambda pkt, x: x),  # noqa: E501
        PacketListField('Metadata', None, XIntField, count_from=lambda pkt: pkt.Len/4)
    ]
...
#                         adjust=lambda pkt, x: 6 if pkt.MDType == 1 else x + 2),  # noqa: E501
        BitFieldLenField('Len', None, 6,
                         length_of='ContextHeaders',
                         adjust=lambda pkt, x: 2 + 4 if pkt.MDType == 1 else 2 + x/4),  # noqa: E501
...
#        ConditionalField(XIntField('NPC', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(ByteField('DataType', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(ByteField('Reserved0', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(BitField('LagHash', 0, 16), lambda pkt: pkt.MDType == 1),
#        ConditionalField(XIntField('NSC', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(BitField('VPN', 0, 16), lambda pkt: pkt.MDType == 1),
        ConditionalField(BitField('Reserved1', 0, 16), lambda pkt: pkt.MDType == 1),
#        ConditionalField(XIntField('SPC', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(ByteField('Reserved2', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(ByteField('Scope', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(BitField('SSAP', 0, 16), lambda pkt: pkt.MDType == 1),
#        ConditionalField(XIntField('SSC', 0), lambda pkt: pkt.MDType == 1),
        ConditionalField(XIntField('Timestamp', 0), lambda pkt: pkt.MDType == 1),
#        ConditionalField(PacketListField("ContextHeaders", None, NSHTLV, count_from="Length"), lambda pkt: pkt.MDType == 2)
        ConditionalField(PacketListField("ContextHeaders", None, NSHTLV, length_from=lambda pkt: (pkt.Len-2)*4), lambda pkt: pkt.MDType == 2)
    ]
