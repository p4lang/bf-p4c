...
from test_mau_common_pkt import *
...
#					[gc.DataTuple('$MULTICAST_BACKUP_PORT',                   backup_port),
			...
			table.entry_add(
				...
				[table.make_key(
					[gc.KeyTuple('ig_md.port',                                port[i]),
					 ...
					 gc.KeyTuple('hdr.transport.nsh_type1.$valid',            0)],
				)],
				[table.make_data(
					...
					'SwitchIngress.ingress_port_mapping.set_port_properties_without_nsh'
				)]
			)
			...
			table.entry_add(
				...
				[table.make_key(
					[gc.KeyTuple('ig_md.port',                                port[i]),
					 gc.KeyTuple('hdr.cpu.$valid',                            0),
					 gc.KeyTuple('hdr.transport.nsh_type1.$valid',            1)],
				)],
				[table.make_data(
					[],
					'SwitchIngress.ingress_port_mapping.set_port_properties_with_nsh'
				)]
			)
			...
			table.entry_add(
				...
				[table.make_data(
					[gc.DataTuple('sap',                                      sap),
					 ...
					 gc.DataTuple('si',                                       si),
					 gc.DataTuple('si_predec',                                si_predec)],
					'SwitchIngress.ingress_port_mapping.set_cpu_port_properties_without_nsh'
				)]
			)
			...
			table.entry_add(
				...
				[table.make_data(
					[],
					'SwitchIngress.ingress_port_mapping.set_cpu_port_properties_with_nsh'
				)]
			)
...
def npb_ing_port_del(self, target, port):
...
def npb_tunnel_network_dst_vtep_add(self, target, tun_type, tun_type_mask, drop):
...
def npb_tunnel_network_dst_vtep_del(self, target, tun_type, tun_type_mask):
...
def npb_tunnel_network_src_vtep_add(self, target, tun_type, tun_type_mask, sap, vpn, port_lag_ptr):
...
def npb_tunnel_network_src_vtep_del(self, target, tun_type, tun_type_mask):
		...
		table.entry_add(
			...
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,      sap_mask),
				 gc.KeyTuple('lkp.mac_src_addr',                 0,        0),
				 gc.KeyTuple('lkp.mac_dst_addr',                 0,        0),
		...
		table.entry_add(
			...
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,            sap_mask),
				 ...
				 gc.KeyTuple('lkp.ip_src_addr[31:0]',            0,              0),
				 gc.KeyTuple('lkp.ip_dst_addr[31:0]',            0,              0),
...
def npb_tunnel_encap_sip_rewrite_add(self, target, bd, sip):
		self.insert_table_entry(
			target,
			'SwitchEgress.tunnel_rewrite.src_addr_rewrite',
			[self.KeyField('eg_md.bd',                         self.to_bytes(bd, 2))],
			'SwitchEgress.tunnel_rewrite.rewrite_ipv4_src',
			[self.DataField('src_addr',                        self.ipv4_to_bytes(sip))])

def npb_tunnel_encap_sip_rewrite_del(self, target, bd):
		self.delete_table_entry(
			target,
			'SwitchEgress.tunnel_rewrite.src_addr_rewrite',
			[self.KeyField('eg_md.bd',                         self.to_bytes(bd, 2))])
...
def npb_tunnel_encap_dip_rewrite_add(self, target, tunnel_ptr, dip):
		self.insert_table_entry(
			target,
			'SwitchEgress.tunnel_rewrite.ipv4_dst_addr_rewrite',
			[self.KeyField('tunnel.index',                     self.to_bytes(tunnel_ptr, 2))],
			'SwitchEgress.tunnel_rewrite.rewrite_ipv4_dst',
			[self.DataField('dst_addr',                        self.ipv4_to_bytes(dip))])

def npb_tunnel_encap_dip_rewrite_del(self, target, tunnel_ptr):
		self.delete_table_entry(
			target,
			'SwitchEgress.tunnel_rewrite.ipv4_dst_addr_rewrite',
			[self.KeyField('tunnel.index',                     self.to_bytes(tunnel_ptr, 2))])
...
def npb_nsh_chain_start_end_del(self, target,
	# ingress
	ig_port, spi, si, sf_bitmask, rmac, nexthop_ptr, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port_num_entries, eg_port
	# egress
):
	cpu_port_del              (self, target)
	npb_ing_port_del          (self, target, ig_port)
