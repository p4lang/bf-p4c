...
control OuterPktValidation(
    ...
    table validate_ipv6 {
        key = {
            ...
            hdr.ipv6.src_addr_3 : ternary; //TODO define the bit range.
#else
...
control ParserValidation(
    ...
    // This lives here for now because outer-validation always gets run.
    // If we change stuff such that this isn't the case (ie. generic parsing),
    // then this table may want to live somewhere else (such that it always
    // gets run).
    ...
    table handle_parser_errors {
        key = {
            ...
            //ig_intr_md_from_prsr.parser_err[1:1]: exact;
            ig_intr_md_from_prsr.parser_err: exact;
        }
        ...
        const entries = {
            ...
            // we see both partial_hdr and src_ext errors on this event.
	...
	apply {
#ifdef PARSER_ERROR_HANDLING_ENABLE
        if (ig_intr_md_from_prsr.parser_err != 0) {
