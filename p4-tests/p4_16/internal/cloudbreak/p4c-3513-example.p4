/*
    Simple program showing usage of functions extract_greedy() and lookahead_greedy().

    Refer to tofino3_base.p4 for definitions.

    The ingress parser in this code looks for packets with the following format:

        - ethernet/IPv4/metadata1_t/metadata1_t

    Specifically, it supports packets with the following content:

              |  14B | 20B|                            40 Bytes                                 |                                40 Bytes                            |
              |      |    |                                                                     |                                                                    |
              |      |    |                            metadata1_t (m1)                         |                  metadata1_t (m2 or m3)                            |
              |      |    |                                                                     |                                                                    |
      p_full= Ether()/IP()/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'dddd'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'/'DDDD'
     p_full1= Ether()/IP()/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'eeee'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'/'EEEE'


    Note: The minimum packet size received in the ingress parser is 64-bytes.  This means that
          for a header to be detected as incomplete, its last byte must be located beyond the
          64th bytes of the ethernet packet.
...
parser ParserImpl(packet_in packet, out headers hdr,
    ...
    state parse_ipv6 {
        extract_greedy(packet, hdr.ipv6);
        packet.extract(hdr.ipv4_inner);
