    ...
    in empty_t recirc_meta)
{
    ...
    state start {
	//XXX(hanw): there is an issue with eliminating copy operation in parser.
        //fabric_metadata.ingress_port = istd.ingress_port;
...
control FabricIngressDeparser(
    packet_out packet,
    out empty_t clone,
    out empty_t resub,
    out bridge_metadata_t normal,
    inout parsed_headers_t hdr,
    in fabric_metadata_t fabric_metadata,
    in psa_ingress_output_metadata_t istd)
{
    FabricCommonDeparser() commonDeparser;
    apply {
	//XXX(hanw): there is an issue with eliminating copy operation in parser.
        //normal.ingress_port = fabric_metadata.ingress_port;
        commonDeparser.apply(packet, hdr);
    }
}

// XXX(hanw): there is an issue with eliminating copy operation
// from parser inlining. Otherwise, we could factor out a large
// chunk of the ingress and egress parser into a common parser.
parser FabricEgressParser(
    packet_in packet,
    out parsed_headers_t hdr,
    inout fabric_metadata_t fabric_metadata,
    ...
    in empty_t clone_e2e_meta)
{
    ...
    state start {
	//XXX(hanw): there is an issue with eliminating copy operation in parser.
        //fabric_metadata.ingress_port = normal_meta.ingress_port;
        transition select(fabric_metadata.ingress_port) {
            CPU_PORT: parse_packet_out;
            default: parse_ethernet;
        }
    }
