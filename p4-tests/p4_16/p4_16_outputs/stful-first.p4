...
control OneBitRead(inout user_metadata_t md, in ingress_intrinsic_metadata_t ig_intr_md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    register<bit<1>, bit<16>>(16w1000) ob1;
    register<bit<1>, bit<16>>(16w1000) ob2;
    ...
    apply {
        md.one_bit_val_1 = one_bit_alu_1.execute(16w1);
        md.one_bit_val_2 = one_bit_alu_2.execute(16w2);
...
control BloomFilter(inout headers_t hdr, inout user_metadata_t md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    ...
    apply {
        bit<18> index_1 = bf_hash_1.get_hash<tuple<bit<8>, bit<30>, bit<30>, bit<16>, bit<16>>>({ hdr.ipv4.proto, hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.src_port, hdr.tcp.dst_port });
        bit<18> index_2 = bf_hash_2.get_hash<tuple<bit<8>, bit<30>, bit<30>, bit<16>, bit<16>>>({ hdr.ipv4.proto, hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.src_port, hdr.tcp.dst_port });
        bit<18> index_3 = bf_hash_3.get_hash<tuple<bit<8>, bit<30>, bit<30>, bit<16>, bit<16>>>({ hdr.ipv4.proto, hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.src_port, hdr.tcp.dst_port });
...
control ClearBloomFilter(inout headers_t hdr, inout user_metadata_t md) {
    ...
    bit<18> index = bf_hash.get_hash<tuple<bit<16>, bit<16>>>({ hdr.pktgen_generic.batch_id, hdr.pktgen_generic.packet_id });
    register<bit<1>, bit<18>>(18w262143) bloom_filter_1;
    register<bit<1>, bit<18>>(18w262143) bloom_filter_2;
    register<bit<1>, bit<18>>(18w262143) bloom_filter_3;
...
control SipSampler(in headers_t hdr, inout user_metadata_t md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    register<bit<32>, bit<18>>(18w143360, 32w1) sampling_cntr;
...
control Flowlet(inout headers_t hdr, inout user_metadata_t md) {
    stateful_param<bit<32>>(32w5000) flowlet_inactive_timeout;
    ...
    bit<15> index = flowlet_hash.get_hash<tuple<bit<8>, bit<30>, bit<30>, bit<16>, bit<16>>>({ hdr.ipv4.proto, hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.src_port, hdr.tcp.dst_port });
    apply {
...
control IpRoute(in headers_t hdr, inout user_metadata_t md) {
    ...
    action_selector<bit<29>>(32w4096, selector_mode_t.FAIR, 32w0, 32w0, next_hop_ecmp_reg) next_hop_ecmp_selector = {
        bit<29> hash() {
            hash<bit<29>>(hash_algorithm_t.CRC16) nexthop_ecmp_hash;
            return nexthop_ecmp_hash.get_hash<tuple<bit<8>, bit<30>, bit<30>, bit<16>, bit<16>>>({ hdr.ipv4.proto, hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.dst_port, hdr.tcp.src_port });
        }
    };
    table nexthop_ecmp {
        key = {
            md.nhop_id: exact @name("md.nhop_id") ;
        }
        actions = {
            set_nexthop();
            @defaultonly NoAction();
...
control Nexthop(inout headers_t hdr, inout user_metadata_t md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    register<bit<16>, bit<12>>(12w4095) scratch;
    ...
    table next_hop {
        key = {
            md.nhop_id: ternary @name("md.nhop_id") ;
        }
        actions = {
            set_eg_ifindex();
            scratch_add();
...
control EgressIfindex(inout user_metadata_t md, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    action_selector<bit<66>>(32w4096, selector_mode_t.FAIR, 32w128, 32w1200, lag_reg) lag_as = {
        bit<66> hash() {
        }
    };
    ...
    table eg_ifindex {
        key = {
            md.eg_ifindex: exact @name("md.eg_ifindex") ;
        }
        actions = {
            set_eg_port();
            @defaultonly NoAction();
