...
const bit<16> MEMCACHED_UDP_PORT = 16w11211;
const bit<8> MEMCACHED_MAGIC_REQ_CLIENT_TO_SERVER = 8w0x80;
struct headers_t {
...
parser MemcachedCommonParser(packet_in pkt, out headers_t hdr) {
    ...
    state parse_memcached {
        pkt.extract<MemcachedHdr_t>(hdr.memcached_hdr);
        transition select(hdr.memcached_hdr.magic) {
            8w0x80: parse_memcached_req;
            default: accept;
        }
    }
    ...
    state parse_memcached_key {
        transition select(hdr.memcached_hdr.keyLen) {
            16w16: parse_memcached_key_16B;
            default: accept;
        }
    }
    state parse_memcached_key_16B {
        pkt.extract<MemcachedFixedSizeKey_t>(hdr.memcached_fixed_size_key);
        transition accept;
    }
    state parse_memcached_extras {
        pkt.extract<MemachedExtras_t>(hdr.memcached_extras);
        transition accept;
    }
}
