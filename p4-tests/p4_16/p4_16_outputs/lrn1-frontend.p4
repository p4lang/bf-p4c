...
control iDprsr(packet_out pkt, inout headers hdr, in metadata md, in ingress_intrinsic_metadata_for_deparser_t ig_intr_dprsr_md) {
    ...
    apply {
        if (ig_intr_dprsr_md.digest_type == 3w0) {
            d_0 = { hdr.ethernet.dmac, hdr.ethernet.smac, hdr.ethernet.etype, md.port };
            d0_0.pack(d_0);
        }
        if (ig_intr_dprsr_md.digest_type == 3w1) {
            d_1 = { md.port, hdr.ethernet.etype, hdr.ethernet.smac, hdr.ethernet.dmac };
            d1_0.pack(d_1);
        }
        if (ig_intr_dprsr_md.digest_type == 3w2) {
            d_2 = { hdr.ipv6.next_hdr, md.port, hdr.ethernet.dmac, hdr.ethernet.smac, hdr.ipv6.src, hdr.ipv6.dst };
            d2_0.pack(d_2);
        }
        if (ig_intr_dprsr_md.digest_type == 3w3) {
            d_3 = { hdr.ipv4.protocol, md.port, hdr.ethernet.dmac, hdr.ethernet.smac, hdr.ipv4.src, hdr.ipv4.dst };
            d3_0.pack(d_3);
        }
        ...
        if (ig_intr_dprsr_md.digest_type == 3w5) {
            d_5 = { hdr.ethernet.smac[3:3] };
            d5_0.pack(d_5);
        }
        if (ig_intr_dprsr_md.digest_type == 3w6) {
            d_6 = { hdr.ethernet.isValid(), hdr.ipv4.isValid(), hdr.ipv6.isValid(), hdr.big.isValid() };
            d6_0.pack(d_6);
        }
