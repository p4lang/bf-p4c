...
parser UPFIngressParser(packet_in pkt, out headers hdr, out upf_ingress_metadata_t ig_md, out ingress_intrinsic_metadata_t ig_intr_md) {
    upf_port_metatdata_t port_md_0;
    dp_ctrl_header_t ether_0;
    dp_ctrl_header_t tmp;
    bit<112> tmp_2;
    state start {
    ...
    state parse_port_metadata {
        port_md_0.setInvalid();
        pkt.extract<upf_port_metatdata_t>(port_md_0);
        tmp_2 = pkt.lookahead<bit<112>>();
        tmp.setValid();
        tmp._pad0 = tmp_2[111:107];
        tmp.ring_id = tmp_2[106:104];
        tmp._pad1 = tmp_2[103:25];
        tmp.port = tmp_2[24:16];
        tmp.etherType = tmp_2[15:0];
        ether_0 = tmp;
        transition select(ether_0.etherType) {
...
control UPFEgressDeparser(packet_out pkt, inout headers hdr, in upf_egress_metadata_t eg_md, in egress_intrinsic_metadata_for_deparser_t eg_intr_md_for_dprsr) {
    ...
    @hidden action act_0() {
        tmp_0 = outer_ipv4_checksum_0.update<tuple_0>({ hdr.outer_ipv4.version, hdr.outer_ipv4.ihl, hdr.outer_ipv4.diffserv, hdr.outer_ipv4.totalLen, hdr.outer_ipv4.identification, hdr.outer_ipv4.flags, hdr.outer_ipv4.fragOffset, hdr.outer_ipv4.ttl, hdr.outer_ipv4.protocol, hdr.outer_ipv4.srcAddr, hdr.outer_ipv4.dstAddr });
        hdr.outer_ipv4.hdrChecksum = tmp_0;
        tmp_1 = inner_ipv4_checksum_0.update<tuple_0>({ hdr.inner_ipv4.version, hdr.inner_ipv4.ihl, hdr.inner_ipv4.diffserv, hdr.inner_ipv4.totalLen, hdr.inner_ipv4.identification, hdr.inner_ipv4.flags, hdr.inner_ipv4.fragOffset, hdr.inner_ipv4.ttl, hdr.inner_ipv4.protocol, hdr.inner_ipv4.srcAddr, hdr.inner_ipv4.dstAddr });
        hdr.inner_ipv4.hdrChecksum = tmp_1;
        pkt.emit<dp_ctrl_header_t>(hdr.dp_ctrl);
        pkt.emit<ethernet_t>(hdr.ethernet);
        pkt.emit<vlan_t>(hdr.vlan);
        pkt.emit<ipv4_t>(hdr.outer_ipv4);
...
control UPFIngress(inout headers hdr, inout upf_ingress_metadata_t ig_md, in ingress_intrinsic_metadata_t ig_intr_md, in ingress_intrinsic_metadata_from_parser_t ig_intr_from_prsr, inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr, inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    ...
    @name("UPFIngress.ig_drp_cntr") DirectCounter<bit<32>>(CounterType_t.PACKETS_AND_BYTES) ig_drp_cntr_0;
    @name("UPFIngress.send_cpu_cntr") DirectCounter<bit<32>>(CounterType_t.PACKETS_AND_BYTES) send_cpu_cntr_0;
    @name("UPFIngress.set_cpu_port") action set_cpu_port(PortId_t cpu_port) {
        ig_md.cpu_port = cpu_port;
    }
    @name("UPFIngress.read_cpu_port_table") table read_cpu_port_table_0 {
        key = {
    ...
    @name("UPFIngress.send_to_cpu") action send_to_cpu() {
        ...
        send_cpu_cntr_0.count();
        hasExited = true;
    }
    @name("UPFIngress.send_from_cpu") action send_from_cpu() {
        ig_intr_md_for_tm.ucast_egress_port = hdr.dp_ctrl.port;
        hasExited = true;
    }
    @name("UPFIngress.forward") action forward(PortId_t eg_port) {
        ig_intr_md_for_tm.ucast_egress_port = eg_port;
    }
    @name("UPFIngress.ul_dl_session_table") table ul_dl_session_table_0 {
        key = {
    ...
    @name("UPFIngress.add_bridged_md") action add_bridged_md() {
    ...
    @hidden table tbl_send_from_cpu {
        actions = {
            send_from_cpu();
        }
        const default_action = send_from_cpu();
    }
    @hidden table tbl_send_to_cpu {
        actions = {
            send_to_cpu();
        }
        const default_action = send_to_cpu();
    }
    @hidden table tbl_add_bridged_md {
        actions = {
            add_bridged_md();
        }
        const default_action = add_bridged_md();
    }
    apply {
        tbl_act_1.apply();
        switch (ul_dl_session_table_0.apply().action_run) {
            NoAction_0: {
                read_cpu_port_table_0.apply();
                if (ig_intr_md.ingress_port == ig_md.cpu_port) 
                    tbl_send_from_cpu.apply();
                else 
                    tbl_send_to_cpu.apply();
            }
        }

        if (!hasExited) 
            tbl_add_bridged_md.apply();
    }
}
...
control UPFEgress(inout headers hdr, inout upf_egress_metadata_t eg_md, in egress_intrinsic_metadata_t eg_intr_md, in egress_intrinsic_metadata_from_parser_t eg_intr_from_prsr, inout egress_intrinsic_metadata_for_deparser_t eg_intr_md_for_dprsr, inout egress_intrinsic_metadata_for_output_port_t eg_intr_md_for_oport) {
    @name("UPFEgress.uplink.decap_gtp") action uplink_decap_gtp_0() {
        hdr.outer_ipv4.setInvalid();
        hdr.outer_udp.setInvalid();
        hdr.gtp_u.setInvalid();
    }
    @name("UPFEgress.downlink.sub_fcs_tofino") action downlink_sub_fcs_tofino_0() {
        eg_md.pkt_length = eg_md.pkt_length + 16w0xfffc;
    }
    @name("UPFEgress.downlink.payload_vlan_len") action downlink_payload_vlan_len_0() {
        eg_md.pkt_length = eg_md.pkt_length + 16w0xfffc;
    }
    @name("UPFEgress.downlink.payload_len") action downlink_payload_len_0() {
        eg_md.pkt_length = eg_md.pkt_length + 16w0xfff2;
    }
    @name("UPFEgress.downlink.compute_len") action downlink_compute_len_0() {
    ...
    @name("UPFEgress.downlink.copy_outer_ip_to_inner_ip") action downlink_copy_outer_ip_to_inner_ip_0() {
        hdr.inner_ipv4.setValid();
        hdr.inner_ipv4 = hdr.outer_ipv4;
        hdr.outer_ipv4.protocol = 8w17;
    }
    @name("UPFEgress.downlink.add_gtp") action downlink_add_gtp_0() {
        hdr.gtp_u.setValid();
    ...
    @name("UPFEgress.downlink.copy_outer_tcp_to_inner_tcp") action downlink_copy_outer_tcp_to_inner_tcp_0() {
        hdr.inner_tcp.setValid();
        hdr.inner_tcp = hdr.outer_tcp;
        hdr.outer_tcp.setInvalid();
    }
    @name("UPFEgress.downlink.copy_outer_udp_to_inner_udp") action downlink_copy_outer_udp_to_inner_udp_0() {
        hdr.inner_udp.setValid();
        hdr.inner_udp = hdr.outer_udp;
        hdr.inner_udp.checksum = hdr.outer_udp.checksum;
    }
    @name("UPFEgress.downlink.add_outer_udp") action downlink_add_outer_udp_0() {
        hdr.outer_udp.setValid();
        hdr.outer_udp.checksum = 16w0;
        hdr.outer_udp.dstPort = 16w2152;
        hdr.outer_udp.srcPort = 16w2152;
    }
    @name("UPFEgress.eg_drp_cntr") DirectCounter<bit<32>>(CounterType_t.PACKETS_AND_BYTES) eg_drp_cntr_0;
    @name("UPFEgress.dl_cntr") DirectCounter<bit<32>>(CounterType_t.PACKETS_AND_BYTES) dl_cntr_0;
    @name("UPFEgress.ul_cntr") DirectCounter<bit<32>>(CounterType_t.PACKETS_AND_BYTES) ul_cntr_0;
    @hidden action act_2() {
    ...
    @hidden table tbl_uplink_decap_gtp {
        actions = {
            uplink_decap_gtp_0();
        }
        const default_action = uplink_decap_gtp_0();
    }
    ...
    @hidden table tbl_downlink_copy_outer_ip_to_inner_ip {
        actions = {
            downlink_copy_outer_ip_to_inner_ip_0();
        }
        const default_action = downlink_copy_outer_ip_to_inner_ip_0();
    }
    @hidden table tbl_downlink_add_gtp {
        actions = {
            downlink_add_gtp_0();
        }
        const default_action = downlink_add_gtp_0();
    }
    @hidden table tbl_downlink_copy_outer_tcp_to_inner_tcp {
        actions = {
            downlink_copy_outer_tcp_to_inner_tcp_0();
        }
        const default_action = downlink_copy_outer_tcp_to_inner_tcp_0();
    }
    @hidden table tbl_downlink_copy_outer_udp_to_inner_udp {
        actions = {
            downlink_copy_outer_udp_to_inner_udp_0();
        }
        const default_action = downlink_copy_outer_udp_to_inner_udp_0();
    }
    @hidden table tbl_downlink_add_outer_udp {
        actions = {
            downlink_add_outer_udp_0();
        }
        const default_action = downlink_add_outer_udp_0();
    }
    @hidden table tbl_downlink_payload_vlan_len {
        actions = {
            downlink_payload_vlan_len_0();
        }
        const default_action = downlink_payload_vlan_len_0();
    }
    @hidden table tbl_downlink_sub_fcs_tofino {
        actions = {
            downlink_sub_fcs_tofino_0();
        }
        const default_action = downlink_sub_fcs_tofino_0();
    }
    @hidden table tbl_downlink_payload_len {
        actions = {
            downlink_payload_len_0();
        }
        const default_action = downlink_payload_len_0();
    }
    @hidden table tbl_downlink_compute_len {
        actions = {
            downlink_compute_len_0();
        }
        const default_action = downlink_compute_len_0();
    }
    apply {
        if (hdr.dp_ctrl.isValid() && hdr.ethernet.isValid()) 
            ...
            if (hdr.gtp_u.isValid()) {
                tbl_act_3.apply();
                tbl_uplink_decap_gtp.apply();
            }
            else {
                tbl_act_4.apply();
                tbl_downlink_copy_outer_ip_to_inner_ip.apply();
                tbl_downlink_add_gtp.apply();
                if (hdr.outer_tcp.isValid()) 
                    tbl_downlink_copy_outer_tcp_to_inner_tcp.apply();
                else 
                    if (hdr.outer_udp.isValid()) 
                        tbl_downlink_copy_outer_udp_to_inner_udp.apply();
                tbl_downlink_add_outer_udp.apply();
                if (hdr.vlan.isValid()) 
                    tbl_downlink_payload_vlan_len.apply();
                tbl_downlink_sub_fcs_tofino.apply();
                tbl_downlink_payload_len.apply();
                tbl_downlink_compute_len.apply();
            }
    }
}
