...
header egr_port_mirror_h {
    ...
    @flexible  bit<48>     ingress_mac_tstamp;
#if !defined(TOFINO_TELEMETRY) || defined(P4C_3670)    
    /* 
     * Unfortunately we have to sacrifice this field in order to fit into
     * the mirror buffer
     */    
    @flexible  bit<48>     ingress_global_tstamp;
#endif
...
#else
    @padding PortId_Pad_t    pad0; PortId_t    ingress_port;
    @padding PortId_Pad_t    pad1; PortId_t    egress_port;
    @padding MirrorId_Pad_t  pad2; MirrorId_t  mirror_session;
                                   bit<16>     pkt_length;
                                   bit<48>     ingress_mac_tstamp;
#if !defined(TOFINO_TELEMETRY) || defined(P4C_3670)    
                                   bit<48>     ingress_global_tstamp;
    ...
    @padding QueueId_PAD_t  pad9;  QueueId_t   egress_qid;
...
#define CPU_PAD(N, field) field
    ...
    action send_to_cpu_ing_mirror(bit<48> dst_mac, bit<48>src_mac) {
        ...
        CPU_PAD( 9, hdr.to_cpu.egress_qid)            = 0;
    ...
    action send_to_cpu_egr_mirror(bit<48> dst_mac, bit<48>src_mac) {
        ...
        CPU_PAD( 9, hdr.to_cpu.egress_qid)            =
                                    meta.egr_port_mirror.egress_qid;
        CPU_PAD(10, hdr.to_cpu.egress_cos)            =
                                    meta.egr_port_mirror.egress_cos;
        CPU_PAD(11, hdr.to_cpu.deflection_flag)       =
                                    meta.egr_port_mirror.deflection_flag;
#else
        CPU_PAD( 3, hdr.to_cpu.enq_qdepth)            = 0;
        CPU_PAD( 4, hdr.to_cpu.enq_congest_stat)      = 0;  
        CPU_PAD( 6, hdr.to_cpu.deq_qdepth)            = 0;
...
#ifdef MODEL_625_FIXED
        /* 
         * This is how original packet length is supposed to be calculated
         * assuming that no modifications (aside from prepending the bridge 
         * header took place in the Ingress control. If more modifications
         * took place, they need to be accounted for. This works on the real
         * hardware
         */
        meta.egr_mirror_pkt_length  =
                    eg_intr_md.pkt_length - (bit<16>)sizeInBytes(meta.bridge);
#else
        /* 
         * This is a workaround that should be used on the current version of
         * the model, because it simply passes the ingress packet length
         */
        ...
        if (eg_dprsr_md.mirror_type == EGR_PORT_MIRROR) {
            egr_port_mirror.emit<egr_port_mirror_h>(
                meta.egr_mirror_session,
                {
                    ...
                    PAD(eg_intr_md.deflection_flag)
#endif
                });
        }
