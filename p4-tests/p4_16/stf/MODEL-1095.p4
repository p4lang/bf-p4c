...
@command_line("--auto-init-metadata")
@command_line("--num-stages-override", 16)
...
typedef bit<12> switch_tunnel_ip_index_t;



typedef bit<14> switch_tunnel_nexthop_t;
...
control IngressIpAcl(inout switch_local_metadata_t local_md,
    ...
    action no_action() { stats.count(); } action set_tc(switch_tc_t tc) { local_md.qos.tc = tc; stats.count(); } action set_color(switch_pkt_color_t color) { local_md.qos.color = color; stats.count(); } action set_meter(switch_meter_index_t index) { local_md.qos.acl_meter_index = index; stats.count(); } action deny() { local_md.flags.acl_deny = true; stats.count(); } action copy_to_cpu(switch_hostif_trap_t trap_id) { local_md.hostif_trap_id = trap_id; stats.count(); } action trap(switch_hostif_trap_t trap_id) { local_md.hostif_trap_id = trap_id; deny(); } action permit(switch_user_metadata_t user_metadata) { local_md.flags.acl_deny = false; local_md.user_metadata = user_metadata; stats.count(); } action redirect_port(switch_user_metadata_t user_metadata, switch_port_lag_index_t egress_port_lag_index) { local_md.flags.acl_deny = false; local_md.egress_port_lag_index = egress_port_lag_index; local_md.acl_port_redirect = true; local_md.user_metadata = user_metadata; stats.count(); } action mirror(switch_mirror_meter_id_t meter_index, switch_mirror_session_t session_id) { local_md.mirror.type = 1; local_md.mirror.src = SWITCH_PKT_SRC_CLONED_INGRESS; local_md.mirror.session_id = session_id; local_md.mirror.meter_index = meter_index; stats.count(); } action set_dtel_report_type(switch_dtel_report_type_t type) { local_md.dtel.report_type = local_md.dtel.report_type | type; stats.count(); }
...
control IngressSystemAcl(
    ...
    action copy_sflow_to_cpu(switch_cpu_reason_t reason_code,
                             ...
                             switch_copp_meter_id_t meter_id,
                             bool disable_learning) {
        copy_to_cpu(reason_code + (bit<16>)local_md.sflow.session_id, qid, meter_id, disable_learning);
    }
    ...
    table system_acl {
        key = {
            ...
            local_md.flags.acl_meter_drop : ternary;
...
//        hdr.gre.flags = 0;
    ...
    action rewrite_erspan_type2(
        ...
        hdr.ipv4.total_len = local_md.pkt_length + 16w20;
    ...
    action rewrite_erspan_type3(
        ...
        hdr.ipv4.total_len = local_md.pkt_length + 16w24;
...
control IngressAclMeter(inout switch_local_metadata_t local_md)(
    ...
    action count() {
        stats.count();
        local_md.flags.acl_meter_drop = false;
    }

    action drop_and_count() {
        stats.count();
        local_md.flags.acl_meter_drop = true;
    }
...
//    PFCWd(512) pfc_wd;
    ...
    action set_egress_port(switch_port_t port) { ig_intr_md_for_tm.ucast_egress_port = port; } table snake { key = { local_md.ingress_port : exact; } actions = { set_egress_port; } size = SNAKE_TABLE_SIZE; }
...
//    PFCWd(512) pfc_wd;
    ...
    apply {
        ...
        if (local_md.pkt_src == SWITCH_PKT_SRC_BRIDGED) {
            port_mirror.apply(eg_intr_md.egress_port, SWITCH_PKT_SRC_CLONED_EGRESS, local_md.mirror);
            if (local_md.tunnel.terminate) {
                tunnel_decap.apply(hdr, local_md);
            } else {
                vlan_decap.apply(hdr, local_md);
            }
            if (local_md.flags.routed) egress_vrf.apply(hdr, local_md);
