...
# 16 "spine/p4src/spine.p4" 2


# 1 "spine/p4src/table_profile.p4" 1
/****************************************************************
 * Copyright (c) Kaloom, 2019
 *
 * This unpublished material is property of Kaloom Inc.
 ...
 */
# 14 "spine/p4src/table_profile.p4" 2


const bit<32> L2_INGRESS_TABLE_SIZE = 1024;
const bit<32> ROUTING_IPV6_TABLE_SIZE = 4096;
 ...
 * forbidden except by express written permission of Kaloom Inc.
 ****************************************************************/
# 21 "spine/p4src/spine.p4" 2
# 1 "spine/p4src/parde.p4" 1
/****************************************************************
 * Copyright (c) Kaloom, 2019
 *
 * This unpublished material is property of Kaloom Inc.
 ...
 * forbidden except by express written permission of Kaloom Inc.
 ****************************************************************/
# 15 "spine/p4src/parde.p4" 2
# 1 "spine/p4src/types.p4" 1
/****************************************************************
 * Copyright (c) Kaloom, 2019
 *
 * This unpublished material is property of Kaloom Inc.
 ...
 * forbidden except by express written permission of Kaloom Inc.
 ****************************************************************/
# 21 "spine/p4src/types.p4" 2

/* Port Metadata (Phase 0). Must be 64 bits. */
header port_metadata_t {
    logical_port_id_t logical_port_id;
    bit<1> disabled; /* All traffic except LLDP is blocked when true */
    bit<55> pad0;
}
...
/* Ingress Metadata */
@pa_alias("ingress", "ig_md.ingress_mac_tstamp", "hdr.bridged_md.ingress_tstamp")
...
parser SwitchEgressParser(
    ...
    state parse_knf {
        pkt.extract(hdr.knf);
        eg_md.tel_metadata.seq_num = hdr.knf.telSequenceNum;
        transition accept;
    }
}
...
control SwitchIngress(
    ...
    apply {
        ...
        if (!hdr.pktgen_ext_header.isValid()) {
            ...
            copp_flow_classification.apply(hdr, ig_tm_md);
            ...
            copp_rate_limiting.apply(ig_intr_md, ig_md.fabric_meta, ig_tm_md,
                    ig_dprsr_md);
        } else {
