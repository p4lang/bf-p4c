...
# 11 "leaf.p4" 2
# 1 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tna.p4" 1
/*
...
# 1 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tofino1arch.p4" 1
/*
...
/* This is the P4-16 core library, which declares some built-in P4 constructs using P4 */
# 23 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tofino1arch.p4" 2
# 1 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tofino.p4" 1
/*
 * Copyright (c) 2015-2019 Barefoot Networks, Inc.
 *
 * All Rights Reserved.
...
/* This is the P4-16 core library, which declares some built-in P4 constructs using P4 */
# 23 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tofino.p4" 2
...
# 24 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tofino1arch.p4" 2
...
# 21 "/home/parallels/bf-sde-9.2.0/install/share/p4c/p4include/tna.p4" 2
# 12 "leaf.p4" 2
...
/* Header structure */
/* Due to recent changes in the parser, the compiler has overlayed VLAN header
 * fields with underlay fields (IPv6 and KNF headers) resulting in errored packets.
 * As a quick fix we can prevent the compiler from overlaying these headers by the
 * use of pa_no_overlay annotation until we find a better optimized way.
 */
@pa_no_overlay("ingress", "hdr.vlan.pcp")
@pa_no_overlay("ingress", "hdr.vlan.cfi")
@pa_no_overlay("ingress", "hdr.vlan.vlanID")
@pa_no_overlay("ingress", "hdr.vlan.etherType")

@pa_no_overlay("egress", "hdr.vlan.pcp")
@pa_no_overlay("egress", "hdr.vlan.cfi")
@pa_no_overlay("egress", "hdr.vlan.vlanID")
@pa_no_overlay("egress", "hdr.vlan.etherType")
struct header_t {
...
parser SwitchIngressParser(
    ...
    state start {
        ...
        ig_md.ip_options_unsupported = 1w0;
...
control PuntUserPackets(
    ...
    Random<bit<3>>() random_value;
    Meter<logical_port_id_t>(COPP_METER_SIZE, MeterType_t.BYTES) user_punt_meter;

    action user_punt_hit(port_id_t cpu_port) {
        //ig_md.fabric_meta.copp_packet_color = user_punt_meter.execute(ig_md.fabric_meta.logical_port_id);
        hdr.dp_ctrl_hdr.setValid();
        hdr.dp_ctrl_hdr.ring_id = random_value.get();
...
control VnetMapping(
    ...
    action kvs_multicast_hit(port_id_t cpu_port) {
        //ig_md.fabric_meta.copp_packet_color = kvs_meter.execute(ig_md.fabric_meta.logical_port_id);
...
control VnetEgressVLANCntr(
    ...
    action set_eth_type_qinq() {
        hdr.inner_ethernet.etherType = ETHERTYPE_QINQ;
    }

    action set_eth_type_miss() {}

    table set_qinq_type {
        key = {
            hdr.inner_ethernet.isValid() : exact;
            hdr.vlan.isValid() : exact;
            hdr.vlan.etherType : exact;
        }
        actions = {
            set_eth_type_qinq;
            set_eth_type_miss;
        }
        const default_action = set_eth_type_miss;
        const entries = {
            (true, true, ETHERTYPE_VLAN) : set_eth_type_qinq;
        }
    }
...
control VRouterPuntChannel(inout ingress_metadata_t ig_md,
    ...
    action set_punt_data(nw_id_t p_nw_id, ipv6_addr_t punt_ip, meter_index_t index) {
        //ig_md.ktep_router_meta.meter_color = unicast_punt_meter.execute(index);
...
control EgressCPUPortEncap(
    ...
    action dp_ctrl_header_encap() {
        hdr.dp_ctrl_hdr.setValid();
        hdr.dp_ctrl_hdr.ring_id = generate_ring_id.execute(0);
        hdr.dp_ctrl_hdr.etherType = ETHERTYPE_DP_CTRL;
...
control L2Ingress(
    ...
    action l2_ingress_send_to_cpu_hit(port_id_t cpu_port) {
        //fabric_meta.copp_packet_color = l2_ingress_meter.execute(fabric_meta.logical_port_id);
        hdr.dp_ctrl_hdr.setValid();
        hdr.dp_ctrl_hdr.ring_id = random_value.get();
        hdr.dp_ctrl_hdr.etherType = ETHERTYPE_DP_CTRL;
        hdr.dp_ctrl_hdr.port = (bit<16>)ig_intr_md.ingress_port;
...
control FabricRouting(
    ...
    action routing_to_host(port_id_t cpu_port) {
        //fabric_meta.copp_packet_color = routing_meter.execute(fabric_meta.logical_port_id);
        hdr.dp_ctrl_hdr.setValid();
        hdr.dp_ctrl_hdr.ring_id = random_value.get();
        hdr.dp_ctrl_hdr.etherType = ETHERTYPE_DP_CTRL;
        hdr.dp_ctrl_hdr.port = (bit<16>)ig_intr_md.ingress_port;
...
control ReplicatedPackets(
    ...
    action rid_punt_channel_hit(dest_id_t punt_dest_id, nw_id_t p_nw_id, meter_index_t index) {
        //eg_md.ktep_router_meta.meter_color = multicast_punt_meter.execute(index);
    ...
    apply {
        ...
        if (!hdr.pktgen_ext_header.isValid()) {
            ...
            if ((ig_md.ktep_router_meta.punt == 1) ||
                ...
                prepare_vrouter_punt.apply(ig_md, ig_intr_md_for_tm, ig_dprsr_md);
