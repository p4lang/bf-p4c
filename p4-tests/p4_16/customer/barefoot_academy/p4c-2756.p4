    ...
    out ingress_intrinsic_metadata_t  ig_intr_md)
{
    ...
    state parse_srv6 {
        ...
        // routing type and last segments should be correct
        transition select(hdr.srv6.routing_type, hdr.srv6.last_entry) {
            (4, 0) : parse_sid_1;
            (4, 1) : parse_sid_2;
            (4, 2) : parse_sid_3;
        }
    ...
    state parse_sid_1 {
        pkt.extract(hdr.segment_list[0]);
        parser_counter.set(hdr.srv6.hdr_ext_len);
        parser_counter.decrement(2);

        transition select(parser_counter.is_zero(), parser_counter.is_negative()) {
            (false, false) : parse_srv6_tlv;
            (true, false)  : accept;
        }
    }
    ...
    state parse_sid_2 {
        pkt.extract(hdr.segment_list[0]);
        pkt.extract(hdr.segment_list[1]);
        parser_counter.set(hdr.srv6.hdr_ext_len);
    ...
    state parse_sid_3 {
        pkt.extract(hdr.segment_list[0]);
        pkt.extract(hdr.segment_list[1]);
        pkt.extract(hdr.segment_list[2]);
        parser_counter.set(hdr.srv6.hdr_ext_len);
        parser_counter.decrement(6);
        transition select(parser_counter.is_zero(), parser_counter.is_negative()) {
            (false, false) : parse_srv6_tlv;
            (true, false)  : accept;
        }    
