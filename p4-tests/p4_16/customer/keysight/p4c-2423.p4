...
/*------------------------------------------------------------------------
    ...
    All Rights Reserved.
*-----------------------------------------------------------------------*/
//#define EMPTY_CONTROLS 1 // define to make ingress & egress empty
...
// MOVE UDF param load to EG, reduce bridged MD
# 25 "pktgen9_16.p4"
//====================
...
struct egress_metadata_t {
    ...
    udf_cntr_params_brdg_t udf1_cntr_params_br; // loaded in ingress to reduce br_md
    udf_cntr_params_brdg_t udf2_cntr_params_br; // loaded in ingress to reduce br_md
...
parser SwitchIngressParser(
    ...
    //         ETHERTYPE_BF_FABRIC: parse_fabric_only;
    //         default: accept;
    //     }
    // }
    state start {
        ...
        /* TEMP - hide, error in sde-9.1.0.11-pr
        /home/chris/9.0.0.nanite_demo.p4-16/pktgen9_16/parser.p4(56): [--Werror=legacy] error: Extraction source for ingress::hdr.bridged_md.ingress_port[8:8] is out of state ingress::TofinoIngressParser_parse_port_metadata's input buffer (32 bytes)
 ...
 */
  ...
  control process_load_udf_cntr_params_ingress(
    ...
    action do_load_udf1_cntr_params_ingress(bit<32> nested_repeat, bit<32> nested_step, bit<2> bit_mask) {
        eg_md.udf1_cntr_params_br.nested_repeat = nested_repeat;
        eg_md.udf1_cntr_params_br.nested_step = nested_step;
        eg_md.udf1_cntr_params_br.bit_mask = bit_mask;
    }
    ...
    action do_load_udf2_cntr_params_ingress(bit<32> nested_repeat, bit<32> nested_step, bit<2> bit_mask) {
        eg_md.udf2_cntr_params_br.nested_repeat = nested_repeat;
        eg_md.udf2_cntr_params_br.nested_step = nested_step;
        eg_md.udf2_cntr_params_br.bit_mask = bit_mask;
    }
...
control process_intermediate_outer_cntr_udf(inout header_t hdr, inout egress_metadata_t eg_md) {
    ...
    RegisterAction<reg_pair_t, bit<32>, bit<32>>(udf1_nested_cntr_udf_reg) udf1_nested_cntr_udf = {
        void apply(inout reg_pair_t value, out bit<32> rv) {
            ...
            if (in_value.lo + 32w1 <= eg_md.udf1_cntr_params_br.nested_repeat) {
                ...
                value.hi = in_value.hi + eg_md.udf1_cntr_params_br.nested_step;
            }
    ...
    RegisterAction<reg_pair_t, bit<32>, bit<32>>(udf2_nested_cntr_udf_reg) udf2_nested_cntr_udf = {
        void apply(inout reg_pair_t value, out bit<32> rv) {
            ...
            in_value = value;
            rv = in_value.hi;
            if (in_value.lo + 32w1 <= eg_md.udf2_cntr_params_br.nested_repeat) {
                value.hi = in_value.hi + eg_md.udf2_cntr_params_br.nested_step;
            }
            if (!(in_value.lo + 32w1 <= eg_md.udf2_cntr_params_br.nested_repeat)) {
                value.hi = (bit<32>)0;
            }
...
control process_cntr_udf(inout header_t hdr, in egress_intrinsic_metadata_t eg_intr_md, inout egress_metadata_t eg_md) {
    ...
    table udf1_cntr_udf_tbl {
        ...
        key = {
            eg_md.udf1_cntr_params_br.bit_mask : exact;
            eg_md.udf1_cntr_params_eg.enable : exact;
            hdr.bridged_md.stream : ternary;
            eg_intr_md.egress_port: ternary;
        }
    ...
    table udf2_cntr_udf_tbl {
        ...
        key = {
            eg_md.udf2_cntr_params_br.bit_mask : exact;
            eg_md.udf2_cntr_params_eg.enable : exact;
            hdr.bridged_md.stream : ternary;
            eg_intr_md.egress_port: ternary;
        }
...
control SwitchIngress(
    ...
    apply {
        hdr.bridged_md.setValid(); // must set valid to emit
...
control SwitchEgress(
    ...
    apply {
        process_load_udf1_cntr_params_0.apply(hdr, eg_intr_md, eg_md); // TODO: why isn't this conditional same as process_load_udf2_cntr_params_0()?
        ...
        process_load_udf_cntr_params_ingress_0.apply(hdr, eg_md, eg_intr_md);
            ...
            if (eg_md.burst_pkt_cntr.drop == 1w1) {
                // TODO - uncomment; w/o ctrl plane, always dropping
