...
@pa_no_overlay("ingress","hdr.bridged_md.ingress_port")
@flexible
...
control process_eg_stream_stats(inout header_t hdr, inout egress_metadata_t eg_md) {
    ...
    display_name="Tx Stream Stats A", display_cols="Port|TxBytesA|TxPacketsA",col_units="|bytes|packets")
    @Api(index=port, label="Tx Stream Stats A", col_labels="Port|TxStreamBytesA|TxStreamPacketsA",col_units="|bytes|packets")
    ...
    display_name="Tx Stream Stats B", display_cols="Port|TxBytesB|TxPacketsB",col_units="|bytes|packets")
    @Api(index=port, label="Tx Stream Stats B", col_labels="Port|TxStreamBytesB|TxStreamPacketsB",col_units="|bytes|packets")
    ...
    @Feature(name="StreamStats", block=eg_port_stats, service=Stats, service_id=".*StreamStats.*",
      index=pgid, display_name="All Stream Stats")
    @Api(index=pgid, label="All Stream Stats")
    ...
    Register<reg_pair_t, bit<32>>(1024,{0,0}) fake_stream_stats_reg;
...
control process_load_cpu_parameters(inout header_t hdr, in ingress_intrinsic_metadata_t ig_intr_md, inout ingress_metadata_t ig_md) {
    ...
    @name(".load_cpu_enabled_parameter") action load_cpu_enabled_parameter() {
        hdr.bridged_md.pcie_cpu_port.enabled = 1w1;
        hdr.bridged_md.pcie_cpu_port.ingress_port = ig_intr_md.ingress_port;
    }
    @name(".load_cpu_enabled_parameter_tbl") table load_cpu_enabled_parameter_tbl {
        actions = {
            _nop;
...
control process_ig_lat_stats(inout header_t hdr, inout ingress_metadata_t ig_md) {
    ...
    Register<bit<32>,bit<16>>(16384) rx_first_stamp_reg;
    ...
    @Feature(name="RxPgidTotLat", block=ig_pgid_lat_stats, service=Stats, service_id=RxPgidTotLat, index=pgid, display_name="Rx Tot Latency", display_cols="PGID|TotLatHi|TotLatLo",col_units="|nsec*2^32|nsec")

    @Api(index=pgid,label="Rx Tot Latency", col_labels="PGID|TotLatHi|TotLatLo",col_units="|nsec*2^32|nsec")
    @brief("PGID Tot latency register")
    @description("PGID Tot latency register, holds the minimum latency for a given flow;double-precision")
...
control process_ig_seq_stats(inout header_t hdr, inout ingress_metadata_t ig_md) {
    ...
    @Feature(name="RxPgidMaxSeqNum", block=ig_pgid_seq_stats, service=Stats, service_id=RxPgidMaxSeqNum, index=pgid, display_name="Rx Max Sequence Number", display_cols="PGID|MaxSeqNum",col_units="|packets")

    @Api(index=pgid, label="Rx Max Sequence Number", col_labels="PGID|MaxSeqNum",col_units="|packets")
...
control pack_stream_port(inout header_t hdr, in ingress_intrinsic_metadata_t ig_intr_md, inout ingress_metadata_t ig_md) {
...
control process_udf_vlist_mac_ip(inout header_t hdr, in egress_intrinsic_metadata_t eg_intr_md, inout egress_metadata_t eg_md) {
    ...
    @name(".udf_vlist_mac_ip_tbl") table udf_vlist_mac_ip_tbl {
        ...
        size = 32768; // larger values cause PHV alloc error
...
control SwitchIngress(
    ...
    // TODO- move these to separate control file?

    @name(".common_drop_pkt") action common_drop_pkt() {
    ...
    @name(".do_drop") action do_drop() {
     ...
     Hash<bit<7>>(HashAlgorithm_t.IDENTITY) identity_hash_stream2qid;

    @name("._set_eg_port") action _set_eg_port(bit<9> eg_port) {
        ig_tm_md.ucast_egress_port = eg_port;

       ig_tm_md.qid = identity_hash_stream2qid.get({2w0 +++ hdr.bridged_md.stream});



    }
    @name(".do_set_unicast") action do_set_unicast(bit<9> eg_port) {
        _set_eg_port(eg_port);
    }
    @name("._set_general_md") action _set_general_md(bit<16> rid, bit<16> xid, bit<9> yid, bit<13> h1, bit<13> h2) {
        ig_tm_md.rid = rid;
    ...
    @name(".do_set_mcast1_md") action do_set_mcast1_md(bit<16> mgid, bit<16> rid, bit<16> xid, bit<9> yid, bit<13> h1, bit<13> h2) {
        ig_tm_md.mcast_grp_a = mgid;
        _set_general_md(rid, xid, yid, h1, h2);
    }
    @name("._nop") action _nop() {
    }
    @name(".do_remap_stream") action do_remap_stream(bit<5> stream_id) {
        hdr.bridged_md.stream = stream_id;
    }
    @name(".set_md_from_fabric_hdr") action set_md_from_fabric_hdr() {
    ...
    @name(".do_clone_to_cpu") action do_clone_to_cpu(bit<32> cpu_mirror_id) {
        // hdr.bridged_md.pcie_cpu_port.ingress_port = ig_intr_md.ingress_port;
// TODO XLATE Clone extern        clone3(CloneType.I2E, (bit<32>)cpu_mirror_id, { hdr.bridged_md.pcie_cpu_port.ingress_port, meta.pad7b.pad0 });
    ...
    @name(".common_drop_tbl") table common_drop_tbl {
        actions = {
            common_drop_pkt;
        }
        size = 1;
    ...
    @name(".ig_port_tbl") table ig_port_tbl {
        actions = {
            do_drop;
            do_set_unicast;
            do_set_mcast1_md;
    ...
    @name(".ig_recirc_port_tbl") table ig_recirc_port_tbl {
        actions = {
            do_set_unicast;
        }
        key = {
    ...
    @name(".ig_remap_stream_tbl") table ig_remap_stream_tbl {
        actions = {
            _nop;
            do_remap_stream;
        }
    ...
    @name(".select_egress_port_tbl") table select_egress_port_tbl {
        actions = {
    ...
    @name(".send_to_cpu_tbl") table send_to_cpu_tbl {
        actions = {
            do_clone_to_cpu;
        }
        size = 1;
    ...
    @name(".pack_stream_port") pack_stream_port() pack_stream_port_0;
    apply {
        ...
        if (hdr.bridged_md.is_pktgen == 1w1) {
            ...
            if (1w0 == ig_intr_md.resubmit_flag && hdr.fabric_hdr.isValid() && hdr.bridged_md.pcie_cpu_port.enabled == 1w1) {
        ...
        pack_stream_port_0.apply(hdr, ig_intr_md, ig_md);
    }
}
control SwitchEgress(
    ...
    // TODO- move these to separate control file?
    @name(".set_fabric_hdr") action set_fabric_hdr() {
        hdr.fabric_hdr.setValid();
        hdr.fabric_hdr.devPort = (bit<16>)hdr.bridged_md.pcie_cpu_port.ingress_port;
        hdr.fabric_hdr.etherType = hdr.outer_eth.etherType;
        hdr.outer_eth.etherType = 16w0x9090;
    }
    @name("._nop") action _nop() {
    }
    @name(".set_fabric_hdr_tbl") table set_fabric_hdr_tbl {
        actions = {
            set_fabric_hdr;
            _nop;
        }
        ...
        if (hdr.bridged_md.is_pktgen == 1w1 && eg_intr_md.egress_port & 9w128 == 9w0) {
            ...
            if (eg_md.burst_pkt_cntr.drop == 1w1) {
                ...
                if (eg_md.tx_instrum == 1w1) {
                    ...
                    calib_tx_tstamp_0.apply(hdr, eg_intr_md);
                }
...
@Attr(target_type=TOFINO2,access=ro,descrip="Target device type")
@Attr(pktgen_apps_per_pipe=16,access=ro,descrip="Number of packet-generator apps per pipe")
