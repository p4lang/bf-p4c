...
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_GTP = 5;
...
parser NpbIngressParser(
 ...
 bit<8> inner_protocol;
    ...
    ig_md.tunnel.id = (bit<32>)hdr.nvgre.vsid;
     transition parse_inner_ethernet;
    }
    state parse_esp {
        pkt.extract(hdr.esp);
    ...
    state extract_gtp_c {
        ...
        ig_md.tunnel.type = SWITCH_TUNNEL_TYPE_GTP;
...
control TunnelEncap(inout switch_header_t hdr,
   ...
   hdr.nsh_extr_underlay.isValid() : exact;
    ...
    action rewrite_ipv4_vxlan(bit<16> vxlan_port) {
        ...
        add_vxlan_header((bit<24>)eg_md.tunnel.id);
        hdr.ethernet.ether_type = 0x0800;
    }
