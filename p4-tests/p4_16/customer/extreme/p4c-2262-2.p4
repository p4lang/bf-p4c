...
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_UNDERLAY = 3;
...
const bit<32> NPB_ING_SFF_SCHD_TABLE_PART2_DEPTH = 1024;
const bit<32> NPB_ING_SF_0_INGRESS_SFP_TABLE_DEPTH = 1024;
...
control VlanXlate(inout switch_header_t hdr,
    ...
    table port_bd_to_vlan_mapping {
        ...
        actions = {
            ...
            set_vlan_tagged;
            set_double_tagged;
            set_underlay_vlan_untagged;
            set_underlay_vlan_tagged;
        }
...
parser NpbEgressParser(
 ...
 value_set<switch_cpu_port_value_set_t>(1) cpu_port;
  ...
  eg_md.vrf = hdr.bridged_md.tunnel.vrf;
        eg_md.tunnel_nsh.terminate = hdr.bridged_md.tunnel.terminate_nsh;
  ...
  hdr.ethernet_underlay.ether_type = 0x894F;
   ...
   rewrite_nsh;
        ...
        const entries = {
            ...
            (false, false, true ) : rewrite_nsh();
            (false, true, true ) : rewrite_nsh();
            (true, false, true ) : rewrite_nsh();
        }
    }
    apply {
        smac_index = 0;
   ...
   switch(ip_rewrite.apply().action_run) {
    rewrite_nsh: { npb_egr_sff_dec_ttl.apply(); }
   }
            smac_rewrite.apply();
        }
    }
...
control TunnelEncap(inout switch_header_t hdr,
 ...
 action add_underlay_nsh_header() {
                ...
                hdr.nsh_extr_underlay.ttl = 0x3f;
 ...
 action add_underlay_l2_header() {
  ...
  add_underlay_nsh_header();
    ...
    apply {
        ...
        if (eg_md.tunnel.type != SWITCH_TUNNEL_TYPE_NONE) {
   if(eg_md.tunnel.type != SWITCH_TUNNEL_TYPE_UNDERLAY) {
                encap_outer.apply();
   }
            tunnel.apply();
        }
...
control npb_ing_sfc_top (
 ...
 action ing_sfc_sfc_classifier_12_permit (
  bit<8> sfc,
  bit<8> nsh_sph_si,
  bit<8> flow_type,
  bit<8> sf_bitmask_local
 ...
 table ing_sfc_sfc_class_12 {
  ...
  actions = {
   NoAction;
   ing_sfc_sfc_classifier_12_permit;
 ...
 action ing_sfc_sfc_classifier_2_permit (
  bit<8> sfc,
  bit<8> nsh_sph_si,
  bit<8> sf_bitmask_local
 ) {
 ...
 table ing_sfc_sfc_class_2 {
  ...
  actions = {
   NoAction;
   ing_sfc_sfc_classifier_2_permit;
   ing_sfc_sfc_classifier_drop;
  }
  const default_action = NoAction;
 ...
 apply {
  ...
  if(hdr.nsh_extr_underlay.isValid()) {
   ...
   if(ing_sfc_flowtype_class_1.apply().hit) {
    ing_sfc_sfc_class_2.apply();
   }
  }
...
action npb_ing_sf_policy_redirect (
 ...
 inout switch_stats_index_t index,
 inout switch_nexthop_t nexthop,
 out bit<1> action_bitmask_internal,
 switch_stats_index_t stats_index,
 bit<8> sfc,
 bit<8> nsh_sph_si,
 bit<8> flow_type,
...
control NpbIngSfPolicyL2(
 ...
 table npb_ing_sf_policy_l2 {
  key = {
   ...
   ig_md.nsh_extr.extr_flow_type : ternary;
  }
  actions = {
   NoAction;
   npb_ing_sf_policy_drop(ig_md, index);
   npb_ing_sf_policy_permit(ig_md, index);
   npb_ing_sf_policy_redirect(ig_md, index, nexthop, action_bitmask_internal);
  }
  const default_action = NoAction;
...
control npb_ing_sf_npb_basic_adv_acl (
 ...
 inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr,
 inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm,
 inout bit<1> action_bitmask_internal
) {
 ...
 apply {
  ...
  if(!(ig_md.bypass & SWITCH_INGRESS_BYPASS_ACL != 0)) {
   if (ig_md.lkp.ip_type == SWITCH_IP_TYPE_NONE || (mac_packet_class_enable && ig_md.flags.mac_pkt_class)) {
    NpbIngSfPolicyL2.apply(ig_md, stats_index, nexthop, action_bitmask_internal);
   }
  }
  if (!(ig_md.bypass & SWITCH_INGRESS_BYPASS_ACL != 0) && (!mac_packet_class_enable || !ig_md.flags.mac_pkt_class)) {
   if (ig_md.lkp.ip_type == SWITCH_IP_TYPE_IPV6) {
    NpbIngSfPolicyL34_v6.apply(ig_md, stats_index, nexthop, action_bitmask_internal);
   } else if (!mac_acl_enable || ig_md.lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
    NpbIngSfPolicyL34_v4.apply(ig_md, stats_index, nexthop, action_bitmask_internal);
   }
  }
  if(!(ig_md.bypass & SWITCH_INGRESS_BYPASS_ACL != 0)) {
  }
...
control npb_ing_sf_npb_basic_adv_top (
 ...
 bit<2> action_bitmask_internal;
    ...
    table ing_sf_action_sel {
        ...
        size = NPB_ING_SF_0_INGRESS_SFP_TABLE_DEPTH;
    }
 apply {
...
control npb_egr_sff_top_part1 (
 ...
 apply {
  ...
  hdr.nsh_extr_underlay.unused = 0;
...
control npb_egr_sff_top_part2 (
 ...
 apply {
  ...
  if(eg_md.nsh_extr.valid == 1) {
   if(eg_md.tunnel_nsh.terminate == false) {
...
control SwitchIngress(
    ...
    apply {
        ...
        tunnel_nsh.apply(hdr, ig_md, ig_md.lkp);
