...
parser NpbIngressParser(
 ...
 bit<8> inner_protocol;
    ...
    state parse_udp {
        pkt.extract(hdr.udp);
        transition select(hdr.udp.dst_port, hdr.udp.dst_port) {
            (_, 4789): parse_vxlan;
            default : accept;
        }
    }
...
control npb_ing_sf_npb_basic_adv_dedup (
 ...
 RegisterAction<pair_t, bit<16>, bit<8>>(test_reg) test_reg_action = {
  void apply(
   inout pair_t reg_value,
   out bit<8> return_value
  ) {
   if(reg_value.hash == flowtable_hash_hi) {
    if(reg_value.data == (bit<16>)ig_intr_md.ingress_port) {
     return_value = 0;
    } else {
     return_value = 1;
    }
 ...
 apply {
  bit<8> return_value;
  flowtable_hash_lo = h1.get({hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.ipv4.protocol, hdr.tcp.src_port, hdr.tcp.dst_port});
  flowtable_hash_hi = h2.get({hdr.ipv4.dst_addr, hdr.ipv4.src_addr, hdr.ipv4.protocol, hdr.tcp.dst_port, hdr.tcp.src_port});
  return_value = test_reg_action.execute(flowtable_hash_lo);
  if(return_value != 0) {
   ig_intr_md_for_dprsr.drop_ctl = 0x1;
  }
 }
 ...
 apply {
  ...
  if(lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
   if (!npb_ing_sf_table_1_v4_exact.apply().hit) {
    npb_ing_sf_table_1_v4_lpm.apply();
   }
  }
