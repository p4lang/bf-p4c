...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			npb_npb_sf2_policy_l2_add    (self, self.target, ig_pipe, dsap=dsap, l2_etype=0x0800, l2_etype_mask=0xffff, strip_tag_vn=1)
...
#			time.sleep(1)
			...
			src_pkt, exp_pkt = npb_simple_1lyr_udp(
				dmac_nsh=dmac, smac_nsh=smac, vn_en=True, spi=spi, si=si, sap=sap, vpn=vpn, ttl=63, scope=1,
			...
			'''
#			base_pkt = simple_tcp_packet2(eth_dst=dmac,tcp_sport=0,  tcp_dport=0,vn_tag=1)
			base_pkt = simple_udp_packet2(eth_dst=dmac,udp_sport=443,udp_dport=0,vn_tag=1)
			print(type(base_pkt))
...
#			print("---------- Debug ----------")
			...
			base_pkt_exp = testutils.simple_udp_packet(eth_dst=dmac,udp_sport=443,udp_dport=0,pktlen=100-6) # subtract 4, since tag will be stripped
			...
			if(PipelineParams.VNTAG_ENABLE == True):
