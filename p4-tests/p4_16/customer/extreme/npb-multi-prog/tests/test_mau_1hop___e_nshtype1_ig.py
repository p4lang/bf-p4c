...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			npb_sfc_sf_sel_nsh_xlate_add(self, self.target, ig_pipe, ta, spi, si, sf_bitmask)
...
#			time.sleep(1)
			...
			src_pkt, exp_pkt = npb_simple_1lyr_udp(
				dmac_nsh=dmac, smac_nsh=smac, ta=ta, spi=spi, si=si, nshtype=1, sap=sap, vpn=vpn, ttl=63, scope=1,
				sf_bitmask=sf_bitmask, start_of_chain=False, end_of_chain=True, scope_term_list=[],
				spi_exp=spi, si_exp=si, ta_exp=ta, nshtype_exp=None, sap_exp=sap, vpn_exp=vpn
			)
