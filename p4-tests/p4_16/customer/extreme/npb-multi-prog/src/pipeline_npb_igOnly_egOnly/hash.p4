...
control HashMask(
	...
	apply {
		...
		// l2
//		if(lkp_hash_mask_en[6:6] == 1) { lkp.mac_type     = 0; }
//		if(lkp_hash_mask_en[7:7] == 1) { lkp.mac_src_addr = 0; }
//		if(lkp_hash_mask_en[8:8] == 1) { lkp.mac_dst_addr = 0; }
		// l3
		if(lkp_hash_mask_en[0:0] == 1) { lkp.ip_src_addr  = 0; }
		if(lkp_hash_mask_en[1:1] == 1) { lkp.ip_dst_addr  = 0; }
		if(lkp_hash_mask_en[2:2] == 1) { lkp.ip_proto     = 0; }
		// l4
		if(lkp_hash_mask_en[3:3] == 1) { lkp.l4_src_port  = 0; }
...
control compute_non_ip_hash0_symmetric(
	in switch_lookup_fields_t lkp,
	out bit<32> hash
) (
	switch_uint32_t coeff = 0x04c11db7 // crc32 -- for making wider hashes, we can pass in other polynoials (see training docs for more info)
...
control compute_non_ip_hash1_symmetric(
	in switch_lookup_fields_t lkp,
	out bit<32> hash
) (
	switch_uint32_t coeff = 0x04c11db7 // crc32 -- for making wider hashes, we can pass in other polynoials (see training docs for more info)
