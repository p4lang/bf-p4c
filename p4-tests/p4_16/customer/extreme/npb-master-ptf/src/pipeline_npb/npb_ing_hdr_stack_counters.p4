...
control IngressHdrStackCounters(
    ...
    DirectCounter<bit<32>>(CounterType_t.PACKETS) inner_inner_stack_hdr_cntrs;
...
//         }
    ...
    action bump_inner_inner_stack_hdr_cntr() {
        inner_inner_stack_hdr_cntrs.count();
    }
    ...
    table inner_inner_stack_hdr_cntr_tbl {
        ...
        counters = inner_inner_stack_hdr_cntrs;
        size = 4;
    }
    ...
    apply {
        ...
        if(TRANSPORT_INGRESS_ENABLE) {
            ...
            // -----------------------------------------------------------------
            if(TRANSPORT_GRE_INGRESS_ENABLE) {
            ...
            // -----------------------------------------------------------------
            if(TRANSPORT_ERSPAN_INGRESS_ENABLE) {
            ...
            // -----------------------------------------------------------------
            if(TRANSPORT_IPV4_VXLAN_INGRESS_ENABLE || TRANSPORT_IPV4_GENEVE_INGRESS_ENABLE) {
                hdr_transport_udp_isValid = hdr.transport.udp.isValid();
            } else {
                hdr_transport_udp_isValid = false;
            }  // TRANSPORT_IPV4_VXLAN_INGRESS_ENABLE || TRANSPORT_IPV4_GENEVE_INGRESS_ENABLE
            ...
            if(TRANSPORT_IPV4_GENEVE_INGRESS_ENABLE) {
        ...
        if(OUTER_GRE_ENABLE || OUTER_NVGRE_ENABLE || OUTER_MPLSoGRE_ENABLE) {
