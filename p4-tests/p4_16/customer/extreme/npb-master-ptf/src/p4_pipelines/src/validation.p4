...
control OuterPktValidation(
    ...
    @name("validate_ethernet")
    table validate_ethernet_etag_vntag {
        key = {
            hdr.ethernet.isValid() : exact;
            hdr.ethernet.src_addr : ternary;
            hdr.ethernet.dst_addr : ternary;
...
*/
        if(OUTER_ETAG_ENABLE && OUTER_VNTAG_ENABLE) {       
			validate_ethernet_etag_vntag.apply();
        } else if(OUTER_ETAG_ENABLE) {
			validate_ethernet_etag.apply();
        } else if(OUTER_VNTAG_ENABLE) {
			validate_ethernet_vntag.apply();
        }  else {
...
#endif // ifdef PARSER_ERROR_HANDLING_ENABLE
