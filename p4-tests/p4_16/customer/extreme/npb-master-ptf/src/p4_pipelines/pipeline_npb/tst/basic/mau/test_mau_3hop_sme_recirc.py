...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_nsh_chain_middle_add(self, self.target,
			#ingress
			[ig_port_1], ig_lag_ptr+1, 0,    ta,       spi, si-(popcount(sf_bitmask_0)),                          sf_bitmask_1, rmac, nexthop_ptr+1, bd, eg_lag_ptr+1, 0+1, 0+1, [recircports[0]], False, 0, dsap,
			#tunnel
...
#		testutils.send_packet(self, ig_port_2, src_pkt)
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port_0], ig_lag_ptr+0,  spi, si,                                                   sf_bitmask_0, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, [recircports[0]],
			#tunnel
		...
		npb_nsh_chain_middle_del(self, self.target,
			#ingress
			[ig_port_1], ig_lag_ptr+1, ta, spi, si-(popcount(sf_bitmask_0)),                          sf_bitmask_1, rmac, nexthop_ptr+1, eg_lag_ptr+1, 0+1, 0+1, [recircports[0]],
			#tunnel
