...
#		time.sleep(1)
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port], spi, si, sf_bitmask, rmac, nexthop_ptr, eg_lag_ptr, 0, 0, [eg_port],
			#tunnel
		...
		# -----------------------------------------------------------
		# -----------------------------------------------------------
		# send same packet, different sap id (should be dropped)
		# -----------------------------------------------------------
		# -----------------------------------------------------------
		# -----------------------------------------------------------
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, 0, sap+1, vpn, spi, si, sf_bitmask, rmac, nexthop_ptr, bd, eg_lag_ptr, 0, 0, [eg_port], False, 0, dsap,
			#tunnel
			tunnel_encap_ptr, EgressTunnelType.NSH.value, tunnel_encap_nexthop_ptr, tunnel_encap_bd, dmac, tunnel_encap_smac_ptr, smac
			#egress
		)
...
#		time.sleep(1)
		...
		src_pkt, exp_pkt = npb_simple_1lyr_udp(
			dmac_nsh=dmac, smac_nsh=smac, spi=spi, si=si, sap=sap+1, vpn=vpn, ttl=63, scope=1,
			sf_bitmask=sf_bitmask, start_of_chain=True, end_of_chain=False, scope_term_list=[],
			spi_exp=spi, si_exp=si, sap_exp=sap+1, vpn_exp=vpn
		)
