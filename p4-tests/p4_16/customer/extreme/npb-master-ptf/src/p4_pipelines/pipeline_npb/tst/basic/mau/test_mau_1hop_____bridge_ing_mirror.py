...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		ig_port = swports[1]
		eg_port = swports[1]
		ig_port_mir = swports[2]
		eg_port_mir = swports[2]
		...
		npb_pre_mirror_add(self, self.target, 5, "INGRESS", eg_port_mir)
		npb_ing_port_mirror_add(self, self.target, ig_port, 5);
...
#		time.sleep(1)
		...
		print("Got HEre1")
		logger.info("Verify Mirrored packet on port %d", eg_port_mir)
		testutils.verify_packet(self, src_pkt, eg_port_mir)
		...
		npb_ing_port_mirror_del(self, self.target, ig_port);
