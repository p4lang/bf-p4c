...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		upper_test_limit = 1500 #Older hardware had 2000 entries and test was failing at that limit. Newer pipeline only have 1500 entries
		...
		for i in range(0, upper_test_limit):
			#print i
			npb_npb_sf0_policy_l2_add(self, self.target, sap=sap+i, vpn=vpn+i, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, flow_class=flow_class_acl, scope=1)
...
#		time.sleep(1)
		...
		for i in range(0, upper_test_limit):
			#print i
			npb_npb_sf0_policy_l2_del(self, self.target, sap=sap+i, vpn=vpn+i, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf)
