...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		#Case 3 (Outer tunnel)
		#npb_tunnel_network_dst_vtep_add(self, self.target, sip_nsh, 0xffffffff, dip_nsh, 0xffffffff, IngressTunnelType.VXLAN.value,   0xf, sap+1, vpn+1, ig_lag_ptr, 0)
		...
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, 
		...
		npb_npb_sf0_policy_l2_add    (self, self.target, sap=sap, vpn=vpn+2,
...
#		time.sleep(1)
		...
		npb_tunnel_inner_sap_del(self, self.target, sap=sap, 
					l3_type        = 0x1,       l3_type_mask        = 0x3,
					l3_proto       = 0x11,      l3_proto_mask       = 0xff,
					l3_sip         = sip_outer, l3_sip_mask         = 0xffffffff,
					l3_dip         = dip_outer, l3_dip_mask         = 0xffffffff,
		...
		npb_npb_sf0_policy_l2_del    (self, self.target, sap=sap, vpn=vpn+2,
