...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		vid2                    = 8 # Arbitrary value
		pcp                     = 5 # Arbitrary value
		...
		npb_nsh_bridge_add(self, self.target, 
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, bd, 0x0800, 0, vid, 5, dmac, eg_lag_ptr, 0, 0, [eg_port], False
			#egress
		)

		npb_bd_to_vlan_add(self, self.target, bd, vid2, pcp)
...
# NOTE: THIS TEST CURRENTLY WILL FAIL.  THIS IS BECAUSE WE ARE PUTTING BRIDGED PACKETS
# IN "OUTER" INSTEAD OF "TRANSPORT".  THE VLAN CODE CURRENTLY WORKS ON THE "TRANSPORT"
# LAYER.  WHAT ENDS UP HAPPENING THEN IS THE VLAN COMES OUT FIRST, BEFORE THE ETHERNET
# OF THE PACKET.  POSSIBLE SOLUTIONS:
#   - HAVING NEXTHOP ADD VLANS INSTEAD OF EGRESS ACL.
#   - HAVING BRIDGED PACKETS GO TO TRANSPORT INSTEAD OUT OUTER.

		src_pkt, exp_pkt = npb_simple_1lyr_udp(
			dmac_nsh=dmac, smac_nsh=smac, vlan_en_nsh=False, vlan_pcp_nsh=pcp, vlan_vid_nsh=vid2, spi=spi, si=si, ta=ta, nshtype=2, sap=sap, vpn=vpn, ttl=63, scope=1,
			dmac=dmac, smac=smac,
			sf_bitmask=sf_bitmask, start_of_chain=True, end_of_chain=True, scope_term_list=[],
			vlan_en_nsh_exp=True, spi_exp=spi, si_exp=si, ta_exp=ta, nshtype_exp=2, sap_exp=sap, vpn_exp=vpn
		)
...
#		base_pkt = testutils.simple_tcp_packet(eth_dst=dmac)
		...
		exp_pkt = \
			...
			scapy.Dot1Q(prio=pcp, id=0, vlan=vid2,  type=0x894f) / \
			scapy.NSH(MDType=1, NextProto=3, NSP=spi, NSI=si, TTL=63, \
				NPC=((2<<24)|(sap)), \
				NSC=((vpn<<16)|(0)), \
				SPC=0, \
		...
		npb_nsh_bridge_del(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, 0x0800, 0, vid, 5, dmac, eg_lag_ptr, 0, 0, [eg_port]
			#egress
		)

		npb_bd_to_vlan_del(self, self.target, tunnel_encap_bd)
