...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.IPINIP.value, tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, sap_new=sap, vpn=vpn+4, terminate=1)
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GTPC.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn+5, terminate=0)
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GTPU.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn+6, terminate=1)
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.ERSPAN.value, tun_type_mask=0xf, sap_new=sap, vpn=vpn+7, terminate=1)
		npb_tunnel_outer_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GRE.value,    tun_type_mask=0xf, sap_new=sap, vpn=vpn+8, terminate=1)
...
#		time.sleep(1)
		...
		src_pkt, exp_pkt = npb_simple_2lyr_gtpc_udp(
			...
			spi_exp=spi, si_exp=si, sap_exp=sap, vpn_exp=vpn+5
		)
		...
		src_pkt, exp_pkt = npb_simple_2lyr_gtpu_ip(
			dmac_nsh=dmac, smac_nsh=smac, spi=spi, si=si, sap=sap, vpn=vpn, ttl=63, scope=1,
			sf_bitmask=sf_bitmask, start_of_chain=True, end_of_chain=False, scope_term_list=[1], 
			spi_exp=spi, si_exp=si, sap_exp=sap, vpn_exp=vpn+6
		)
