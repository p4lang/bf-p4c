...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr+0, 0, sap, vpn, spi, si,                          sf_bitmask, rmac, nexthop_ptr+0, bd, eg_lag_ptr+0, 0+0, 0+0, [eg_port], False, 0, dsap,
			#tunnel
			tunnel_encap_ptr+0, EgressTunnelType.NSH.value, tunnel_encap_nexthop_ptr+0, tunnel_encap_bd+0, dmac, tunnel_encap_smac_ptr+0, smac
			#egress
		)
		...
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.VXLAN.value,  tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
		...
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GTPC.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=0)
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GTPU.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.ERSPAN.value, tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
		npb_tunnel_inner_sap_add(self, self.target, sap=sap, tun_type=IngressTunnelType.GRE.value,    tun_type_mask=0xf, sap_new=sap, vpn=vpn+2, terminate=1)
...
#		time.sleep(1)
		...
		src_pkt, exp_pkt = npb_simple_3lyr_vxlan_gre_ip(
			...
			sf_bitmask=sf_bitmask, start_of_chain=True, end_of_chain=False, scope_term_list=[1, 1],
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr+0, spi, si,                          sf_bitmask, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, [eg_port],
			#tunnel
