...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_bd_to_vlan_add(self, self.target, tunnel_encap_bd, vid, pcp)
...
#		time.sleep(1)
		...
		src_pkt, exp_pkt = npb_simple_1lyr_udp(
			dmac_nsh=dmac, smac_nsh=smac, vlan_en_nsh=True, vlan_pcp_nsh=pcp, vlan_vid_nsh=vid, spi=spi, si=si, ta=ta, nshtype=2, sap=sap, vpn=vpn, ttl=63, scope=1,
...
#		base_pkt = testutils.simple_tcp_packet(eth_dst=dmac)
		...
		exp_pkt = \
				...
				scapy.Dot1Q(prio=pcp, id=0, vlan=vid,  type=0x894f) / \
