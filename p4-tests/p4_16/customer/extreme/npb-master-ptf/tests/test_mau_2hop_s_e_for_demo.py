...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			ig_port_1 = ig_swports[ig_pipe][1]
			eg_port_1 = eg_swports[1]
			ig_port_2 = ig_swports[ig_pipe][1]
			eg_port_2 = eg_swports[1]
			ig_port_3 = ig_swports[ig_pipe][2]
			eg_port_3 = eg_swports[2] # derek changed from [4] so that this test can run on the hw, which only has 4 cpu ports total.
...
#			sf_bitmask_0            = 1 # Bit 0 = ingress, bit 1 = multicast, bit 2 = egress
			...
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.VXLAN.value,  tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.IPINIP.value, tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.GRE.value,    tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.GTPC.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=0)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.GTPU.value,   tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			npb_tunnel_inner_sap_add(self, self.target,ig_pipe, sap=sap, tun_type=IngressTunnelType.ERSPAN.value, tun_type_mask=0xf, sap_new=sap, vpn=vpn, terminate=1)
			...
			npb_npb_sf0_policy_l34_v4_add(self, self.target,ig_pipe, sap=sap, l3_proto=6, l3_proto_mask=0xff, l4_src=80,  l4_src_mask=0xffff, flow_class=flow_class_acl, sfc_enable=1, sfc=sfc_1) # hop 0 - reclassify to chain #1
# DEREK MOVED DROPPING TO EGRESS, TO TEST THAT LOGIC
#			npb_npb_sf0_policy_l34_v4_add(self, self.target,ig_pipe, sap=sap, l3_proto=6, l3_proto_mask=0xff, l4_src=443, l4_src_mask=0xffff, flow_class=flow_class_acl, drop=1)                  # hop 0 - reclassify to drop
			npb_npb_sf0_policy_l34_v4_add(self, self.target,ig_pipe, sap=sap, l3_proto=6, l3_proto_mask=0xff, l4_src=443, l4_src_mask=0xffff, flow_class=flow_class_acl, sfc_enable=1, sfc=sfc_2) # hop 0 - reclassify to chain #2

			# -----------------
			# Ingress SFP Sel
			# -----------------

			npb_npb_sfp_sel_add(self, self.target,ig_pipe, vpn, flow_class_sfp, sfc_0, 0, 0+0, [spi_0], [si_0], [sf_bitmask_0]) # hop 0
			npb_npb_sfp_sel_add(self, self.target,ig_pipe, vpn, flow_class_sfp, sfc_1, 0, 0+1, [spi_1], [si_1], [sf_bitmask_0]) # hop 0 (port 80)
			npb_npb_sfp_sel_add(self, self.target,ig_pipe, vpn, flow_class_sfp, sfc_2, 0, 0+2, [spi_2], [si_2], [sf_bitmask_0]) # hop 0 (port 443)

			# -----------------
			# Egress SF(s)
			# -----------------

			npb_npb_sf2_policy_l34_v4_add    (self, self.target,ig_pipe, dsap=dsap, l3_proto=6, l3_proto_mask=0xff, l4_src=443, l4_src_mask=0xffff, drop=1)                                       # hop 0 - reclassify to drop
