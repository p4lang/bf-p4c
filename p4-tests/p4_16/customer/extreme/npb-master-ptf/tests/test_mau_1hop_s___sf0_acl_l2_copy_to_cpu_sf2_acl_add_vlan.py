...
#			npb_pre_port_add(self, self.target, ig_pipe, eg_port2)
			...
			npb_npb_sf2_policy_l2_add    (self, self.target, ig_pipe, dsap=dsap, l2_etype=0x0800, l2_etype_mask=0xffff, add_tag_vlan_bd=5)

			npb_npb_sf2_policy_hdr_edit_add(self, self.target, ig_pipe, bd=5, pcp=0, vid=2080)
...
#			time.sleep(1)
			...
			src_pkt, exp_pkt = npb_simple_1lyr_udp(
				...
				vlan_en_exp=True, vlan_en_exp_vid=2080,
...
#				tunnel_encap_ptr, tunnel_encap_nexthop_ptr, tunnel_encap_bd, tunnel_encap_smac_ptr
			...
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.NONE.value,   tun_type_mask=0xf)

			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.VXLAN.value,  tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.IPINIP.value, tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.NVGRE.value,  tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GRE.value,    tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GTPC.value,   tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.GTPU.value,   tun_type_mask=0xf)
			npb_npb_sf0_policy_l2_del(self, self.target, ig_pipe, sap=sap, vpn=vpn, tun_type=IngressTunnelType.ERSPAN.value, tun_type_mask=0xf)

#			npb_pre_port_del(self, self.target, ig_pipe, eg_port2)
			npb_pre_mirror_del(self, self.target, ig_pipe, 251) # this is defined in the p4 code: SWITCH_MIRROR_SESSION_CPU_INGRESS = 251
