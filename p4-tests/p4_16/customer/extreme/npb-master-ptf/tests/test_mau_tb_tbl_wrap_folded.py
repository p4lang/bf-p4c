...
from test_mau_tb_top_folded import *
...
def cpu_port_add(self, target, port):
		...
		if(port[i] not in cpu_port_static_port_list): # barefoot allows duplicate writes to parser tables for some reason (bug?), so we have to disallow them here....
			...
			try:
				...
				if(PipelineParams.CPU_ENABLE == True):
					...
					table = self.bfrt_info.table_get("%s.cpu_port" % iprsr)
					table.entry_add(
						target,
						[table.make_key(
...
def cpu_port_del(self, target, port):
...
#			table = self.bfrt_info.table_get('%s.rmac.rmac' % ictl_0)
#			table.entry_add(
#				target,
#				[table.make_key(
...
#				[table.make_data(
#					[],
#					'%s.rmac.rmac_hit' % ictl_0
#				)]
#			)

			table = self.bfrt_info.table_get("%s.my_mac_lo" % iprsr)
			table.entry_add(
				target,
				[table.make_key(
			...
			table = self.bfrt_info.table_get("%s.my_mac_hi" % iprsr)
			table.entry_add(
				target,
				[table.make_key(
	...
	for i in range(len(port)):
		...
		try:
			...
			if(PipelineParams.CPU_HDR_CONTAINS_EG_PORT == False):
				...
				table = self.bfrt_info.table_get('%s.$PORT_METADATA' % iprsr)
				table.entry_add(
					target,
					[table.make_key(
				...
				table = self.bfrt_info.table_get('%s.ingress_port_mapping.port_mapping' % ictl_0)
				table.entry_add(
					target,
					[table.make_key(
					...
					[table.make_data(
						[],
						'%s.ingress_port_mapping.set_port_properties' % ictl_0
					)]
				)
				...
				table.entry_add(
					...
					[table.make_data(
						[gc.DataTuple('port_lag_index',                           port_lag_ptr),
						 gc.DataTuple('l2_fwd_en',                                bridging_enable)],
						'%s.ingress_port_mapping.set_cpu_port_properties' % ictl_0
					)]
				)
...
#					[gc.DataTuple('$MULTICAST_BACKUP_PORT',                   backup_port),
		...
		try:
			...
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.port_mapping' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
...
#					[gc.KeyTuple('port_lag_index',                            port_lag_ptr)],
				...
				[table.make_data(
					...
					'%s.npb_ing_top.npb_ing_sfc_top.set_port_properties' % ictl_0
				)]
			)
...
def npb_ing_port_mirror_add(self, target, port, session_id):
		...
		table = self.bfrt_info.table_get('%s.ingress_port_mapping.port_mirror.port_mirror' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.ingress_port_mapping.port_mirror.set_mirror_id' % ictl_0,
			)]
		)
...
def npb_port_vlan_to_bd_add(self, target, port_lag_index, vid_valid, vid_valid_mask, vid, vid_mask, member_ptr, bd):
		...
		try:
			# bottom
			table = self.bfrt_info.table_get('%s.ingress_port_mapping.bd_action_profile' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
				...
				[table.make_data(
					[gc.DataTuple('bd',                                   bd),
					 gc.DataTuple('rid',                                  0)],
					'%s.ingress_port_mapping.set_bd_properties' % ictl_0
				)]
			)
		...
		try:
			# top
			table = self.bfrt_info.table_get('%s.ingress_port_mapping.port_vlan_to_bd_mapping' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
...
#					[gc.DataTuple('rid',                                  0)],
		...
		try:
			# top
			table = self.bfrt_info.table_get('%s.ingress_port_mapping.vlan_to_bd_mapping' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
...
def npb_tunnel_dmac_add(self, target, ig_port_lag_ptr, ethertype, ethertype_mask, vid_isValid, vid_isValid_mask, vid, vid_mask, dst_addr, dst_addr_mask, eg_port_lag_ptr):
		if(PipelineParams.BRIDGING_ENABLE == True):

			table = self.bfrt_info.table_get('%s.dmac.dmac' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
#					[gc.KeyTuple('vlan_tag.isValid',                      vid_isValid, vid_isValid_mask),
				...
				[table.make_data(
					...
					'%s.dmac.dmac_hit' % ictl_0
				)]
			)

def npb_tunnel_dmac_del(self, target, ig_port_lag_ptr, ethertype, ethertype_mask, vid_isValid, vid_isValid_mask, vid, vid_mask, dst_addr, dst_addr_mask):
		if(PipelineParams.BRIDGING_ENABLE == True):
...
def npb_tunnel_network_dst_vtep_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.tunnel_transport.dst_vtep' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
...
#				 gc.DataTuple('port_lag_index',                           port_lag_ptr),
				...
				'%s.npb_ing_top.npb_ing_sfc_top.tunnel_transport.dst_vtep_hit' % ictl_0
			)]
		)
...
def npb_tunnel_network_src_vtep_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.tunnel_transport.src_vtep' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('src_addr',                                  gc.ipv4_to_bytes(l3_src), l3_src_mask),
			...
			[table.make_data(
				[gc.DataTuple('sap',                                      sap),
				 gc.DataTuple('vpn',                                      vpn)],
#				 gc.DataTuple('port_lag_index',                           port_lag_ptr)],
				'%s.npb_ing_top.npb_ing_sfc_top.tunnel_transport.src_vtep_hit' % ictl_0
			)]
		)
...
def npb_tunnel_network_sap_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.tunnel_network.sap' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('sap',                              sap),
			...
			[table.make_data(
				[gc.DataTuple('sap',                             sap_new),
				 gc.DataTuple('vpn',                             vpn)],
				'%s.npb_ing_top.npb_ing_sfc_top.tunnel_network.sap_hit' % ictl_0
			)]
		)
...
def npb_tunnel_outer_sap_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.tunnel_outer.sap_tcam' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.npb_ing_top.npb_ing_sfc_top.tunnel_outer.sap_tcam_hit' % ictl_0
			)]
		)
...
def npb_tunnel_inner_sap_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.tunnel_inner.sap_tcam' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,         0xffff),
			...
			[table.make_data(
				[gc.DataTuple('sap',                             sap_new),
				 ...
				 gc.DataTuple('terminate',                       terminate),
				 gc.DataTuple('drop',                            drop)],
				'%s.npb_ing_top.npb_ing_sfc_top.tunnel_inner.sap_tcam_hit' % ictl_0
			)]
		)
...
def npb_npb_sfc_sf_sel_add(self, target, spi, si, si_predec):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.ing_sfc_sf_sel' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('spi',                               spi),
			...
			[table.make_data(
				[gc.DataTuple('si_predec',                        si_predec)],
				'%s.npb_ing_top.npb_ing_sfc_top.ing_sfc_sf_sel_hit' % ictl_0
			)]
		)
...
def npb_sfc_sf_sel_nsh_xlate_add(self, target, ta, ttl, spi, si, si_predec):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sfc_top.ing_sfc_sf_sel_nsh_xlate' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('ttl',                             ttl),
				 ...
				 gc.DataTuple('si',                              si),
				 gc.DataTuple('si_predec',                      si_predec)],
				'%s.npb_ing_top.npb_ing_sfc_top.ing_sfc_sf_sel_nsh_xlate_hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_action_sel_add(self, target, spi, si, bitmask):
	try:
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_action_sel' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('spi',                               spi),
			...
			[table.make_data(
				[],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_action_sel_hit' % ictl_0
			)]
		)
	except:
...
def npb_npb_sf0_len_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_ip_len_rng' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_ip_len_rng_hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_l4_src_port_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_l4_src_port_rng' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('rng_bitmask',                        l3_len_rng)],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_l4_src_port_rng_hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_l4_dst_port_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_l4_dst_port_rng' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('rng_bitmask',                        l3_len_rng)],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.ing_sf_l4_dst_port_rng_hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_policy_l2_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.mac_acl.acl' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.mac_acl.hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_policy_l34_v4_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.ipv4_acl.acl' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,            sap_mask),
			...
			[table.make_data(
				[gc.DataTuple('flow_class',                      flow_class),
				 ...
				 gc.DataTuple('sfc_enable',                      sfc_enable),
				 gc.DataTuple('sfc',                             sfc)],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.ipv4_acl.hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_policy_l34_v6_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.ipv6_acl.acl' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,            sap_mask),
			...
			[table.make_data(
				[gc.DataTuple('flow_class',                      flow_class),
				 ...
				 gc.DataTuple('sfc_enable',                      sfc_enable),
				 gc.DataTuple('sfc',                             sfc)],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.ipv6_acl.hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_policy_l7_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.l7_acl.acl' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('sap',                              sap,            sap_mask),
			...
			[table.make_data(
				[gc.DataTuple('drop',                            drop),
				 ...
				 gc.DataTuple('sfc_enable',                      sfc_enable),
				 gc.DataTuple('sfc',                             sfc)],
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_top.acl.l7_acl.hit' % ictl_0
			)]
		)
...
def npb_npb_sf0_policy_sfp_sel_hash_add(self, target, vpn, flowclass):
	try:
			...
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_hash_lkp_1.ing_flow_class' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
				...
				[table.make_data(
					...
					'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_hash_lkp_1.ing_flow_class_hit' % ictl_0
				)]
			)

#			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_hash_lkp_2.ing_flow_class' % ictl_0)
#			table.entry_add(
#				target,
#				[table.make_key(
#					[gc.KeyTuple('vpn',                              vpn, 0xffff)],
...
#				[table.make_data(
#					[gc.DataTuple('flow_class',                      flowclass)],
#					'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_hash_lkp_2.ing_flow_class_hit' % ictl_0
#				)]
#			)
...
def npb_npb_sf0_policy_sfp_sel_single_add(self, target, sfc, sfc_member_ptr, spi, si, si_predec):
		if(PipelineParams.SFF_SCHD_SIMPLE == True):
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.ing_schd' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
					[gc.KeyTuple('sfc',                              sfc)],
				...
				[table.make_data(
					[gc.DataTuple('spi',                             spi),
					 gc.DataTuple('si',                              si),
					 gc.DataTuple('si_predec',                       si_predec)],
					'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.ing_schd_hit' % ictl_0
				)]
			)
		else:

			# bottom
#			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.schd_selector' % ictl_0)
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.schd_action_profile' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
...
def npb_npb_sf0_policy_sfp_sel_multi_add(self, target, sfc, sfc_group_ptr, sfc_member_ptr, spi, si, si_predec):
		if(PipelineParams.SFF_SCHD_SIMPLE == True):
			self.insert_table_entry(
				target,
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.ing_schd' % ictl_0,
...
#				table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.schd_selector' % ictl_0)
			...
			# middle
#			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.schd_selector_sel' % ictl_0)
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.schd_selector' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
...
def npb_npb_sf0_policy_sfp_sel_multi_del(self, target, sfc, sfc_group_ptr, sfc_member_ptr, spi, si):
		if(PipelineParams.SFF_SCHD_SIMPLE == True):
			self.delete_table_entry(
				target,
				'%s.npb_ing_top.npb_ing_sf_npb_basic_adv_sfp_sel.ing_schd' % ictl_0,
...
def npb_npb_sff_fib_add(self, target, spi, si, nexthop_ptr, end_of_chain):
		...
		try:
			...
			table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sff_top.ing_sff_fib' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
					[gc.KeyTuple('spi',                              spi),
				...
				'%s.npb_ing_top.npb_ing_sff_top.redirect' % ictl_0
				)]
			)
...
def npb_npb_sf1_action_sel_add(self, target, spi, si, bitmask, mgid):
		...
		table = self.bfrt_info.table_get('%s.npb_ing_top.npb_ing_sf_multicast_top_part1.ing_sf_action_sel' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('spi',                               spi),
			...
			[table.make_data(
				...
				'%s.npb_ing_top.npb_ing_sf_multicast_top_part1.ing_sf_action_sel_hit' % ictl_0,
			)]
		)
...
def npb_nexthop_add(self, target, nexthop_ptr, bd, port_lag_ptr):
		...
		try:
			...
			table = self.bfrt_info.table_get('%s.nexthop.nexthop' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
				...
				[table.make_data(
					[gc.DataTuple('bd',                              bd),
					 gc.DataTuple('port_lag_index',                  port_lag_ptr)],
					'%s.nexthop.set_nexthop_properties' % ictl_0
				)]
			)
			...
			table = self.bfrt_info.table_get('%s.rewrite.nexthop_rewrite' % ectl_0)
			table.entry_add(
				target,
				[table.make_key(
...
def npb_nexthop_tunnel_encap_part2_add(self, target, nexthop_ptr, tun_type):
		...
		try: 
			...
			table.entry_add(
				...
				[table.make_data(
					...
					'%s.rewrite.rewrite_l2_with_tunnel' % ectl_0
				)]
			)
		...
		try:
			...
			table.entry_add(
				...
				[table.make_data(
					...
					'%s.nexthop.set_nexthop_properties_tunnel' % ictl_0
				)]
			)
			...
			npb_nexthop_tunnel_encap_part2_add(self, target, nexthop_ptr, tun_type)

		except:
			print("A table operation failed. This is typically due to adding or removing a duplicate, and can be ignored 11.")

def npb_nexthop_part2_del(self, target, nexthop_ptr):
...
def npb_nexthop_del(self, target, nexthop_ptr):
		...
		try:
			...
			npb_nexthop_part2_del(self, target, nexthop_ptr)
...
def npb_tunnel_encap_nexthop_add(self, target, tunnel_ptr, port_lag_ptr, outer_nexthop_ptr):
		...
		table = self.bfrt_info.table_get('%s.outer_fib.fib' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.outer_fib.set_nexthop_properties' % ictl_0
			)]
		)
...
def npb_lag_single_add(self, target, port_lag_ptr, port_lag_member_ptr, port):
		...
		try:
			# bottom
#			table = self.bfrt_info.table_get('%s.lag.lag_selector' % ictl_0)
			table = self.bfrt_info.table_get('%s.lag.lag_action_profile' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
				...
				[table.make_data(
					...
					'%s.lag.set_lag_port' % ictl_0
				)]
			)

			# top
			table = self.bfrt_info.table_get('%s.lag.lag' % ictl_0)
			table.entry_add(
				target,
				[table.make_key(
					[gc.KeyTuple('port_lag_index',                   port_lag_ptr)],
...
#			table = self.bfrt_info.table_get('%s.lag.lag_selector' % ictl_0)
		...
		# middle
#		table = self.bfrt_info.table_get('%s.lag.lag_selector_sel' % ictl_0)
		table = self.bfrt_info.table_get('%s.lag.lag_selector' % ictl_0)
		table.entry_add(
			target,
			[table.make_key(
...
def npb_lag_multi_del(self, target, port_lag_ptr, port_lag_group_ptr, port_lag_member_ptr, port):
		port_lag_member_ptr_list = []
...
def npb_egr_port_add(self, target, port, eg_port_is_cpu, port_lag_ptr):
	...
	for i in range(len(port)):
		...
		try:
			...
			if(eg_port_is_cpu):
				...
				table = self.bfrt_info.table_get('%s.egress_port_mapping.port_mapping' % ectl_2)
				table.entry_add(
					target,
					[table.make_key(
						[gc.KeyTuple('port',                                      port[i])],
					...
					[table.make_data(
						[gc.DataTuple('port_lag_index',                           port_lag_ptr)],
						'%s.egress_port_mapping.port_cpu' % ectl_2
					)]
				)

				if(PipelineParams.SPLIT_EG_PORT_TBL == True):
					table = self.bfrt_info.table_get('%s.cpu_rewrite.cpu_port_rewrite' % ectl_2)
					table.entry_add(
						target,
						[table.make_key(
							[gc.KeyTuple('port',                                      port[i])],
						...
						[table.make_data(
							[],
							'%s.cpu_rewrite.cpu_rewrite' % ectl_2
						)]
					)
				...
				table.entry_add(
					...
					[table.make_data(
						[gc.DataTuple('port_lag_index',                           port_lag_ptr)],
						'%s.egress_port_mapping.port_normal' % ectl_2
					)]
				)
				'''
				if(PipelineParams.SPLIT_EG_PORT_TBL == True):
					...
					table.entry_add(
						...
						[table.make_data(
							[],
							'%s.cpu_rewrite.normal_rewrite' % ectl_2
						)]
					)
				'''
...
def npb_egr_port_mirror_add(self, target, port, session_id):
		...
		table = self.bfrt_info.table_get('%s.egress_port_mapping.port_mirror.port_mirror' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('port',                             port)],
			...
			[table.make_data(
				[gc.DataTuple('session_id',                      session_id)],
				'%s.egress_port_mapping.port_mirror.set_mirror_id' % ectl_2,
			)]
		)
...
def npb_mult_rid_identical_add(self, target, rid, bd):

		table = self.bfrt_info.table_get('%s.multicast_replication.rid' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('bd',                              bd)],
				'%s.multicast_replication.rid_hit_identical_copies' % ectl_2,
			)]
		)

def npb_mult_rid_identical_del(self, target, rid):
...
def npb_mult_rid_unique_add(self, target, rid, bd, spi, si, nexthop_index, tunnel_index, outer_nexthop_index):
		...
		table.entry_add(
			...
			[table.make_data(
				...
				'%s.multicast_replication.rid_hit_unique_copies' % ectl_2,
			)]
		)

def npb_mult_rid_unique_del(self, target, rid):
...
def npb_npb_sf2_action_sel_add(self, target, spi, si, bitmask, dsap):
	try:
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_action_sel' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('spi',                              spi),
			...
			[table.make_data(
				...
				'%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_action_sel_hit' % ectl_2,
			)]
		)
	except:
...
def npb_npb_sf2_len_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_ip_len_rng' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('ip_len',                              low=l3_len_lo, high=l3_len_hi)],
			...
			[table.make_data(
				[gc.DataTuple('rng_bitmask',                        l3_len_rng)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_ip_len_rng_hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_l4_src_port_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_l4_src_port_rng' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('l4_src_port',                         low=l3_len_lo, high=l3_len_hi)],
			...
			[table.make_data(
				[gc.DataTuple('rng_bitmask',                        l3_len_rng)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_l4_src_port_rng_hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_l4_dst_port_rng_add(self, target, l3_len_lo, l3_len_hi, l3_len_rng):
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_l4_dst_port_rng' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('l4_dst_port',                         low=l3_len_lo, high=l3_len_hi)],
			...
			[table.make_data(
				[gc.DataTuple('rng_bitmask',                        l3_len_rng)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.egr_sf_l4_dst_port_rng_hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_policy_l2_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_mac_acl.acl' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('drop',                            drop),
				 ...
				 gc.DataTuple('mirror_session_id',               mirror_id),
				 gc.DataTuple('cpu_reason_code',                 cpu_reason_code)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_mac_acl.hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_policy_l34_v4_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_ipv4_acl.acl' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('dsap',                             dsap,           dsap_mask),
			...
			[table.make_data(
				[gc.DataTuple('drop',                            drop),
				 ...
				 gc.DataTuple('mirror_session_id',               mirror_id),
				 gc.DataTuple('cpu_reason_code',                 cpu_reason_code)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_ipv4_acl.hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_policy_l34_v6_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_ipv6_acl.acl' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('dsap',                             dsap,           dsap_mask),
			...
			[table.make_data(
				[gc.DataTuple('drop',                            drop),
				 ...
				 gc.DataTuple('mirror_session_id',               mirror_id),
				 gc.DataTuple('cpu_reason_code',                 cpu_reason_code)],
				'%s.npb_egr_top.npb_egr_sf_proxy_top.acl.egress_ipv6_acl.hit' % ectl_2
			)]
		)
...
def npb_npb_sf2_policy_hdr_edit_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sf_proxy_top.npb_egr_sf_proxy_hdr_edit.bd_to_vlan_mapping' % ectl_2)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('bd',                               bd)],
			...
			[table.make_data(
				...
				'%s.npb_egr_top.npb_egr_sf_proxy_top.npb_egr_sf_proxy_hdr_edit.set_vlan_tagged' % ectl_2
			)]
		)
...
def npb_egr_sff_fib_nshtype1_add(self, target,
		...
		table = self.bfrt_info.table_get('%s.npb_egr_top.npb_egr_sff_top.egr_sff_fib' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('spi',                             spi),
			...
			[table.make_data(
				...
				'%s.npb_egr_top.npb_egr_sff_top.fwd_pkt_nsh_hdr_ver_1' % ectl_0
			)]
		)
...
def npb_rewrite_bd_mapping_add(self, target, bd, smac_ptr):
		self.insert_table_entry(
			target,
			'%s.rewrite.egress_bd.bd_mapping' % ectl_0,
			[self.KeyField('bd',                               self.to_bytes(bd, 2))],
			'%s.rewrite.egress_bd.set_bd_properties' % ectl_0,
...
def npb_rewrite_smac_rewrite_add(self, target, smac_ptr, smac):
		self.insert_table_entry(
			target,
			'%s.rewrite.smac_rewrite' % ectl_0,
			[self.KeyField('smac_index',                       self.to_bytes(smac_ptr, 2))],
			'%s.rewrite.rewrite_smac' % ectl_0,
...
def npb_tunnel_encap_add(self, target):
	try:
		...
		table = self.bfrt_info.table_get('%s.tunnel_encap.tunnel' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[],
				'%s.tunnel_encap.rewrite_mac_in_mac' % ectl_0
			)]
		)
		...
		table.entry_add(
			...
			[table.make_data(
				[],
				'%s.tunnel_encap.rewrite_ipv4_gre' % ectl_0
			)]
		)
		...
		table.entry_add(
			...
			[table.make_data(
				[],
				'%s.tunnel_encap.rewrite_ipv6_gre' % ectl_0
			)]
		)
		...
		table.entry_add(
			...
			[table.make_data(
				[],
				'%s.tunnel_encap.rewrite_ipv4_erspan' % ectl_0
			)]
		)
		...
		table.entry_add(
			...
			[table.make_data(
				[],
				'%s.tunnel_encap.rewrite_ipv4_udp' % ectl_0
			)]
		)
...
def npb_tunnel_encap_nexthop_rewrite_add(self, target, outer_nexthop_ptr, bd, dmac):
	try:
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.nexthop_rewrite' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_tunnel' % ectl_0
			)]
		)
...
def npb_tunnel_encap_bd_mapping_add(self, target, bd, smac_ptr):
	try:
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.egress_bd.bd_mapping' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('bd',                               bd)],
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.egress_bd.set_bd_properties' % ectl_0
			)]
		)
...
def npb_tunnel_encap_sip_rewrite_v4_add(self, target, bd, sip):
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.src_addr_rewrite' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_ipv4_src' % ectl_0
			)]
		)
...
def npb_tunnel_encap_sip_rewrite_v6_add(self, target, bd, sip):
		...
		table.entry_add(
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_ipv6_src' % ectl_0
			)]
		)
...
def npb_tunnel_encap_dip_rewrite_v4_add(self, target, tunnel_ptr, dip):
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.ipv4_dst_addr_rewrite' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_ipv4_dst' % ectl_0
			)]
		)
...
def npb_tunnel_encap_dip_rewrite_v6_add(self, target, tunnel_ptr, dip):
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.ipv6_dst_addr_rewrite' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('tunnel.index',                     tunnel_ptr)], # aka dip_index!!!
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_ipv6_dst' % ectl_0
			)]
		)
...
def npb_tunnel_encap_smac_rewrite_add(self, target, smac_ptr, smac):
	try:
		...
		table = self.bfrt_info.table_get('%s.tunnel_rewrite.smac_rewrite' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.tunnel_rewrite.rewrite_smac' % ectl_0
			)]
		)
...
def npb_port_bd_to_vlan_add(self, target, port_lag_index, bd, vid, pcp):
		...
		table = self.bfrt_info.table_get('%s.vlan_xlate.port_bd_to_vlan_mapping' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('vid',                             vid),
				 gc.DataTuple('pcp',                             pcp)],
				'%s.vlan_xlate.set_vlan_tagged' % ectl_0
			)]
		)
...
def npb_bd_to_vlan_add(self, target, bd, vid, pcp):
		...
		table = self.bfrt_info.table_get('%s.vlan_xlate.bd_to_vlan_mapping' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('bd',                               bd)],
...
def dtel_flow_report_array1_add(self, target,
	index               = 0,
	data                = 0
):

		table = self.bfrt_info.table_get('%s.dtel.flow_report0.array1' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('%s.dtel.flow_report0.array1.f1'  % ectl_0, data)]
			)]
		)

def dtel_flow_report_array1_del(self, target,
	index               = 0
):
...
def dtel_flow_report_array2_add(self, target,
	index               = 0,
	data                = 0
):

		table = self.bfrt_info.table_get('%s.dtel.flow_report0.array2' % ectl_0)
		print(table)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				[gc.DataTuple('%s.dtel.flow_report0.array2.f1'  % ectl_0, data)]
			)]
		)

def dtel_flow_report_array2_del(self, target,
	index               = 0
):
		...
		#register2 = self.bfrt_info.register_get('%s.dtel.flow_report0.array2' % ectl_0)
		#print(table)
		#print register2
		#register2.write(0,0)
		#table.dump(table=True)
		#array_name = '%s.dtel.flow_report0.array2' % ectl_0
		#print array_name
		#table.entry_del(target,[table.make_key(
		#		[gc.KeyTuple('$REGISTER_INDEX',                  index)]
		#	)], )
		#report_array = '%s.dtel.flow_report0.array2' % ectl_0
		#report_array = 0
...
def dtel_queue_report_thresholds_add(self, target,
	index               = 0,
	qdepth              = 0,
	latency             = 0
	):

		table = self.bfrt_info.table_get('%s.dtel.queue_report.thresholds' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('$REGISTER_INDEX',                  index)]
			...
			[table.make_data(
				[gc.DataTuple('%s.dtel.queue_report.thresholds.qdepth'  % ectl_0, qdepth),
				 gc.DataTuple('%s.dtel.queue_report.thresholds.latency' % ectl_0, latency)],
			)]
		)

def dtel_queue_report_thresholds_del(self, target,
	index               = 0,
	):
...
def dtel_queue_report_quotas_add(self, target,
	index               = 0,
	counter             = 0,
	latency             = 0
	):

		table = self.bfrt_info.table_get('%s.dtel.queue_report.quotas' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('$REGISTER_INDEX',                  index)]
			...
			[table.make_data(
				[gc.DataTuple('%s.dtel.queue_report.quotas.counter' % ectl_0, counter),
				 gc.DataTuple('%s.dtel.queue_report.quotas.latency' % ectl_0, latency)],
			)]
		)

def dtel_queue_report_quotas_del(self, target,
	index               = 0,
	):
...
def dtel_queue_report_queue_alert_table_add(self, target,
	qid                 = 0,
	port                = 0,
	quota               = 0,
	quantization_mask   = 0
		...
		table = self.bfrt_info.table_get('%s.dtel.queue_report.queue_alert' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
				[gc.KeyTuple('qid',                              qid),
			...
			[table.make_data(
				...
				'%s.dtel.queue_report.set_qalert' % ectl_0
			)]
		)

def dtel_queue_report_queue_alert_table_del(self, target,
	qid                 = 0,
	port                = 0
	):
...
def dtel_queue_report_check_quota_table_update_add(self, target,
	pkt_src             = 0,
	qalert              = 0,
	qid                 = 0,
	port                = 0,
	index               = 0
		...
		table = self.bfrt_info.table_get('%s.dtel.queue_report.check_quota' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
			...
			[table.make_data(
				...
				'%s.dtel.queue_report.check_latency_and_update_quota_' % ectl_0
			)]
		)

def dtel_queue_report_check_quota_table_reset_add(self, target,
	pkt_src             = 0,
	qalert              = 0,
	qid                 = 0,
	port                = 0,
	index               = 0
		...
		table.entry_add(
			...
			[table.make_data(
				[gc.DataTuple('index',                           index)],
				'%s.dtel.queue_report.reset_quota_' % ectl_0
			)]
		)

def dtel_queue_report_check_quota_table_del(self, target,
	pkt_src             = 0,
	qalert              = 0,
	qid                 = 0,
	port                = 0
...
def dtel_config_update_add(self, target,
	...
	report_type_out     = 0
		...
		table = self.bfrt_info.table_get('%s.dtel_config.config' % ectl_0)
		table.entry_add(
			target,
			[table.make_key(
...
#				'%s.dtel_config.update' % ectl_0
				action
			)]
		)

def dtel_config_mirror_add(self, target,
	pkt_src             = 0, pkt_src_mask             = 0,
	report_type         = 0, report_type_mask         = 0,
	drop_report_flag    = 0, drop_report_flag_mask    = 0,
	flow_report_flag    = 0, flow_report_flag_mask    = 0,
...
#				 gc.KeyTuple('eg_md.drop_reason',                drop_reason,         drop_reason_mask),
			...
			[table.make_data(
				[],
#				'%s.dtel_config.%s' %(ectl_0, action)
				action
			)]
		)

def dtel_config_del(self, target,
	pkt_src             = 0, pkt_src_mask             = 0,
	report_type         = 0, report_type_mask         = 0,
	drop_report_flag    = 0, drop_report_flag_mask    = 0,
	flow_report_flag    = 0, flow_report_flag_mask    = 0,
...
def npb_lag_del(self, target,
	eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port
):
	if(len(eg_port) == 1):
		npb_lag_single_del        (self, target, eg_port_lag_ptr,                        eg_port_lag_member_ptr)
	else:
		npb_lag_multi_del         (self, target, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port)

#######################################

def dtel_flow_report_array_add(self, target,
	index = 0,
	data0 = 0,
	data1 = 0
):

	dtel_flow_report_array1_add(self, target, index, data0)
	dtel_flow_report_array2_add(self, target, index, data1)

def dtel_flow_report_array_del(self, target,
	index = 0
):
	
	#dtel_flow_report_array1_del(self, target, index)
	dtel_flow_report_array2_del(self, target, index)
	

#######################################

def dtel_queue_report_queue_alert_add(self, target,
	port              = 0,
	qid               = 0,
	quota             = 0,
	quantization_mask = 0,
	...
	dtel_queue_report_thresholds_add(self, target,
		index             = reg_index,
	...
	dtel_queue_report_queue_alert_table_add(self, target,
		qid               = qid,
		port              = port,
		quota             = quota,
...
def dtel_queue_report_queue_alert_del(self, target,
	port              = 0,
	qid               = 0,
	reg_index         = 0
):
	'''
	# set dtel queue report (register)
	dtel_queue_report_thresholds_del(self, target,
		index             = reg_index
	)
	'''
	# set dtel queue report (table)
	dtel_queue_report_queue_alert_table_del(self, target,
		qid               = qid,
		port              = port
	)
...
def dtel_queue_report_check_quota_add(self, target,
	port             = 0,
	qid              = 0,
	reg_index        = 0,
	reg_data_latency = 0
	...
	dtel_queue_report_quotas_add(self, self.target,
		index             = reg_index,
		counter           = 0, # gets reloaded automatically in the p4 code
		latency           = reg_data_latency 
	)
...
def dtel_queue_report_check_quota_del(self, target,
	port             = 0,
	qid              = 0,
	reg_index        = 0
):
...
def dtel_config_init_add(self, target,
	flow_report_en       = True,
	...
	if(queue_report_en):
		action_ = "%s.dtel_config.mirror_switch_local_and_set_q_bit" %(ectl_0)
	elif(flow_report_en):
		action_ = "%s.dtel_config.mirror_switch_local" %(ectl_0)
	else:
		action_ = "NoAction"
	...
	dtel_config_update_add(self, self.target,
		...
		drop_report_valid = 0,                                                         drop_report_valid_mask = 0x0,
		action = "%s.dtel_config.update" %(ectl_0),
...
def npb_nsh_chain_start_end_add(self, target,
	...
	cpu_port_add              (self, target, ig_port)
...
#	npb_npb_sfc_sf_sel_add    (self, target, spi, si, si-(popcount(sf_bitmask&6))) # only 'and' with sf's that are after the sff
	...
	npb_lag_add               (self, target, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port)

	npb_egr_port_add          (self, target, eg_port, eg_port_is_cpu, eg_port_lag_ptr)
	if(sf_bitmask&4):
...
def npb_nsh_chain_start_end_del(self, target,
	...
	cpu_port_del              (self, target, ig_port)
...
#	npb_npb_sfc_sf_sel_del    (self, target, spi, si) # adding this allows acting as a middle too
	...
	npb_nexthop_del           (self, target, nexthop_ptr)
	npb_lag_del               (self, target, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port)
...
#	npb_port_vlan_to_bd_add   (self, self.target, ig_port_lag_ptr, 0, 0, 0, 0, bd_member_ptr, bd)
	npb_tunnel_rmac_add       (self, target, rmac);
	npb_tunnel_dmac_add       (self, target, ig_port_lag_ptr, ethertype, ethertype_mask, vid_isValid, vid_isValid_mask, vid, vid_mask, dmac, dmac_mask, eg_port_lag_ptr);
#	npb_nexthop_add           (self, target, nexthop_ptr, bd, eg_port_lag_ptr)
	npb_lag_add               (self, target, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port)
	npb_egr_port_add          (self, target, eg_port, eg_port_is_cpu, eg_port_lag_ptr)
...
#	npb_port_vlan_to_bd_del   (self, self.target, ig_port_lag_ptr, 0, 0, 0, 0, bd_member_ptr)
	npb_tunnel_rmac_del       (self, target, rmac);
	npb_tunnel_dmac_del       (self, target, ig_port_lag_ptr, ethertype, ethertype_mask, vid_isValid, vid_isValid_mask, vid, vid_mask, dmac, dmac_mask);
#	npb_nexthop_del           (self, target, nexthop_ptr)
	npb_lag_del               (self, target, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port)
	npb_egr_port_del          (self, target, eg_port)
...
def npb_nsh_bridge_no_eg_lag_add(self, target,
	#ingress
#	ig_port, ig_port_lag_ptr, rmac, bd, bd_member_ptr, dmac, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port
...
def npb_nsh_bridge_no_eg_lag_del(self, target,
	#ingress
#	ig_port, ig_port_lag_ptr, rmac, bd, bd_member_ptr, dmac, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port
	ig_port, ig_port_lag_ptr, rmac, nexthop_ptr, ethertype, ethertype_mask, vid_isValid, vid_isValid_mask, vid, vid_mask, dmac, dmac_mask, eg_port_lag_ptr, eg_port_lag_group_ptr, eg_port_lag_member_ptr, eg_port
	#egress
