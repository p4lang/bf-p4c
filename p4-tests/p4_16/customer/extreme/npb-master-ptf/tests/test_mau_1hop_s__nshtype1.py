...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_egr_sff_fib_nshtype1_add(self, self.target, spi, si, ta)
...
#		time.sleep(1)
		...
		src_pkt, exp_pkt = npb_simple_1lyr_udp(
			...
			spi_exp=spi, si_exp=si, ta_exp=ta, nshtype_exp=1, sap_exp=sap, vpn_exp=vpn
		)

#		print "----------------------"
#		print type(exp_pkt)
#		print exp_pkt.__class__
#
#		print "----------------------"
#
#		counter = 0
#		while True:
#			layer = exp_pkt.exp_pkt.getlayer(counter)
#			if layer is None:
#				break
#			print layer.name,
#			counter += 1
...
#		print "----------------------"
		...
		npb_egr_sff_fib_nshtype1_del(self, self.target, spi, si)
