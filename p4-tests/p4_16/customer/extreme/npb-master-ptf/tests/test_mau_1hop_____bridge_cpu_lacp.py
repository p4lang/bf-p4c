...
#		dmac2= '23:22:22:22:22:22'
		...
		npb_nsh_bridge_add(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, bd, 0x8809, 0, vid, dmac2,  eg_lag_ptr,   0,   0,   [eg_port], False
			#egress
		)

		npb_nsh_bridge_add(self, self.target,
			#ingress
			[ig_port2], ig_lag_ptr, rmac, nexthop_ptr, bd, 0x8809, 0, vid, dmac, eg_lag_ptr+1, 0+1, 0+1, [eg_port2], True
			#egress
		)
...
#		exp_pkt = base_pkt
		...
		npb_nsh_bridge_del(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, 0x8809, 0, vid, dmac2,  eg_lag_ptr,   0,   0,   1, [eg_port]
			#egress
		)

		npb_nsh_bridge_del(self, self.target,
			#ingress
			[ig_port2], ig_lag_ptr, rmac, nexthop_ptr, 0x8809, 0, vid, dmac, eg_lag_ptr+1, 0+1, 0+1, 1, [eg_port2]
			#egress
		)
