...
#		dmac2= '23:22:22:22:22:22'
		...
		bd                      = 2 # Arbitrary value egress bd
		...
		npb_nsh_bridge_add(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, bd, 0x8809, 0xffff, 0, 0x1, vid, 0xfff, dmac2, 0xffffffffffff,  eg_lag_ptr,   0,   0,   [eg_port], False
			#egress
		)

		npb_nsh_bridge_add(self, self.target,
			#ingress
			[ig_port2], ig_lag_ptr, rmac, nexthop_ptr, bd, 0x8809, 0xffff, 0, 0x1, vid, 0xfff, dmac, 0xffffffffffff, eg_lag_ptr+1, 0+1, 0+1, [eg_port2], True
			#egress
		)
...
#		exp_pkt = base_pkt
		...
		npb_nsh_bridge_del(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr, rmac, nexthop_ptr, 0x8809, 0xffff, 0, 0x1, vid, 0xfff, dmac2, 0xffffffffffff,  eg_lag_ptr,   0,   0,   [eg_port]
			#egress
		)

		npb_nsh_bridge_del(self, self.target,
			#ingress
			[ig_port2], ig_lag_ptr, rmac, nexthop_ptr, 0x8809, 0xffff, 0, 0x1, vid, 0xfff, dmac, 0xffffffffffff, eg_lag_ptr+1, 0+1, 0+1, [eg_port2]
			#egress
		)
