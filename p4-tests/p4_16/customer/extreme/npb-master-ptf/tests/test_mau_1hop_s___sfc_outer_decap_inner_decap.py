...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		npb_nsh_chain_start_add(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr+0, 0, sap, vpn, spi, si,                          sf_bitmask, rmac, nexthop_ptr+0, bd, eg_lag_ptr+0, 0+0, 0+0, [eg_port], 0, dsap,
			#tunnel
...
#		time.sleep(1)
		...
		npb_nsh_chain_start_del(self, self.target,
			#ingress
			[ig_port], ig_lag_ptr+0, spi, si,                          sf_bitmask, rmac, nexthop_ptr+0, eg_lag_ptr+0, 0+0, 0+0, 1, [eg_port],
			#tunnel
