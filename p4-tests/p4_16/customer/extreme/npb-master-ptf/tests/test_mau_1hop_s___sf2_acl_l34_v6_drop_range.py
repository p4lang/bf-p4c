...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			npb_npb_sf2_len_rng_add(self, self.target, ig_pipe,    0x0, 0x1fff, 0) # <  8k
			npb_npb_sf2_len_rng_add(self, self.target, ig_pipe, 0x2000, 0xffff, 1) # >= 8k

			npb_npb_sf2_policy_l34_v6_add(self, self.target, ig_pipe, dsap=dsap, l3_len_rng=0x0000, l3_len_rng_mask=0xffff, drop=0) # don't drop
			npb_npb_sf2_policy_l34_v6_add(self, self.target, ig_pipe, dsap=dsap, l3_len_rng=0x0001, l3_len_rng_mask=0xffff, drop=1) # drop
