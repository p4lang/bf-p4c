...
control npb_ing_flowtable_v4 (
 ...
 RegisterAction<pair_t, bit<32>, bit<3>>(test_reg) test_reg_action = {
  void apply(
   inout pair_t reg_value,
   out bit<3> return_value
  ) {
   if((reg_value.hash == flowtable_hash[31:16]) && (reg_value.data[8:0] != ig_intr_md.ingress_port)) {
    return_value = 1;
   } else {
    return_value = 0;
   }
 ...
 apply {
  ...
  ig_intr_md_for_dprsr.drop_ctl = test_reg_action.execute((bit<32>)flowtable_hash);
 }
}
