...
control npb_ing_flowtable_v4 (
 ...
 Hash<bit<32>>(HashAlgorithm_t.IDENTITY) h2;
 ...
 apply {
  ...
  flowtable_hash2 = h2.get({1w1, ig_intr_md.ingress_port, flowtable_hash[31:16]});

  // ***** use hashed key to index register *****
  return_value = test_reg_action.execute((bit<32>)flowtable_hash);

  // ***** drop packet? *****
  if((return_value[31:31] == 1) && (return_value[24:16] == ig_intr_md.ingress_port) && (return_value[15:0] == flowtable_hash[31:16])) {
