...
state parse_inner_inner_ethernet {
    pkt.extract(hdr.inner_inner_ethernet);
    transition select(hdr.inner_inner_ethernet.ether_type) {
        0x8100 : parse_inner_inner_vlan;
        0x0800 : parse_inner_inner_ipv4;
        0x86dd : parse_inner_inner_ipv6;
        default : accept;
    }
}
state parse_inner_inner_vlan {
    pkt.extract(hdr.inner_inner_vlan_tag);
    transition select(hdr.inner_inner_vlan_tag.ether_type) {
        0x0800 : parse_inner_inner_ipv4;
        0x86dd : parse_inner_inner_ipv6;
        default : accept;
    }
...
state parse_inner_inner_igmp {
    pkt.extract(hdr.inner_inner_igmp);
    transition accept;
}
state parse_inner_inner_ipv4 {
    pkt.extract(hdr.inner_inner_ipv4);
    transition select(hdr.inner_inner_ipv4.ihl,
                      hdr.inner_inner_ipv4.flags,
                      hdr.inner_inner_ipv4.frag_offset) {
        (5, 0, 0) : parse_inner_inner_ipv4_no_options_frags;
        (5, 2, 0) : parse_inner_inner_ipv4_no_options_frags;
        default : accept;
    }
}
state parse_inner_inner_ipv4_no_options_frags {
    transition select(hdr.inner_inner_ipv4.protocol) {
        50 : parse_inner_inner_esp;
        47 : parse_inner_inner_gre;
        132 : parse_inner_inner_sctp;
        1 : parse_inner_inner_icmp;
        2 : parse_inner_inner_igmp;
        6 : parse_inner_inner_tcp;
        17 : parse_inner_inner_udp;
        default : accept;
    }
}
...
state parse_inner_inner_icmp {
    pkt.extract(hdr.inner_inner_icmp);
    transition accept;
}
state parse_sctp {
...
state parse_inner_inner_sctp {
    pkt.extract(hdr.inner_inner_sctp);
    transition accept;
}
...
state parse_inner_inner_udp {
    pkt.extract(hdr.inner_inner_udp);
    transition accept;
}
state parse_inner_inner_tcp {
    pkt.extract(hdr.inner_inner_tcp);
    transition accept;
}
state parse_mpls {
    transition accept;
...
state parse_inner_inner_esp {
    pkt.extract(hdr.inner_inner_esp);
    transition accept;
}
state parse_gre {
 pkt.extract(hdr.gre);
...
state parse_inner_inner_gre {
    pkt.extract(hdr.inner_inner_gre);
    transition accept;
}
state parse_nvgre {
...
parser SwitchEgressParser(
  ...
  eg_md.tenant_id = hdr.bridged_md.tenant_id;
        eg_md.flow_type = hdr.bridged_md.flow_type;
        eg_md.service_func_chain = hdr.bridged_md.service_func_chain;
        eg_md.eg_service_func_bitmask = hdr.bridged_md.eg_service_func_bitmask;
        transition parse_ethernet;
    }
...
control SwitchIngressDeparser(
    ...
    apply {
        ...
        pkt.emit(hdr.inner_inner_ethernet);
        pkt.emit(hdr.inner_inner_vlan_tag);
        pkt.emit(hdr.inner_inner_ipv4);
        pkt.emit(hdr.inner_inner_ipv6);
        pkt.emit(hdr.inner_inner_icmp);
        pkt.emit(hdr.inner_inner_igmp);
        pkt.emit(hdr.inner_inner_udp);
        pkt.emit(hdr.inner_inner_tcp);
        pkt.emit(hdr.inner_inner_sctp);
        pkt.emit(hdr.inner_inner_gre);
        pkt.emit(hdr.inner_inner_esp);
    }
}
...
control IngressTunnel(in switch_header_t hdr,
 ...
 bool erspan;
    ...
    action erspan_type_hit() {
        erspan = true;
    }
    table erspan_type_table {
        key = {
            hdr.erspan_type1.isValid() : exact;
        ...
        const default_action = NoAction;
        const entries = {
            (true, false) : erspan_type_hit();
            (false, true ) : erspan_type_hit();
        }
    }
    apply {
  ...
  erspan_type_table.apply();
        if (ig_md.tunnel.terminate) {
   if(erspan) {
    ig_md.tunnel.id = hdr.inner_vxlan.vni;
   }
            vni_to_bd_mapping.apply();
   if(erspan) {
    ig_md.tenant_id = hdr.inner_vxlan.vni;
             pkt_validation_inner.apply(hdr, lkp, ig_md.flags, ig_md.drop_reason);
   } else {
    ig_md.tenant_id = hdr.vxlan.vni;
...
control npb_ing_sfc_sel_class (
 ...
 action hit(
  ...
  hdr.nsh_extr.spi = nsh_sph_path_identifier;
  hdr.nsh_extr.si = nsh_sph_index;
  ig_md.flow_type = flow_type;
  ig_md.service_func_chain = service_func_chain;
 }
 action miss(
 ) {
 }
...
control npb_ing_sf_npb_basic_adv_top (
 ...
 bit<16> hash;
 bit<8> new_flow_type;
 bit<8> new_nsh_sph_index;
 bit<8> new_service_func_chain;
 apply {
  hdr.nsh_extr.si = hdr.nsh_extr.si - 1;
...
control npb_ing_sff_top (
 ...
 bit<1> end_of_chain_;
 bit<3> service_func_bitmask_;
 bit<2> ig_md_ig_service_func_bitmask;
 action dmac_miss(
 ) {
  ig_md.egress_ifindex = SWITCH_IFINDEX_FLOOD;
  end_of_chain_ = 0;
  service_func_bitmask_ = 0;
 }
 action dmac_hit(
  switch_ifindex_t ifindex,
  switch_port_lag_index_t port_lag_index,
  bit<1> end_of_chain,
  bit<3> service_func_bitmask
 ) {
  ig_md.egress_ifindex = ifindex;
  ig_md.egress_port_lag_index = port_lag_index;
  ig_md.checks.same_if = ig_md.ifindex ^ ifindex;
  end_of_chain_ = end_of_chain;
  service_func_bitmask_ = service_func_bitmask;
 }
 ...
 table myTable {
  key = {
   hdr.nsh_extr.spi : selector;
   hdr.nsh_extr.si : selector;
  }
  actions = {
   dmac_miss;
   dmac_hit;
 ...
 apply {
  if(hdr.nsh_extr.isValid()) {
   hdr.nsh_extr.ttl = hdr.nsh_extr.ttl - 1;
   if(hdr.nsh_extr.ttl == 0) {
    ig_intr_md_for_dprsr.drop_ctl = 0x1;
   }
   myTable.apply();
   ig_md_ig_service_func_bitmask = service_func_bitmask_[1:0];
   ig_md.eg_service_func_bitmask = service_func_bitmask_[2:2];
   if(end_of_chain_ == 1) {
    hdr.nsh_extr.setInvalid();
   }
   if(ig_md_ig_service_func_bitmask[0:0] == 1) {
...
control npb_egr_sff_top (
 ...
 apply {
  if(hdr.nsh_extr.isValid()) {
   if(eg_md.eg_service_func_bitmask[0:0] == 1) {
...
control SwitchIngress(
    ...
    action add_bridged_md() {
        ...
        hdr.bridged_md.tenant_id = ig_md.tenant_id;
        hdr.bridged_md.flow_type = ig_md.flow_type;
        hdr.bridged_md.service_func_chain = ig_md.service_func_chain;
        hdr.bridged_md.eg_service_func_bitmask = ig_md.eg_service_func_bitmask;
    }
