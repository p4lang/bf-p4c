...
# 1 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/core.p4" 1
/*
...
# 1 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/t2na.p4" 1
/*
 * Copyright (c) 2015-2019 Barefoot Networks, Inc.
 *
 * All Rights Reserved.
...
/* This is the P4-16 core library, which declares some built-in P4 constructs using P4 */
# 23 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/t2na.p4" 2
# 1 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/tofino2.p4" 1
/*
 * Copyright (c) 2015-2019 Barefoot Networks, Inc.
 *
 * All Rights Reserved.
...
/* This is the P4-16 core library, which declares some built-in P4 constructs using P4 */
# 23 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/tofino2.p4" 2
...
# 24 "/home/parallels/bf-sde-9.1.1/install/share/p4c/p4include/t2na.p4" 2
...
struct switch_tunnel_metadata_term_only_t {
    bool terminate;
}
...
@pa_container_size("ingress", "ig_md.lkp_1.l4_src_port", 8)
...
//  switch_ingress_checks_t checks;
 ...
 switch_cpu_reason_t cpu_reason;
    ...
    switch_lookup_fields_t lkp_1; /* this also doubles as the "main" lookup structure */
...
//#endif
    ...
    switch_tunnel_metadata_term_only_t tunnel_1;
    switch_tunnel_metadata_term_only_t tunnel_2;
...
//  switch_egress_checks_t checks;
    ...
    switch_lookup_fields_t lkp_1; /* this also doubles as the "main" lookup structure */
 switch_tunnel_type_t lkp_1_tunnel_type;
 switch_tunnel_type_t lkp_2_tunnel_type;
...
@pa_container_size("ingress" ,"hdr.cpu.ether_type", 16)
...
control IngressAcl(
 ...
 in switch_header_inner_t hdr_2,
 in switch_tunnel_metadata_term_only_t tunnel_2,
 in udf_h hdr_l7_udf,
...
*/
 ...
 apply {
  ...
  // ----- l2 -----
  mac_acl.apply(lkp, hdr_0, ig_md, ig_intr_md_for_dprsr, length_bitmask, int_ctrl_flags, nexthop, drop, terminate, scope, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);
  ...
  if (lkp.ip_type == SWITCH_IP_TYPE_IPV6) {
   ...
   ipv6_acl.apply(lkp, hdr_0, ig_md, ig_intr_md_for_dprsr, length_bitmask, int_ctrl_flags, nexthop, drop, terminate, scope, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);

  } else if (lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
   ipv4_acl.apply(lkp, hdr_0, ig_md, ig_intr_md_for_dprsr, length_bitmask, int_ctrl_flags, nexthop, drop, terminate, scope, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);
  }


  // ----- l7 -----

  l7_acl.apply(lkp, hdr_0, hdr_l7_udf, ig_md, ig_intr_md_for_dprsr, length_bitmask, int_ctrl_flags, nexthop, drop, terminate, scope, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);
...
//          hdr.ethernet.ether_type : ternary;
   ...
   eg_md.lkp_1_tunnel_outer_type : ternary @name("tunnel_outer_type");
   eg_md.lkp_1_tunnel_inner_type : ternary @name("tunnel_inner_type");
...
# 1305 "acl.p4"
 ...
 apply {
  ...
  egress_mac_acl.apply(lkp, eg_md, eg_intr_md_for_dprsr, length_bitmask, drop, terminate, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);
  ...
  if (lkp.ip_type == SWITCH_IP_TYPE_IPV6) {
   ...
   egress_ipv6_acl.apply(lkp, eg_md, eg_intr_md_for_dprsr, length_bitmask, drop, terminate, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);

  } else if (lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
   egress_ipv4_acl.apply(lkp, eg_md, eg_intr_md_for_dprsr, length_bitmask, drop, terminate, copy_to_cpu, redirect_to_cpu, cpu_reason, copp_meter_id);
  }
...
# 94 "npb_ing_parser.p4"
        ...
        //ig_md.lkp_1.tunnel_type = 0; // do this always (above)
...
# 129 "npb_ing_parser.p4"
        ...
        //ig_md.lkp_2.tunnel_type = 0; // do this always (above)
...
//      ig_md.ifindex = port_md.ifindex;
  ig_md.nsh_md.l2_fwd_en = (bool)port_md.l2_fwd_en;
    ...
    state parse_transport_nsh_tagged {
        ...
        // ig_md.lkp_0.tunnel_id = (bit<TUNNEL_ID_WIDTH>)pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;
        ...
        // ig_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.transport.vlan_tag[0].vid;
        ...
        // ig_md.lkp_1.tunnel_id[15:0] = (hdr.transport.vlan_tag[0].pcp ++ hdr.transport.vlan_tag[0].cfi ++ hdr.transport.vlan_tag[0].vid);
...
// #ifdef CPU_PARSER_EXTRACTS_HDR_FIELDS_ENABLE
...
// #endif
        ...
        ig_md.lkp_1.mac_type = hdr.cpu.ether_type; // todo: confirm
...
// #endif
        transition select(hdr.cpu.ether_type) {
            0x893F : parse_outer_br; // todo: will we see on cpu pkt?
            0x8926 : parse_outer_vn; // todo: will we see on cpu pkt?
            0x8100: parse_outer_vlan_0;
            0x88A8: parse_outer_vlan_0;
            //ETHERTYPE_MPLS: parse_outer_mpls; // todo: will we see on cpu pkt?
            //ETHERTYPE_ARP : parse_outer_arp;  // todo: will we see on cpu pkt?
    ...
    state parse_outer_vlan_0 {
        ...
        // [--Werror=type-error] error: cast: Cannot unify T function<T>() to bit<32>
        // ig_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)pkt.lookahead<vlan_tag_grouped_h>().pcp_cfi_vid;
     ...
     pkt.extract(hdr.outer.vlan_tag[0]);
        ...
        // error: Field is extracted in the parser into multiple containers, but the container slices after the
        // first aren't byte aligned: 77:ingress::ig_md.lkp_1.tunnel_id<32> ^0 ^bit[0..15] meta solitary mocha
        // ig_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.outer.vlan_tag[0].vid;
    ...
    state parse_outer_vlan_1 {
     pkt.extract(hdr.outer.vlan_tag[1]);
        ...
        //ig_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.outer.vlan_tag[1].vid;
...
// #endif /* ERSPAN_TRANSPORT_ENABLE */            
    ...
    state extract_outer_gtp_c {
        ...
        ig_md.lkp_1.tunnel_id = hdr.outer.gtp_v2_base.teid;
        //ig_md.lkp_1.tunnel_id = 0;
     transition accept;
    }
  ...
  // ---------------
    ...
    state parse_outer_vlan_0_scope0 {
     pkt.extract(hdr.outer.vlan_tag[0]);
        ...
        //eg_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.outer.vlan_tag[0].vid;
    ...
    state parse_outer_vlan_1_scope0 {
     pkt.extract(hdr.outer.vlan_tag[1]);
        ...
        //eg_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.outer.vlan_tag[1].vid;
...
# 579 "npb_egr_parser.p4"
    ...
    state parse_outer_vxlan_scope0 {
        ...
        pkt.extract(hdr.outer.vxlan);
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_VXLAN;
    ...
    state parse_outer_ipinip_set_tunnel_type_scope0 {
        ...
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_IPINIP;
    ...
    state parse_outer_gre_scope0 {
     pkt.extract(hdr.outer.gre);
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_GRE;
    ...
    state parse_outer_nvgre_scope0 {
     pkt.extract(hdr.outer.nvgre);
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_NVGRE;
    ...
    state extract_outer_gtp_c_scope0 {
        pkt.extract(hdr.outer.gtp_v2_base);
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_GTPC;

        eg_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_GTPC;
        eg_md.lkp_1.tunnel_id = hdr.outer.gtp_v2_base.teid;
        //eg_md.lkp_1.tunnel_id = 0;
    ...
    state extract_outer_gtp_u_scope0 {
        pkt.extract(hdr.outer.gtp_v1_base);
        eg_md.lkp_1_tunnel_type = SWITCH_TUNNEL_TYPE_GTPU;
    ...
    state parse_inner_vlan_scope1 {
        ...
        eg_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_VLAN;
        //eg_md.lkp_1.tunnel_id = (bit<TUNNEL_ID_WIDTH>)hdr.inner.vlan_tag[0].vid;
        transition select(hdr.inner.vlan_tag[0].ether_type) {
            0x0800 : parse_inner_ipv4_scope1;
            0x86dd : parse_inner_ipv6_scope1;
            default : accept;
...
# 1026 "npb_egr_parser.p4"
    ...
    state parse_inner_gre_scope0 {
        pkt.extract(hdr.inner.gre);
        eg_md.lkp_2_tunnel_type = SWITCH_TUNNEL_TYPE_GRE;
        transition accept;
    }
    ...
    state extract_inner_gtp_c_scope0 {
        pkt.extract(hdr.inner.gtp_v2_base);
        eg_md.lkp_2_tunnel_type = SWITCH_TUNNEL_TYPE_GTPC;
     transition accept;
    }
    ...
    state extract_inner_gtp_c_scope1 {
        ...
        eg_md.lkp_2_tunnel_type = SWITCH_TUNNEL_TYPE_GTPC;
        eg_md.lkp_1.tunnel_type = SWITCH_TUNNEL_TYPE_GTPC;
        eg_md.lkp_1.tunnel_id = hdr.inner.gtp_v2_base.teid;
        //eg_md.lkp_1.tunnel_id = 0;
        transition accept;
    }
    ...
    state extract_inner_gtp_u_scope0 {
        pkt.extract(hdr.inner.gtp_v1_base);
        eg_md.lkp_2_tunnel_type = SWITCH_TUNNEL_TYPE_GTPU;
        transition accept;
    }
...
control IngressPortMapping(
 ...
 action terminate_cpu_packet() {
  ...
  //hdr.outer.ethernet.ether_type = hdr.cpu.ether_type; // DEREK: MOVED TO PARSER

// #ifndef CPU_PARSER_EXTRACTS_HDR_FIELDS_ENABLE
// 		// ig_md.bypass = hdr.cpu.reason_code; // DEREK: MOVED TO PARSER
// 		ig_md.port = (switch_port_t) hdr.cpu.ingress_port; // DEREK: MOVED TO PARSER
// 		ig_md.egress_port_lag_index = (switch_port_lag_index_t) hdr.cpu.port_lag_index; // DEREK: MOVED TO PARSER
// 		hdr.outer.ethernet.ether_type = hdr.cpu.ether_type; // DEREK: MOVED TO PARSER
// #endif
...
//		ig_md.qos.tc = tc;
  ...
  hdr.transport.nsh_type1.sap = sap; // 16 bits
  hdr.transport.nsh_type1.vpn = vpn; // 16 bits
 ...
 table port_mapping {
  ...
  actions = {
   ...
   set_cpu_port_properties_with_nsh;
   set_cpu_port_properties_without_nsh;
  }
...
control EgressPortMapping(
 ...
 action cpu_rewrite() {
  ...
  hdr.outer.ethernet.ether_type = 0x9000;
 }
...
control IngressTunnelOuterInner(
 ...
 in switch_lookup_fields_t lkp_2,
 in switch_header_inner_t hdr_2,
 inout switch_tunnel_metadata_term_only_t tunnel_2
) (
...
control TunnelDecapOuter(
 ...
 in switch_tunnel_metadata_term_only_t tunnel
) (
 switch_tunnel_mode_t mode
) {
 // -------------------------------------
...
control npb_ing_sfc_top (
 ...
 inout switch_tunnel_metadata_t tunnel_0,
 inout switch_header_outer_t hdr_1,
 inout switch_tunnel_metadata_term_only_t tunnel_1,
 inout switch_header_inner_t hdr_2,
 inout switch_tunnel_metadata_term_only_t tunnel_2,
...
//			if(action_bitmask_internal[0:0] == 1) {
    ...
    acl.apply(ig_md.lkp_1, ig_md, ig_intr_md_for_dprsr, ig_intr_md_for_tm, length_bitmask_internal, hdr_0, hdr_2, ig_md.tunnel_2, hdr_l7_udf, ig_md.nsh_md.int_ctrl_flags);
...
# 158 "npb_ing_top.p4"
  ...
  tunnel_decap_transport_ingress.apply(hdr_0, tunnel_0);
...
//			if(eg_md.action_bitmask[0:0] == 1) {
    ...
    acl.apply(eg_md.lkp_1, eg_md, eg_intr_md_for_dprsr, length_bitmask_internal, hdr_0);
...
# 69 "npb_egr_top.p4"
   // outer means two back from current scope (scope-2), inner means one back from current scope (scope-1)
   eg_md.lkp_1_tunnel_outer_type = SWITCH_TUNNEL_TYPE_NONE;
   eg_md.lkp_1_tunnel_inner_type = SWITCH_TUNNEL_TYPE_NONE;
...
# 86 "npb_egr_top.p4"
   // outer means two back from current scope (scope-2), inner means one back from current scope (scope-1)
   eg_md.lkp_1_tunnel_outer_type = SWITCH_TUNNEL_TYPE_NONE;
   eg_md.lkp_1_tunnel_inner_type = eg_md.lkp_1_tunnel_type;
