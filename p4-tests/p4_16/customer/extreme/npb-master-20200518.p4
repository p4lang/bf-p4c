...
struct nsh_metadata_t {
 ...
 bit<16> hash; // ingress only (for sfp sel)
...
// @alias pragma is field-based - decided not to go this route....
// #ifndef EGRESS_HEADERS_X2
//   @pa_alias("ingress", "hdr.outer", "hdr.outer_scope1")
//   @pa_alias("ingress", "hdr.inner", "hdr.inner_scope1")
// #endif // EGRESS_HEADERS_X2
...
/*
	action scope_tunnel_vlan() {
		...
		lkp.tunnel_id   = (switch_tunnel_id_t)hdr.vlan_tag[0].vid;
	}
*/
 action scope_tunnel_vni() {
  lkp.tunnel_type = tunnel.type;
  lkp.tunnel_id = tunnel.id;
 }
...
control IngressIpv4Acl(
 ...
 table acl {
  key = {
   lkp.ip_src_addr[31:0] : ternary; lkp.ip_dst_addr[31:0] : ternary; lkp.ip_proto : ternary; lkp.ip_tos : ternary; lkp.l4_src_port : ternary; lkp.l4_dst_port : ternary; lkp.tcp_flags : ternary; lkp.tunnel_type : ternary; lkp.tunnel_id : ternary;
...
*/
 ...
 apply {
  ...
  l7_acl.apply(lkp, hdr_0, hdr_l7_udf, ig_md, ig_intr_md_for_dprsr, nexthop, length_bitmask, drop, terminate, scope, int_ctrl_flags);
...
//  #endif
*/
    ig_md.nsh_md.hash = ig_md.nsh_md.hash_2;
   }
...
control EgressIpv4Acl(
 ...
 table acl {
  key = {
   lkp.ip_src_addr[31:0] : ternary; lkp.ip_dst_addr[31:0] : ternary; lkp.ip_proto : ternary; lkp.ip_tos : ternary; lkp.tcp_flags : ternary; lkp.l4_src_port : ternary; lkp.l4_dst_port : ternary; lkp.tunnel_type : ternary; lkp.tunnel_id : ternary;
//          hdr.ethernet.ether_type : ternary;
...
# 166 "npb_ing_parser.p4"
    ...
    state parse_transport_nsh {
     ...
     pkt.extract(hdr.transport.nsh_type1);
        ...
        // todo: Transition based on NSH next-proto - don't assume enet
    ...
    // GRE - Transport
    //-------------------------------------------------------------------------
# 254 "npb_ing_parser.p4"
    //-------------------------------------------------------------------------
    // ERSPAN - Transport
    //-------------------------------------------------------------------------
# 292 "npb_ing_parser.p4"
    ///////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////
    // "Outer" Headers / Stack
    ///////////////////////////////////////////////////////////////////////////
...
# 590 "npb_ing_parser.p4"
    ...
    state parse_outer_vxlan {
        ...
        ig_md.tunnel_1.id = (bit<24>)hdr.outer.vxlan.vni;

        ig_md.lkp.tunnel_type = SWITCH_TUNNEL_TYPE_VXLAN;
        ig_md.lkp.tunnel_id = (bit<24>)hdr.outer.vxlan.vni;
...
// #endif /* ERSPAN_TRANSPORT_ENABLE */            
  ...
  ig_md.tunnel_1.id = (bit<24>)hdr.outer.nvgre.vsid;

        ig_md.lkp.tunnel_type = SWITCH_TUNNEL_TYPE_NVGRE;
        ig_md.lkp.tunnel_id = (bit<24>)hdr.outer.nvgre.vsid;
    ...
    state extract_outer_gtp_c {
        ...
        // ig_md.tunnel_1.id = hdr.outer.gtp_v2_base.teid;
    ...
    state extract_outer_gtp_u {
        ...
        // ig_md.tunnel_1.id = hdr.outer.gtp_v1_base.teid;
...
# 573 "npb_egr_parser.p4"
    ...
    state parse_outer_vxlan_scope0 {
        ...
        eg_md.tunnel_1.id = (bit<24>)hdr.outer.vxlan.vni;

        eg_md.lkp.tunnel_type = SWITCH_TUNNEL_TYPE_VXLAN;
        eg_md.lkp.tunnel_id = (bit<24>)hdr.outer.vxlan.vni;
    ...
    state parse_outer_nvgre_scope0 {
     pkt.extract(hdr.outer.nvgre);
        ...
        eg_md.tunnel_1.id = (bit<24>)hdr.outer.nvgre.vsid;

        eg_md.lkp.tunnel_type = SWITCH_TUNNEL_TYPE_NVGRE;
        eg_md.lkp.tunnel_id = (bit<24>)hdr.outer.nvgre.vsid;
    ...
    state parse_inner_vlan_scope1 {
        ...
        eg_md.lkp.pcp = hdr.inner.vlan_tag[0].pcp;
...
# 92 "npb_egr_deparser.p4"
        // ***** TRANSPORT *****
        pkt.emit(hdr.transport.ethernet);
        pkt.emit(hdr.transport.vlan_tag);
        pkt.emit(hdr.transport.nsh_type1);
# 105 "npb_egr_deparser.p4"
        // ***** OUTER *****
        pkt.emit(hdr.outer.ethernet);
        pkt.emit(hdr.outer.e_tag);
        pkt.emit(hdr.outer.vn_tag);
...
// derek: commented this out, because on tofino 1 it causes all kinds ov valids to not fit (compiler bug?)
      vlan_to_bd_mapping.apply();
...
control IngressTunnelTransport(
 inout switch_ingress_metadata_t ig_md,
 inout switch_lookup_fields_t lkp,
 inout switch_header_transport_t hdr_0,
 inout switch_tunnel_metadata_t tunnel_0
...
control IngressTunnelOuterInner(
 ...
 inout switch_lookup_fields_t lkp,
 inout switch_header_transport_t hdr_0,
 inout switch_lookup_fields_t lkp_2,
 inout switch_header_inner_t hdr_2,
 inout switch_tunnel_metadata_t tunnel_2
) (
...
control TunnelDecapOuter(
 ...
 apply {
  ...
  if(tunnel.terminate) {
   ...
   // -------------------------------------
   // temporary, until barefoot bug fixed!
   // -------------------------------------
# 627 "tunnel.p4"
  }
...
control npb_ing_sfc_top (
 ...
 IngressTunnelTransport(NPB_ING_SFC_TUNNEL_NETWORK_SAP_TABLE_DEPTH) tunnel_network;
...
//			hdr_0.nsh_type1.timestamp                    = 0;                     // FOR SIMS
   ...
   if(hdr_0.ethernet.isValid()) {
    ...
    tunnel_transport.apply(ig_md, ig_md.flags.ipv4_checksum_err_0, hdr_0, tunnel_0, ig_intr_md_for_dprsr, hit);


    if(hit) {
//					tunnel_transport_sap.apply();
     tunnel_network.apply(ig_md, ig_md.lkp, hdr_0, tunnel_0);
    }
...
//			flow_class_internal     : selector;
   ig_md.nsh_md.hash[15:0] : selector;
...
//  Scoper_l7() scoper_l7;
 ...
 npb_ing_sf_npb_basic_adv_sfp_hash() npb_ing_sf_npb_basic_adv_sfp_hash_lkp;
...
//      }
  ...
  npb_ing_sf_npb_basic_adv_sfp_hash_lkp.apply(
   hdr_0,
   ig_md,
   ig_intr_md,
   ig_intr_md_from_prsr,
...
//		if(eg_md.tunnel_0.terminate == false) { // check for si of 0 (or underflow, since we used a saturating subtracts)
  if(eg_md.nsh_md.end_of_path == false) { // check for si of 0 (or underflow, since we used a saturating subtracts)
   ...
   hdr.nsh_type1.setInvalid(); // it's the end of the line for this nsh chain....
  }
...
//         const entries = {
// 
//             //enet   vlan0  arp    ipv4   ipv6   icmp,  igmp,  udp    tcp    sctp   gtp_v1 gtp_v2
// 
//             // None (invalid)
//             ( false, false, false, false, false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2
//             ( true,  false, false, false, false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3
//             ( true,  false, true,  false, false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  true,  false, false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, true,  false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / ICMP
//             ( true,  false, false, true,  false, true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / IGMP
//             ( true,  false, false, true,  false, false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / UDP
//             ( true,  false, false, true,  false, false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / TCP
//             ( true,  false, false, true,  false, false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / SCTP
//             ( true,  false, false, true,  false, false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//
//             // L2 / L3 / UDP / GTP-U
//             ( true,  false, false, true,  false, false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L2 / L3 / UDP / GTP-C
//             ( true,  false, false, true,  false, false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, true,  false, false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
//             ( true,  false, false, false, true,  false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
//             ( true,  true,  false, false, true,  false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
//
//             // L3
//             ( false, false, false, true,  false, false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / ICMP
//             ( false, false, false, true,  false, true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  true,  false, false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / IGMP
//             ( false, false, false, true,  false, false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, true,  false, false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / UDP
//             ( false, false, false, true,  false, false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, true,  false, false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / TCP
//             ( false, false, false, true,  false, false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, false, true,  false, false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / SCTP
//             ( false, false, false, true,  false, false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, false, false, true,  false, false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / UDP / GTP-U
//             ( false, false, false, true,  false, false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, true,  false, false, true,  false ): bump_inner_stack_hdr_cntr;
//                                                               
//             // L3 / UDP / GTP-C
//             ( false, false, false, true,  false, false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
//             ( false, false, false, false, true,  false, false, true,  false, false, false, true  ): bump_inner_stack_hdr_cntr;
// 
//         }
    }
...
# 608 "npb_ing_hdr_stack_counters.p4"
        hdr_inner_arp_isValid = false;
        hdr_inner_icmp_isValid = false;
        hdr_inner_igmp_isValid = false;
        ...
        hdr_inner_gtp_v2_base_isValid = false;
...
// #endif // PARDE_INNER_GRE_ENABLE
// 
// #ifdef PARDE_INNER_ESP_ENABLE    
//         hdr_inner_esp_isValid = hdr.inner.esp.isValid();
// #else
//         hdr_inner_esp_isValid = false;
// #endif // PARDE_INNER_ESP_ENABLE
...
# 112 "npb_ing_set_lkp.p4"
    // -----------------------------
 // Apply
    // -----------------------------

    apply {
# 130 "npb_ing_set_lkp.p4"
...
# 127 "npb_egr_set_lkp.p4"
    // -----------------------------
 // Apply
    // -----------------------------

    apply {
# 140 "npb_egr_set_lkp.p4"
  // ipv6: would like to do this stuff in the parser, but can't because tos
  // field isn't byte aligned...
