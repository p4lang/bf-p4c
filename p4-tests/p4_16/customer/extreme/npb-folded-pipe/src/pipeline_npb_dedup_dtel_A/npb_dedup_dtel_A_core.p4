#ifndef _NPB_DEDUP_DTEL_A_CORE_
#define _NPB_DEDUP_DTEL_A_CORE_
...
//		dtel.apply(hdr.transport, ig_md.lkp_1, ig_md, ig_md.hash[15:0], ig_intr_md_for_dprsr, ig_intr_md_for_tm);
		...
		ig_intr_md_for_tm.ucast_egress_port = ig_intr_md_for_tm.ucast_egress_port | 0x100; // for folded pipe (set the high bit) - FOR FOLD IN PIPES 2 and 3
//		ig_intr_md_for_tm.ucast_egress_port = ig_intr_md_for_tm.ucast_egress_port & 0x0ff; // for folded pipe (clr the high bit) - FOR FOLD IN PIPES 0 and 1
...
//  TunnelDecap() tunnel_decap;
	...
	Npb_Egr_Top_Folded(INSTANCE_DEPLOYMENT_PARAMS) npb_egr_top;
	...
	action hit(
		...
		eg_md.dtel.report_type = SWITCH_DTEL_REPORT_TYPE_FLOW; // for testing
		eg_md.dtel.session_id = 0x5; // for testing
	}

	table mirror_session_to_dtel_session {
		key = {
...
//      eg_intr_md_for_dprsr.drop_ctl = 0;
		...
		if (eg_md.flags.bypass_egress == false) {
			...
			// we need to hash here, because the hash that comes across in the bridged-metadata may have had fields masked out -- so we don't want to use it for dtel.
			// TODO: ingress parser currently doesn't parse mirrored packets -- this will need to be fixed so we have something to hash on....
/*
			if (eg_md.lkp_1.ip_type == SWITCH_IP_TYPE_NONE) {
				non_ip_hash.apply(eg_md.lkp_1, eg_md.hash[31:0]);
			} else if (eg_md.lkp_1.ip_type == SWITCH_IP_TYPE_IPV4) {
				ipv4_hash.apply(eg_md.lkp_1, eg_md.hash[31:0]);
			} else {
				ipv6_hash.apply(eg_md.lkp_1, eg_md.hash[31:0]);
			}
*/
			eg_md.dtel.hash = eg_md.hash;
...
#ifdef DTEL_ENABLE
				// 1st packet: original pkt

				// TODO: don't know where/how this stuff will ultimately be set....
				eg_md.dtel.report_type = SWITCH_DTEL_REPORT_TYPE_FLOW; // for testing
				eg_md.dtel.session_id = 0x5; // for testing
//				mirror_session_to_dtel_session.apply();

				// from mirror_rewrite.p4
  #ifdef INT_V2
// TODO: Fix!
//				hdr.transport.dtel.report_length = eg_md.pkt_length >> 2;
  #endif
			} else {
				// 2nd packet: mirrored pkt (encap it)
				eg_md.nexthop = 5; // for testing
				eg_md.tunnel_0.dip_index = 5; // for testing
				eg_md.tunnel_nexthop = 6; // for testing
#endif
			}

			// ----------------------------------

			// 1st & 2nd packet: run dtel
#ifdef DTEL_ENABLE
			dtel.apply(hdr.transport, eg_md, eg_intr_md, eg_intr_md_from_prsr, eg_md.dtel.hash);
