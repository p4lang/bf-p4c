...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			eg_port_3 = eg_swports[3] # derek changed from [4] so that this test can run on the hw, which only has 4 cpu ports total.
			...
			npb_nsh_chain_start_add(self, self.target,ig_pipe,
				#ingress
				[ig_port], ig_lag_ptr, 0, sap, vpn, spi, si, sf_bitmask, rmac, nexthop_ptr, bd, eg_lag_ptr, 0, 0, [eg_port_0, eg_port_1, eg_port_2, eg_port_3], False, 0, dsap,
				#tunnel
...
#			time.sleep(1)
			...
			logger.info("Verify packet on port %d, %d, %d, or %d", eg_port_0, eg_port_1, eg_port_2, eg_port_3)
			testutils.verify_packets_any(self, exp_pkt, [eg_port_0, eg_port_1, eg_port_2, eg_port_3])
