...
#		self.target = gc.Target(self.dev, pipe_id=0xffff)
	...
	def runTest(self):
		...
		for ig_pipe in ig_pipes:
			...
			#Src Port Range
			npb_npb_sf0_l4_src_port_rng_add(self, self.target, ig_pipe,    0x0, 0x1fff, 0) # <  8k
			npb_npb_sf0_l4_src_port_rng_add(self, self.target, ig_pipe, 0x2000, 0xffff, 1) # >= 8k

			npb_npb_sf0_policy_l34_v6_add(self, self.target, ig_pipe, sap=sap, l4_src_rng=0, l4_src_rng_mask=0xffff,  flow_class=flow_class_acl, drop=0) # don't drop
			npb_npb_sf0_policy_l34_v6_add(self, self.target, ig_pipe, sap=sap, l4_src_rng=1, l4_src_rng_mask=0xffff,  flow_class=flow_class_acl, drop=1) # drop
...
#			testutils.verify_packets(self, exp_pkt, [eg_port])
			...
			npb_npb_sf0_l4_src_port_rng_del(self, self.target, ig_pipe,    0x0, 0x1fff) # <  8k
			npb_npb_sf0_l4_src_port_rng_del(self, self.target, ig_pipe, 0x2000, 0xffff) # >= 8k

			npb_npb_sf0_policy_l34_v6_del(self, self.target, ig_pipe, sap=sap, l4_src_rng=0, l4_src_rng_mask=0xffff) # don't drop
			npb_npb_sf0_policy_l34_v6_del(self, self.target, ig_pipe, sap=sap, l4_src_rng=1, l4_src_rng_mask=0xffff) # drop
			...
			#Dest Port Range
			npb_npb_sf0_l4_dst_port_rng_add(self, self.target, ig_pipe,    0x0, 0x1fff, 0) # <  8k
			npb_npb_sf0_l4_dst_port_rng_add(self, self.target, ig_pipe, 0x2000, 0xffff, 1) # >= 8k

			npb_npb_sf0_policy_l34_v6_add(self, self.target, ig_pipe, sap=sap, l4_dst_rng=0, l4_dst_rng_mask=0xffff,  flow_class=flow_class_acl, drop=0) # don't drop
			npb_npb_sf0_policy_l34_v6_add(self, self.target, ig_pipe, sap=sap, l4_dst_rng=1, l4_dst_rng_mask=0xffff,  flow_class=flow_class_acl, drop=1) # drop
