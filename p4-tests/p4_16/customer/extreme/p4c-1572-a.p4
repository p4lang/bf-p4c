...
const switch_tunnel_type_t SWITCH_TUNNEL_TYPE_MAC_IN_MAC_NSH = 3;
...
struct switch_ingress_metadata_t {
    ...
    mac_addr_t underlay_ethernet_dst_addr;
    mac_addr_t underlay_ethernet_src_addr;
    bit<16> underlay_ethernet_ether_type;
    bit<3> underlay_vlan_tag_pcp;
    bit<1> underlay_vlan_tag_cfi;
    vlan_id_t underlay_vlan_tag_vid;
    bit<16> underlay_vlan_tag_ether_type;
    bit<4> underlay_ipv4_version;
    bit<4> underlay_ipv4_ihl;
    bit<8> underlay_ipv4_diffserv;
    bit<16> underlay_ipv4_total_len;
    bit<16> underlay_ipv4_identification;
    bit<3> underlay_ipv4_flags;
    bit<13> underlay_ipv4_frag_offset;
    bit<8> underlay_ipv4_ttl;
    bit<8> underlay_ipv4_protocol;
    bit<16> underlay_ipv4_hdr_checksum;
    ipv4_addr_t underlay_ipv4_src_addr;
    ipv4_addr_t underlay_ipv4_dst_addr;
    bit<1> underlay_gre_C;
    bit<1> underlay_gre_R;
    bit<1> underlay_gre_K;
    bit<1> underlay_gre_S;
    bit<1> underlay_gre_s;
    bit<3> underlay_gre_recurse;
    bit<5> underlay_gre_flags;
    bit<3> underlay_gre_version;
    bit<16> underlay_gre_proto;
 bit<1> nsh_extr_valid;
 bit<1> nsh_extr_end_of_chain;
...
struct switch_header_t {
 ...
 switch_mirror_metadata_h mirror;
    ...
    ipv4_h ipv4_underlay;
    gre_h gre_underlay;
    nsh_extr_h nsh_extr_underlay;
    erspan_type1_h erspan_type1_underlay;
    erspan_type2_h erspan_type2_underlay;
    mpls_h[4] mpls;
...
const bit<32> NPB_EGR_SFF_TABLE_DEPTH = 1024;
...
parser ParserUnderlayL2_MdBackup(
    packet_in pkt,
    inout switch_header_t hdr,
    ...
    state start {
        ...
        ig_md.underlay_ethernet_dst_addr = hdr.ethernet.dst_addr;
        ig_md.underlay_ethernet_src_addr = hdr.ethernet.src_addr;
        ig_md.underlay_ethernet_ether_type = hdr.ethernet.ether_type;
    ...
    state parse_vlan {
     pkt.extract(hdr.vlan_tag.next);
        ig_md.underlay_vlan_tag_pcp = hdr.vlan_tag.last.pcp;
        ig_md.underlay_vlan_tag_cfi = hdr.vlan_tag.last.cfi;
        ig_md.underlay_vlan_tag_vid = hdr.vlan_tag.last.vid;
        ig_md.underlay_vlan_tag_ether_type = hdr.vlan_tag.last.ether_type;
...
parser NpbIngressParser(
    ...
    Checksum<bit<16>>(ChecksumAlgorithm_t.CSUM16) ipv4_checksum_again;
    Checksum<bit<16>>(ChecksumAlgorithm_t.CSUM16) inner_ipv4_checksum;
    ParserUnderlayL2() parser_underlay_l2;
    ParserUnderlayL2_MdBackup() parser_underlay_l2_md_backup;
 bit<16> ether_type;
 ...
 bit<8> inner_protocol;
    ...
    state parse_ethernet {
        parser_underlay_l2_md_backup.apply(pkt, hdr, ig_md, ether_type);
        transition select(ether_type) {
            0x894F : parse_nsh_underlay;
            0x0806 : parse_arp;
            0x8847 : parse_mpls;
            0x0800 : parse_ipv4;
            0x86dd : parse_ipv6;
    ...
    state parse_nsh_underlay {
        pkt.extract(hdr.nsh_extr_underlay);
        transition parse_ethernet_again_nsh;
    }
    state parse_ethernet_again_nsh {
    ...
    state parse_ethernet_again_erspan {
        pkt.extract(hdr.ethernet);
        ether_type = hdr.ethernet.ether_type;
        transition select(hdr.ethernet.ether_type) {
            0x893F : parse_br;
    ...
    state parse_ipv4 {
        ...
        ig_md.underlay_ipv4_version = hdr.ipv4.version;
        ig_md.underlay_ipv4_ihl = hdr.ipv4.ihl;
        ig_md.underlay_ipv4_diffserv = hdr.ipv4.diffserv;
        ig_md.underlay_ipv4_total_len = hdr.ipv4.total_len;
        ig_md.underlay_ipv4_identification = hdr.ipv4.identification;
        ig_md.underlay_ipv4_flags = hdr.ipv4.flags;
        ig_md.underlay_ipv4_frag_offset = hdr.ipv4.frag_offset;
        ig_md.underlay_ipv4_ttl = hdr.ipv4.ttl;
        ig_md.underlay_ipv4_protocol = hdr.ipv4.protocol;
        ig_md.underlay_ipv4_hdr_checksum = hdr.ipv4.hdr_checksum;
        ig_md.underlay_ipv4_src_addr = hdr.ipv4.src_addr;
        ig_md.underlay_ipv4_dst_addr = hdr.ipv4.dst_addr;
    ...
    state parse_ipv4_again {
        pkt.extract(hdr.ipv4);
        protocol = hdr.ipv4.protocol;
        ipv4_checksum_again.add(hdr.ipv4);
        transition select(hdr.ipv4.ihl, hdr.ipv4.flags, hdr.ipv4.frag_offset) {
            (5, 3w2 &&& 3w5, 0): parse_ipv4_no_options_frags_again;
            default : accept;
        }
    }
    state parse_ipv4_no_options_frags_again {
        ig_md.flags.ipv4_checksum_again_err = ipv4_checksum_again.verify();
        transition select(hdr.ipv4.protocol) {
            0x2f: parse_gre_again;
            default: parse_l3_protocol;
        }
   }
    state parse_ipv6 {
    ...
    state parse_gre {
     pkt.extract(hdr.gre);
        ig_md.underlay_gre_C = hdr.gre.C;
        ig_md.underlay_gre_R = hdr.gre.R;
        ig_md.underlay_gre_K = hdr.gre.K;
        ig_md.underlay_gre_S = hdr.gre.S;
        ig_md.underlay_gre_s = hdr.gre.s;
        ig_md.underlay_gre_recurse = hdr.gre.recurse;
        ig_md.underlay_gre_flags = hdr.gre.flags;
        ig_md.underlay_gre_version = hdr.gre.version;
        ig_md.underlay_gre_proto = hdr.gre.proto;
        transition select(
            hdr.gre.C,
            hdr.gre.R,
            hdr.gre.K,
            ...
            (0,0,0,0,0,0,0,0x88BE): parse_erspan_t1_underlay;
            (0,0,0,1,0,0,0,0x88BE): parse_erspan_t2_underlay;
            default: accept;
        }
    }
    state parse_gre_again {
     pkt.extract(hdr.gre);
        transition select(
            hdr.gre.C,
            hdr.gre.R,
    ...
    state parse_erspan_t1_underlay {
        pkt.extract(hdr.erspan_type1_underlay);
        transition parse_ethernet_again_erspan;
    }
    state parse_erspan_t2_underlay {
        pkt.extract(hdr.erspan_type2_underlay);
        transition parse_ethernet_again_erspan;
    }
}
parser NpbEgressParser(
 ...
 bit<8> inner_protocol;
    ...
    state parse_bridged_metadata {
        ...
        transition select(eg_md.nsh_extr_valid) {
            0: parse_underlay_l2;
...
control TunnelEncap(inout switch_header_t hdr,
   ...
   SWITCH_TUNNEL_TYPE_MAC_IN_MAC_NSH : rewrite_mac_in_mac_nsh();
        }
    }
    apply {
    }
