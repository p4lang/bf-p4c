...
header snoop_untagged_h {
    ...
    bit<8> protocol;
    bit<16> hdr_checksum;
    ipv4_addr_t src_addr_ip;
    ipv4_addr_t dst_addr_ip;
    bit<1> C;
    bit<1> R;
    bit<1> K;
    bit<1> S;
...
header_union snoop_h {
    snoop_untagged_h untagged;
    snoop_tagged_h tagged;
}
...
control Vtepv6_NSH<T>(in ipv6_addr_t src_addr,
                  in ipv6_addr_t dst_addr,
                  inout switch_ingress_metadata_t ig_md,
                  out switch_ifindex_t vtep_ifindex)(
                  switch_uint32_t src_vtep_table_size,
...
control Vtep_NSH<T>(in ipv4_addr_t src_addr,
                in ipv4_addr_t dst_addr,
                inout switch_ingress_metadata_t ig_md,
                out switch_ifindex_t vtep_ifindex)(
                switch_uint32_t src_vtep_table_size,
...
control IngressTunnel(in switch_header_t hdr,
    ...
    switch_ifindex_t vtep_ifindex_nsh;
    Vtep_NSH<ipv4_addr_t>(
        IPV4_SRC_TUNNEL_TABLE_SIZE, DEST_TUNNEL_TABLE_SIZE) ipv4_vtep_nsh;
    Vtepv6_NSH<ipv6_addr_t>(
        IPV6_SRC_TUNNEL_TABLE_SIZE, DEST_TUNNEL_TABLE_SIZE) ipv6_vtep_nsh;
    ...
    action set_vni_properties_nsh(
        ...
        ig_md.ifindex_nsh = vtep_ifindex;
    }
    ...
    apply {
        switch(rmac.apply().action_run) {
            rmac_hit : {
                if (lkp.ip_type == SWITCH_IP_TYPE_IPV4) {
                    ...
                    ipv4_vtep_nsh.apply(
                        lkp_nsh.ipv4_src_addr, lkp_nsh.ipv4_dst_addr, ig_md, vtep_ifindex_nsh);
                } else if (lkp.ip_type == SWITCH_IP_TYPE_IPV6) {
                }
            }
        }
        ...
        if (ig_md.tunnel_nsh.type != SWITCH_TUNNEL_TYPE_NONE) {
...
control npb_ing_sfc_top (
 ...
 action table_2_hit (
  ...
  ig_md.nsh_extr.extr_tenant_id = ig_md.bd;
...
if((ig_md.nsh_extr_valid == 0) || (ig_md.nsh_extr_end_of_chain == 1)) {
        ...
        ig_md.ifindex = ig_md.ifindex_nsh;
}
