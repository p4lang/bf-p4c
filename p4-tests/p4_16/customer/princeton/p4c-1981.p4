/*
  RTT measurement based on TCP syn/ack
  
  Version 001, last modified 2019 07 16
  
  Author: Willie Chang, Javed Aman, Xiaoqi Chen
  Copyright Princeton Univ., all rights reserved.
  
  Part of this code includes Barefoot Tofino specific platform code and is subject to NDA.
  
  
  Known bugs:
  1. Using if-else calling action exec(){salu.execute()} will cause the second SALU blackbox not to run??
  2.
  3.
 */
...
control SwitchIngressDeparser(
    ...
    //TODO!!! ipv4 checksum for report header
...
control SwitchIngress(
        ...
        action compute_eack_last_if_syn(){
            ...
            //save 1 stage later, by putting this into "++ 2w0" as "++ 2w1"
        }
        ...
        Hash<bit<32>>(HashAlgorithm_t.IDENTITY) copy32;
        ...
        apply {
            ...
            //debug, do not eliminate
            hdr.ethernet.dst_addr[31:0]=copy32.get({ig_md.table_1_read});
             
            // single stage for now!
            // however, for multiple stages:
            ...
            // send out report headers.
             if(ig_md.pkt_type==PKT_TYPE_SEQ){
                ...
                hdr.ethernet.src_addr=0x01010101;
            }else 
            if(ig_md.pkt_type==PKT_TYPE_ACK){
                if(ig_md.table_1_read==0){
                    hdr.ethernet.src_addr=0x0a0a0a0a;
                    hdr.report.payload_1=0;
                }else{
                    hdr.report.payload_1=(TIMESTAMP-ig_md.table_1_read);
                    hdr.ethernet.src_addr=0x0f0f0f0f;
                }
            }
