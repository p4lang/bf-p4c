...
header pktgen_generic_h {
    ...
    bit<16> batch_id; // Overloaded to port# or lsbs of key
    bit<16> packet_id;
}

header recirc_h {
    bit<4> tag; // Must be 0xF
    bit<4> rtype; // 0: TDB, 1: pktgen port down, 2:pktgen recirc, 3:TDB, ...
    bit<8> pad;
    bit<16> key;
}
...
// FIXME the size should be 262144
...
// FIXME the size should be 131072 
register<bit<1>, bit<17>>(131071) lag_reg;
register<bit<1>, bit<29>>(131072) next_hop_ecmp_reg;

//-----------------------------------------------------------------------------
// Common ingress/egress parser
//-----------------------------------------------------------------------------
parser PacketParser(
        packet_in pkt, inout headers_t hdr, inout user_metadata_t md) {  
    state start {
        transition  select(pkt.lookahead<bit<8>>()) {
            0x01 &&& 0xE7: parse_pktgen_port_down; // Pipe x, App 1
            0x02 &&& 0xE7: parse_pktgen_recirc;  // Pipe x, App 2
            0x03 &&& 0xE7: parse_pktgen_hw_clr;  // Pipex, App 3
    ...
    state parse_recirc_pkt {
        ...
        transition select(hdr.recirc_hdr.rtype) {
            RECIRC_TYPE_PG_PORT_DOWN : parse_pktgen_port_down;
            default : accept;
        }
    }
...
control BloomFilter(inout headers_t hdr,
    ...
    apply {
        bit<18> index_1 = bf_hash_1.get_hash({
            hdr.ipv4.proto,
            hdr.ipv4.src_addr,
            hdr.ipv4.dst_addr,
            hdr.tcp.src_port,
        ...
        bit<18> index_2 = bf_hash_2.get_hash({
            hdr.ipv4.proto,
            hdr.ipv4.src_addr,
            hdr.ipv4.dst_addr,
            hdr.tcp.src_port,
        ...
        bit<18> index_3 = bf_hash_3.get_hash({
            hdr.ipv4.proto,
            hdr.ipv4.src_addr,
            hdr.ipv4.dst_addr,
            hdr.tcp.src_port,
...
control ClearBloomFilter(inout headers_t hdr,
                         inout user_metadata_t md) {
...
// ----------------------------------------------------------------------------
control SipSampler(in headers_t hdr,
                 inout user_metadata_t md,
                 inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    register<bit<32>, bit<18>>(143360, 1) sampling_cntr; // Fills 35 + 1 spare RAMs
...
control IpRoute(in headers_t hdr, inout user_metadata_t md) {
    ...
    // Next Hop ECMP Table
    action_selector<bit<29>>(4096,
                             selector_mode_t.FAIR,
                             0,
                             0,
...
control EcmpFailover(inout headers_t hdr,
    ...
    table make_key_ecmp_fast_update {
        key = {
            hdr.pktgen_recirc.key : exact; //TODO : add mask
...
//    on a stateful ALU.
// ----------------------------------------------------------------------------
control Nexthop(inout headers_t hdr,
                inout user_metadata_t md,
                inout ingress_intrinsic_metadata_for_tm_t ig_intr_md_for_tm) {
    // FIXME the size should be 4096
...
control IfindexCounter(
    ...
    action drop_it() {
        run_ifid_cntr();
        ig_intr_md_for_tm.drop_ctl = 0x7; //TODO add enum for drop_ctr in tofino.p4 ??
    }

    table ifindex {
        key = { md.ifindex : exact; }
        actions = {
...
control PgenPass1(inout headers_t hdr,
    ...
    apply {
        ...
        } else if (hdr.pktgen_recirc.isValid()) {
