...
control PLT_bloom_filter_way_ctrl(in bit<32> index, in Metadata metadata, out bit<32> result) {
    StatefulALU<StateType>(32w262144, 32w0, 32w0, 32w0, 32w0, 32w0) PLT_bloom_filter_way = {
    ...
    apply {
        result = PLT_bloom_filter_way.execute_instr(op_num.INSTRUCTION_1, index, metadata.latency_digest, 32w0);
    }
}
