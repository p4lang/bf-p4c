#include <core.p4>
#include <v1model.p4>

struct meta_t {
    bit<32> a;
    bit<8>  b;
    bit<5>  c;
    bit<3>  d;
}

header one_byte_t {
    bit<8> a;
}

header zero_byte_t {
    bit<8> a;
}

struct metadata {
    @name("meta") 
    meta_t meta;
}

struct headers {
    @name("one") 
    one_byte_t  one;
    @name("zero") 
    zero_byte_t zero;
}

parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name("oneb") state oneb {
        packet.extract(hdr.one);
        meta.meta.b = (bit<8>)hdr.one.a;
        meta.meta.a = (bit<32>)32w0x7;
        meta.meta.c = (bit<5>)5w1;
        meta.meta.d = (bit<3>)3w2;
        transition accept;
    }
    @name("start") state start {
        transition zerob;
    }
    @name("twob") state twob {
        packet.extract(hdr.one);
        meta.meta.c = (bit<5>)5w2;
        transition accept;
    }
    @name("zerob") state zerob {
        packet.extract(hdr.zero);
        transition select(hdr.zero.a) {
            8w0: oneb;
            default: twob;
        }
    }
}

control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".do_nothing") action do_nothing() {
    }
    @name(".action_0") action action_0(bit<32> p) {
        hdr.one.a = (bit<8>)(hdr.one.a + 8w1);
        meta.meta.a = (bit<32>)(meta.meta.a + p);
    }
    @name("table_i0") table table_i0 {
        actions = {
            do_nothing;
            action_0;
        }
        key = {
            hdr.one.a  : ternary;
            meta.meta.a: exact;
            meta.meta.b: exact;
            meta.meta.c: exact;
            meta.meta.d: exact;
        }
        size = 512;
    }
    apply {
        table_i0.apply();
    }
}

control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.zero);
        packet.emit(hdr.one);
    }
}

control verifyChecksum(in headers hdr, inout metadata meta) {
    apply {
    }
}

control computeChecksum(inout headers hdr, inout metadata meta) {
    apply {
    }
}

V1Switch(ParserImpl(), verifyChecksum(), ingress(), egress(), computeChecksum(), DeparserImpl()) main;
