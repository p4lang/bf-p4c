...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".reg") register<bit<1>>(32w1000) reg_1;
    @name("sful1") register_action<bit<1>, bit<1>>(reg_1) sful1 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = value;
            value = 1w1;
        }
    };
    @name("sful2") register_action<bit<1>, bit<1>>(reg_1) sful2 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = value;
            value = 1w0;
        }
    };
    @name("sful3") register_action<bit<1>, bit<1>>(reg_1) sful3 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = value;
        }
    };
    @name("sful4") register_action<bit<1>, bit<1>>(reg_1) sful4 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = ~value;
        }
    };
