...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".run_bloom_filter_1") action run_bloom_filter_1() {
        meta.meta.is_not_member = bloom_filter_alu_1.execute((bit<32>)meta.meta.hash_1);
    }
    @name(".run_bloom_filter_2") action run_bloom_filter_2() {
        meta.meta.is_not_member = bloom_filter_alu_2.execute();
    }
    @name(".run_bloom_filter_3") action run_bloom_filter_3() {
        meta.meta.is_not_member = bloom_filter_alu_3.execute();
    }
    ...
    @name(".set_hash_1") action set_hash_1() {
        hash<bit<18>, bit<18>, tuple<bit<32>, bit<32>, bit<16>>, bit<36>>(meta.meta.hash_1, HashAlgorithm.random, 18w0, { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.srcPort }, 36w262144);
    }
    @name(".set_hash_2") action set_hash_2() {
        hash<bit<18>, bit<18>, tuple<bit<32>, bit<32>, bit<16>>, bit<36>>(meta.meta.hash_2, HashAlgorithm.crc32, 18w0, { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.srcPort }, 36w262144);
    }
