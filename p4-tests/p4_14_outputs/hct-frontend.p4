...
value, out bit<32> rv) {
            ...
            if (in_value_11.hi < 32w143359) 
                value.hi = in_value_11.hi + 32w1;
            if (in_value_11.hi >= 32w143359) 
                value.hi = 32w0;
            if (in_value_11.lo < 32w1576960 && in_value_11.hi < 32w143359) 
                value.lo = in_value_11.lo + 32w1;
            if (in_value_11.hi >= 32w143359) 
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".ingress_port_meter") action ingress_port_meter_3(bit<32> meter_idx) {
        tmp_12 = hdr.ig_intr_md_for_tm.drop_ctl;
        execute_meter_with_color<meter, bit<32>, bit<3>>(ingress_port_meter_0, meter_idx, tmp_11, tmp_12);
