...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Trevorton") state Trevorton {
        packet.extract<Macksburg_0>(hdr.Wenden);
...
control Oakford(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Delcambre") action Delcambre() {
        {
            bit<18> temp;
            hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(temp, HashAlgorithm.identity, 18w0, { meta.Weinert.Raceland, hdr.Dougherty[0].Yakima }, 19w262144);
    ...
    @name(".Surrency") action Surrency() {
        {
            bit<18> temp_0;
            hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(temp_0, HashAlgorithm.identity, 18w0, { meta.Weinert.Raceland, hdr.Dougherty[0].Yakima }, 19w262144);
...
control Tunis(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Grampian") table Grampian {
        ...
        size = 1;
        default_action = Seattle(1w1, 1w1);
    }
    apply {
        Grampian.apply();
        Cowley.apply();
...
control Woolsey(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Pilger") action Pilger() {
        RioLinda.execute_meter<bit<2>>((bit<32>)meta.Redmon.Whitlash, hdr.ig_intr_md_for_tm.packet_color);
    }
...
control computeChecksum(inout headers hdr, inout metadata meta) {
    ...
    Checksum16() Benonine;
    apply {
        hdr.Longford.McGovern = Menifee.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Longford.Lovett, hdr.Longford.Slagle, hdr.Longford.Titonka, hdr.Longford.Wyandanch, hdr.Longford.Warden, hdr.Longford.Tulia, hdr.Longford.Macon, hdr.Longford.Salamonia, hdr.Longford.Lindsborg, hdr.Longford.Brashear, hdr.Longford.Sedan, hdr.Longford.Kinross });
        hdr.Uniopolis.McGovern = Benonine.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Uniopolis.Lovett, hdr.Uniopolis.Slagle, hdr.Uniopolis.Titonka, hdr.Uniopolis.Wyandanch, hdr.Uniopolis.Warden, hdr.Uniopolis.Tulia, hdr.Uniopolis.Macon, hdr.Uniopolis.Salamonia, hdr.Uniopolis.Lindsborg, hdr.Uniopolis.Brashear, hdr.Uniopolis.Sedan, hdr.Uniopolis.Kinross });
    }
}
