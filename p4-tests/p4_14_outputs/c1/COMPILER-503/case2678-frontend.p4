...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Gerlach") RegisterAction<bit<1>, bit<32>, bit<1>>(Ilwaco) _Gerlach = {
        void apply(inout bit<1> value, out bit<1> rv) {
            bit<1> _Devore_in_value;
            _Devore_in_value = value;
            value = _Devore_in_value;
            rv = value;
        }
    };
    ...
    @name(".Leeville") action _Leeville_0() {
        hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(_Devore_temp, HashAlgorithm.identity, 18w0, { meta.Lowemont.Rocky, hdr.Melrude[0].Chamois }, 19w262144);
        _Devore_tmp = _Vinita.execute((bit<32>)_Devore_temp);
        meta.Stout.Verndale = _Devore_tmp;
    }
    @name(".Kaweah") action _Kaweah_0() {
        hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(_Devore_temp_0, HashAlgorithm.identity, 18w0, { meta.Lowemont.Rocky, hdr.Melrude[0].Chamois }, 19w262144);
