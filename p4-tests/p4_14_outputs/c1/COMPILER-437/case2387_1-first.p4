...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Bangor") state Bangor {
        packet.extract<BelAir>(hdr.Kahaluu);
...
control Macksburg(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @command_line("--no-dead-code-elimination") @name(".Goulding") table Goulding {
        actions = {
            Olathe();
            @defaultonly NoAction();
        }
...
control Victoria(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    stateful_alu() Kekoskee;
    stateful_alu() Woodsboro;
    @name(".Pacifica") action Pacifica() {
        Woodsboro.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Tarlton.Atoka, hdr.Maryhill[0].Ladner });
    }
    ...
    @name(".Kerrville") action Kerrville() {
        Kekoskee.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Tarlton.Atoka, hdr.Maryhill[0].Ladner });
    }
