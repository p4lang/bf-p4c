...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Lenwood") RegisterAction<bit<1>, bit<32>, bit<1>>(Godley) _Lenwood = {
        void apply(inout bit<1> value, out bit<1> rv) {
            ...
            value = _Gabbs_in_value;
            rv = ~_Gabbs_in_value;
        }
    };
    @name(".Craig") action _Craig_0() {
        hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(_Gabbs_temp, HashAlgorithm.identity, 19w0, { hdr.eg_intr_md.egress_port, meta.Upalco.Kelvin }, 20w524288);
    ...
    @name(".Giltner") RegisterAction<bit<1>, bit<32>, bit<1>>(Pearce) _Giltner = {
        void apply(inout bit<1> value, out bit<1> rv) {
            ...
            value = _Nevis_in_value;
            rv = ~_Nevis_in_value;
        }
    };
    @name(".Boysen") action _Boysen_0() {
        hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(_Nevis_temp, HashAlgorithm.identity, 19w0, { hdr.eg_intr_md.egress_port, meta.Upalco.Kelvin }, 20w524288);
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Kenmore") RegisterAction<bit<1>, bit<32>, bit<1>>(Norma) _Kenmore_0 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            ...
            value = _Counce__Chatcolet_in_value;
            rv = ~_Counce__Chatcolet_in_value;
        }
    };
    @name(".Spivey") RegisterAction<bit<1>, bit<32>, bit<1>>(Lesley) _Spivey_0 = {
        void apply(inout bit<1> value, out bit<1> rv) {
            bit<1> _Counce__Chatcolet_in_value_0;
            _Counce__Chatcolet_in_value_0 = value;
            value = _Counce__Chatcolet_in_value_0;
            rv = value;
        }
    };
    @name(".Joshua") action _Joshua() {
        hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(_Counce__Chatcolet_temp, HashAlgorithm.identity, 19w0, { hdr.ig_intr_md.ingress_port, hdr.Panola[0].Harshaw }, 20w524288);
    ...
    @name(".Bruce") action _Bruce() {
        hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(_Counce__Chatcolet_temp_0, HashAlgorithm.identity, 19w0, { hdr.ig_intr_md.ingress_port, hdr.Panola[0].Harshaw }, 20w524288);
    ...
    @name(".GlenDean") action _GlenDean_4(bit<32> Drake) {
        if (meta.Gresston.Ovett >= Drake) 
            _Visalia_tmp = meta.Gresston.Ovett;
        else 
            _Visalia_tmp = Drake;
    ...
    @name(".GlenDean") action _GlenDean_5(bit<32> Drake) {
        if (meta.Gresston.Ovett >= Drake) 
            _Stidham_tmp = meta.Gresston.Ovett;
        else 
            _Stidham_tmp = Drake;
    ...
    @name(".GlenDean") action _GlenDean_6(bit<32> Drake) {
        if (meta.Gresston.Ovett >= Drake) 
            _Minto_tmp = meta.Gresston.Ovett;
        else 
            _Minto_tmp = Drake;
    ...
    @name(".GlenDean") action _GlenDean_7(bit<32> Drake) {
        if (meta.Gresston.Ovett >= Drake) 
            _Fairborn_tmp = meta.Gresston.Ovett;
        else 
            _Fairborn_tmp = Drake;
    ...
    @name(".GlenDean") action _GlenDean_8(bit<32> Drake) {
        if (meta.Gresston.Ovett >= Drake) 
            _Elsmere_tmp = meta.Gresston.Ovett;
        else 
            _Elsmere_tmp = Drake;
