...
control Bagwell(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    stateful_alu() Goodwin;
    stateful_alu() Hulbert;
    ...
    @name(".Woodland") action Woodland() {
        Goodwin.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Harding.Honokahua, hdr.Casper[0].Mariemont });
    }
    @name(".Glazier") action Glazier() {
        Hulbert.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Harding.Honokahua, hdr.Casper[0].Mariemont });
    }
