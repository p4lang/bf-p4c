...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Tahuya.Goldsmith") stateful_alu() _Tahuya_Goldsmith_0;
    @name(".Tahuya.Mizpah") stateful_alu() _Tahuya_Mizpah_0;
    @name(".Eaton") action _Eaton() {
        meta.Perrin.Denhoff = hdr.Lamison[0].Floris;
        meta.Perrin.Willard = 1w1;
    }
    @name(".Craigmont") action _Craigmont() {
        _Tahuya_Goldsmith_0.execute_stateful_alu_from_hash<tuple_0>({ meta.Riverwood.Renick, hdr.Lamison[0].Floris });
    }
    @name(".Osakis") action _Osakis(bit<1> Naubinway) {
        meta.Wanatah.Lepanto = Naubinway;
    }
    @name(".Baytown") action _Baytown() {
        _Tahuya_Mizpah_0.execute_stateful_alu_from_hash<tuple_0>({ meta.Riverwood.Renick, hdr.Lamison[0].Floris });
    }
...
control verifyChecksum(in headers hdr, inout metadata meta) {
    ...
    apply {
        tmp_7 = Burnett.get<tuple_5>({ hdr.Halfa.Broussard, hdr.Halfa.Miller, hdr.Halfa.Geeville, hdr.Halfa.Belgrade, hdr.Halfa.Sandoval, hdr.Halfa.Cutler, hdr.Halfa.Biscay, hdr.Halfa.Windber, hdr.Halfa.DesPeres, hdr.Halfa.Chatom, hdr.Halfa.Niota, hdr.Halfa.Stonebank });
        if (hdr.Halfa.Laketown == tmp_7) 
            mark_to_drop();
