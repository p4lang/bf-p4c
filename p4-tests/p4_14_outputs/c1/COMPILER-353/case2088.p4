...
control Parnell(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Ledger") action Ledger() {
        Tillatoba.execute_stateful_alu_from_hash({ meta.Wolsey.Perryton, hdr.Moodys[0].Salome });
    }
    ...
    @name(".Quinault") action Quinault() {
        Gonzalez.execute_stateful_alu_from_hash({ meta.Wolsey.Perryton, hdr.Moodys[0].Salome });
    }
