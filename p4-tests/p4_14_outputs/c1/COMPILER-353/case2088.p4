...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Alcester") state Alcester {
        packet.extract(hdr.Arvonia);
        transition Attica;
    }
    @name(".Attica") state Attica {
        packet.extract(hdr.Willette);
    ...
    @name(".Epsie") state Epsie {
        packet.extract(hdr.Pengilly);
        transition accept;
    }
    @name(".Gunder") state Gunder {
        packet.extract(hdr.Kearns);
    ...
    @name(".Pathfork") state Pathfork {
        packet.extract(hdr.Pierson);
        transition Alcester;
    }
    @name(".Shipman") state Shipman {
        packet.extract(hdr.Donner);
        meta.Goodlett.Lisle = 2w1;
        transition Verdery;
    }
    @name(".Triplett") state Triplett {
        packet.extract(hdr.Reynolds);
...
control Brookwood(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @immediate(0) @name(".Paradise") table Paradise {
        ...
        key = {
            hdr.Suring.isValid()   : ternary;
            hdr.Wimberley.isValid(): ternary;
            hdr.Batchelor.isValid(): ternary;
            hdr.Kearns.isValid()   : ternary;
        }
        size = 6;
    }
    @action_default_only("DosPalos") @immediate(0) @name(".Pardee") table Pardee {
        ...
        key = {
            hdr.Suring.isValid()   : ternary;
            hdr.Wimberley.isValid(): ternary;
            hdr.Weskan.isValid()   : ternary;
            hdr.Neubert.isValid()  : ternary;
            hdr.Wailuku.isValid()  : ternary;
            hdr.Batchelor.isValid(): ternary;
            hdr.Kearns.isValid()   : ternary;
            hdr.Valmont.isValid()  : ternary;
            hdr.Shelbina.isValid() : ternary;
            hdr.Willette.isValid() : ternary;
        }
        size = 256;
    }
    apply {
...
control Filley(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @ways(1) @name(".Glenside") table Glenside {
        ...
        key = {
            meta.Roseau.Rippon: exact;
            meta.Roseau.Surrey: exact;
        }
        size = 1;
...
control Floral(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Gamewell") table Gamewell {
        ...
        key = {
            meta.Goodlett.Paradis  : exact;
            meta.Goodlett.Blitchton: exact;
            meta.Goodlett.DewyRose : exact;
            meta.Goodlett.Blossom  : exact;
        }
        size = 65536;
    }
    @name(".Halley") table Halley {
        ...
        key = {
            meta.Goodlett.Deferiet: ternary;
            meta.Goodlett.Huxley  : exact;
            meta.Goodlett.Gomez   : exact;
        }
        size = 512;
    }
    ...
    @action_default_only("DosPalos") @name(".Selby") table Selby {
        ...
        key = {
            meta.Wolsey.Perryton   : exact;
            meta.Rixford.Houston   : ternary;
            meta.Rixford.TonkaBay  : ternary;
            meta.Goodlett.Albemarle: ternary;
            meta.Goodlett.Parkway  : ternary;
...
control Gibbstown(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Longdale") action Longdale() {
        hash(meta.Cisco.Ferrum, HashAlgorithm.crc32, (bit<32>)0, { hdr.Shelbina.Davisboro, hdr.Shelbina.Clarkdale, hdr.Shelbina.Stonefort, hdr.Shelbina.Sawpit }, (bit<64>)4294967296);
    }
    @name(".Lordstown") action Lordstown() {
        hash(meta.Cisco.Ferrum, HashAlgorithm.crc32, (bit<32>)0, { hdr.Valmont.Emmalane, hdr.Valmont.Shirley, hdr.Valmont.LeeCity }, (bit<64>)4294967296);
    }
...
control Johnsburg(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Ottertail") action Ottertail() {
        digest<Blakeslee>((bit<32>)0, { meta.Denby.Angwin, meta.Goodlett.DewyRose, hdr.Wailuku.Paoli, hdr.Wailuku.Shelbiana, hdr.Valmont.Shirley });
    }
...
control LeSueur(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Sebewaing") table Sebewaing {
        ...
        key = {
            meta.Ellicott.Alamosa: exact;
            meta.Wolsey.Farthing : ternary;
            meta.Roseau.Oakville : ternary;
        }
        size = 512;
    }
    apply {
...
control Malesus(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Craigmont") action Craigmont() {
        hash(meta.Cisco.Abilene, HashAlgorithm.crc32, (bit<32>)0, { hdr.Willette.Berwyn, hdr.Willette.Toulon, hdr.Willette.Paoli, hdr.Willette.Shelbiana, hdr.Willette.Hannibal }, (bit<64>)4294967296);
    }
...
control Nixon(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Faysville") table Faysville {
        ...
        key = {
            hdr.Willette.Paoli    : ternary;
            hdr.Willette.Shelbiana: ternary;
        }
        size = 512;
    }
...
control Northway(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Gamaliel") action Gamaliel() {
        hash(meta.Cisco.Duster, HashAlgorithm.crc32, (bit<32>)0, { hdr.Valmont.Shirley, hdr.Valmont.LeeCity, hdr.Kearns.Heizer, hdr.Kearns.Frederika }, (bit<64>)4294967296);
    }
...
control Parnell(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Gonzalez") register_action<bit<1>, bit<1>>(BigWells) Gonzalez = {
    ...
    @name(".Tillatoba") register_action<bit<1>, bit<1>>(Skillman) Tillatoba = {
        void apply(inout bit<1> value, out bit<1> rv) {
            bit<1> in_value;
            in_value = value;
            rv = 1w0;
    ...
    @name(".Ledger") action Ledger() {
        {
            bit<18> temp;
            hash(temp, HashAlgorithm.identity, 18w0, { meta.Wolsey.Perryton, hdr.Moodys[0].Salome }, 19w262144);
    ...
    @name(".Quinault") action Quinault() {
        {
            bit<18> temp_0;
            hash(temp_0, HashAlgorithm.identity, 18w0, { meta.Wolsey.Perryton, hdr.Moodys[0].Salome }, 19w262144);
...
control Pineridge(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".CityView") table CityView {
        ...
        key = {
            meta.Roseau.Pevely    : exact;
            meta.Roseau.Nightmute : exact;
            meta.Roseau.Orrstown  : exact;
            hdr.Valmont.isValid() : ternary;
            hdr.Shelbina.isValid(): ternary;
        }
        size = 512;
...
control Rankin(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Othello") table Othello {
        ...
        key = {
            meta.Wolsey.Buras      : ternary;
            hdr.Moodys[0].isValid(): exact;
            hdr.Moodys[0].Salome   : ternary;
        }
        size = 4096;
    }
    @name(".Pembine") table Pembine {
        ...
        key = {
            hdr.Willette.Berwyn: exact;
            hdr.Willette.Toulon: exact;
            hdr.Valmont.LeeCity: exact;
            meta.Goodlett.Lisle: exact;
        }
        size = 1024;
...
control Ribera(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Greenbush") action Greenbush(bit<32> Cullen) {
        Jayton.execute_meter((bit<32>)Cullen, meta.Tenino.Higganum);
    }
...
control Sabetha(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Chappell") table Chappell {
        ...
        key = {
            meta.Goodlett.Eldora : exact;
            meta.Goodlett.Everton: exact;
        }
        size = 3;
    }
...
control Trooper(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Kaeleku") action Kaeleku() {
        digest<Palco>((bit<32>)0, { meta.Denby.Angwin, meta.Goodlett.Paradis, meta.Goodlett.Blitchton, meta.Goodlett.DewyRose, meta.Goodlett.Blossom });
    }
...
control WolfTrap(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @stage(10) @name(".Pittsboro") table Pittsboro {
        ...
        key = {
            meta.Wolsey.Forkville  : exact;
            meta.Wolsey.Chevak     : ternary;
            meta.Goodlett.Punaluu  : ternary;
            meta.Goodlett.Grapevine: ternary;
        }
        size = 80;
    }
    apply {
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.Pierson);
        packet.emit(hdr.Arvonia);
        packet.emit(hdr.Willette);
        packet.emit(hdr.Moodys[0]);
        packet.emit(hdr.Pengilly);
        packet.emit(hdr.Shelbina);
        packet.emit(hdr.Valmont);
        packet.emit(hdr.Kearns);
        packet.emit(hdr.Donner);
        packet.emit(hdr.Wailuku);
        packet.emit(hdr.Neubert);
        packet.emit(hdr.Weskan);
    }
}
