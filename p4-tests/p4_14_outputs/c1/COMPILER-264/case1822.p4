...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Cowen") state Cowen {
        packet.extract(hdr.Ashley);
        transition accept;
    }
    @name(".Despard") state Despard {
        packet.extract(hdr.Markville);
    ...
    @name(".Wolcott") state Wolcott {
        packet.extract(hdr.Lampasas);
        transition accept;
    }
    @name(".Yocemento") state Yocemento {
        packet.extract(hdr.McQueen);
...
control Barnwell(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Tunis") action Tunis() {
        digest<Tahlequah>((bit<32>)0, { meta.Cascadia.Licking, meta.SomesBar.Cisco, meta.SomesBar.Tilghman, meta.SomesBar.Brunson, meta.SomesBar.Sonoita });
    }
...
control Brodnax(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".LaJoya") table LaJoya {
        ...
        key = {
            meta.Larue.Goodlett: exact;
            meta.Anthon.Santos : selector;
        }
        size = 1024;
...
control Caban(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Bessie") table Bessie {
        ...
        key = {
            meta.Moylan.Capitola    : exact;
            hdr.Tarlton[0].McGonigle: exact;
        }
        size = 1024;
    ...
    @name(".Eastover") table Eastover {
        ...
        key = {
            hdr.Charco.Lakebay   : exact;
            hdr.Charco.GunnCity  : exact;
            hdr.McQueen.Taopi    : exact;
            meta.SomesBar.Kountze: exact;
        }
        size = 1024;
    ...
    @name(".Wardville") table Wardville {
        ...
        key = {
            meta.Moylan.Capitola    : ternary;
            hdr.Tarlton[0].isValid(): exact;
            hdr.Tarlton[0].McGonigle: ternary;
        }
        size = 4096;
    }
...
control DuQuoin(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @idletime_precision(1) @name(".Barnsboro") table Barnsboro {
        ...
        key = {
            meta.ElkPoint.Brewerton: exact;
            meta.Weskan.Nashua     : exact;
        }
        size = 65536;
        default_action = Dushore();
    }
    ...
    @atcam_partition_index("Bonduel.LaMarque") @atcam_number_partitions(16384) @name(".Rapids") table Rapids {
        ...
        key = {
            meta.Bonduel.LaMarque   : exact;
            meta.Bonduel.Jones[19:0]: lpm;
        }
        size = 131072;
        default_action = Dushore();
    }
...
control Halaula(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    register_action<bit<1>, bit<1>>(Onamia) Fajardo = {
    ...
    register_action<bit<1>, bit<1>>(Mahopac) Gastonia = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = 1w0;
            value = value;
            rv = value;
    ...
    @name(".Wyatte") action Wyatte() {
        {
            bit<18> temp;
            hash(temp, HashAlgorithm.identity, 18w0, { meta.Moylan.Monahans, hdr.Tarlton[0].McGonigle }, 19w262144);
    ...
    @name(".Renville") action Renville() {
        {
            bit<18> temp_0;
            hash(temp_0, HashAlgorithm.identity, 18w0, { meta.Moylan.Monahans, hdr.Tarlton[0].McGonigle }, 19w262144);
...
control Hanapepe(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Callimont") table Callimont {
        ...
        key = {
            meta.Larue.Mentone    : exact;
            meta.Larue.Malaga     : exact;
            meta.Larue.Greenbush  : exact;
            hdr.McQueen.isValid() : ternary;
            hdr.Lampasas.isValid(): ternary;
            meta.Larue.Coamo[0:0] : ternary;
        }
        size = 512;
    }
...
control Mabelvale(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Cypress") action Cypress() {
        ...
        hdr.ig_intr_md_for_tm.mcast_grp_a = (bit<16>)meta.Larue.Rosalie + 16w0;
    }
    ...
    @ways(1) @name(".Hartfield") table Hartfield {
        ...
        key = {
            meta.Larue.Stillmore: exact;
            meta.Larue.Mayview  : exact;
        }
        size = 1;
...
control Oakes(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Olmstead") action Olmstead() {
        hash(meta.Anthon.Santos, HashAlgorithm.crc32, (bit<32>)0, { hdr.Charco.Lakebay, hdr.Charco.GunnCity, hdr.Charco.Amity, hdr.Charco.Silica, hdr.Charco.Browning }, (bit<64>)65536);
    }
    @pa_solitary("ingress", "SomesBar.Brunson") @pa_solitary("ingress", "SomesBar.Sonoita") @pa_solitary("ingress", "SomesBar.Kingman") @pa_solitary("ig_intr_md_for_tm.ucast_egress_port") @pa_solitary("ingress", "ig_intr_md_for_tm.mcast_grp_a") @name(".Dushore") action Dushore() {
        ;
    }
    @immediate(0) @name(".Baranof") table Baranof {
        ...
        key = {
            hdr.Ganado.isValid()   : ternary;
            hdr.Bogota.isValid()   : ternary;
            hdr.Ashley.isValid()   : ternary;
            hdr.Nuremberg.isValid(): ternary;
            hdr.Counce.isValid()   : ternary;
            hdr.Sagamore.isValid() : ternary;
            hdr.Lenox.isValid()    : ternary;
            hdr.McQueen.isValid()  : ternary;
            hdr.Lampasas.isValid() : ternary;
            hdr.Charco.isValid()   : ternary;
        }
        size = 256;
        default_action = Dushore();
    }
...
control Parshall(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    register_action<bit<1>, bit<1>>(Ashville) Challenge = {
        void apply(inout bit<1> value, out bit<1> rv) {
            rv = 1w0;
            value = 1w1;
        }
    ...
    @name(".Elihu") table Elihu {
        ...
        key = {
            meta.Moylan.Monahans  : exact;
            meta.Myrick.IowaCity  : ternary;
            meta.Myrick.Globe     : ternary;
            meta.SomesBar.Ranchito: ternary;
            meta.SomesBar.Skene   : ternary;
    ...
    @name(".Hammocks") table Hammocks {
        ...
        key = {
            meta.SomesBar.Cisco   : exact;
            meta.SomesBar.Tilghman: exact;
            meta.SomesBar.Brunson : exact;
            meta.SomesBar.Sonoita : exact;
        }
        size = 65536;
    }
    @name(".Risco") table Risco {
        ...
        key = {
            meta.SomesBar.Kingman  : ternary;
            meta.SomesBar.Alvwood  : exact;
            meta.SomesBar.Scottdale: exact;
        }
        size = 512;
    }
    apply {
...
control Sudden(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Paxico") table Paxico {
        ...
        key = {
            hdr.Charco.Amity : ternary;
            hdr.Charco.Silica: ternary;
        }
        size = 512;
    }
    @name(".Talmo") table Talmo {
        ...
        key = {
            hdr.Charco.Lakebay : ternary;
            hdr.Charco.GunnCity: ternary;
        }
        size = 512;
...
control Volcano(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Powelton") action Powelton() {
        digest<Lamison>((bit<32>)0, { meta.Cascadia.Licking, meta.SomesBar.Brunson, hdr.Counce.Amity, hdr.Counce.Silica, hdr.McQueen.Dowell });
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.Charco);
        packet.emit(hdr.Tarlton[0]);
        packet.emit(hdr.Stone);
        packet.emit(hdr.Lampasas);
        packet.emit(hdr.McQueen);
        packet.emit(hdr.Lenox);
        packet.emit(hdr.Markville);
        packet.emit(hdr.Counce);
        packet.emit(hdr.Nuremberg);
        packet.emit(hdr.Ashley);
    }
}
