...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @atcam_partition_index("h1.f1") @atcam_number_partitions(16384) @name(".t") table t {
        ...
        key = {
            ...
            hdr.ethHdr.dmac[27:0]: lpm @name("hdr.ethHdr.dmac[27:0]") ;
        }
        size = 131072;
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit<EthernetHdr_0>(hdr.ethHdr);
        packet.emit<VlanHdr_0>(hdr.vlanHdr);
        packet.emit<Ipv4Hdr_0>(hdr.ipv4Hdr);
    }
}
