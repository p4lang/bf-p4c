...
control Hiawassee(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Riverlea") action Riverlea() {
        Noyack.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Harding.Honokahua, hdr.Casper[0].Mariemont });
    }
    @name(".Cedonia") action Cedonia() {
        Kasilof.execute_stateful_alu_from_hash<tuple<bit<6>, bit<12>>>({ meta.Harding.Honokahua, hdr.Casper[0].Mariemont });
    }
