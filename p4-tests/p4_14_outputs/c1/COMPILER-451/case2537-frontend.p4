...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Kasilof") RegisterAction<bit<1>, bit<32>, bit<1>>(Hobucken) _Kasilof = {
        void apply(inout bit<1> value, out bit<1> rv) {
            bit<1> _Hiawassee_in_value;
            _Hiawassee_in_value = value;
            value = _Hiawassee_in_value;
            rv = value;
        }
    };
    ...
    @name(".Riverlea") action _Riverlea_0() {
        hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(_Hiawassee_temp, HashAlgorithm.identity, 18w0, { meta.Harding.Honokahua, hdr.Casper[0].Mariemont }, 19w262144);
        _Hiawassee_tmp = _Noyack.execute((bit<32>)_Hiawassee_temp);
        meta.Nondalton.Telma = _Hiawassee_tmp;
    }
    @name(".Cedonia") action _Cedonia_0() {
        hash<bit<18>, bit<18>, tuple<bit<6>, bit<12>>, bit<19>>(_Hiawassee_temp_0, HashAlgorithm.identity, 18w0, { meta.Harding.Honokahua, hdr.Casper[0].Mariemont }, 19w262144);
