...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".GCpcTj") state GCpcTj {
        packet.extract<WpkNOl>(hdr.OGAwff);
    ...
    @name(".HwAhMc") state HwAhMc {
        packet.extract<xZGoIJ>(hdr.KkUuiN);
        transition accept;
    }
    @name(".IEpHDt") state IEpHDt {
        packet.extract<LvNiTF>(hdr.mJKpnm);
    ...
    @name(".WEpwHS") state WEpwHS {
        packet.extract<cbNiss>(hdr.ReMSVq);
    ...
    @name(".ZBfdXg") state ZBfdXg {
        packet.extract<teszwH>(hdr.XKlCuY);
    ...
    @name(".ihhcZZ") state ihhcZZ {
        packet.extract<ChMpQB_0>(hdr.jxruBM);
        transition accept;
    }
    @name(".kERnnN") state kERnnN {
        packet.extract<UJNFaQ>(hdr.RCeMTB);
    ...
    @name(".sNXzVW") state sNXzVW {
        packet.extract<WpkNOl>(hdr.vQBhIB);
        transition accept;
    }
...
control ingressProcessing(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".tNoIui") action tNoIui_3(bit<8> tDVDRX, bit<1> CzrSAo, bit<1> jKuAKM, bit<1> zvrvfA, bit<1> XKAzdx) {
        meta.zOxPzX.EzUaHZ = tDVDRX;
        meta.zOxPzX.uGjozA = CzrSAo;
        meta.zOxPzX.iNqMBP = jKuAKM;
        meta.zOxPzX.uaMZSu = zvrvfA;
    ...
    @name(".tNoIui") action tNoIui_4(bit<8> tDVDRX, bit<1> CzrSAo, bit<1> jKuAKM, bit<1> zvrvfA, bit<1> XKAzdx) {
        meta.zOxPzX.EzUaHZ = tDVDRX;
        meta.zOxPzX.uGjozA = CzrSAo;
        meta.zOxPzX.iNqMBP = jKuAKM;
        meta.zOxPzX.uaMZSu = zvrvfA;
    ...
    @name(".EmgemB") table EmgemB {
        ...
        key = {
            meta.YxcqTZ.WpWIBS: exact @name("meta.YxcqTZ.WpWIBS") ;
            hdr.RCeMTB.bKQlQb : exact @name("hdr.RCeMTB.bKQlQb") ;
        }
        size = 1024;
    ...
    @name(".HIjCXW") table HIjCXW {
        ...
        key = {
            meta.YxcqTZ.SxlBOA: exact @name("meta.YxcqTZ.SxlBOA") ;
        }
        size = 8092;
    ...
    @name(".JrMSBe") table JrMSBe {
        ...
        key = {
            hdr.OGAwff.IBpboJ: exact @name("hdr.OGAwff.IBpboJ") ;
            hdr.OGAwff.SjPEmg: exact @name("hdr.OGAwff.SjPEmg") ;
        }
        size = 64;
    ...
    @name(".KjYLTh") table KjYLTh {
        ...
        key = {
            hdr.XKlCuY.Vkxquf: exact @name("hdr.XKlCuY.Vkxquf") ;
        }
        size = 4096;
    ...
    @name(".UnqUFr") table UnqUFr {
        ...
        key = {
            hdr.OGAwff.IBpboJ : exact @name("hdr.OGAwff.IBpboJ") ;
            hdr.OGAwff.SjPEmg : exact @name("hdr.OGAwff.SjPEmg") ;
            hdr.XKlCuY.NkPqcV : exact @name("hdr.XKlCuY.NkPqcV") ;
            meta.tHxLKe.pIQUaB: exact @name("meta.tHxLKe.pIQUaB") ;
        }
        size = 1024;
    ...
    @name(".WVyiva") table WVyiva {
        ...
        key = {
            hdr.ReMSVq.pZSyjJ: exact @name("hdr.ReMSVq.pZSyjJ") ;
        }
        size = 4096;
    ...
    @name(".XzzeTs") table XzzeTs {
        ...
        key = {
            meta.YxcqTZ.WpWIBS  : ternary @name("meta.YxcqTZ.WpWIBS") ;
            hdr.RCeMTB.isValid(): exact @name("hdr.RCeMTB.isValid()") ;
            hdr.RCeMTB.bKQlQb   : ternary @name("hdr.RCeMTB.bKQlQb") ;
        }
        size = 4096;
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit<WpkNOl>(hdr.OGAwff);
        packet.emit<UJNFaQ>(hdr.RCeMTB);
        packet.emit<ChMpQB_0>(hdr.jxruBM);
        packet.emit<xZGoIJ>(hdr.KkUuiN);
        packet.emit<teszwH>(hdr.XKlCuY);
        packet.emit<LvNiTF>(hdr.mJKpnm);
        packet.emit<cbNiss>(hdr.ReMSVq);
        packet.emit<WpkNOl>(hdr.vQBhIB);
    }
}
