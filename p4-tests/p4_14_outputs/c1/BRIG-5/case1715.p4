...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".EsYZnM") state EsYZnM {
        packet.extract(hdr.SozCdH);
        transition accept;
    }
    @name(".MeKmKm") state MeKmKm {
        packet.extract(hdr.ujAsAs);
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".IJasQc") action IJasQc() {
        digest<uajPdr>((bit<32>)0x0, { meta.IYRPFS.JiMzUK, meta.cfDyxP.kLfTIU, meta.cfDyxP.FpUAZo, meta.cfDyxP.eHdmgU, meta.cfDyxP.vAMWZi });
    }
    ...
    @name(".STOiil") action STOiil() {
        digest<HgFUwb>((bit<32>)0x0, { meta.IYRPFS.JiMzUK, meta.cfDyxP.eHdmgU, hdr.oPHKWO.KaAvFG, hdr.oPHKWO.wNAybQ, hdr.ujAsAs.tqMJMA });
    }
    ...
    @name(".BeBzRQ") table BeBzRQ {
        ...
        key = {
            hdr.QUzEun.GpVggy : exact;
            hdr.QUzEun.qTArPp : exact;
            hdr.ujAsAs.zMkMTl : exact;
            meta.cfDyxP.nZmyfl: exact;
        }
        size = 1024;
    }
    ...
    @name(".OuIfwj") table OuIfwj {
        ...
        key = {
            meta.BdarYZ.XzqkeH  : ternary;
            hdr.LkPrni.isValid(): exact;
            hdr.LkPrni.BaAnQa   : ternary;
        }
        size = 4096;
    }
    @name(".RFvNXO") table RFvNXO {
        ...
        key = {
            meta.YEKoZE.eOdtuN: exact;
            meta.YEKoZE.QeEJZF: exact;
            meta.YEKoZE.jvCpRl: exact;
        }
        size = 65536;
    }
    @name(".RSLvpr") table RSLvpr {
        ...
        key = {
            meta.snSjFB.WwpGkv: exact;
            meta.OlPBiN.gVnEFV: lpm;
        }
        size = 16384;
    }
    ...
    @ways(1) @name(".iAPDQh") table iAPDQh {
        ...
        key = {
            meta.cfDyxP.eguEQM: exact;
            meta.cfDyxP.YWkHqd: exact;
        }
        size = 1;
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.QUzEun);
        packet.emit(hdr.LkPrni);
        packet.emit(hdr.hPpXZx);
        packet.emit(hdr.SozCdH);
        packet.emit(hdr.ujAsAs);
        packet.emit(hdr.hSwWXC);
        packet.emit(hdr.qAVGBC);
        packet.emit(hdr.oPHKWO);
    }
}
