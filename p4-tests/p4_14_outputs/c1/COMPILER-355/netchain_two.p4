...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_netchain") state parse_netchain {
        packet.extract(hdr.netchain_hdr);
    ...
    @name(".parse_netchain_val") state parse_netchain_val {
        packet.extract(hdr.netchain_value);
        transition accept;
    }
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".seq_alu") register_action<bit<32>, bit<32>>(seq_register) seq_alu = {
        void apply(inout bit<32> value, out bit<32> rv) {
            rv = 32w0;
            rv = 32w1;
            if (hdr.netchain_hdr.seq > value) 
                value = hdr.netchain_hdr.seq;
        }
    };
    @name(".value_alu_1") register_action<bit<32>, bit<32>>(value_register_1) value_alu_1 = {
        void apply(inout bit<32> value, out bit<32> rv) {
            rv = 32w0;
    ...
    @name(".value_alu_2") register_action<bit<32>, bit<32>>(value_register_2) value_alu_2 = {
        void apply(inout bit<32> value, out bit<32> rv) {
            rv = 32w0;
            if (meta.md.write_valid == 1w1) 
    ...
    @name(".run_seq_alu") action run_seq_alu() {
        meta.md.write_valid = (bit<1>)seq_alu.execute((bit<32>)meta.md.seq_index);
    }
    @name(".run_value_alu_1") action run_value_alu_1() {
        hdr.netchain_value.val1 = value_alu_1.execute((bit<32>)meta.md.val_index1);
    }
    @name(".run_value_alu_2") action run_value_alu_2() {
        hdr.netchain_value.val2 = value_alu_2.execute((bit<32>)meta.md.val_index2);
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        ...
        packet.emit(hdr.udp);
        packet.emit(hdr.netchain_hdr);
        packet.emit(hdr.netchain_value);
        packet.emit(hdr.tcp);
    }
}
