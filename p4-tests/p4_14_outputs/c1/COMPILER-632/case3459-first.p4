...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".Adams") state Adams {
        packet.extract<Longville>(hdr.Hayfork);
...
control Arapahoe(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Timnath") table Timnath {
        ...
        size = 256;
        default_action = Rumson(16w0xffff, 16w0xffff, 16w0xffff, 16w0xffff, 8w0xff, 6w0x3f, 8w0xff, 8w0xff, 1w1);
    }
    apply {
        Timnath.apply();
    }
...
control Burgdorf(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Edinburgh") action Edinburgh(bit<32> Karluk) {
        Alabaster.count(Karluk);
    }
    @name(".Valentine") action Valentine(bit<32> LaneCity) {
        Northome.execute_meter<bit<2>>(LaneCity, hdr.ig_intr_md_for_tm.packet_color);
    }
...
control Chicago(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Savery") table Savery {
        ...
        size = 256;
        default_action = Swifton(16w0xffff, 16w0xffff, 16w0xffff, 16w0xffff, 8w0xff, 6w0x3f, 8w0xff, 8w0xff, 1w1);
    }
    apply {
        Savery.apply();
    }
...
control Cornville(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @use_hash_action(1) @name(".Asherton") table Asherton {
        ...
        size = 256;
        default_action = Avondale(24w0, 24w0, 12w0);
    }
...
control Grapevine(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".MoonRun") table MoonRun {
        ...
        size = 1;
        default_action = Quinault(1w0, 1w0);
    }
...
control Hercules(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @use_hash_action(1) @name(".Simnasho") table Simnasho {
        ...
        size = 65536;
        default_action = Ontonagon(12w0, 1w0, 3w1);
    }
    apply {
...
control Kelsey(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Gamewell") table Gamewell {
        ...
        size = 256;
        default_action = Baltimore(16w0xffff, 16w0xffff, 16w0xffff, 16w0xffff, 8w0xff, 6w0x3f, 8w0xff, 8w0xff, 1w1);
    }
    apply {
        Gamewell.apply();
    }
...
control Ludowici(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Larwill") action Larwill() {
        {
            bit<19> temp;
            hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(temp, HashAlgorithm.identity, 19w0, { hdr.ig_intr_md.ingress_port, hdr.Mission[0].Ozona }, 20w524288);
    ...
    @name(".Waiehu") action Waiehu() {
        {
            bit<19> temp_0;
            hash<bit<19>, bit<19>, tuple<bit<9>, bit<12>>, bit<20>>(temp_0, HashAlgorithm.identity, 19w0, { hdr.ig_intr_md.ingress_port, hdr.Mission[0].Ozona }, 20w524288);
...
control Success(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Vantage") table Vantage {
        ...
        size = 256;
        default_action = Bayne(16w0xffff, 16w0xffff, 16w0xffff, 16w0xffff, 8w0xff, 6w0x3f, 8w0xff, 8w0xff, 1w1);
    }
    apply {
        Vantage.apply();
    }
...
control Tuttle(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".Hematite") table Hematite {
        ...
        size = 256;
        default_action = Nelson(16w0xffff, 16w0xffff, 16w0xffff, 16w0xffff, 8w0xff, 6w0x3f, 8w0xff, 8w0xff, 1w1);
    }
    apply {
        Hematite.apply();
    }
...
control verifyChecksum(in headers hdr, inout metadata meta) {
    ...
    Checksum16() Weches;
    apply {
        if (hdr.Amory.Tallassee == (LaMarque.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Amory.Wyanet, hdr.Amory.Elkins, hdr.Amory.Houston, hdr.Amory.Sheldahl, hdr.Amory.Lofgreen, hdr.Amory.Kanorado, hdr.Amory.Snowflake, hdr.Amory.Tulia, hdr.Amory.Teaneck, hdr.Amory.Brush, hdr.Amory.Gresston, hdr.Amory.Worthing }))) 
            mark_to_drop();
        if (hdr.Weissert.Tallassee == (Weches.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Weissert.Wyanet, hdr.Weissert.Elkins, hdr.Weissert.Houston, hdr.Weissert.Sheldahl, hdr.Weissert.Lofgreen, hdr.Weissert.Kanorado, hdr.Weissert.Snowflake, hdr.Weissert.Tulia, hdr.Weissert.Teaneck, hdr.Weissert.Brush, hdr.Weissert.Gresston, hdr.Weissert.Worthing }))) 
            mark_to_drop();
    }
}
...
control computeChecksum(inout headers hdr, inout metadata meta) {
    ...
    Checksum16() Weches;
    apply {
        hdr.Amory.Tallassee = LaMarque.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Amory.Wyanet, hdr.Amory.Elkins, hdr.Amory.Houston, hdr.Amory.Sheldahl, hdr.Amory.Lofgreen, hdr.Amory.Kanorado, hdr.Amory.Snowflake, hdr.Amory.Tulia, hdr.Amory.Teaneck, hdr.Amory.Brush, hdr.Amory.Gresston, hdr.Amory.Worthing });
        hdr.Weissert.Tallassee = Weches.get<tuple<bit<4>, bit<4>, bit<6>, bit<2>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>({ hdr.Weissert.Wyanet, hdr.Weissert.Elkins, hdr.Weissert.Houston, hdr.Weissert.Sheldahl, hdr.Weissert.Lofgreen, hdr.Weissert.Kanorado, hdr.Weissert.Snowflake, hdr.Weissert.Tulia, hdr.Weissert.Teaneck, hdr.Weissert.Brush, hdr.Weissert.Gresston, hdr.Weissert.Worthing });
    }
}
