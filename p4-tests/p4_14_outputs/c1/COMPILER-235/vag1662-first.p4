...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_vlan_tag") state parse_vlan_tag {
        ...
        meta.m.etherType = hdr.vlan_tag.etherType;
    ...
    @name(".start") state start {
        ...
        meta.m.etherType = hdr.ethernet.etherType;
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".protocol_based_vlan") table protocol_based_vlan {
        ...
        key = {
            meta.m.etherType: exact @name("meta.m.etherType") ;
        }
        size = 1024;
        @name(".vlan_profile") implementation = action_profile(32w2048);
        default_action = NoAction();
    ...
    apply {
        if (!hdr.vlan_tag.isValid() || hdr.vlan_tag.vid == 12w0) 
            if (subnet_based_vlan.apply().hit) 
                ;
            else 
                if (mac_based_vlan.apply().hit) 
                    ;
                else 
                    if (protocol_based_vlan.apply().hit) 
                        ;
                    else 
