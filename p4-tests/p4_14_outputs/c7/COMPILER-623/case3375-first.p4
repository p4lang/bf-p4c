...
control encap_process(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @ternary(1) @name(".tunnel_dst_datacenter_rewrite") table tunnel_dst_datacenter_rewrite {
        ...
        key = {
            meta.tunnel_metadata.dst_datacenter_index: exact @name("meta.tunnel_metadata.dst_datacenter_index") ;
        }
        size = 256;
        default_action = NoAction();
    }
    @ternary(1) @name(".tunnel_dst_server_rewrite") table tunnel_dst_server_rewrite {
        ...
        key = {
            meta.tunnel_metadata.dst_server_index: exact @name("meta.tunnel_metadata.dst_server_index") ;
        }
        size = 2048;
        default_action = NoAction();
    }
    @ternary(1) @name(".tunnel_dst_tor_rewrite") table tunnel_dst_tor_rewrite {
        ...
        key = {
            meta.tunnel_metadata.dst_tor_index: exact @name("meta.tunnel_metadata.dst_tor_index") ;
        }
        size = 2048;
        default_action = NoAction();
    }
...
control process_tunnel(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".tunnel_check") table tunnel_check {
        ...
        key = {
            hdr.vxlan.isValid()      : exact @name("hdr.vxlan.isValid()") ;
            hdr.ipv6.dstAddr_first32 : exact @name("hdr.ipv6.dstAddr_first32") ;
            hdr.ipv6.dstAddr_second32: exact @name("hdr.ipv6.dstAddr_second32") ;
        }
        size = 2;
        default_action = NoAction();
    }
