...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".b_em_direct") RegisterAction<bit<16>, bit<32>, bit<16>>(r_em_direct) b_em_direct_0 = {
        void apply(inout bit<16> value, out bit<16> rv) {
            value = value + 16w1;
            rv = value;
        }
    };
    @name(".b_em_indirect") RegisterAction<bit<16>, bit<32>, bit<16>>(r_em_indirect) b_em_indirect_0 = {
        void apply(inout bit<16> value, out bit<16> rv) {
            value = value + 16w5;
            rv = value;
        }
    };
    @name(".b_hash_act") RegisterAction<bit<8>, bit<32>, bit<8>>(r_hash_act) b_hash_act_0 = {
        void apply(inout bit<8> value, out bit<8> rv) {
            value = value + 8w5;
            rv = value;
        }
    };
    @name(".b_no_key") RegisterAction<bit<16>, bit<32>, bit<16>>(r_no_key) b_no_key_0 = {
        void apply(inout bit<16> value, out bit<16> rv) {
            value = value + 16w5;
            rv = value;
        }
    };
    @name(".b_t_direct") RegisterAction<bit<16>, bit<32>, bit<16>>(r_t_direct) b_t_direct_0 = {
        void apply(inout bit<16> value, out bit<16> rv) {
            value = value + 16w1;
            rv = value;
        }
    };
    @name(".b_t_indirect") RegisterAction<bit<16>, bit<32>, bit<16>>(r_t_indirect) b_t_indirect_0 = {
        void apply(inout bit<16> value, out bit<16> rv) {
            value = value + 16w5;
            rv = value;
        }
    ...
    @name(".a_em_direct") action a_em_direct() {
        hash<bit<12>, bit<12>, tuple_0, bit<13>>(temp_5, HashAlgorithm.random, 12w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 13w4096);
        tmp = b_em_direct_0.execute((bit<32>)temp_5);
        hdr.pkt.field_e_16 = tmp;
    }
    @name(".a_em_indirect") action a_em_indirect() {
        hash<bit<13>, bit<13>, tuple_0, bit<14>>(temp_6, HashAlgorithm.random, 13w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 14w8192);
    ...
    @name(".a_hash_act") action a_hash_act() {
        hash<bit<10>, bit<10>, tuple_0, bit<11>>(temp_7, HashAlgorithm.random, 10w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 11w1024);
        tmp_1 = b_hash_act_0.execute((bit<32>)temp_7);
        hdr.pkt.field_j_8 = tmp_1;
    }
    @name(".a_no_key") action a_no_key() {
        hash<bit<16>, bit<16>, tuple_0, bit<17>>(temp_8, HashAlgorithm.random, 16w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 17w65536);
        tmp_2 = b_no_key_0.execute((bit<32>)temp_8);
        hdr.pkt.field_i_8 = (bit<8>)tmp_2;
    }
    @name(".a_t_direct") action a_t_direct() {
        hash<bit<12>, bit<12>, tuple_0, bit<13>>(temp_9, HashAlgorithm.random, 12w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 13w4096);
        tmp_3 = b_t_direct_0.execute((bit<32>)temp_9);
        hdr.pkt.field_g_16 = tmp_3;
    }
    @name(".a_t_indirect") action a_t_indirect() {
        hash<bit<13>, bit<13>, tuple_0, bit<14>>(temp_10, HashAlgorithm.random, 13w0, { hdr.pkt.field_a_32, hdr.pkt.field_b_32 }, 14w8192);
