...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_ipv4_options_64b") state parse_ipv4_options_64b {
        packet.extract<ipv4_options_64b_t>(hdr.ipv4_options_64b);
        transition parse_ipv4_no_options;
    }
    @name(".parse_ipv4_options_96b") state parse_ipv4_options_96b {
        packet.extract<ipv4_options_96b_t>(hdr.ipv4_options_96b);
        transition parse_ipv4_no_options;
    }
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        ...
        packet.emit<ipv4_options_96b_t>(hdr.ipv4_options_96b);
        packet.emit<ipv4_options_64b_t>(hdr.ipv4_options_64b);
