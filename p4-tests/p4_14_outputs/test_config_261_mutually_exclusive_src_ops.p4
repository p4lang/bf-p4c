...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".parse_a") state parse_a {
        packet.extract(hdr.pkt_a);
        transition accept;
    }
    @name(".parse_b") state parse_b {
        packet.extract(hdr.pkt_b);
        transition accept;
    }
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".set_hash") action set_hash() {
        hash(hdr.eth.blah1, HashAlgorithm.crc32, (bit<32>)0, { meta.meta.x }, (bit<64>)4294967296);
    }
    @name(".t_a") table t_a {
        actions = {
            set_a;
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit(hdr.eth);
        packet.emit(hdr.pkt_b);
        packet.emit(hdr.pkt_a);
    }
}
