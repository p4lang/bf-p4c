...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".change_fields") action change_fields_6(bit<16> dstPort, bit<9> eg_port) {
        hdr.tcp.dstPort = dstPort;
        hdr.ig_intr_md_for_tm.ucast_egress_port = eg_port;
    }
    @name(".change_fields") action change_fields_7(bit<16> dstPort, bit<9> eg_port) {
        hdr.tcp.dstPort = dstPort;
        hdr.ig_intr_md_for_tm.ucast_egress_port = eg_port;
    }
    ...
    @name(".wide_deep_routing") table wide_deep_routing {
        ...
        key = {
            hdr.ipv4.dstAddr: exact @name("ipv4.dstAddr") ;
            hdr.ipv4.srcAddr: exact @name("ipv4.srcAddr") ;
            meta.md.index1  : exact @name("md.index1") ;
            meta.md.index2  : exact @name("md.index2") ;
    ...
    @stage(2) @name(".wide_routing") table wide_routing {
        ...
        key = {
            ...
            meta.md.index2  : exact @name("md.index2") ;
            hdr.tcp.srcPort : exact @name("tcp.srcPort") ;
            meta.md.index3  : exact @name("md.index3") ;
            meta.md.index4  : exact @name("md.index4") ;
            meta.md.index5  : exact @name("md.index5") ;
            meta.md.index6  : exact @name("md.index6") ;
            hdr.ipv4.ttl    : exact @name("ipv4.ttl") ;
        }
        size = 1024;
