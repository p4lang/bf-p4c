...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_fabric_header_cpu") state parse_fabric_header_cpu {
        ...
        transition select(hdr.fabric_header_cpu.ingressPort) {
...
control process_ingress_knet(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".set_egress_port") action set_egress_port() {
        hdr.ig_intr_md_for_tm.ucast_egress_port = (bit<9>)hdr.fabric_header_cpu.ingressPort;
    }
    @name(".add_cpu_header") action add_cpu_header(bit<1> pad1, bit<3> fabric_color, bit<5> fabric_qos, bit<8> dst_device, bit<16> dst_port_or_group, bit<2> reserved1, bit<16> ingress_ifindex, bit<16> ingress_bd, bit<16> reason_code) {
        ...
        hdr.fabric_header.pad1 = pad1;
        hdr.fabric_header.fabricColor = fabric_color;
        hdr.fabric_header.fabricQos = fabric_qos;
        hdr.fabric_header.dstDevice = dst_device;
        hdr.fabric_header.dstPortOrGroup = dst_port_or_group;
        hdr.fabric_header_cpu.setValid();
        hdr.fabric_header_cpu.reserved = reserved1;
        hdr.fabric_header_cpu.ingressIfindex = ingress_ifindex;
        hdr.fabric_header_cpu.ingressBd = ingress_bd;
        hdr.fabric_header_cpu.reasonCode = reason_code;
        hdr.fabric_header_cpu.ingressPort = (bit<16>)hdr.ig_intr_md.ingress_port;
    ...
    @name(".fabric_tbl") table fabric_tbl {
        actions = {
            set_egress_port;
        }
        key = {
    ...
    @name(".port_tbl") table port_tbl {
        ...
        size = 288;
        default_action = add_cpu_header(pad1 = 0, fabric_color = 0, fabric_qos = 0, dst_device = 0, dst_port_or_group = 0, reserved1 = 0, ingress_ifindex = 0, ingress_bd = 0, reason_code = 0);
    }
    apply {
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".process_ingress_knet") process_ingress_knet() process_ingress_knet_0;
    apply {
        process_ingress_knet_0.apply(hdr, meta, standard_metadata);
    }
}
