...
@name("ethernet_split_dmac_t") header ethernet_split_dmac_t_0 {
    bit<8>  mac_upper_byte;
    bit<8>  lb_port;
    bit<32> lower_dmac;
    bit<48> srcAddr;
    bit<16> etherType;
...
struct headers {
    ...
    ipv6_t                                         inner_ipv6;
    @name("outer_eth") 
    ethernet_split_dmac_t_0                        outer_eth;
    @name("outer_ipv4") 
    ipv4_t                                         outer_ipv4;
    @name("outer_ipv6") 
    ipv6_t                                         outer_ipv6;
...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_inner_ipv4") state parse_inner_ipv4 {
        ...
        meta.meta.tunnel_type = 3w2;
        transition accept;
    }
    @name(".parse_inner_ipv6") state parse_inner_ipv6 {
        packet.extract<ipv6_t>(hdr.inner_ipv6);
        meta.meta.tunnel_type = 3w3;
        transition accept;
    }
    @name(".parse_inner_non_ip") state parse_inner_non_ip {
    ...
    @name(".parse_outer_eth") state parse_outer_eth {
        packet.extract<ethernet_split_dmac_t_0>(hdr.outer_eth);
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".rewrite_outer_ethtype_ipv4") action rewrite_outer_ethtype_ipv4() {
        hdr.outer_eth.etherType = 16w0x800;
    }
    @name(".rewrite_outer_ethtype_ipv6") action rewrite_outer_ethtype_ipv6() {
    ...
    @name(".rewrite_tunnel_tbl") table rewrite_tunnel_tbl {
        ...
        key = {
            meta.meta.tunnel_type: exact @name("meta.meta.tunnel_type") ;
        }
        max_size = 8;
        default_action = NoAction();
    }
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".set_lb_hashed_index_ipv6") action set_lb_hashed_index_ipv6() {
        hash<bit<4>, bit<4>, tuple<bit<128>, bit<128>>, bit<8>>(meta.meta.lb_hash, HashAlgorithm.crc16, 4w0, { hdr.inner_ipv6.srcAddr, hdr.inner_ipv6.dstAddr }, 8w16);
    }
    ...
    @name(".set_hashed_lb_port") action set_hashed_lb_port(bit<8> lb_port) {
        meta.meta.lb_port_num = lb_port;
    }
    @name(".rewrite_outer_dmac") action rewrite_outer_dmac() {
...
control DeparserImpl(packet_out packet, in headers hdr) {
    apply {
        packet.emit<ethernet_split_dmac_t_0>(hdr.outer_eth);
