...
control process_egress_system_acl(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".egress_copy_to_cpu") action egress_copy_to_cpu() {
        clone3<tuple<bit<16>, bit<14>, bit<16>, bit<9>>>(CloneType.E2E, 32w250, { meta.ingress_metadata.bd, meta.ingress_metadata.ifindex, meta.fabric_metadata.reason_code, meta.ingress_metadata.ingress_port });
    }
    ...
    @name(".egress_mirror") action egress_mirror(bit<32> session_id) {
        ...
        clone3<tuple<bit<32>, bit<10>>>(CloneType.E2E, session_id, { meta.i2e_metadata.ingress_tstamp, meta.i2e_metadata.mirror_session_id });
    }
...
control process_validate_outer_header(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".validate_outer_ethernet") table validate_outer_ethernet {
        ...
        key = {
            ...
            hdr.vlan_tag_[1].isValid(): ternary @name("vlan_tag_[1].$valid$") ;
        }
        size = 512;
        default_action = NoAction();
    }
...
control process_mac_acl(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".acl_mirror") action acl_mirror(bit<32> session_id, bit<14> acl_stats_index, bit<8> acl_meter_index, bit<2> nat_mode, bit<8> ingress_cos, bit<8> tc, bit<8> color) {
        meta.i2e_metadata.mirror_session_id = (bit<10>)session_id;
        clone3<tuple<bit<32>, bit<10>>>(CloneType.I2E, session_id, { meta.i2e_metadata.ingress_tstamp, meta.i2e_metadata.mirror_session_id });
        meta.acl_metadata.acl_stats_index = acl_stats_index;
        meta.nat_metadata.ingress_nat_mode = nat_mode;
    }
