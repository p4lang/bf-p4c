...
int read_bin(std::istream &in) {
    ...
    while (in.read((char *)&atom_typ, 4)) {
        if ((atom_typ >> 24) == 'H') {
            ...
            if (disasm)
                disasm->input_binary(reg_addr, 'R', &reg_data, 1);
            ...
            if (disasm)
                disasm->input_binary(addr, 'B', &data[0], count);
            ...
            if (disasm)
                disasm->input_binary(addr, 'D', &data[0], count*width/4);
            ...
            if (!in.read((char *)&data[0], count*4)) return -1;
            if (disasm)
                disasm->input_binary(reg_addr, 'S', &data[0], count);
        } else {
            fprintf(stderr, "\n");
