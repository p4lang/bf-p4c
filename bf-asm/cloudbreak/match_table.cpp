/* mau table template specializations for cloudbreak -- #included directly in match_tables.cpp */

// FIXME -- factor better with JBay code?

/**  Write next table setup, which is JBay/Cloudbreak-specific.  `tbl` here is the ternary indirect
 * table if there is one, or the match table otherwise */
template<> void MatchTable::write_next_table_regs(Target::Cloudbreak::mau_regs &regs, Table *tbl) {
