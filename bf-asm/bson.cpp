...
int32_t get32(std::istream &in) {
    char data[4];
    in.read(data, sizeof(data));
    return (data[0] & 0xffU) | ((data[1] & 0xffU) << 8) |
           ((data[2] & 0xffU) << 16) | ((data[3] & 0xffU) << 24);
}
int64_t get64(std::istream &in) {
    char data[8];
    in.read(data, sizeof(data));
    return (data[0] & 0xffULL) | ((data[1] & 0xffULL) << 8) |
           ((data[2] & 0xffULL) << 16) | ((data[3] & 0xffULL) << 24) |
           ((data[4] & 0xffULL) << 32) | ((data[5] & 0xffULL) << 40) |
...
std::istream &operator>>(std::istream &in, bson_wrap<json::vector> o) {
    ...
    while (uint8_t type = get8(in)) {
        ...
        case 0x08:
            switch (get8(in)) {
            case 0: out.push_back(false); break;
            case 1: out.push_back(true); break;
            default:
...
std::istream &operator>>(std::istream &in, bson_wrap<json::map> o) {
    ...
    while (uint8_t type = get8(in)) {
        ...
        case 0x08:
            switch (get8(in)) {
            case 0: out[key] = mkuniq<False>(False()); break;
            case 1: out[key] = mkuniq<True>(True()); break;
            default:
                std::cerr << "invalid boolean value" << std::endl;
                in.setstate(std::ios::failbit);
                break; }
...
static std::unique_ptr<json::vector> map_is_vector(json::map &m) {
    int idx = 0;
    for (auto &el : m) {
        if (*el.first != std::to_string(idx).c_str())
            return nullptr;
        ++idx; }
...
std::ostream &operator<<(std::ostream &out, bson_wrap<const obj> json) {
    if (auto m = json.o.as_map())
        return out << binary(*m);
    if (auto v = json.o.as_vector())
        return out << binary(*v);
