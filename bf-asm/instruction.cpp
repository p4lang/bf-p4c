...
static std::set<target_t>   tofino12 = std::set<target_t>({TOFINO,
#if HAVE_JBAY
    JBAY,
#endif
});
...
#if HAVE_JBAY
static std::set<target_t>   jb_targets = std::set<target_t>({JBAY,
                                                               });

static AluOP3Src::Decode    jb_opBMSET    ("bitmasked-set", jb_targets, 0x0e);   // NOLINT
static CondMoveMux::Decode  jb_opCondMove ("cmov",    jb_targets, 0x6, true,  5, "conditional-move");  // NOLINT
