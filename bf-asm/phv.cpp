    ...
    for (int i = 0; i < Target::NUM_MAU_STAGES(); i++) {
        ...
        for (auto &slot : phv.user_defined) {
            ...
            for (auto field_name : slot.second.second) {
                ...
                // field is in the header or generated by the compiler.
                // Driver does not use is_compiler_generated (other than
                // requiring it).  p4i does use it for display purposes.
