...
template<class REGS> void StatefulTable::write_merge_regs(REGS &regs, MatchTable *match,
    ...
    unsigned per_entry_en_mux = per_flow_enable_bit + address_shift();

    // When addr is generated by hash distribution, the exact_shiftount is used
    // to shift for the pfe and not the meter_addr. Hence, in this case we set
    ...
    // employed.
    if (args.size() > 1) {
        if (args[1].type == Call::Arg::HashDist)
            per_entry_en_mux = 0; }
