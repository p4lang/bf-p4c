...
ErrorMode::mode_t ErrorMode::str2mode(const value_t &v) {
    if (CHECKTYPE(v, tSTR)) {
        ...
        if (v != "no_config")
            error(v.lineno, "Not a valid error mode: %s", v.s);
    }
    return NO_CONFIG;
}
...
const char *ErrorMode::mode2str(ErrorMode::mode_t m) {
    switch (m) {
    case NO_CONFIG: return "no_config";
    case PROPAGATE: return "propagate";
    case MAP_TO_IMMEDIATE: return "map_to_immediate";
    case DISABLE_ALL_TABLES: return "disable";
    case PROPAGATE_AND_MAP: return "propagate_and_map";
    case PROPAGATE_AND_DISABLE: return "propagate_and_disable";
...
const char *ErrorMode::type2str(ErrorMode::type_t t) {
    switch (t) {
    case TCAM_MATCH: return "tcam_match";
    case TIND_ECC: return "tind_ecc";
    case GFM_PARITY: return "gfm_parity";
    case EMM_ECC: return "emm_ecc";
    case PREV_ERROR: return "prev_err";
    case ACTIONDATA_ERROR: return "actiondata";
    case IMEM_PARITY_ERROR: return "imem_parity";
    ...
    if (data.type == tSTR) {
        ...
        for (auto &kv : MapIterChecked(data.map)) {
            ...
            if (t >= LATE_ERROR && m > PROPAGATE)
                error(kv.value.lineno, "%s error mode can only propagate, not %s",
                      type2str(t), mode2str(m));
            mode[t] = m; } }
}

#if HAVE_FLATROCK
// undefined specialization to avoid compile error if HAVE_FLATROCK gets turned on
template<> void ErrorMode::write_regs(Target::Flatrock::mau_regs &, const Stage *, gress_t);
#endif

template<class REGS> void ErrorMode::write_regs(REGS &regs, const Stage *stage, gress_t gress) {
    auto &merge = regs.rams.match.merge;
    int tcam_err_delay = stage->tcam_delay(gress) ? 1 : 0;
    int fifo_err_delay = stage->pipelength(gress) - stage->pred_cycle(gress)
                       - Target::MAU_ERROR_DELAY_ADJUST();
...
#define HANDLE_ERROR_CASES(REG, HAVE_O_ERR_EN)                          \
    ...
    if (propagate) {
        ...
        case Stage::NONE:
            if (stage->stageno == 0) {
                // stage 0 does not have stage_dep set, but counts as if it was match dependent (on
                // the parser).  FIXME -- should just always set stage_dep to MATCH_DEP for stage 0?
                // fall through
    ...
    if (mode[ACTIONDATA_ERROR])
        merge.actiondata_error_ctl |= 1 << gress;
    if (mode[IMEM_PARITY_ERROR])
        merge.imem_parity_error_ctl |= 1 << gress;
...
FOR_ALL_REGISTER_SETS(INSTANTIATE_TARGET_TEMPLATE, void ErrorMode::write_regs,
        mau_regs &, const Stage *, gress_t);
