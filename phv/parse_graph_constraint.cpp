...
void ParseGraphConstraint::postorder(const IR::Tofino::ParserMatch *pm) {
    ...
    bool is_inserted;
    std::tie(new_item, is_inserted) =
        local_extracts_.insert(std::make_pair(pm, PHV::Bits(0)));
    ...
    if (nullptr != pm->next) {
        ...
        LOG2("Setting conflicts from " << findContext<const IR::Tofino::ParserState>()->name <<
             " to " << pm->next->name); }
...
void ParseGraphConstraint::postorder(const IR::Tofino::ParserState *ps) {
    ...
    CHECK(subtree_extracts_.count(ps->name) == 0) << ": " << *ps << " already exists ";
