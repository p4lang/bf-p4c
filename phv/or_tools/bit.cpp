...
void Bit::set_container(Container *container) {
    ...
    CHECK(nullptr == container_) << ": Cannot reassign container for " << name();
    CHECK(nullptr != container) << ": Invalid container in group for " << name();
...
void Bit::set_offset(IntVar *base_offset, const int &relative_offset) {
    ...
    offset_ = base_offset_ = base_offset;
    relative_offset_ = relative_offset;
    if (relative_offset)
        offset_ = base_offset->solver()->MakeSum(base_offset, relative_offset);
}
...
IntVar *Bit::SetDeparsedHeader(const Bit &prev_bit, const Byte &prev_byte) {
    ...
    CHECK(nullptr != prev_bit.container()) << ": No previous container for " << name();
    CHECK(nullptr != prev_bit.base_offset()) << ": No previous offset for " << name();
    CHECK(nullptr != container()) << ": No container for " << name();
    CHECK(nullptr != container()->container()) << ": No container expr for " << name();
    ...
    std::vector<IntVar*> last_bytes({mau_group()->is_8b(), last_byte_16b, last_byte_32b});

    return solver->MakeIsEqualVar(solver->MakeSum(last_bytes), solver->MakeIntConst(1));
}
...
IntExpr *Bit::MakeBit() {
    if (nullptr == bit_) {
        operations_research::Solver *s = base_offset()->solver();
        bit_ = s->MakeSum(s->MakeProd(container_->container(), PHV::kMaxContainer), offset_); }
    return bit_;
}
}  // namespace or_tools
