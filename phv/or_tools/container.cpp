...
IntExpr *Container::deparser_group(const gress_t &thread) {
    if (nullptr == deparser_group_) {
        ...
        for (auto boundaries : PHV::kDeparserGroups)
            if (boundaries.at(0) < PHV::kTPhvContainerOffset || mau_group()->is_t_phv())
                flags.push_back(MakeDeparserGroupFlags(boundaries, &lt_flags));
...
void Container::set_mau_group(MauGroup *mg) {
    CHECK(mg) << ": MAU group is null";
    CHECK(!mau_group_ || mg == mau_group_) << ": Cannot rewrite MAU group" << mau_group_;
    if (!container_) {
        CHECK(!mau_group_) << ": Invalid MAU group for " << container_in_group_;
        LOG2("Creating container from " << mg->mau_group() << " and " << container_in_group_);
        Solver *s = container_in_group_->solver();
        container_ = s->MakeSum(s->MakeProd(mg->mau_group(), PHV::kNumContainersPerMauGroup),
                                container_in_group_); }
    mau_group_ = mg;
}
...
void Container::SetConflict(Container *c) {
    ...
    if (c->mau_group() == mau_group()) {
        ...
        solver->AddConstraint(solver->MakeNonEquality(container(), c->container())); }
}
...
Container::MakeDeparserGroupFlags(const std::vector<int> &boundaries,
    ...
    CHECK(1 == boundaries.size() || 3 == boundaries.size() ||
          5 == boundaries.size() || 7 == boundaries.size())
        << ": Found " << boundaries.size() << " elements in boundary vector";
    Solver *s = solver();
    bool is_lt = false;
    int expected_sum = 0;
