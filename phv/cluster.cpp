...
void Cluster::sanity_check_clusters(const std::string& msg, const PhvInfo::Field *lhs) {
    if (lhs && dst_map_i[lhs]) {
        ...
        if (dst_map_i[lhs]->count(lhs) != 1) {
            WARNING("*****cluster.cpp:sanity_FAIL*****cluster member count > 1.."
                << msg << lhs << "-->" << *(dst_map_i[lhs]));
        }
        // forall x elem (b,d,e), x-->(b,d,e)
        for (auto rhs : *(dst_map_i[lhs])) {
            if (dst_map_i[rhs] != dst_map_i[lhs]) {
                WARNING("*****cluster.cpp:sanity_FAIL*****cluster member pointers inconsistent.."
                    << msg << lhs << "-->" << rhs);
            }
        }
    }
...
void Cluster::sanity_check_fields_use(const std::string& msg,
    ...
    if (sx.size()) {
        WARNING("*****cluster.cpp:sanity_FAIL*****fields_use cluster+all_minus_cluster != all .."
            << msg << sx);
    }
    //
    ...
    if (sx.size()) {
        WARNING("*****cluster.cpp:sanity_FAIL*****fields_use.. cl intersect all_minus_cluster != 0"
            << msg << sx);
    }
    //
    ...
    set_difference(s1.begin(), s1.end(), all.begin(), all.end(), std::back_inserter(sx));
    if (sx.size()) {
        WARNING("*****cluster.cpp:sanity_FAIL*****fields_use all != cluster+pov+no_mau+pov_mau .."
            << msg << sx);
    }
}
