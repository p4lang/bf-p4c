...
std::set<PHV::Bit> Constraints::GetEqual(const PHV::Bit &b, const Equal &e) const {
    if (!equalities_[e].count(b))
        return std::set<PHV::Bit>({b});
    return equalities_[e].at(b);
}

void Constraints::SetEqual_(const PHV::Bit &bit1, const PHV::Bit &bit2, const Equal &eq) {
...
inline void Constraints::SetMatchBits(const std::set<PHV::Bit> &bits,
    ...
    auto comp = [this](const PHV::Bit &b1, const PHV::Bit &b2) -> bool {
                   return !GetByte(b1).Contains(b2) && b1 < b2; };
...
template<> void
Constraints::SetEqual<PHV::Bit>(const PHV::Bit &bit1, const PHV::Bit &bit2, const Equal &eq) {
    SetEqual_(bit1, bit2, eq);
}

template<class T> void
Constraints::SetConstraints(const Equal &e, T set_equal, std::set<PHV::Bit> bits) {
...
void Constraints::UnionEqualities(const Equal &a, const Equal &b) {
...
void Constraints::SetExactMatchBits(const int &stage, const std::set<PHV::Bit> &bits) {
    if (exact_match_bits_.size() <= size_t(stage))
        exact_match_bits_.resize(stage+1);
    SetMatchBits(bits, &exact_match_bits_.at(stage));
    LOG2("Found " << exact_match_bits_.at(stage).size() << " exact match bits in stage " << stage);
}

void Constraints::SetTcamMatchBits(const int &stage, const std::set<PHV::Bit> &bits) {
    if (tcam_match_bits_.size() <= size_t(stage))
        tcam_match_bits_.resize(stage+1);
    SetMatchBits(bits, &tcam_match_bits_.at(stage));
    LOG2("Found " << tcam_match_bits_.at(stage).size() << " TCAM match bits in stage " << stage);
}

void Constraints::SetContainerConflict(const PHV::Bit &b1, const PHV::Bit &b2) {
...
void Constraints::SetParseConflict(const PHV::Bits &old_bits, const PHV::Bits &new_bits) {
