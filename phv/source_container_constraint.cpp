    ...
    for (auto it = dst_src_pairs_.begin(); it != dst_src_pairs_.end(); ++it) {
        for (auto it2 = dst_src_pairs_.begin(); it2 != it; ++it2) {
            ...
            if ((true == is_equal(it->first, it2->first)) &&
                ...
                std::tie(dst_dist, is_valid) = constraints_.GetDistance(it->first, it2->first);
                CHECK(true == is_valid)
                    << ": Unknown distance between " << it->first << " and " << it2->first;
                std::tie(src_dist, is_valid) = constraints_.GetDistance(it->second, it2->second);
                if (true == is_valid) {
