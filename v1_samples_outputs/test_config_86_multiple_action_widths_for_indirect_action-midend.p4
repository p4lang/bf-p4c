...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("action_0") action action(bit<8> my_param_a) {
        hdr.pkt.field_a = my_param_a;
    }
    @name("action_1") action action_6(bit<16> my_param_c) {
        hdr.pkt.field_c = my_param_c;
    }
    @name("action_2") action action_7(bit<32> my_param_e) {
        hdr.pkt.field_e = my_param_e;
    }
    @name("action_3") action action_8(bit<32> my_param_f, bit<32> my_param_g) {
        hdr.pkt.field_f = my_param_f;
        hdr.pkt.field_g = my_param_g;
    }
    @name("action_4") action action_9(bit<32> my_param_h, bit<32> my_param_i, bit<32> my_param_j, bit<32> my_param_k) {
        hdr.pkt.field_h = my_param_h;
        hdr.pkt.field_i = my_param_i;
        hdr.pkt.field_j = my_param_j;
        hdr.pkt.field_k = my_param_k;
    }
    @name("action_5") action action_10(bit<32> my_param_h, bit<32> my_param_i, bit<32> my_param_j, bit<32> my_param_k, bit<32> my_param_l) {
        hdr.pkt.field_h = my_param_h;
        hdr.pkt.field_i = my_param_i;
        hdr.pkt.field_j = my_param_j;
        hdr.pkt.field_k = my_param_k;
