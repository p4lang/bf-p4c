...
WORKSPACE=$1
P4C_VERSION=$2
p4c_tofino_branch=master

cd $WORKSPACE
git clone --recursive git@github.com:barefootnetworks/bf-p4c-compilers
...
docker run --rm -it -v ${WORKSPACE}:/tmp/p4c-build -w /tmp/p4c-build \
