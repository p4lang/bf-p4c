...
Visitor::profile_t FindExpressionsForFields::init_apply(const P4::IR::Node* root) {
    profile_t rv = Inspector::init_apply(root);
    fieldNameToExpressionsMap.clear();
    return rv;
}

bool FindExpressionsForFields::preorder(const P4::IR::HeaderOrMetadata* h) {
    LOG5("Header: " << h->name);
    LOG5("Header type: " << h->type);
    for (auto f : h->type->fields) {
        if (auto hs = h->to<P4::IR::HeaderStack>()) {
            for (int i = 0; i < hs->size; i++) {
                ...
                P4::IR::Member* mem =
                    new P4::IR::Member(
                        ftype,
                        new P4::IR::HeaderStackItemRef(
                            new P4::IR::ConcreteHeaderRef(h), new P4::IR::Constant(i)),
                        f->name);
            ...
            if (name.endsWith("$always_deparse") ||
                name.endsWith(BFN::BRIDGED_MD_INDICATOR.string_view()))
                continue;
            P4::IR::Member* mem = gen_fieldref(h, f->name);
            if (!mem) continue;
            fieldNameToExpressionsMap[name] = mem;
            LOG5("  Added field: " << name << ", " << mem);
        }
    }
    if (auto hs = h->type->to<P4::IR::HeaderStack>()) {
        for (int i = 0; i < hs->size; i++) {
...
Visitor::profile_t ReplaceAllAliases::init_apply(const P4::IR::Node* root) {
    if (LOGGING(3)) {
...
P4::IR::Node* ReplaceAllAliases::preorder(P4::IR::BFN::Pipe* pipe) {
    visit(pipe->thread[INGRESS].hw_constrained_fields);
    visit(pipe->thread[EGRESS].hw_constrained_fields);
    return pipe;
}

P4::IR::Node* ReplaceAllAliases::preorder(P4::IR::Expression* expr) {
    ...
    if (!aliasMap.count(f->name)) {
        ...
        return expr; }
    auto* ixbarread = expr->to<P4::IR::MAU::TableKey>();
    if (ixbarread) {
    ...
    if (getParent<P4::IR::BFN::HardwareConstrainedField>() &&
    ...
    const P4::IR::Member* replacementMember = fieldExpressions.at(replacementField->name);

    auto* sl = expr->to<P4::IR::Slice>();
    if (sl) {
        if (f->size == replacementField->size) {
            auto* newMember = new P4::IR::BFN::AliasMember(replacementMember, sl->e0);
            P4::IR::Slice* newSlice = new P4::IR::Slice(newMember, sl->getH(), sl->getL());
            LOG2("    Replaced A: " << expr << " -> " << newSlice);
            return newSlice;
        } else {
            P4::IR::Slice* destSlice = new P4::IR::Slice(replacementMember, sl->getH(), sl->getL());
            auto* newAliasSlice = new P4::IR::BFN::AliasSlice(destSlice, sl);
            LOG2("    Replaced B: " << expr << " -> " << newAliasSlice);
            return newAliasSlice;
        }
    } else {
        if (f->size == replacementField->size) {
            auto* newMember = new P4::IR::BFN::AliasMember(replacementMember, expr);
            LOG2("    Replaced C: " << expr << " -> " << newMember);
            return newMember;
        } else {
            P4::IR::Slice* destSlice = new P4::IR::Slice(replacementMember, f->size - 1, 0);
            auto* newAliasSlice = new P4::IR::BFN::AliasSlice(destSlice, expr);
...
Visitor::profile_t AddValidityBitSets::init_apply(const P4::IR::Node* root) {
    aliasedFields.clear();
...
P4::IR::Node* AddValidityBitSets::preorder(P4::IR::MAU::Instruction* inst) {
...
P4::IR::Node* AddValidityBitSets::postorder(P4::IR::MAU::Action* action) {
    ...
    for (const auto* f : actionToPOVMap.at(action->name)) {
        ...
        auto* oneExpr = new P4::IR::Constant(P4::IR::Type_Bits::get(pov->type->width_bits()), 1);
        auto* prim = new P4::IR::MAU::Instruction("set"_cs, { pov, oneExpr });
