...
/** Replace the "$valid" suffix for header stacks with an index into the
 * "$stkvalid" field instead.
 *
...
class ValidToStkvalid : public Transform {
    ...
    //
    // XXX(cole): This is because Brig doesn't currently
    // support extracting constants to field slices in the parser (see
    // BRIG-584).
