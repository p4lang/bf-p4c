...
std::optional<const P4::IR::Operation_Binary*>
CheckOperations::getSrcBinop(const P4::IR::MAU::Primitive* prim) const {
    prim = prim->apply(RemoveCasts());
    if (prim->name != "modify_field") return std::nullopt;
    if (prim->operands.size() < 2) return std::nullopt;
    if (auto* binop = prim->operands[1]->to<P4::IR::Operation_Binary>())
        return binop;
    return std::nullopt;
}

bool CheckOperations::isModBitMask(const P4::IR::MAU::Primitive* prim) const {
    auto* binop = getSrcBinop(prim).value_or(nullptr);
    if (!binop || binop->getStringOp() != "|") return false;
    auto leftBinop = binop->left->to<P4::IR::Operation_Binary>();
    auto rightBinop = binop->right->to<P4::IR::Operation_Binary>();
