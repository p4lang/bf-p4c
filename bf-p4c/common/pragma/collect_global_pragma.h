...
class CollectGlobalPragma : public Inspector {
    ...
    std::vector<const P4::IR::Annotation*> global_pragmas_;

    /// Given an P4::IR::StructField* node, extract the header name associated with that node. @returns
    /// an empty string if the name annotation does not actually have an associated name.
    cstring getStructFieldName(const P4::IR::StructField*) const;
    ...
    bool preorder(const P4::IR::Annotation *) override;
    ...
    bool preorder(const P4::IR::StructField*) override;

 public:
    const std::vector<const P4::IR::Annotation*>& global_pragmas() const { return global_pragmas_; }
    ...
    const P4::IR::Annotation *exists(const char *pragma_name) const;
};
