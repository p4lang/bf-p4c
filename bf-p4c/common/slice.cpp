...
static P4::IR::Constant* sliceConstant(const P4::IR::Constant* k, int lo, int hi) {
    int size = hi-lo+1;
    big_int value = (k->value >> lo) & bigBitMask(size);
    return new P4::IR::Constant(P4::IR::Type::Bits::get(size), value);
}
...
const P4::IR::Expression *MakeSliceDestination(const P4::IR::Expression *e, int lo, int hi) {
    LOG6("\tExpression: " << e);
    LOG6("\tLo: " << lo << ", Hi: " << hi);
    if (auto s = e->to<P4::IR::Slice>())
        LOG6("\tOriginal Expression: " << s->e0);
    if (e->is<P4::IR::MAU::MultiOperand>())
        return new P4::IR::Slice(e, hi, lo);
    ...
    if ((lo - slLoHi.first) >= e->type->width_bits()) {
        LOG6("\tReturning 0's");
        return new P4::IR::Constant(P4::IR::Type::Bits::get(hi-lo+1), 0); }
    if (hi > slLoHi.second) {
...
const P4::IR::Expression *MakeSliceSource(const P4::IR::Expression *read, int lo, int hi, const
    ...
    if (read->is<P4::IR::MAU::MultiOperand>())
        return new P4::IR::Slice(read, hi, lo);
    if (auto k = read->to<P4::IR::Constant>()) {
    ...
    if (lo == readLoHi.first && hi == readLoHi.second) {
        ...
        return read; }
    if (auto sl = read->to<P4::IR::Slice>()) {
        LOG6("\tEncountered a slice [" << sl->getL() << ", " << sl->getH() << "]");
        LOG6("\tOriginal expression: " << sl->e0);
        BUG_CHECK(lo >= int(sl->getL()) && hi <= int(sl->getH()),
                  "MakeSlice slice on slice type mismatch");
...
const P4::IR::Expression *MakeSlice(const P4::IR::Expression *e, int lo, int hi) {
    ...
    if (auto a = e->to<P4::IR::BAnd>())
        return new P4::IR::BAnd(e->srcInfo, MakeSlice(a->left, lo, hi), MakeSlice(a->right, lo, hi));
    if (auto o = e->to<P4::IR::BOr>())
        return new P4::IR::BOr(e->srcInfo, MakeSlice(o->left, lo, hi), MakeSlice(o->right, lo, hi));
    if (auto o = e->to<P4::IR::BXor>())
        return new P4::IR::BXor(e->srcInfo, MakeSlice(o->left, lo, hi), MakeSlice(o->right, lo, hi));
    if (auto cc = e->to<P4::IR::Concat>()) {
        ...
        return new P4::IR::Concat(cc->srcInfo, MakeSlice(cc->left, 0, hi - rsize),
    ...
    if (auto *neg = e->to<P4::IR::Neg>())
        return new P4::IR::Neg(e->srcInfo, MakeSlice(neg->expr, lo, hi));
    return new P4::IR::Slice(e, hi, lo);
}

safe_vector<const P4::IR::Expression *> convertMaskToSlices(const P4::IR::Mask *mask) {
    BUG_CHECK(mask != nullptr, "Cannot convert nullptr P4::IR::Mask");
    safe_vector<const P4::IR::Expression *> slice_vector;

    auto value = mask->right->to<P4::IR::Constant>()->value;
    auto expr = mask->left;
    unsigned zero_pos = 0;
...
const P4::IR::Expression *MakeWrappedSlice(const P4::IR::Expression *e, int lo, int hi, int wrap_size) {
    ...
    if (e->is<P4::IR::MAU::MultiOperand>() || e->is<P4::IR::MAU::HashDist>()
        || e->is<P4::IR::MAU::RandomNumber>()) {
        return new P4::IR::MAU::WrappedSlice(e, hi, lo, wrap_size);
    } else {
