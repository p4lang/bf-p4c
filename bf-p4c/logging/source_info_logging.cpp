...
void CollectSourceInfoLogging::addSymbol(const CollectSourceInfoLogging::Symbol& symbol) {
    ...
    if (symbol.inst.isValid()) {
        ...
        endLine = line + (symbol.inst.getEnd().getLineNumber() -
                            symbol.inst.getStart().getLineNumber());
    }
    // TODO(1): else { return; }
    // Make file path relative to sourceRoot (ignore paths outside sourceRoot)
    if (file.find(sourceRoot) == 0)
        file.erase(0, sourceRoot.length() + 1);
    ...
    auto* logSymbol =
        new Source_Info_Schema_Logger::SymbolDefinition(
            symbol.name.c_str(),
    ...
    for (const auto& ref : symbol.refs) {
        file = ""; line = 0; endLine = 0; column = 0;
        if (ref.isValid()) {
...
void CollectSourceInfoLogging::addSymbolReference(const IR::Node* ref,
    ...
    if (decl->is<IR::P4Table>()
            ...
            || decl->is<IR::Declaration_Variable>()
        ) {
    ...
    // ! This is rather a hack, the right way is to modify IR::Parameter to provide FQN
        ...
        if ((action = findContext<IR::P4Action>()) &&
            !ref->is<IR::Member>() && !findContext<IR::Member>()) {
    ...
    if (declType != nullptr) {
        ...
        if (declType->is<IR::Type_Name>())
            type += "_" + refMap.getDeclaration(declType->to<IR::Type_Name>()->path)->
                              node_type_name();
        else
    ...
    std::cout << std::endl << "\t";
    std::cout << "/" << ref->node_type_name()
        << "(" << (ref->is<IR::IDeclaration>() ?
                    ref->to<IR::IDeclaration>()->externalName() :
                    ref->toString()) << ")";
    std::cout << "(" << (ref->getSourceInfo().isValid() ?
                            ref->getSourceInfo().toPositionString() + ":" +
    ...
    if (decl) {
        ...
        std::cout << "(" << (decl->getSourceInfo().isValid() ?
...
bool CollectSourceInfoLogging::preorder(const IR::Argument* argument) {
    ...
    if (!expr || !expr->is<IR::PathExpression>())
        return true;
