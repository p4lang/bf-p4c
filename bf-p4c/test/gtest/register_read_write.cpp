...
auto defs = R"(
    struct headers_t { }
    struct local_metadata_t { bit<5> result_5b; bit<32> result_32b; })";
...
/*
 * For downcasting and slicing, only the ArchTranslation and RegisterReadWrite passes
 * are needed. The rest of the passes are needed for upcasting.
 */
...
TEST(RegisterReadWrite, ReadSlice) {
    ...
    Match::CheckList expected {
        ...
        "};",
        "action reg_16b_action() {",
            "ig_md.result_5b = reg_16b_0_register_action.execute(ig_intr_md.ingress_port)[4:0];",
        "}",
        "apply {",
            "reg_16b_action();",
        "}"
...
TEST(RegisterReadWrite, ReadUpcast) {
    ...
    Match::CheckList expected {
        ...
        "action reg_16b_action() {",
            ...
            "$concat_to_slice`\\2` = reg_16b_0_register_action.execute(ig_intr_md.ingress_port);",
