...
TEST_F(CheckUninitializedReadTest, TestForTofino) {
    auto defs = R"(
header H1
{
    bit<8> f1;
...
struct headers_t { H1 h1; H2 h2;}
struct local_metadata_t { bit<8> f1; }
)";
    auto ingress_parser = R"(
state start {
...
apply{
    ...
    auto blk = TestCode(TestCode::Hdr::Tofino1arch, TestCode::tofino_shell(),
        {defs, ingress_parser, ingress, ingress_deparser});
    ...
    auto res = output.find("warning: ingress::hdr.h2.f1 is read in ingress deparser, but it is "
        "totally or partially uninitialized");
    ASSERT_TRUE(res != std::string::npos);

    // padding field should be ignored.
    res = output.find("warning: ingress::hdr.h2.f2 is read in ingress deparser, but it is "
        "totally or partially uninitialized");
    ASSERT_TRUE(res == std::string::npos);

    // pov bit should always be initialized.
    res = output.find("warning: ingress::hdr.h2.$valid is read in ingress deparser, but it is "
        "totally or partially uninitialized");
    ASSERT_TRUE(res == std::string::npos);

    // Although drop ctl is partially initialized in ingress, but it is initialized by the
    // architecture when ingress starts.
    res = output.find("warning: ingress::ig_intr_md_for_dprsr.drop_ctl is read in ingress deparser,"
        " but it is totally or partially uninitialized");
    ASSERT_TRUE(res == std::string::npos);
}

TEST_F(CheckUninitializedReadTest, TestForTofino2) {
    auto program = R"(
/* -*- P4_16 -*- */
...
Switch(pipe) main;
    ...
    // $tmp1 is compiler generated padding field for resubmit field, should be ignored if not
    // initialized.
    auto res = output.find("warning: ingress::$tmp1 is read in ingress deparser, but it is "
        "totally or partially uninitialized");
    ASSERT_TRUE(res == std::string::npos);

    // ig_dprsr_md is partially written in the program, but since it is pov bit protected, it should
    // be ignored.
    res = output.find("warning: ingress::ig_intr_md_for_dprsr.drop_ctl is read in ingress"
        " deparser, but it is totally or partially uninitialized");
    ASSERT_TRUE(res == std::string::npos);
}
