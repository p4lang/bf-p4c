...
TEST(symbolic_bitvec_tests, basic) {
     ...
     * 
     */
    auto expr_simpl = new Expr(
        new Expr(nullptr, nullptr, ExprNodeType::BIT_NODE, 2),
        new Expr(nullptr, nullptr, ExprNodeType::BIT_NODE, 3),
        ExprNodeType::OR_NODE);
    ...
    // eq
    // ~a[0] && b[0] == ~a[0] && b[0]
    EXPECT_TRUE(Expr(new Expr(a.get(0), nullptr, ExprNodeType::NEG_NODE),
                     b.get(0),
                     ExprNodeType::AND_NODE).eq(new Expr(
                        new Expr(a.get(0), nullptr, ExprNodeType::NEG_NODE),
                        b.get(0),
                        ExprNodeType::AND_NODE)));
    // a[0] && b[0] == b[0] && a[0]
    EXPECT_TRUE(Expr(a.get(0),
                     b.get(0),
                     ExprNodeType::AND_NODE).eq(new Expr(
                        b.get(0),
                        a.get(0),
    ...
    EXPECT_TRUE(Expr(new Expr(a.get(0), a.get(1), ExprNodeType::AND_NODE),
                     b.get(0),
                     ExprNodeType::AND_NODE).eq(new Expr(
                        b.get(0),
                        new Expr(
    ...
    EXPECT_FALSE(Expr(new Expr(a.get(0), a.get(1), ExprNodeType::AND_NODE),
                      b.get(0),
                      ExprNodeType::AND_NODE).eq(new Expr(
                        a.get(0),
                        new Expr(b.get(0), a.get(1), ExprNodeType::AND_NODE),
                        ExprNodeType::AND_NODE)));
}
