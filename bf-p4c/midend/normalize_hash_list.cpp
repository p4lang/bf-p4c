...
class TemporaryVariableInserter {
 private:
    ...
    class InjectInitializations : public Transform {
     ...
     public:
        explicit InjectInitializations(
     ...
     private:
        std::vector<const IR::PathExpression*> findActions(
 ...
 public:
    ...
    IR::PathExpression* createTemporaryVariable(
        const std::string& name_,
        const IR::Type* var_type_,
...
TemporaryVariableInserter::TemporaryVariableInserter(
    const ::P4::ReferenceMap* ref_map_) :
    ref_map(ref_map_) {
}
...
IR::PathExpression* TemporaryVariableInserter::createTemporaryVariable(
    ...
    if (init_expr_ != nullptr) {
        auto* assignment_(new IR::AssignmentStatement(
            init_expr_->getSourceInfo(), tmp_var_, init_expr_));
        if (action_ != nullptr) {
...
void TemporaryVariableInserter::injectIntoControl(IR::P4Control* control_) {
    if (!declarations.empty()) {
        ...
        control_->body = control_->body->apply(
            InjectInitializations(ref_map, &action_initializations, &stmt_initializations))
...
const IR::Node* ProcessFields::preorder(IR::StructExpression* expr) {
    return expr;
}

const IR::Node* ProcessFields::preorder(IR::Constant* constant) {
    return skipExpression(constant);
}

const IR::Node* ProcessFields::preorder(IR::PathExpression* pe) {
    return skipExpression(pe);
}

const IR::Node* ProcessFields::preorder(IR::Member* member) {
    return skipExpression(member);
}

const IR::Node* ProcessFields::preorder(IR::TempVar* tmp) {
    return skipExpression(tmp);
}

const IR::Node* ProcessFields::preorder(IR::Padding* padding) {
    return skipExpression(padding);
}

const IR::Node* ProcessFields::preorder(IR::Neg* neg) {
    return skipExpression(neg);
}

const IR::Node* ProcessFields::preorder(IR::Slice* slice) {
    return skipExpression(slice);
}
...
const IR::Node* ProcessFields::replaceByTemporaryVariable(IR::Expression* expr_) {
    ...
    auto* tmp_var_(inserter->createTemporaryVariable(
        "$hash_field_argument" + std::to_string(tempvar_index++),
        expr_type_,
        expr_,
        stmt_,
        action_));
...
ProcessFieldArgument::ProcessFieldArgument(
    ::P4::TypeMap* type_map_,
    TemporaryVariableInserter* inserter_) :
    type_map(type_map_),
    inserter(inserter_) {
...
void ProcessFieldArgument::postorder(IR::Argument* arg) {
    arg->expression = arg->expression->apply(
        ProcessFields(type_map, inserter), getContext());
}
...
const IR::Node* PickUpHasher::preorder(IR::MethodCallExpression* mc) {
    if (!NormalizeHashList::isHasher(mc))
        return mc;

    /* -- find all hash field list items which should be replaced by a temporary variable */
    mc->arguments = mc->arguments->apply(
        ProcessFieldArgument(type_map, &inserter), getContext());
...
NormalizeHashList::NormalizeHashList(
    ::P4::ReferenceMap* ref_map_,
    ::P4::TypeMap* type_map_,
    ::P4::TypeChecking* type_checking_) {
    addPasses({
        type_checking_,
        new PickUpHasher(ref_map_, type_map_),

        /* -- new variables are created, clear the map and move local declarations */
        new P4::ClearTypeMap(type_map_),
        type_checking_
    });
}
...
bool NormalizeHashList::isHasher(const IR::MethodCallExpression* mc) {
    ...
    if (this_base_type == nullptr)
        return false;
    if (this_base_type->name.name != "Hash")
        return false;

    /* -- the extern type is OK, now check method name */
    if (member_method->member.name != "get")
        return false;
