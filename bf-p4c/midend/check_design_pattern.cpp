...
bool BFN::DigestEmitMustSpecifyType::preorder(const IR::MethodCallExpression* expr) {
    auto* mi = P4::MethodInstance::resolve(expr, refMap, typeMap, true);

    std::set<cstring> externsToCheck = {"Mirror", "Resubmit", "Digest", "Pktgen"};
    std::set<cstring> methodToCheck = {"emit", "pack"};
    ...
    std::set<cstring> structAllowed = { "Digest" };
    ...
    if (auto *em = mi->to<P4::ExternMethod>()) {
        ...
        if (auto lexp = param->type->to<IR::ListExpression>()) {
            if (lexp->size() == 0) {
                ::error(ErrorType::ERR_UNSUPPORTED, " field list cannot be empty, use emit()?");
                return false; } }
