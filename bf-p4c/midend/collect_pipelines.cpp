...
bool CollectPipelines::preorder(const IR::Declaration_Instance* di) {
    ...
    if (name == "Switch") {
        for (auto argument : *di->arguments) {
            ...
            BUG_CHECK(pathExp && pathExp->type->is<IR::Type_Package>(),
                      "pipe expression %1% has unexpected type",
                      argument->expression);
            ...
            for (auto dec : decs) {
                if (dec)
                    pipelines->declarationToPipe[dec].insert(pipelines->pipes.size() - 1);
            }
        }
    ...
    if (name == "Pipeline") {
        ...
        for (auto *argument : *di->arguments) {
            ...
            BUG_CHECK(call,
                      "Cannot handle pipelines with non-call expression %1% (in %2%)",
                      argument->expression, di);
            BUG_CHECK(call->constructedType,
                      "Cannot get the callee in expression %1% (in %2%)",
                      call, di);
            auto *type = call->constructedType->to<IR::Type_Name>();
            BUG_CHECK(type, "Cannot extract type from type name in expression %1% (in %2%)",
                      call, di);
