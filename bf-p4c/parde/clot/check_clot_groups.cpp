...
bool CheckClotGroups::preorder(const IR::BFN::AbstractDeparser *dp) {
    if (LOGGING(5))
        dump(::Log::Detail::fileLogOutput(__FILE__), dp);
...
void CheckClotGroups::postorder(const IR::BFN::AbstractDeparser *dp) {
    ...
    for (auto &span : field_dictionary) {
        ...
        if (span.clot) {
            if (++clots_in_group > DEPARSER_CHUNKS_PER_GROUP ||
                ...
                LOG2("insert gap of " << gap << " chunks before clot " << span.clot->tag <<
    ...
    if (next_chunk > DEPARSER_CHUNK_GROUPS * DEPARSER_CHUNKS_PER_GROUP) {
        error(ErrorType::ERR_OVERLIMIT, "%sToo many field dictionary chunks (%d) required in "
              "%s deparser, need to have fewer or smaller headers", dp->srcInfo,
              next_chunk, toString(dp->gress));
    }
}
