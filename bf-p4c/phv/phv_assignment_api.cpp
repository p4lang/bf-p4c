...
std::map<cstring, std::list<const PhvInfo::Field::alloc_slice *>>
    PHV_Assignment_API::phv_name_alloc_slices_map_i;
ordered_map<cstring, PHV::Container *> PHV_Assignment_API::phv_name_container_map_i;

//
//***********************************************************************************
//
// PHV_Assignment_API::apply_visitor
//
//***********************************************************************************
//
//
const IR::Node *
PHV_Assignment_API::apply_visitor(const IR::Node *node, const char *name) {
    //
    if (name) {
        LOG1(name);
    }
...
void
PHV_Assignment_API::end_apply() {
    //
    LOG3(*this);
    //
    sanity_check_fields("PHV_Assignment_API::end_apply()");
}

void
PHV_Assignment_API::create_field_container_map() {
    //
    phv_name_alloc_slices_map_i.clear();  // static, avoid duplicates upon backtracking re-entrancy
    //
    for (auto &f : phv_i) {
        ...
        for (auto &slice : f.alloc_i) {
            ...
            //
            f.phv_assignment_api()->
                phv_name_alloc_slices_map()[slice.container.toString()].push_back(&slice);
            ...
            f.phv_assignment_api()->
                phv_name_container_map()[slice.container.toString()] = &(slice.container);
        }
    }  // for
    // sort alloc slices in name_alloc_slices_map
    for (auto &e : phv_name_alloc_slices_map_i) {
        e.second.sort(
            [](const PhvInfo::Field::alloc_slice *l, const PhvInfo::Field::alloc_slice *r) {
...
std::tuple<
    ...
    PHV::Container *,
    const std::pair<int, int>>
PHV_Assignment_API::make_tuple(const PhvInfo::Field::alloc_slice *slice) {
    //
    assert(slice);
    //
    ...
    //
    const PhvInfo::Field *f = slice->field;
...
//
void
PHV_Assignment_API::sanity_check_fields(const std::string& msg) {
    //
    const std::string msg_1 = msg + ".....sanity_check_fields()";
    for (auto &f : phv_i) {
        if (!f.phv_assignment_api()) {
            LOG1("*****phv_assignment_api.cpp:sanity_FAIL*****....."
                << "..... PHV_Assignment_API extended object not created for field ..... "
                << f);
        }
    }
...
void
PHV_Assignment_API::sanity_check_fields_containers(const std::string& msg) {
    //
...
void
PHV_Assignment_API::sanity_check_container_holes(const std::string& msg) {
    //
    ...
    for (auto &e : phv_name_alloc_slices_map_i) {
        ...
        if (holes_list_1 != holes_list_2) {
            LOG1("*****phv_assignment_api.cpp:sanity_FAIL*****....."
                << "..... PHV_Assignment_API container holes inconsistent ..... "
                << std::endl
                << holes_list_1
                << " -- "
                << holes_list_2);
...
//
bool
PHV_Assignment_API::field_allocated(
    PhvInfo::Field *f,
    bool contiguously) {
    //
    std::list<std::tuple<
        ...
        PHV::Container *,
        const std::pair<int, int>>> tuple_list;
    bool contiguous = f->phv_assignment_api()->field_to_containers(f, tuple_list);
...
PHV_Assignment_API::field_to_containers(
    ...
    for (auto &e : f_api->field_container_map()) {
        ...
        if (lo >= range.first && lo <= range.second) {
            const PhvInfo::Field::alloc_slice *slice = e.second;
            tuple_list.push_back(make_tuple(slice));
            lo += slice->width;
        }
    }
...
bool
PHV_Assignment_API::container_holes(PHV::Container& phv_num) {
...
void
PHV_Assignment_API::container_holes(
...
//
void
PHV_Assignment_API::container_to_fields(
    PHV::Container& phv_num,
    std::list<std::tuple<
        const PhvInfo::Field *,
        const std::pair<int, int>,
...
//
void
PHV_Assignment_API::fields_written(
    PHV::Container& phv_num,
    std::list<std::tuple<
        const PhvInfo::Field *,
        const std::pair<int, int>,
...
//
void
PHV_Assignment_API::fields_overlayed(
    PHV::Container& phv_num,
    std::list<std::tuple<
        const PhvInfo::Field *,
        const std::pair<int, int>,
...
//
void
PHV_Assignment_API::fields_sliced(
    PHV::Container& phv_num,
    std::list<std::tuple<
        const PhvInfo::Field *,
        const std::pair<int, int>,
...
std::ostream &operator<<(
    ...
    //
    const PhvInfo::Field *f = std::get<0>(tuple);
    const std::pair<int, int> field_range = std::get<1>(tuple);
    ...
    std::list<std::tuple<
        ...
        PHV::Container *,
        const std::pair<int, int>>> tuple_list;
    const_cast<PhvInfo::Field *>(f)->phv_assignment_api()->container_to_fields(*c, tuple_list);
    for (auto &t : tuple_list) {
        const PhvInfo::Field *f = std::get<0>(t);
...
std::ostream &operator<<(
    std::ostream &out,
    ordered_map<std::pair<int, int>, const PhvInfo::Field::alloc_slice *>& field_container_map) {
    //
    for (auto &e : field_container_map) {
        std::pair<int, int> range = e.first;
        const PhvInfo::Field::alloc_slice *slice = e.second;
        const PhvInfo::Field *field = slice->field;
        const PHV::Container c = slice->container;
...
std::ostream &operator<<(
    std::ostream &out,
    std::map<cstring, std::list<const PhvInfo::Field::alloc_slice *>>& name_alloc_slices_map) {
    //
...
std::ostream &operator<<(std::ostream &out, PHV_Assignment_API &phv_assignment_api) {
    ...
    std::list<std::tuple<
        ...
        PHV::Container *,
        const std::pair<int, int>>> tuple_list;
    out << ".................... PHV Assignment API: Fields to Containers ........................."
        << std::endl;
    for (auto &f : phv_assignment_api.phv()) {
        if (!phv_assignment_api.uses()->is_referenced(&f)) {  // disregard unreferenced fields
            continue;
        }
        assert(f.phv_assignment_api());
    ...
    out << ".................... PHV Assignment API: Containers to Fields ........................."
        << std::endl;
    out << phv_assignment_api.phv_name_alloc_slices_map()
        << std::endl;
