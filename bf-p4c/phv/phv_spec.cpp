...
bitvec
PhvSpec::range(PHV::Kind kind, unsigned start, unsigned length) const {
    bitvec containers;
...
bitvec PhvSpec::tagalongGroup(unsigned groupIndex) const {
    return range(PHV::Kind::TB, groupIndex * 4, 4)
         | range(PHV::Kind::TW, groupIndex * 4, 4)
         | range(PHV::Kind::TH, groupIndex * 6, 6);
}

const bitvec& PhvSpec::individuallyAssignedContainers() const {
