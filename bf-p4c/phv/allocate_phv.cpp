    ...
    if (LOGGING(4)) {
        ...
        for (const auto* cluster : sc) {
            ...
            cluster->forall_fieldslices([&] (const PHV::FieldSlice& fs) {
                ...
                LOG_DEBUG4(TAB1 "Field " << fs.field() << " has "
                           << p->nOpportunities(fs.field()) << "opportunities");
            });
        }
    }
}
...
// trivial_alloc_score_is_better prefers minimal packing of fieldslices.
bool trivial_alloc_score_is_better(const AllocScore& left, const AllocScore& right) {
    AllocScore delta = left - right;
    auto weighted_delta = weighted_sum(delta);
    std::vector<AllocScoreCmpCond> conds;
    conds = {
            {n_overlay_bits, weighted_delta[n_overlay_bits], false},
            {n_inc_containers, weighted_delta[n_inc_containers], true},
            {n_packing_bits, weighted_delta[n_packing_bits], false},
        };
    auto rst = prioritized_cmp(conds);
    if (rst.second != "equal") {
        LOG_DEBUG6("left-hand-side score is better because: " << rst.second);
...
std::list<PHV::SuperCluster*> remove_singleton_slicelist_metadata(
    ...
    for (auto* super_cluster : cluster_groups) {
        ...
        // TODO(yumin): we should fix this in make_clusters and remote this function.
...
bool is_deparser_zero_sc(const PHV::SuperCluster* sc) {
    return sc->all_of_fieldslices(
...
AllocScore ScoreContext::make_score(
    const PHV::Transaction& alloc,
    const PhvInfo& phv,
    const ClotInfo& clot,
    const PhvUse& uses,
...
bool CoreAllocation::satisfies_constraints(
    ...
    if (f->no_split() && int(group.width()) < f->size) {
        LOG_DEBUG5(TAB1 "Constraint violation: Can't split field size " << f->size << " across "
                   << group.width() << " containers");
        return false; }
    return true;
}
...
bool CoreAllocation::satisfies_constraints(
    ...
    if (req && !group.is(*req)) {
        LOG_DEBUG5(TAB1 "Constraint violation: " << slice << " must go to "
                   << *req << " because of @pa_container_size");
        return false; }
...
bool CoreAllocation::satisfies_constraints(
    ...
    if (std::adjacent_find(slices.begin(), slices.end(), DifferentContainer) != slices.end()) {
        LOG_DEBUG5(TAB1 "Constraint violation: Slices placed together must be placed "
                   "in the same container: " << slices);
        return false; }
    ...
    if (std::any_of(slices.begin(), slices.end(), IsDeparsed)) {
        ...
        if (!std::all_of(slices.begin(), slices.end(), IsDeparsedOrDigest)) {
            LOG_DEBUG5(TAB1 "Constraint violation: Mix of deparsed/not deparsed fields "
                       "cannot be placed together: " << slices);
            return false; }
        ...
        if (container_size < aggregate_size) {
            LOG_DEBUG5(TAB1 "Constraint violation: Slices placed together are " << aggregate_size
                       << "b wide and cannot fit in an " << container_size << "b container");
            return false; }
        ...
        if (container_size > aggregate_size) {
            LOG_DEBUG5(TAB1 "Constraint violation: Deparsed slices placed together are "
                       << aggregate_size << "b wide but do not completely fill "
    ...
    for (auto& slice : slices) {
        bool isDeparsedOrMau = utils_i.uses.is_deparsed(slice.field()) ||
            utils_i.uses.is_used_mau(slice.field());
    ...
    if (not_adjacent && solitary) {
        if (!std::all_of(pack_fields.begin(), pack_fields.end(), [](const PHV::Field* f) {
            ...
            LOG_DEBUG5(TAB1 "Constraint violation: Slice list contains multiple fields, one of "
    ...
    for (auto& alloc_slice : slices) {
        if (alloc_slice.field()->used_in_wide_arith()) {
            ...
            if (alloc_slice.field()->bit_is_wide_arith_lo(lo_bit)) {
                if ((cidx % 2) != 0) {
                    LOG_DEBUG5(TAB1 "Constraint violation: Starting wide "
                    ...
                    if (as.field() == alloc_slice.field()) {
                        ...
                        if (lo_bit2 == (lo_bit - 32)) {
                            ...
                            if (cidx != (lo_container_idx + 1)) {
                                LOG_DEBUG5(TAB1 "Constraint violation: Starting wide arith hi at "
                    ...
                    if (!state_to_vec.count(state) && prev_field != field->name) {
                       ...
                       for (auto &sv : state_to_vec) {
                           if (!utils_i.parser_info.graph(parser).is_mutex(sv.first, state)) {
                               LOG_DEBUG5(TAB1 "Constraint violation: Extracting fields in the "
                                          "slice list in non-mutex states " << sv.first->name <<
                                          " and " << state->name);
                               return false;
                           }
                       }
    ...
    for (const auto* f : containerBytesFields) {
        ...
        LOG_DEBUG6(TAB2 "Field: " << f->name << "\n\t"
                   "allocated_bits: " << allocated_bits << ", container_bits: " << containerBits
        ...
        if (containerBytes > f->getMaxContainerBytes()) {
            LOG_DEBUG5(TAB1 "Constraint violation: Violating maximum container bytes allowed for "
                       "field " << f->name);
            return false;
        }
    }
...
bool CoreAllocation::satisfies_constraints(
    ...
    if (parserGroupGress) {
        if ((isExtracted || singleGressParserGroups) && (*parserGroupGress != f->gress)) {
            LOG_DEBUG5(TAB1 "Constraint violation: Container " << c << " has parser group gress "
            ...
            for (auto ef : utils_i.field_to_parser_states.field_to_extracts.at(f)) {
                ...
                for (auto esl :
                    ...
                    if (esl_state == ef_state && ef->source->equiv(*esl->source) &&
                        ...
                        LOG_DEBUG5(TAB1 "Constraint violation: Slices of field " << f << " and "
                                   << container_field << " have same extract source in the same "
                                   "state " << esl_state->name);
                        return false;
                    }
                }
            }
                ...
                for (auto sl : (*cs).slices) {
                    ...
                    for (auto e : utils_i.field_to_parser_states.field_to_extracts.at(sl.field())) {
                        ...
                        if (*other_write_mode == IR::BFN::ParserWriteMode::CLEAR_ON_WRITE) {
                            LOG_DEBUG5(TAB1 "Constraint violation: Container " << c << " has parser"
                                       " write mode " << *write_mode << " but " << other << " has "
                                       "conflicting parser write mode " << *other_write_mode);
                            return false;
                        }
                    }
    ...
    for (auto &sl : slices) {
        ...
        if ((sl.field()->is_solitary() || slice.field()->is_solitary()) && disjoint) {
            ...
            LOG_DEBUG5(TAB1 "Constraint violation: Field " << sl.field()->name << " has solitary "
                       "constraint and is already placed in this container");
            return false;
        } else if (slice.field()->is_solitary() && !disjoint) {
            LOG_DEBUG5(TAB1 "Constraint violation: Slice has solitary constraint, but there are "
...
CoreAllocation::satisfies_constraints(const PHV::ContainerGroup& g, const PHV::SuperCluster& sc) {
    ...
    if (int(g.width()) < sc.max_width()) {
        LOG_DEBUG5(TAB1 "Constraint violation: Container size " << g.width() << " is too small for "
                   "max field width " << sc.max_width());
        return false; }
    ...
    for (auto* slice_list : sc.slice_lists()) {
        ...
        for (auto& slice : *slice_list)
            size += slice.size();
        if (int(g.width()) < size) {
            LOG_DEBUG5(TAB1 "Constraint violation: Container size " << g.width() << " is too small "
                       "for slice list width " << size);
            return false; }
    }
    ...
    for (auto& cnd_slice : candidate_slices) {
        ...
        for (auto& alc_slice : alloc_slices) {
            ...
            for (int stg : write_stages) {
                ...
                if (lr_overlap) {
                    LOG_DEBUG4("Found overlapping liverange between allocated " << alc_slice
                               << " and candidate " << cnd_slice);
                    LOG_DEBUG4(TAB2 "Stopped considering mocha container due to overlapping "
    ...
    for (const auto& slice : slices) {
        if (slice.field()->bit_used_in_wide_arith(slice.range().lo)) {
            if (slice.field()->bit_is_wide_arith_lo(slice.range().lo)) {
                ...
                LOG_DEBUG7(TAB1 "Slice " << slice << " seems to be wide arith lo with linked "
                           "slice list: " << hi_slice);
            } }
...
bool CoreAllocation::generateNewAllocSlices(
    ...
    for (auto& entry : slices) {  // Iterate over the initialization of the new field and the dark
        ...
        for (auto *prim : dest.getInitPrimitive().getARApostPrims()) {
            ...
            if (!sameEarly && sameLate) {
                LOG_DEBUG4(TAB2 "A.Updating earliest liveness for: "
                           << prim->getDestinationSlice());
        ...
        for (auto *prim : dest.getInitPrimitive().getARApriorPrims()) {
            ...
            if (!sameEarly && sameLate) {
                LOG_DEBUG4(TAB2 "B.Updating earliest liveness for: "
                           << prim->getDestinationSlice());
                LOG_DEBUG4(TAB3 "to " << dest.getEarliestLiveness());
                prim->setDestinationEarliestLiveness(dest.getEarliestLiveness());
            }
...
boost::optional<PHV::Transaction> CoreAllocation::try_alloc(
        ...
        const ScoreContext& score_ctx) const {
    if (isClotSuperCluster(utils_i.clot, super_cluster)) {
...
void PHV::AllocUtils::bind_slices(const PHV::ConcreteAllocation& alloc, PhvInfo& phv) {
    for (auto container_and_slices : alloc) {
        ...
        for (PHV::AllocSlice slice : container_and_slices.second.slices) {
            ...
            if (init_points) {
                slice.setMetaInit();
                LOG_DEBUG5(TAB1 "Adding " << init_points->size() << " initialization points for "
                           "field slice: " << slice.field()->name << " " << slice);
...
const IR::Node *AllocatePHV::apply_visitor(const IR::Node* root_, const char *) {
    ...
    for (auto* anno : root->global_pragmas) {
        if (anno->name.name == PragmaParserGroupMonogress::name) {
            ...
            LOG_DEBUG4(TAB1 "FOUND pa_parser_group_monogress pragma");
        }
    }
    ...
    for (const auto& field : phv_i) {
        BUG_CHECK(!utils_i.mutex()(field.id, field.id),
    ...
    for (auto* sc : cluster_groups)
        if (isClotSuperCluster(utils_i.clot, *sc))
            to_remove.insert(sc);
    ...
    if (utils_i.settings.trivial_alloc) {
        ...
        if (!unallocated.empty()) {
            diagnoseFailures(unallocated_list);
...
boost::optional<PHV::Transaction> CoreAllocation::try_deparser_zero_alloc(
    ...
    if (isClotSuperCluster(utils_i.clot, cluster)) {
...
BruteForceAllocationStrategy::tryAllocationFailuresFirst(
    ...
    std::list<PHV::SuperCluster*> cluster_groups =
        remove_unreferenced_clusters(utils_i.uses, cluster_groups_input);
    ...
    if (deparser_zero_superclusters.size() > 0) {
        LOG_DEBUG1("Failed: Deparser Zero field allocation: "
                   << deparser_zero_superclusters.size());
    }
    ...
    auto* packing_opportunities = new FieldPackingOpportunity(
            cluster_groups, utils_i.actions,
            utils_i.uses, utils_i.defuse, utils_i.mutex());
...
BruteForceAllocationStrategy::tryAllocation(
    ...
    if (!succ) {
        LOG_DEBUG1(log_prefix << "Failed: After " << max_try << " tries, "
...
BruteForceAllocationStrategy::sortClusters(std::list<PHV::SuperCluster*>& cluster_groups) {
    ...
    for (auto* cluster : cluster_groups) {
        ...
        cluster->forall_fieldslices([&] (const PHV::FieldSlice& fs) {
            auto* f = fs.field();
            if (utils_i.uses.is_extracted(f) ||
                utils_i.defuse.hasUninitializedRead(f->id)) {
...
TrivialAllocStrategy::PhvStatus::PhvStatus() {
    for (const auto& s : {PHV::Size::b8, PHV::Size::b16, PHV::Size::b32}) {
        next_container_idx[s] = 0;
    }
}

PHV::Container TrivialAllocStrategy::PhvStatus::next_container(PHV::Size s) const {
...
void TrivialAllocStrategy::PhvStatus::inc_next_container(PHV::Size s) {
    BUG_CHECK(next_container_idx.count(s), "cannot find next container for this size: %1%", s);
    next_container_idx[s]++;
}

TrivialAllocStrategy::TrivialAllocStrategy(const PHV::AllocUtils& utils, const CoreAllocation& core,
                                           PhvInfo& phv, int pipe_id)
    : utils_i(utils), core_i(core), phv_i(phv), pipe_id_i(pipe_id) {}

std::vector<PHV::AllocSlice> TrivialAllocStrategy::gen_alloc_slices_from_tx(
...
void TrivialAllocStrategy::bind_alloc_slices(const std::vector<PHV::AllocSlice>& slices) {
    for (const auto& slice : slices) {
        auto* field = phv_i.field(slice.field()->id);
...
TrivialAllocStrategy::PartialAllocResult TrivialAllocStrategy::alloc_clusters(
    PHV::Transaction& rst,
    ...
    for (auto* sc : clusters) {
        ...
        if (is_deparser_zero_sc(sc)) {
            if (auto partial_alloc = core_i.try_deparser_zero_alloc(empty_alloc, *sc, phv_i)) {
                ...
                LOG5("deparser zero allocated: " << sc);
                continue;
            }
        }
...
ordered_set<const PHV::SuperCluster*> TrivialAllocStrategy::allocate(
    ...
    clusters = remove_unreferenced_clusters(utils_i.uses, clusters);

    // XXX(yumin): seems okay to ignore strided headers in trivial alloc.
    // create strided clusters
    // clusters = create_strided_clusters(utils_i.strided_headers, clusters);

    const int max_slicing_try = 256;
    ordered_set<const PHV::SuperCluster*> unallocated_clusters;
