...
convertStartStateToNormalState(const IR::ParserState *state, cstring newName) {
    ...
    auto newState = new IR::ParserState(state->srcInfo, name, annotations,
...
convertStartStateToNormalState(IR::P4Parser *parser, cstring newName) {
    ...
    auto *newState = convertStartStateToNormalState(
            origStartState->to<IR::ParserState>(), newName);
...
const IR::ParserState*
    addNewStartState(cstring name, cstring nextState) {
        auto *startState =
...
const IR::Statement* createAdvanceCall(cstring pkt, int bits) {
    auto *method = new IR::Member(new IR::PathExpression(IR::ID(pkt)),
                                  IR::ID("advance"));
...
const IR::Statement* createSetMetadata(cstring header,
    ...
    auto *member = new IR::Member(new IR::PathExpression(header),
                                  IR::ID(field));
...
const IR::Statement* createSetMetadata(cstring param, cstring header,
                  cstring field, int bitWidth, int constant) {
    auto *member = new IR::Member(new IR::Member(new IR::PathExpression(param),
                                  IR::ID(header)), IR::ID(field));
    auto *value = new IR::Constant(IR::Type::Bits::get(bitWidth), constant);
    return new IR::AssignmentStatement(member, value);
}
