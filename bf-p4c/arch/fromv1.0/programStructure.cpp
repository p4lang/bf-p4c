...
TnaProgramStructure::convertHashAlgorithm(Util::SourceInfo srcInfo, IR::ID algorithm) {
    ...
    if (!mce)
        FATAL_ERROR("hash algorithm %1% is not supported in tofino", algorithm);
    ...
    if (alg_name == "crc_poly") {
        ...
        return new IR::ConstructorCallExpression(srcInfo,
                new IR::Type_Specialized(new IR::Type_Name(mc_name), typeArgs), arguments);
...
const IR::Declaration_Instance*
TnaProgramStructure::convertDirectCounter(const IR::Counter* c, cstring newName) {
    LOG3("Synthesizing " << c);
...
const IR::Declaration_Instance*
TnaProgramStructure::convertDirectMeter(const IR::Meter* c, cstring newName) {
    LOG3("Synthesizing " << c);
    auto annos = addGlobalNameAnnotation(c->name, c->annotations);
    auto args = new IR::Vector<IR::Argument>();
    auto kindarg = counterType(c);
...
const IR::Statement*
TnaProgramStructure::convertMeterCall(const IR::Meter* meterToAccess) {
...
const IR::Statement*
TnaProgramStructure::convertCounterCall(cstring counterToAccess) {
    ExpressionConverter conv(this);
...
const IR::Declaration_Instance*
TnaProgramStructure::convert(const IR::Register* reg, cstring newName,
    ...
    auto type = reg->direct ? new IR::Type_Name("DirectRegister") :
        new IR::Type_Name("Register");
    auto typeArgs = new IR::Vector<IR::Type>();
 ...
 */
IR::BlockStatement *
generate_arch_neutral_hash_block_statement(P4V1::ProgramStructure *structure,
 ...
 */
IR::BlockStatement *
generate_tna_hash_block_statement(P4V1::TnaProgramStructure* structure,
        const IR::Primitive *prim,
        const cstring temp, ExpressionConverter &conv, unsigned num_ops,
    ...
    if (!dest) {
        ...
        if (dest->type->width_bits() > ttype->width_bits()) {
            ...
            block->push_back(new IR::AssignmentStatement(dest,
                new IR::Concat(new IR::Constant(IR::Type::Bits::get(padding_width), 0),
                               methodCall)));
        } else if (dest->type->width_bits() < ttype->width_bits()) {
            block->push_back(new IR::AssignmentStatement(dest,
                        new IR::Slice(methodCall, dest->type->width_bits()-1, 0)));
        } else {
...
// which is not extracted from the frontend function call.
const IR::P4Table*
TnaProgramStructure::convertTable(const IR::V1Table* table, cstring newName,
                               IR::IndexedVector<IR::Declaration> &stateful,
                               std::map<cstring, cstring> &mapNames) {
    // Generate p4Table from frontend call
    auto p4Table = ProgramStructure::convertTable(table, newName, stateful, mapNames);
...
IR::ConstructorCallExpression*
TnaProgramStructure::createHashExtern(const IR::Expression* expr, const IR::Type* ttype) {
...
const IR::Declaration_Instance*
TnaProgramStructure::convertActionProfile(const IR::ActionProfile* action_profile,
        cstring newName) {
    ...
    if (action_selector) {
        ...
        if (action_selector->mode == "resilient") {
            args->push_back(new IR::Argument(
                        new IR::Member(new IR::TypeNameExpression("SelectorMode_t"),
                            "RESILIENT")));
        } else {
            args->push_back(new IR::Argument(
                        new IR::Member(new IR::TypeNameExpression("SelectorMode_t"),
...
TnaProgramStructure::controlType(IR::ID name) {
...
const IR::Declaration_Instance*
TnaProgramStructure::convert(const IR::CounterOrMeter* cm, cstring newName) {
...
class FindHeaderReference : public Inspector {
 ...
 public:
    ...
    void postorder(const IR::V1Table* tbl) override {
        ...
        auto *action_selector = action_profile ?
            structure->action_selectors.get(action_profile->selector.name) : nullptr;
...
TnaProgramStructure::createApplyArguments(cstring name) {
    auto args = new IR::Vector<IR::Argument>();
...
std::vector<cstring>
TnaProgramStructure::findActionInTables(const IR::V1Control* control) {
...
TnaProgramStructure::convertParser(const IR::V1Parser* parser,
        IR::IndexedVector<IR::Declaration>* stateful) {
    ExpressionConverter conv(this);
...
const IR::ParserState*
TnaProgramStructure::createMirrorState(gress_t gress, unsigned index,
        const IR::Expression* expr, cstring headerType, cstring nextState) {
...
IR::IndexedVector<IR::ParserState>*
TnaProgramStructure::createMirrorStates() {
    auto states = new IR::IndexedVector<IR::ParserState>();
...
const IR::ParserState*
TnaProgramStructure::createResubmitState(gress_t gress, unsigned index,
        const IR::Expression* expr, cstring headerType, cstring nextState) {
    auto statements = new IR::IndexedVector<IR::StatOrDecl>();
...
IR::IndexedVector<IR::ParserState>*
TnaProgramStructure::createResubmitStates() {
    auto states = new IR::IndexedVector<IR::ParserState>();
    auto selectCases = new IR::Vector<IR::SelectCase>();
...
TnaProgramStructure::createEgressParserStates() {
    ...
    if (useBridgeMetadata()) {
        ...
        for (auto f : bridgedFields) {
            ...
            auto info = bridgedFieldInfo.at(f);
            auto fn = convertLinearPathToStructFieldName(&info);
            auto* member = new IR::Member(new IR::Member(new IR::PathExpression("meta"),
                        IR::ID("bridged_header")), IR::ID(fn));
            auto* bridgedMember = info.components.back()->apply(cloner);
            bridgedMember = bridgedMember->apply(ConvertConcreteHeaderRefToPathExpression());
...
const IR::StatOrDecl*
TnaProgramStructure::createBridgeEmit() {
...
void TnaProgramStructure::parseUpdateLocationAnnotation(
        std::set<gress_t>& updateLocations, const IR::Annotation* annot, cstring pragma) {
...
void TnaProgramStructure::createChecksumUpdateStatements(
    ...
    forAllChecksums(calculated_fields,
            [&](const IR::CalculatedField* cf, const IR::FieldListCalculation* flc,
                const IR::CalculatedField::update_or_verify uov, const IR::FieldList* fl) {
        ...
        auto method = new IR::Member(
                new IR::PathExpression(checksumInstName), IR::ID("update"));
        auto args = new IR::Vector<IR::Argument>();
        args->push_back(new IR::Argument(le));

        for (auto name : flc->algorithm->names)
            if (name == "csum16_udp")
                args->push_back(new IR::Argument(new IR::BoolLiteral(1)));
        ...
        if (condition)
            mc = new IR::IfStatement(condition, mc, nullptr);
        ...
        if (cinfo.deparserUpdateLocations.count(gress))
            controlLocals.push_back(checksumInst);
...
void TnaProgramStructure::collectBridgedFieldsInControls() {
    ...
    for (auto s : reachableFromEgress) {
        ...
        auto p4ctrl = ProgramStructure::convertControl(v1ctrl, s);
        p4ctrl = p4ctrl->apply(ConvertMetadata(this));
        p4ctrl->apply(CollectBridgedFields(EGRESS,
                    mayWrite[EGRESS], mayRead[EGRESS],
                    bridgedFieldInfo));
    }
...
bool TnaProgramStructure::useBridgeMetadata() {
    ...
    forAllResidualChecksums(calculated_fields, [&](const IR::FieldList*) {
        useResidualChecksum = true;
    });
...
void TnaProgramStructure::collectBridgedFieldsInParsers() {
    ...
    for (auto s : reachableFromStartState) {
        ...
        v1parser->apply(CollectBridgedFields(EGRESS,
                    mayWrite[EGRESS], mayRead[EGRESS],
                    bridgedFieldInfo));
    }

    for (auto& field : mayRead[EGRESS]) {
        if (field.startsWith("ig_intr_md"))
            bridgedFields.insert(field);
    }
...
void TnaProgramStructure::getStructFieldsFromFieldList(
        IR::IndexedVector<IR::StructField>* fields, const IR::ListExpression* expr) {
...
void TnaProgramStructure::createType(cstring typeName, cstring headerName) {
    ...
    if (tna_intr_md_types.count(typeName)) {
        if (types.get(typeName))
            BUG("Cannot create type %1%, already exist!", typeName);
...
bool CollectBridgedFields::preorder(const IR::Member* expr) {
    ...
    if (exprIsRead)
        LOG3("expr " << expr << " is read");
    if (exprIsWrite)
        LOG3("expr " << expr << " is written");
...
unsigned long computeHashOverFieldList(const IR::Expression* expr,
        std::map<unsigned long, unsigned>& hashIndexMap, bool reserve_entry_zero) {
...
IR::Node* FixParserPriority::preorder(IR::AssignmentStatement* assign) {
    ...
    while (auto slice = left->to<IR::Slice>())
        left = slice->e0;
    while (auto cast = left->to<IR::Cast>())
        left = cast->expr;
    ...
    if (!path)
        return assign;
    if (path->to_cstring() == "ig_prsr_ctrl.priority")
        return nullptr;
    return assign;
}

const IR::P4Program*
TnaProgramStructure::create(Util::SourceInfo info) {
    ////////////////////////////////////////////////////////////////////
...
TnaConverter::TnaConverter() {
    setStopOnError(true); setName("TnaConverter");
