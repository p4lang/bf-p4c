...
class FixParamType : public Transform {
    const IR::Node* postorder(IR::Parameter* param) override {
        ...
        if (tn->path->name == "ingress_intrinsic_metadata_from_parser_aux_t")
            return new IR::Parameter(param->name, param->direction,
...
class ConvertConcreteHeaderRefToPathExpression : public Transform {
    const IR::Node* postorder(IR::ConcreteHeaderRef* href) override {
        ...
        auto expr = new IR::PathExpression(href->srcInfo,
                href->type, new IR::Path(href->ref->name));
        return expr;
    }
};
...
class TnaProgramStructure : public ProgramStructure {
    ...
    const IR::Declaration_Instance*
        convertActionProfile(const IR::ActionProfile* action_profile, cstring newName) override;

    // counter and meter
    const IR::Declaration_Instance*
        convertDirectCounter(const IR::Counter* c, cstring newName) override;
    const IR::Declaration_Instance*
        convertDirectMeter(const IR::Meter* c, cstring newName) override;
    const IR::Statement*
        convertMeterCall(const IR::Meter* m) override;
    const IR::Statement*
        convertCounterCall(cstring c) override;
    const IR::Expression* counterType(const IR::CounterOrMeter* cm) override;
    const IR::Declaration_Instance*
        convert(const IR::Register* reg, cstring newName, const IR::Type *regElementType) override;
    ...
    const IR::ParserState* createMirrorState(gress_t, unsigned,
            const IR::Expression*, cstring, cstring);
    const IR::ParserState* createResubmitState(gress_t, unsigned,
            const IR::Expression*, cstring, cstring);
    const IR::SelectCase* createSelectCase(gress_t gress,
            unsigned digestId, const IR::ParserState* newState);
    ...
    void createChecksumVerifyStatements(
            gress_t gress);
 ...
 public:
    ...
    void forAllResidualChecksums(
            std::vector<const IR::CalculatedField*> calculated_fields, Func function) {
...
class ConvertMetadata : public Transform {
    ...
    const IR::Member* mkMember(cstring meta, cstring strct, cstring field, int width) {
        auto mem = new IR::Member(new IR::Member(
                    new IR::PathExpression(IR::ID(meta)), strct), field);
...
unsigned long computeHashOverFieldList(const IR::Expression* expr,
        std::map<unsigned long, unsigned>& hm, bool reserve_entry_zero = false);
 ...
 public:
    ...
    IR::Node* preorder(IR::Member* member) {
        ...
        if (auto hdr = path->components.front()->to<IR::PathExpression>()) {
            if (paths.count(hdr->path->name)) {
                auto meta = new IR::Member(new IR::Member(new IR::PathExpression(prefix),
                            IR::ID(hdr->path->name)), member->member);
            ...
            if (paths.count(hdr->ref->name)) {
                auto meta = new IR::Member(new IR::Member(new IR::PathExpression(prefix),
                            IR::ID(hdr->ref->name)), member->member);
                LOG3("adding " << prefix << " to " << member);
                return meta;
            }
        }
    ...
    IR::Node* preorder(IR::PathExpression* path) {
        ...
        if (paths.count(path->path->name)) {
            auto meta = new IR::Member(new IR::PathExpression(prefix),
                    IR::ID(path->path->name));
...
class ParserCounterSelectCaseConverter : public Transform {
 ...
 public:
    ...
    const IR::Node* preorder(IR::AssignmentStatement* assign) {
        ...
        auto path = *linearizer->linearPath;
        if (path.to_cstring() != "ig_prsr_ctrl.parser_counter")
            return assign;
        ...
        if (right->is<IR::Sub>())
            right = right->apply(StrengthReduction());
        ...
        if (right->to<IR::Constant>() || right->to<IR::Member>()) {
            ...
            auto maskWidth = IR::Type::Bits::get(
                    Device::currentDevice() == Device::TOFINO ? 3 : 8);
                    ...
                    if (member) {
                        ...
                        if (auto* rot = shl->right->to<IR::Constant>()) {
                            ...
                            if (auto* field = left->to<IR::Member>()) {
                                ...
                                auto shr = new IR::Constant(counterWidth,
                                        8 - rot->asUnsigned() - lo);
...
struct ParserCounterSelectExprConverter : Transform {
    ...
    const IR::Node* postorder(IR::P4Parser* parser) {
        if (need_parser_counter.count(parser->name) != 0) {
            ...
            return new IR::P4Parser(parser->srcInfo, parser->name,
                    parser->type, parser->constructorParams,
                    *locals, parser->states); }
        return parser; }
};
 ...
 public:
    ...
    const IR::Node* preorder(IR::Property* prop) override {
        if (prop->name == "support_timeout") {
            return new IR::Property("idle_timeout", prop->annotations,
                    prop->value, prop->isConstant);
        }
        return prop;
    }
};
...
class ModifyParserForChecksum : public Modifier {
 ...
 public:
    ...
    collectResidualChecksumPayloadFields() {
        ...
        for (auto d : descendants) {
            forAllExtracts(d, [&](const IR::Member* member) {
                rv.insert(member);
            });
        }
        return rv;
    ...
    void postorder(IR::ParserState* state) override {
        ...
        forAllExtracts(state, [&](const IR::Member* member) {
            createAddCallsForVerifyChecksum(addCalls,
                structure->verifyChecksums, member, checksum, gress, state->name);
        });
        forAllExtracts(state, [&](const IR::Member* member) {
        ...
        if (addCalls->size() != 0)
            state->components.append(*addCalls);
    ...
    void postorder(IR::MethodCallExpression* mce) override {
        ...
        if (em->actualExternType->name != "Checksum" || em->method->name != "update")
            return;
        ...
        if (structure->residualChecksums.empty())
            return;
...
class InsertChecksumError : public PassManager {
 public:
    ...
    struct ComputeEndStates : public Inspector {
        ...
        EndStates& endStates;
        explicit ComputeEndStates(TnaProgramStructure* structure,
            ...
            for (auto& kv : endStates[parser->name]) {
                ...
                for (auto endState : kv.second) {
                    if (endState == state->name) {
                        ...
                        LOG3("verify " << toString(parser->name) << " "
                             << decl << " in state " << endState);
                    }
                }
            }
            return state;
    ...
    explicit InsertChecksumError(TnaProgramStructure* structure,
            const P4ParserGraphs* graph) {
        addPasses({new ComputeEndStates(structure, graph, endStates),
                   new InsertBeforeAccept(structure, endStates),
                   new InsertEndStates(endStates)});
    }
};
...
IR::BlockStatement *generate_tna_hash_block_statement(
        P4V1::TnaProgramStructure * structure, const IR::Primitive *prim,
        const cstring temp, ExpressionConverter &conv, unsigned num_ops,
