...
class Node {
 public:
  ...
  explicit Node(UniqueId uniq_id, int id) :
    unique_id_(uniq_id), id_(id) {}
  ~Node() {
...
    */
  ...
  /**
    ...
    */
  double get_tables_on_worst_path(const std::map<UniqueId, PowerMemoryAccess>& table_memory_access,
                                  std::set<Node*>& rv);
