...
class  SaluOredIf : public Inspector {
    ...
    const P4::IR::Expression *top_if = nullptr;
    const P4::IR::AssignmentStatement *top_st = nullptr;

    // Second level-if condition
    const P4::IR::Expression *second_if = nullptr;
    const P4::IR::AssignmentStatement *second_st = nullptr;

    // Third level-if condition
    const P4::IR::Expression *third_if = nullptr;
    const P4::IR::AssignmentStatement *third_st = nullptr;
    ...
    void analyze_pattern() {
        ...
        auto land_node = top_if->to<P4::IR::LAnd>();
        if (!land_node) return;
        ...
        auto or_st = top_st->right->to<P4::IR::BOr>();
        if (!or_st) return;
 ...
 public:
    bool preorder(const P4::IR::IfStatement *if_node) override {
        ...
        if (!top_if) {
            ...
            top_st = if_node->ifTrue->to<P4::IR::AssignmentStatement>();
            ...
            second_st = if_node->ifTrue->to<P4::IR::AssignmentStatement>();
            ...
            third_st = if_node->ifTrue->to<P4::IR::AssignmentStatement>();
...
static unsigned eval_predicate(const P4::IR::Expression *exp) {
    if (auto *land = exp->to<P4::IR::LAnd>())
        return eval_predicate(land->left) & eval_predicate(land->right);
    if (auto *lor = exp->to<P4::IR::LOr>())
        return eval_predicate(lor->left) | eval_predicate(lor->right);
    if (auto *lnot = exp->to<P4::IR::LNot>())
        return ~eval_predicate(lnot->expr);
    if (auto *reg = exp->to<P4::IR::MAU::SaluCmpReg>())
...
static bool equiv(const P4::IR::Expression *a, const P4::IR::Expression *b) {
    if (*a == *b) return true;
    if (typeid(*a) != typeid(*b)) return false;
    if (auto ma = a->to<P4::IR::Member>()) {
        auto mb = b->to<P4::IR::Member>();
        return ma->member == mb->member && equiv(ma->expr, mb->expr); }
    if (auto ua = a->to<P4::IR::Operation::Unary>()) {
        auto ub = b->to<P4::IR::Operation::Unary>();
        return equiv(ua->expr, ub->expr); }
    if (auto ea = a->to<P4::IR::Operation::Binary>()) {
        auto eb = b->to<P4::IR::Operation::Binary>();
        return equiv(ea->left, eb->left) && equiv(ea->right, eb->right); }
    if (auto ea = a->to<P4::IR::Operation_Ternary>()) {
        auto eb = b->to<P4::IR::Operation_Ternary>();
        return equiv(ea->e0, eb->e0) && equiv(ea->e1, eb->e1) && equiv(ea->e2, eb->e2); }
    if (auto pa = a->to<P4::IR::PathExpression>()) {
        auto pb = b->to<P4::IR::PathExpression>();
        return pa->path->name == pb->path->name; }
    if (auto ka = a->to<P4::IR::Constant>()) {
        auto kb = b->to<P4::IR::Constant>();
        return ka->value == kb->value; }
    if (auto ka = a->to<P4::IR::BoolLiteral>()) {
        auto kb = b->to<P4::IR::BoolLiteral>();
        return ka->value == kb->value; }
    if (auto ea = a->to<P4::IR::MAU::IXBarExpression>()) {
        auto eb = b->to<P4::IR::MAU::IXBarExpression>();
        return equiv(ea->expr, eb->expr); }
    if (auto sa = a->to<P4::IR::MAU::SaluReg>()) {
        auto sb = b->to<P4::IR::MAU::SaluReg>();
        return sa->equiv(*sb); }
    if (auto sa = a->to<P4::IR::MAU::SaluRegfileRow>()) {
        auto sb = b->to<P4::IR::MAU::SaluRegfileRow>();
        return sa->equiv(*sb); }
    return false;
}

static bool equiv(const P4::IR::Vector<P4::IR::Expression> *a, const P4::IR::Vector<P4::IR::Expression> *b) {
...
bool P4::IR::MAU::StatefulAlu::alu_output() const {
...
static bool is_address_output(const P4::IR::Expression *e) {
    if (auto *r = e->to<P4::IR::MAU::SaluReg>())
        return r->name == "address";
    return false;
}

static bool is_learn(const P4::IR::Expression *e) {
    if (auto *r = e->to<P4::IR::MAU::SaluReg>())
        return r->name == "learn";
    return false;
}
...
bool CreateSaluInstruction::applyArg(const P4::IR::PathExpression *pe, cstring field) {
    Log::TempIndent indent;
    LOG4("applyArg(" << pe << ", " << field << ") etype = " << etype << indent);
    assert(dest == nullptr || !islvalue(etype));
    const P4::IR::Expression *e = nullptr;
    ...
    if (field && regtype->is<P4::IR::Type_StructLike>() && argType->equiv(*regtype)) {
        BUG_CHECK(field_idx == 0 || (local && local->use == LocalVar::NONE),
                  "invalid reuse of local %s.%s", pe, field);
        field_idx = 0;
        for (auto f : regtype->to<P4::IR::Type_StructLike>()->fields) {
            if (f->name == field) {
                argType = f->type;
                break; }
            ++field_idx; }
    ...
    case param_t::VALUE:        /* inout value or local var */
        ...
        if (local && local->use == LocalVar::REGFILE) {
            ...
            e = new P4::IR::MAU::SaluReg(pe->srcInfo, argType, name, field_idx > 0);
        }
        break;
    ...
    case param_t::HASH:         /* in digest */
        if (islvalue(etype)) {
            error(ErrorType::ERR_UNSUPPORTED, "Writing in param %s in %s not supported", pe,
                  action_type_name);
            return false; }
        e = new P4::IR::MAU::SaluReg(pe->srcInfo, argType, "phv_" + name, field_idx > 0);
        break;
    case param_t::LEARN:        /* in learn */
        if (islvalue(etype)) {
            error(ErrorType::ERR_UNSUPPORTED, "Writing in param %s in %s not supported", pe,
                  action_type_name);
            return false; }
        e = new P4::IR::MAU::SaluReg(pe->srcInfo, argType, "learn"_cs, false);
        break;
...
bool CreateSaluInstruction::canBeIXBarExpr(const P4::IR::Expression *e) {
    return CanBeIXBarExpr(e, [this](const P4::IR::PathExpression *pe) -> bool
...
void CreateSaluInstruction::clearFuncState() {
    ...
    math = P4::IR::MAU::StatefulAlu::MathUnit();
...
bool CreateSaluInstruction::preorder(const P4::IR::Function *func) {
    ...
    LOG5(func);
    action = new P4::IR::MAU::SaluAction(func->srcInfo, name, func);
    ...
    for (auto i = 1U; i < params->parameters.size(); ++i) {
        if (auto rt = params->parameters.at(i)->type->to<P4::IR::Type_Enum>()) {
            if (return_encoding)
                error(ErrorType::ERR_UNSUPPORTED,
                      "%1%: Multiple enum return values are not supported", func);
            action->return_encoding = return_encoding =
                new P4::IR::MAU::SaluAction::ReturnEnumEncoding(rt);
            return_enum_word = out_word; }
        if (params->parameters.at(i)->direction == P4::IR::Direction::Out)
            out_word++; }
    return true;
}

void CreateSaluInstruction::postorder(const P4::IR::Function *func) {
    ...
    for (auto &kv : output_address_subword_predicate) {
        ...
        output->operands.push_back(new P4::IR::MAU::SaluFunction(kv.second, "lmatch"_cs)); }
        ...
        if (instr) {
            if (Device::statefulAluSpec().OutputWords > 1) {
                ...
                instr->operands.insert(
                    ...
                    new P4::IR::MAU::SaluReg(instr->operands.front()->type,
    ...
    if (action->action.empty()) {
        ...
        action->action.push_back(new P4::IR::MAU::SaluInstruction("nop"_cs));
...
void CreateSaluInstruction::doAssignment(const Util::SourceInfo &srcInfo) {
    ...
    if (etype == IF && operands.empty() && pred_operands.size() == 1) {
        ...
        if (old_predicate)
            predicate = new P4::IR::LAnd(old_predicate, predicate);
        etype = OUTPUT;
        operands.push_back(new P4::IR::Constant(1)); }
    ...
    if (dest && dest->use != LocalVar::ALUHI) {
        ...
        if (auto src = operands.back()->to<P4::IR::MAU::SaluReg>()) {
            if (dest->pair) {
                use = LocalVar::MEMALL;
            } else if (src->hi) {
                use = LocalVar::MEMHI;
                ...
                use = LocalVar::MEMLO; }
        } else if (operands.back()->is<P4::IR::MAU::SaluRegfileRow>()) {
    ...
    if (!dest) {
        if (etype == OUTPUT_ALUHI) {
            ...
            operands.insert(operands.begin(),
                            new P4::IR::MAU::SaluReg(val->srcInfo, val->type, "hi"_cs, true));
            createInstruction();
            operands.clear();
            operands.push_back(new P4::IR::MAU::SaluReg(val->srcInfo, val->type, "alu_hi"_cs, true));
            etype = OUTPUT; }
        createInstruction();
...
void CreateSaluInstruction::captureAssigstateProps() {
    ...
    auto assig = findContext<P4::IR::AssignmentStatement>(assignCtxt);
...
void CreateSaluInstruction::checkWriteAfterWrite() {
    ...
    if (elem == written_dest.end()) {
        bool zero_assigned = assig_st->right->is<P4::IR::Constant>() &&
                             assig_st->right->to<P4::IR::Constant>()->value == 0;
        bool false_assigned = assig_st->right->is<P4::IR::BoolLiteral>() &&
                             assig_st->right->to<P4::IR::BoolLiteral>()->value == false;
...
bool CreateSaluInstruction::preorder(const P4::IR::AssignmentStatement *as) {
...
static const P4::IR::Expression *negatePred(const P4::IR::Expression *e) {
    if (auto a = e->to<P4::IR::LAnd>())
        return new P4::IR::LOr(e->srcInfo, negatePred(a->left), negatePred(a->right));
    if (auto a = e->to<P4::IR::LOr>())
        return new P4::IR::LAnd(e->srcInfo, negatePred(a->left), negatePred(a->right));
    if (auto a = e->to<P4::IR::LNot>())
        return a->expr;
    return new P4::IR::LNot(e);
}

bool CreateSaluInstruction::preorder(const P4::IR::IfStatement *s) {
    ...
    pred_operands.clear();
    predicate = old_predicate ? new P4::IR::LAnd(new_predicate, old_predicate) : new_predicate;
...
bool CreateSaluInstruction::preorder(const P4::IR::Mux *mux) {
    struct {
        etype_t                         etype;
        LocalVar                        *dest;
        P4::IR::Vector<P4::IR::Expression>      operands;
...
void CreateSaluInstruction::doPrimary(const P4::IR::Expression *e, const P4::IR::PathExpression *pe,
                                      cstring field) {
    if (!pe || !applyArg(pe, field)) {
        if (negate) e = new P4::IR::Neg(e);
        operands.push_back(e);
        LOG4("primary operand: " << operands.back()); }
    auto *reg = operands.empty() ? nullptr : operands.back()->to<P4::IR::MAU::SaluReg>();
    if (etype == IF && (!reg || reg->name != "learn") && e->type->is<P4::IR::Type::Boolean>() &&
        !getParent<P4::IR::Operation::Relation>()) {
        operands.push_back(new P4::IR::Constant(0));
        setupCmp("neq"_cs);
        LOG4("  -- adding != 0 comparison"); }
}

bool CreateSaluInstruction::preorder(const P4::IR::PathExpression *pe) {
    doPrimary(pe, pe, cstring());
    return false;
}

bool CreateSaluInstruction::preorder(const P4::IR::Member *mem) {
    doPrimary(mem, mem->expr->to<P4::IR::PathExpression>(), mem->member);
    return false;
}

bool CreateSaluInstruction::preorder(const P4::IR::StructExpression *se) {
    P4::IR::Vector<P4::IR::Expression> copy = operands;
...
bool CreateSaluInstruction::preorder(const P4::IR::Constant *c) {
...
bool CreateSaluInstruction::preorder(const P4::IR::BoolLiteral *bl) {
    if (etype == IF && bl->value == 0)
        return false;
    auto *c = new P4::IR::Constant(bl->srcInfo, bl->value ? negate ? -1 : 1 : 0);
    LOG4("Constant operand: " << c);
    operands.push_back(c);
    return false;
}
...
static bool checkSlice(const P4::IR::Slice *sl, unsigned width) {
    return sl->getL() == 0 && sl->getH() == width - 1;
}

bool CreateSaluInstruction::preorder(const P4::IR::Slice *sl) {
    ...
    if (etype == NONE) {
        ...
        keep_slice = true; }
    if (etype == IF) {
        const P4::IR::PathExpression *pe = nullptr;
        if (auto mem = sl->e0->to<P4::IR::Member>()) {
            pe = mem->expr->to<P4::IR::PathExpression>();
        } else {
            pe = sl->e0->to<P4::IR::PathExpression>();
        }
        if (pe) {
            for (auto p : params->parameters) {
                if (p->name == pe->path->name) {
    ...
    if (keep_slice) {
        ...
        if (save_negate)
            operands.back() = new P4::IR::Neg(operands.back());
    }
    negate = save_negate;
    return false;
}
...
bool CreateSaluInstruction::preorder(const P4::IR::MAU::Primitive *prim) {
    cstring method;
    ...
    if (prim->name == "min" || prim->name == "max") {
        ...
        if (operands.size() > 0) {
            if (auto *reg = operands.at(0)->to<P4::IR::MAU::SaluReg>()) {
                if (reg->hi)
        ...
        auto mu = gref ? gref->obj->to<P4::IR::Declaration_Instance>() : nullptr;
        BUG_CHECK(mu, "typechecking failure?");
        math.valid = true;
        unsigned i = mu->arguments->size()-1;
        BUG_CHECK(i >= 1 && i <= 3, "typechecking failure");
        if (auto k = mu->arguments->at(0)->expression->to<P4::IR::BoolLiteral>()) {
            math.exp_invert = k->value;
            if (auto k = mu->arguments->at(1)->expression->to<P4::IR::Constant>())
                math.exp_shift = k->asInt();
            BUG_CHECK(i == 3, "typechecking failure");
        } else if (auto m = mu->arguments->at(0)->expression->to<P4::IR::Member>()) {
            BUG_CHECK(i < 3, "typechecking failure");
            if (m->member == "MUL") {
                math.exp_shift = 0;
        ...
        if (mu->arguments->at(i)->expression->is<P4::IR::ListExpression>() ||
                mu->arguments->at(i)->expression->is<P4::IR::StructExpression>()) {
            BUG_CHECK(i >= 2, "typechecking failure");
            if (auto k = mu->arguments->at(i-1)->expression->to<P4::IR::Constant>())
                math.scale = k->asInt();
            else
            ...
            for (auto e : *components) {
                if (auto k = e->to<P4::IR::Constant>())
                    math.table[i] = k->asInt();
                --i; }
        } else if (auto k = mu->arguments->at(i)->expression->to<P4::IR::Constant>()) {
            double val = k->asUint64();
            BUG_CHECK(i == 1 || i == 2, "typechecking failure");
            if (i == 2) {
                if ((k = mu->arguments->at(i-1)->expression->to<P4::IR::Constant>()))
                    val = k->asUint64() / val;
                else
        ...
        operands.push_back(new P4::IR::MAU::SaluReg(prim->srcInfo, prim->type, "address"_cs, false));
        address_subword = 0;
        if (prim->operands.size() == 2) {
            auto k = prim->operands.at(1)->to<P4::IR::Constant>();
        ...
        for (int i = 1; i < int(prim->operands.size()); ++i) {
            ...
            if (auto *b = prim->operands.at(i)->to<P4::IR::BoolLiteral>()) {
                operands.clear();
                operands.push_back(new P4::IR::Constant(0));
            ...
            } else if (auto *cmpreg = pred_operands.at(0)->to<P4::IR::MAU::SaluCmpReg>()) {
                if (cmpreg->index != i-1)
        ...
        operands.clear();
        etype = OUTPUT;
        operands.push_back(new P4::IR::MAU::SaluReg(prim->type, "predicate"_cs, false));
        ...
        etype = OUTPUT;
        operands.push_back(new P4::IR::MAU::SaluReg(prim->type, "minmax"_cs, false));
...
bool CreateSaluInstruction::preorder(const P4::IR::MAU::SaluRegfileRow *regfile) {
    const P4::IR::Expression *e = regfile;
    if (negate)
        e = new P4::IR::Neg(e);
    negate_regfile = negate;
...
const P4::IR::Expression *CreateSaluInstruction::reuseCmp(const P4::IR::MAU::SaluInstruction *cmp,
                                                            int idx) {
    ...
    if (opcode == cmp->name)
        return new P4::IR::MAU::SaluCmpReg(name, idx);
    if (negate_op.at(opcode) == cmp->name)
        return new P4::IR::LNot(new P4::IR::MAU::SaluCmpReg(name, idx));
    return nullptr;
}
...
void CreateSaluInstruction::setupCmp(cstring op) {
    ...
    operands.insert(operands.begin(), new P4::IR::MAU::SaluCmpReg(name, idx));
    cmp_instr.push_back(createInstruction());
    if (!name.startsWith("cmp")) name = "cmp" + name;
    pred_operands.push_back(new P4::IR::MAU::SaluCmpReg(name, idx));
...
static bool can_be_uus_cmp(const P4::IR::Operation::Relation *rel, const P4::IR::Type::Bits *type) {
    if (!type->isSigned) return false;
    if (rel->left->is<P4::IR::Add>() || rel->right->is<P4::IR::Add>() ||
        rel->left->is<P4::IR::Sub>() || rel->right->is<P4::IR::Sub>()) {
...
bool CreateSaluInstruction::preorder(const P4::IR::Operation::Relation *rel, cstring op, bool eq) {
    ...
    if (etype == IF) {
        ...
        Pattern::Match<P4::IR::Constant> k;
        if (Device::statefulAluSpec().CmpMask &&
            ...
            if (!eq) {
                ...
                if (auto t = rel->left->type->to<P4::IR::Type::Bits>()) {
...
bool CreateSaluInstruction::preorder(const P4::IR::Cast *c) {
    if (c->type->width_bits() > c->expr->type->width_bits() && canBeIXBarExpr(c)) {
        operands.push_back(new P4::IR::MAU::IXBarExpression(c));
        if (negate)
            operands.back() = new P4::IR::Neg(operands.back());
        return false; }
    return true;
...
bool CreateSaluInstruction::preorder(const P4::IR::BFN::SignExtend *c) {
    if (canBeIXBarExpr(c)) {
        operands.push_back(new P4::IR::MAU::IXBarExpression(c));
        if (negate)
            operands.back() = new P4::IR::Neg(operands.back());
...
void CreateSaluInstruction::postorder(const P4::IR::Cast *c) {
    if (etype == IF && c->type->to<P4::IR::Type::Boolean>())
        setupCmp("neq"_cs);
}

void CreateSaluInstruction::postorder(const P4::IR::BFN::ReinterpretCast *c) {
    if (etype == IF && c->type->to<P4::IR::Type::Boolean>())
        setupCmp("neq"_cs);
}

inline void condition_error(const P4::IR::Node *node, const char *operation) {
    error(ErrorType::ERR_UNSUPPORTED, "%sIn Stateful ALU, %s can only be used in a condition.",
          node->srcInfo, operation);
}
inline void only_operation_error(const P4::IR::Node *node, const char *operation,
...
void CreateSaluInstruction::postorder(const P4::IR::LNot *e) {
    if (etype == IF) {
...
void CreateSaluInstruction::postorder(const P4::IR::LAnd *e) {
    if (etype == IF) {
        ...
        pred_operands.back() = new P4::IR::LAnd(e->srcInfo, pred_operands.back(), r);
...
void CreateSaluInstruction::postorder(const P4::IR::LOr *e) {
    if (etype == IF) {
        ...
        auto r = pred_operands.back();
        pred_operands.pop_back();
        pred_operands.back() = new P4::IR::LOr(e->srcInfo, pred_operands.back(), r);
...
bool CreateSaluInstruction::preorder(const P4::IR::Add *e) {
...
bool CreateSaluInstruction::preorder(const P4::IR::AddSat *e) {
    checkAndReportComplexInstruction(e);
    if (etype == VALUE || (etype == OUTPUT && outputAluHi())) {
...
bool CreateSaluInstruction::preorder(const P4::IR::Sub *e) {
    checkAndReportComplexInstruction(e);
    if (etype == IF) {
        visit(e->left, "left");
        negate = !negate;
...
bool CreateSaluInstruction::preorder(const P4::IR::SubSat *e) {
    checkAndReportComplexInstruction(e);
    if (etype == VALUE || (etype == OUTPUT && outputAluHi())) {
...
void CreateSaluInstruction::postorder(const P4::IR::BAnd *e) {
    checkAndReportComplexInstruction(e);
    if (etype == VALUE || etype == OUTPUT_ALUHI) {
        if (e->left->is<P4::IR::Cmpl>())
            opcode = "andca"_cs;
        else if (e->right->is<P4::IR::Cmpl>())
            opcode = "andcb"_cs;
        else
            opcode = "and"_cs;
    } else if (etype == IF) {
        ...
        operands.pop_back();
        if (!r->is<P4::IR::Constant>()) {
            if (!operands.back()->is<P4::IR::Constant>())
                error("%s: mask operand must be a constant", r->srcInfo);
            std::swap(r, operands.back()); }
        operands.back() = new P4::IR::BAnd(e->srcInfo, operands.back(), r);
        LOG4("BAnd rewrite opeands: " << operands.back());
    } else {
        only_operation_error(e, "conjunction", salu); }
}

void CreateSaluInstruction::checkAndReportComplexInstruction(const P4::IR::Operation_Binary* op) const {
...
bool CreateSaluInstruction::isComplexInstruction(const P4::IR::Operation_Binary *op) const {
    ...
    //
    // P4::IR::L* operators (LAnd, LOr, etc.) are working with predicates

    bool ret = false;
    for (auto oper : {op->left, op->right}) {
        ret |= oper->is<P4::IR::Add>()  | oper->is<P4::IR::AddSat>();
        ret |= oper->is<P4::IR::Sub>()  | oper->is<P4::IR::SubSat>();
        ret |= oper->is<P4::IR::BAnd>() | oper->is<P4::IR::BOr>() | oper->is<P4::IR::BXor>();
        ret |= oper->is<P4::IR::Div>()  | oper->is<P4::IR::Mod>();
        ret |= oper->is<P4::IR::Neg>();
    }

    return ret;
}

bool CreateSaluInstruction::preorder(const P4::IR::Neg *) {
    if (etype == OUTPUT && outputAluHi()) etype = OUTPUT_ALUHI;
    return true;
}

bool CreateSaluInstruction::preorder(const P4::IR::BAnd *) {
    if (etype == OUTPUT && outputAluHi()) etype = OUTPUT_ALUHI;
    return true;
}

bool CreateSaluInstruction::preorder(const P4::IR::BXor *) {
    if (etype == OUTPUT && outputAluHi()) etype = OUTPUT_ALUHI;
    return true;
}

bool CreateSaluInstruction::preorder(const P4::IR::BOr *e) {
    if (etype == OUTPUT && outputAluHi()) etype = OUTPUT_ALUHI;
    if (isComplexInstruction(e)) {
        if (etype == VALUE && operands.size() == 1) {
            ...
            auto* dest = action->action.back()->to<P4::IR::MAU::SaluInstruction>()->getOutput();
            ...
            dest = action->action.back()->to<P4::IR::MAU::SaluInstruction>()->getOutput();
            or_targets.emplace(dest);
        } else if (etype == IF) {
...
void CreateSaluInstruction::postorder(const P4::IR::BOr *e) {
    if (isComplexInstruction(e)) {
...
bool CreateSaluInstruction::preorder(const P4::IR::Concat *e) {
    if (Pattern(0).match(e->left)) {
        // ElimCasts introduces these zero-extend concats for type casts we can ignore
        visit(e->right, "right");
        return false; }
    if (canBeIXBarExpr(e)) {
        operands.push_back(new P4::IR::MAU::IXBarExpression(e));
        if (negate)
            operands.back() = new P4::IR::Neg(operands.back());
        return false; }
    error(ErrorType::ERR_UNSUPPORTED,
...
void CreateSaluInstruction::postorder(const P4::IR::BXor *e) {
    checkAndReportComplexInstruction(e);
    if (etype == VALUE || etype == OUTPUT_ALUHI) {
        if (e->left->is<P4::IR::Cmpl>() || e->right->is<P4::IR::Cmpl>())
            opcode = "xnor"_cs;
        else
            opcode = "xor"_cs;
    } else {
...
void CreateSaluInstruction::postorder(const P4::IR::Neg *e) {
    ...
    if (operands.size() <= 2) {
        ...
        operands.push_back(new P4::IR::Constant(negated->srcInfo, negated->type, 0));
    } else {
        only_operation_error(e, "negation", salu);
    }
}
void CreateSaluInstruction::postorder(const P4::IR::Cmpl *e) {
...
void CreateSaluInstruction::postorder(const P4::IR::Concat *e) {
    if (operands.size() < 2) return;  // can only happen if there has been an error
...
bool CreateSaluInstruction::divmod(const P4::IR::Operation::Binary *e, cstring op) {
    if (etype == OUTPUT && operands.empty() && Device::statefulAluSpec().DivModUnit) {
        ...
        predicate = saved_predicate;
        etype = OUTPUT;
        operands.push_back(new P4::IR::MAU::SaluReg(e->type, op, false));
    } else {
        only_operation_error(e, "div/mod operation", salu); }
    return false;
}

static bool canOutputDirectly(const P4::IR::Expression *e) {
    if (auto sl = e->to<P4::IR::Slice>())
        e = sl->e0;
    if (auto m = e->to<P4::IR::Member>())
        return !m->expr->is<P4::IR::TypeNameExpression>();
    if (e->is<P4::IR::TempVar>()) return true;
    if (e->is<P4::IR::MAU::SaluReg>()) return true;
    return false;
}

bool CreateSaluInstruction::outputEnumAsPredicate(const P4::IR::Member *mem) {
...
void CreateSaluInstruction::splitWideInstructions() {
    P4::IR::Vector<P4::IR::MAU::SaluInstruction> code;
    std::array<const P4::IR::Expression *, 4> words = { nullptr };
        ...
        if (auto *dest = inst->getOutput()) {
            if (dest->type->width_bits() > 32) {
                if (inst->name == "max8" || inst->name == "max16" ||
                    ...
                    unsigned i = dest->to<P4::IR::MAU::SaluReg>()->hi ? 2 : 0;
    ...
    for (int i = outputs.size()-1; i >= 0; --i) {
        ...
        auto *reg = src->to<P4::IR::MAU::SaluReg>();
...
void CreateSaluInstruction::assignOutputAlus() {
    ...
    for (const auto &[param, operand] : output_param_operands) {
        if (operand) {
            if (const auto *slice = operand->to<P4::IR::Slice>(); slice) {
...
const P4::IR::MAU::SaluInstruction *CreateSaluInstruction::setup_output() {
    ...
    if (is_address_output(operands.back()) && address_subword) {
        if (predicate && output_address_subword_predicate.count(output_index)) {
            ...
            if (subword_predicate)
                subword_predicate = new P4::IR::LOr(subword_predicate, predicate);
        } else {
    ...
    if (output) {
        ...
        if (output->operands.size() == operands.size()) {
            if (predicate)
                operands[0] = new P4::IR::LOr(output->operands[0], operands[0]);
            else
                return output;
        } else if (predicate) {
            return output; } }
    return output = new P4::IR::MAU::SaluInstruction("output"_cs, out_idx, &operands);
}

const P4::IR::MAU::SaluInstruction *CreateSaluInstruction::createInstruction() {
    const P4::IR::MAU::SaluInstruction *rv = nullptr;
    auto k = operands.back()->to<P4::IR::Constant>();
    ...
    auto generate_alu_a = [this](const P4::IR::Expression *val, const P4::IR::Expression *predicate,
                                 const P4::IR::MAU::SaluReg *reg, bool is_mem) {
        ...
        if (is_mem) {
            ...
            val = new P4::IR::MAU::SaluReg(reg->srcInfo, reg->type, trimmed_name, reg->hi);
        }

        const auto *dest_op = new P4::IR::MAU::SaluReg(val->srcInfo, val->type, "hi"_cs, true);
        if (predicate) {
            insert_instruction(
                new P4::IR::MAU::SaluInstruction("alu_a"_cs, 1, predicate, dest_op, val));
        } else {
            insert_instruction(new P4::IR::MAU::SaluInstruction("alu_a"_cs, 0, dest_op, val));
        }
        LOG3("  add " << *action->action.back());
        return new P4::IR::MAU::SaluReg(val->srcInfo, val->type, "alu_hi"_cs, true);
    };

    switch (etype) {
    case IF:
        insert_instruction(rv = new P4::IR::MAU::SaluInstruction(opcode, 0, &operands));
        LOG3("  add " << *action->action.back());
        break;
    case VALUE:
    case MATCH:
        ...
        if (regtype->width_bits() == 1) {
            ...
            if (salu->selector && salu->selector->mode == P4::IR::MAU::SelectorMode::FAIR)
                opcode += "_at";
            rv = onebit = new P4::IR::MAU::SaluInstruction(opcode);
        } else {
            int out_idx = 0;
            if (predicate) {
                operands.insert(operands.begin(), predicate);
            ...
            if (opcode == "divmod")
                out_idx = -1;
            insert_instruction(rv = new P4::IR::MAU::SaluInstruction(opcode, out_idx, &operands));
            LOG3("  add " << *action->action.back()); }
        break;
    case OUTPUT:
        checkWriteAfterWrite();
        ...
        if (outputs.size() > size_t(output_index) && outputs[output_index] &&
            ...
            if (canOutputDirectly(operands.at(0))) {
                ...
                const cstring MEM_PREFIX = "mem_"_cs;
                const auto *reg = operands.at(0)->to<P4::IR::MAU::SaluReg>();
                ...
                if (!equiv(new_operand, output->operands.back())) {
                    ...
                    const auto *reg = val->to<P4::IR::MAU::SaluReg>();
                    const bool is_mem = reg && reg->name.startsWith(MEM_PREFIX.string());
        ...
        if (regtype->width_bits() == 1) {
            ...
            rv = onebit = new P4::IR::MAU::SaluInstruction(opcode);
            operands.clear();
            operands.push_back(new P4::IR::MAU::SaluReg(P4::IR::Type::Bits::get(1), "alu_lo"_cs, false));
        } else if (k && k->value == 0) {
            ...
            if (salu->pred_shift >= 0) {
                ...
                salu->pred_shift = 28; }
            operands.at(0) = new P4::IR::MAU::SaluReg(k->srcInfo, k->type, "predicate"_cs, false);
        } else if (outputEnumAsPredicate(operands.at(0)->to<P4::IR::Member>())) {
            auto psize = 1 << Device::statefulAluSpec().CmpUnits.size();
            // FIXME -- should shift up to the top 16 bits of the word to maximize space
            // for the comb_shift constant, but doing so break action bus allocation
            salu->pred_shift = 0;   // = 28 - psize;
 ...
 */
static bool check_instructions(const P4::IR::MAU::SaluInstruction *si1,
    const P4::IR::MAU::SaluInstruction *si2) {
...
void CreateSaluInstruction::insert_instruction(const P4::IR::MAU::SaluInstruction *si_insert) {
    ...
    //
    // Instructions are merged together via the P4::IR::BOr node where left and right
    ...
    for (auto it = action->action.begin(); it != action->action.end(); it++) {
        auto si_orig = (*it)->to<P4::IR::MAU::SaluInstruction>();
        BUG_CHECK(si_orig,
        ...
        if (similar_inst) {
            auto merged_pred = new P4::IR::LOr(si_insert->operands.at(0), si_orig->operands.at(0));
...
bool CreateSaluInstruction::preorder(const P4::IR::Declaration_Variable *v) {
    auto vt = v->type->to<P4::IR::Type::Bits>();
...
bool CreateSaluInstruction::preorder(const P4::IR::Declaration_Instance *di) {
    ...
    auto type = di->type;
    if (auto st = type->to<P4::IR::Type_Specialized>()) {
        type = st->baseType;
...
bool CheckStatefulAlu::preorder(P4::IR::MAU::StatefulAlu *salu) {
    large_constants.clear();

    auto &device = Device::statefulAluSpec();
    const P4::IR::Type *regtype = nullptr;
    if (salu->selector) {
        // selector action
        BUG_CHECK(salu->width == 1 && salu->dual == false, "wrong size for selector action");
        regtype = P4::IR::Type::Bits::get(1);
    } else {
        BUG_CHECK(salu->reg && salu->reg->type->to<P4::IR::Type_Specialized>(), "invalid SALU");
        regtype = getType(salu->reg->type->to<P4::IR::Type_Specialized>()->arguments->at(0)); }
    auto bits = regtype->to<P4::IR::Type::Bits>();
    if (auto str = regtype->to<P4::IR::Type_Struct>()) {
        auto nfields = str->fields.size();
        bool sameTypes = false;
        if (nfields > 0)
            bits = getType(str->fields.at(0)->type)->to<P4::IR::Type::Bits>();
        if (nfields > 1) {
    ...
    if (bits->size == 1) {
        ...
        if (salu->selector) {
            ...
            if (salu->selector->mode == P4::IR::MAU::SelectorMode::RESILIENT) {
        ...
        for (auto sc : set_clr) {
            ...
            auto instr_action = new P4::IR::MAU::SaluAction(P4::IR::ID(sc + "_alu$0"));
            salu->instruction.addUnique(sc + "_alu$0", instr_action);
            auto set_clr_instr = new P4::IR::MAU::SaluInstruction(sc);
            instr_action->action.push_back(set_clr_instr);
        }
    }

    const P4::IR::MAU::SaluAction *first = nullptr;;
    for (auto salu_action : Values(salu->instruction)) {
        ...
        for (const P4::IR::MAU::Primitive *act_prim : salu_action->action) {
            ...
            if (act_prim->name.startsWith("min") || act_prim->name.startsWith("max"))
                break;
            for (const P4::IR::Expression *op : act_prim->operands) {
...
void CheckStatefulAlu::postorder(P4::IR::MAU::SaluInstruction *si) {
    if (Device::currentDevice() == Device::TOFINO &&
        ...
        for (const auto *op : si->operands) {
            const auto *constant = op->to<P4::IR::Constant>();
...
bool CheckStatefulAlu::preorder(P4::IR::MAU::Primitive *prim) {
    ...
    for (const auto *op : prim->operands) {
        if (const auto *constant = op->to<P4::IR::Constant>();
...
void CheckStatefulAlu::postorder(P4::IR::MAU::StatefulAlu *salu) {
    const auto &device = Device::statefulAluSpec();
...
unsigned CheckStatefulAlu::AddressLmatchUsage::eval_cmp(const P4::IR::Expression *e) {
    if (auto *cmp = e->to<P4::IR::MAU::SaluCmpReg>()) {
        return regmasks[cmp->index];
    } else if (auto *And = e->to<P4::IR::LAnd>()) {
        return eval_cmp(And->left) & eval_cmp(And->right);
    } else if (auto *Or = e->to<P4::IR::LOr>()) {
        return eval_cmp(Or->left) | eval_cmp(Or->right);
    } else if (auto *Not = e->to<P4::IR::LNot>()) {
...
bool CheckStatefulAlu::AddressLmatchUsage::preorder(const P4::IR::MAU::SaluAction *) {
    inuse_mask = 0;
    return true;
}
bool CheckStatefulAlu::AddressLmatchUsage::preorder(const P4::IR::MAU::SaluCmpReg *cmp) {
...
bool CheckStatefulAlu::AddressLmatchUsage::safe_merge(
    const P4::IR::Expression *a, const P4::IR::Expression *b, unsigned inuse
) {
    return (eval_cmp(a) & inuse) == (eval_cmp(b) & inuse);
}


bool CheckStatefulAlu::AddressLmatchUsage::preorder(const P4::IR::MAU::SaluFunction *fn) {
...
bool CheckStatefulAlu::preorder(P4::IR::MAU::SaluFunction *fn) {
    if (fn->name != "lmatch") return false;
    fn->expr = lmatch_usage.lmatch_operand;
    return false;
}

bool P4::IR::MAU::SaluAction::ReturnEnumEncoding::operator<=(
        const P4::IR::MAU::SaluAction::ReturnEnumEncoding &a) const {
...
const P4::IR::MAU::SaluAction::ReturnEnumEncoding *P4::IR::MAU::SaluAction::ReturnEnumEncoding::merge(
        const P4::IR::MAU::SaluAction::ReturnEnumEncoding *a) const {
...
bool FixupStatefulAlu::FindEncodings::preorder(const P4::IR::MAU::SaluAction *act) {
...
const P4::IR::MAU::SaluAction *FixupStatefulAlu::UpdateEncodings::preorder(P4::IR::MAU::SaluAction *act) {
...
const P4::IR::Operation::Relation *FixupStatefulAlu::UpdateEncodings::preorder(
        P4::IR::Operation::Relation *rel) {
    Pattern::Match<P4::IR::Expression> e;
    Pattern::Match<P4::IR::Member> k;
    bool iseq = true;
    if (!(e == k).match(rel) && (iseq = false, !(e != k).match(rel))) return rel;
    auto enum_type = k->type->to<P4::IR::Type_Enum>();
    ...
    if (iseq)
        ...
        rel = new P4::IR::Equ(rel->srcInfo, rel->type, rel->left, rel->right);
    rel->left = new P4::IR::BAnd(self.pred_type, e, new P4::IR::Constant(self.pred_type, mask));
    rel->right = new P4::IR::Constant(self.pred_type, 0);
    prune();
    return rel;
}

const P4::IR::Expression *FixupStatefulAlu::UpdateEncodings::preorder(P4::IR::Member *exp) {
    auto enum_type = exp->type->to<P4::IR::Type_Enum>();
    if (!enum_type || !self.encodings.count(enum_type)) return exp;
    if (!exp->expr->is<P4::IR::TypeNameExpression>()) return exp;
    auto encoding = self.encodings.at(enum_type).encoding;
    const P4::IR::MAU::Primitive *prim;
    if (getParent<P4::IR::AssignmentStatement>() || getParent<P4::IR::BFN::SavedRVal>()) {
        // ok
    } else if ((prim = getParent<P4::IR::MAU::Primitive>()) && prim->name == "modify_field") {
        // ok
    } else {
    ...
    return new P4::IR::Constant(self.pred_type, val);
}

const P4::IR::BFN::ParserRVal *FixupStatefulAlu::UpdateEncodings::postorder(P4::IR::BFN::SavedRVal *e) {
    if (auto k = e->source->to<P4::IR::Constant>())
        return new P4::IR::BFN::ConstantRVal(e->srcInfo, k);
    return e;
}

const P4::IR::Expression *FixupStatefulAlu::UpdateEncodings::preorder(P4::IR::Expression *exp) {
    if (exp->type->is<P4::IR::Type_Enum>())
