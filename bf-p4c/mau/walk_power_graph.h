...
class WalkPowerGraph : public MauInspector {
 public:
  ...
  WalkPowerGraph(const NextTable* next_table_properties,
                 BuildPowerGraph* graphs,
                 const bool& exceeds_stages,
                 const std::map<UniqueId, PowerMemoryAccess>& table_memory_access,
                 const BFN_Options& options,
                 MauFeatures* mau_features,
                 std::map<gress_t, MprSettings*>& mpr_settings) :
    next_table_properties_(next_table_properties), graphs_(graphs),
    exceeds_stages_(exceeds_stages), options_(options),
    table_memory_access_(table_memory_access),
    mau_features_(mau_features), mpr_settings_(mpr_settings) {
