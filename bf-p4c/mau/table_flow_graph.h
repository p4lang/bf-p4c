...
struct FlowGraph {
    typedef boost::adjacency_list<
        ...
        boost::property<boost::vertex_table_t, const P4::IR::MAU::Table*>,
    ...
    ordered_map<const P4::IR::MAU::Table*, int> tableToVertexIndex;

    /// All tables in this graph, excluding the nullptr table, used to represent the sink node.
    std::set<const P4::IR::MAU::Table*> tables;

    /// The dominator set for each table in the graph. Lazily computed.
    mutable std::optional<std::map<const P4::IR::MAU::Table*,
                                     std::set<const P4::IR::MAU::Table*>>> dominators;
    ...
    std::map<const P4::IR::MAU::Table*, typename Graph::vertex_descriptor> tableToVertex;

    /// Reverse map of above
    std::map<typename Graph::vertex_descriptor, const P4::IR::MAU::Table*> vertexToTable;
    ...
    /// in the graph.
    bool can_reach(const P4::IR::MAU::Table* t1, const P4::IR::MAU::Table* t2) const {
    ...
    /// always executed.
    const std::set<const P4::IR::MAU::Table*> get_dominators(const P4::IR::MAU::Table* table) const;

    /// Determines whether the given table is executed on all paths.
    bool is_always_reached(const P4::IR::MAU::Table*) const;
    ...
    /// dst.
    std::vector<const P4::IR::MAU::Table*> find_path(const P4::IR::MAU::Table* src,
                                                 const P4::IR::MAU::Table* dst) {
        auto path = path_finder.find_path(get_vertex(src), get_vertex(dst));
        std::vector<const P4::IR::MAU::Table*> result;
        for (auto node : path) {
    ...
    typename Graph::vertex_descriptor get_vertex(const P4::IR::MAU::Table* tbl) const {
    ...
    const std::set<const P4::IR::MAU::Table*> get_tables() const {
        return tables;
    }

    /// @return the vertex associated with the given table, creating the vertex if one does not
    /// already exist.
    typename Graph::vertex_descriptor add_vertex(const P4::IR::MAU::Table* table) {
    ...
    std::vector<const P4::IR::MAU::Table*> topological_sort() const {
    ...
    struct DumpTableDetails {
        virtual void dump(std::ostream&, const P4::IR::MAU::Table*) const { }
    };

    static std::string viz_node_name(const P4::IR::MAU::Table* tbl);
...
// P4::IR::MAU::Table::visit_children; specifically, the calls to flow_clone, visit(Node, label),
...
class FindFlowGraph : public MauInspector {
 private:
     ...
     * [t4]  [t5]
     */
    const P4::IR::MAU::Table* next_table;
    ...
    std::pair<bool, cstring> next_incomplete(const P4::IR::MAU::Table *t);
