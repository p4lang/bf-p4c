 ...
 private:
    ...
    class LBUse {
     public:
        ...
        const P4::IR::MAU::Table* dest;  // The destination related with this use
        explicit LBUse(const P4::IR::MAU::Table* d)  // Dummy constructor for lookup by destination
                : fst(-1), lst(-1), dest(d) {}
        LBUse(const P4::IR::MAU::Table* d, size_t f, size_t l)  // Construct a LBUse for a dest
                : fst(f), lst(l), dest(d) {}
        ...
        void extend(const P4::IR::MAU::Table*);  // Extends this LB to a new source table
    ...
    class LocalizeSeqs : public PassManager {
        using TableToSeqSet =
              ordered_map<const P4::IR::MAU::Table *, ordered_set<const P4::IR::MAU::TableSeq *>>;
        using TableToTableSet =
              ordered_map<const P4::IR::MAU::Table *, ordered_set<const P4::IR::MAU::Table *>>;
        using SeqToThreads =
              ordered_map<const P4::IR::MAU::TableSeq *,
                          safe_vector<safe_vector<const P4::IR::MAU::Table *>>>;
     ...
     public:
        ...
        bool can_propagate_to(const P4::IR::MAU::Table *src, const P4::IR::MAU::Table *dst) const {
    ...
    static inline UniqueId get_uid(const P4::IR::MAU::Table* t) {
    ...
    ordered_map<const P4::IR::MAU::Table *, std::pair<int, int>>    use_next_table;
    class FindNextTableUse;
    bool uses_next_table(const P4::IR::MAU::Table *tbl) const {
    ...
    std::map<UniqueId, ordered_set<const P4::IR::MAU::TableSeq *>> dest_ts;
    ...
    class Prop : public MauInspector {
        ...
        void add_table_seq(const P4::IR::MAU::Table*, std::pair<cstring, const P4::IR::MAU::TableSeq*>);
    ...
    class LBAlloc : public MauInspector {
        JbayNextTable& self;
        profile_t init_apply(const P4::IR::Node* root) override;  // Allocates long branches to tags
        bool preorder(const P4::IR::BFN::Pipe*) override;  // Early abort
    ...
    class TagReduce : public MauTransform {
        ...
        std::map<int, std::vector<P4::IR::MAU::Table*>> stage_dts;  // Map from stage # to DTs
        void alloc_dt_mems();  // Allocates memories for DTs in stage_dts

        // Map from table sequences to dumb tables to add to sequence
        std::map<const P4::IR::MAU::TableSeq*, std::vector<P4::IR::MAU::Table*>> dumb_tbls;

        profile_t init_apply(const P4::IR::Node* root) override;
        // Early abort if push-down is sufficient. Adds DTs and allocates them memories
        P4::IR::Node* preorder(P4::IR::BFN::Pipe*) override;
        P4::IR::Node* preorder(P4::IR::MAU::Table*) override;  // Set always run tags
        P4::IR::Node* preorder(P4::IR::MAU::TableSeq*) override;  // Add specified tables to table sequences
     public:
