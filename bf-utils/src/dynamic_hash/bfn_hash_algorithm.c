...
bool verify_algorithm(bfn_hash_algorithm_t *alg, char **error_message) {
  ...
  if (alg->final_xor &&
      determine_msb(alg->final_xor) + 1 > alg->hash_bit_width) {
 ...
 */
void build_standard_crc_algorithm(bfn_hash_algorithm_t *alg,
...
static void construct_stream(bfn_hash_algorithm_t *alg,
                             uint64_t val,
                             uint8_t *stream) {
