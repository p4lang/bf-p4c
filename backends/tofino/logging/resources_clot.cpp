...
std::vector<ClotResourcesLogging::ClotUsage*>
&ClotResourcesLogging::getUsageData(gress_t gress, unsigned tag) {
...
static bool areFieldListsEqual(const std::vector<ClotResourcesLogging::ClotField *> &clot1,
    ...
    for (std::size_t i = 0; i < clot1.size(); i++) {
        ...
        if (clot1[i]->get_name() != clot2[i]->get_name()
         ...
         && clot1[i]->get_field_msb() != clot2[i]->get_field_msb()) return false;
    }
...
static bool areClotUsagesEqual(ClotResourcesLogging::ClotUsage *u1,
    ...
    return u1->get_length() == u2->get_length()
        && u1->get_offset() == u2->get_offset()
