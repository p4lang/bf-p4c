...
#include "backends/tofino/phv/live_range_split.h"
#include "backends/tofino/phv/fieldslice_live_range.h"
#include "backends/tofino/phv/optimize_phv.h"
#include "backends/tofino/phv/parser_extract_balance_score.h"
...
#include "backends/tofino/phv/legacy_action_packing_validator.h"
...
const IR::Node *AllocatePHV::apply_visitor(const IR::Node* root_, const char *) {
    ...
    for (auto* sc : cluster_groups)
        if (PHV::AllocUtils::is_clot_allocated(utils_i.clot, *sc))
            to_remove.insert(sc);
