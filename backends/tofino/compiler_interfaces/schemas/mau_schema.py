...
class ActionParameterInfo(jsl.Document):
    ...
    parameters = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                ...
                "bit_width": jsl.IntField(
                    required=True, description="The bit width of this action parameter."
                ),
            },
            required=True,
            description="",
        ...
        required=True,
        description="Array of action parameters used in this action.",
    )
...
class StageEntryFormat(jsl.Document):
    ...
    entries = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "entry_number": jsl.IntField(
                    required=True, description="An integer indicating the entry number."
                ),
                "fields": jsl.ArrayField(
                    items=jsl.DictField(
                        properties={
                            ...
                            "bit_width": jsl.IntField(
                                required=True,
                                description="The bit width of this field (or field slice).",
                            ),
                            "start_bit": jsl.IntField(
                                required=True,
                                description="An integer indicating the start bit, from the least significant bit, for this field.",
                            ),
                            "memory_start_bit": jsl.IntField(
                                required=True,
                                description="An integer indicating the start bit in the memory table word, from the least significant bit, for this field.",
                            ),
                        },
                        required=True,
                        description="",
        ...
        required=True,
        description="Information about each entry in the packing",
    )
...
class StageActionParameterMap(jsl.Document):
    ...
    bit_width = jsl.IntField(
        required=True, description="The bit width of this parameter (or parameter slice)."
    )
    start_bit = jsl.IntField(
        required=True,
        description="An integer indicating the start bit, from the least significant bit, for this parameter.",
    )
    # Should we include the action parameter slot position for the parameter slice?
    location = jsl.StringField(
        required=True,
        enum=["action_data", "match_overhead"],
        description="The location of this action parameter (or parameter slice), either an action data table or immediate match overhead.",
    )

    action_data_slot_size = jsl.IntField(
        required=True,
        enum=[8, 16, 32],
        description="The bit width of this action slot size this action parameter (or parameter slice) is mapped to.",
    )
    action_slot_number = jsl.IntField(
        required=True,
        enum=list(range(0, 32)),
        description="The action slot number this action parameter (or parameter slice) is mapped to.",
    )
...
class StageActionFormat(jsl.Document):
    ...
    action_format = jsl.DocumentField(
        "StageEntryFormat",
        as_ref=True,
        required=True,
        description="The action-specific pack format in this stage.",
    )
    parameter_map = jsl.ArrayField(
        jsl.DocumentField("StageActionParameterMap", as_ref=True, required=True),
        required=True,
        description="A list of the mappings for action parameter (or parameter slice) to action data bus locations.",
    )
...
class StageMemoryDetails(jsl.Document):
    ...
    entries_per_table_word = jsl.IntField(
        required=True, description="Number of entries that are packed into a given table word."
    )
    table_type = jsl.StringField(
        required=True,
        enum=["action", "match", "meter", "selection", "stateful", "statistics", "tind"],
        description="The table type the memory is used for.",
    )
    num_memories = jsl.IntField(
        required=True, description="The total number of memory blocks in use."
    )
...
class StageMemoryDetailsWithEntryWidth(StageMemoryDetails):
    ...
    entry_bit_width_requested = jsl.IntField(
        required=True,
        description="The expected entry bit width (including overhead), assuming ideal packing.",
    )
    entry_bit_width_allocated = jsl.IntField(
        required=True, description="The entry bit width actually allocated."
    )
...
class StageMemoryDetailsWithEntryWidthAndIdeal(StageMemoryDetailsWithEntryWidth):
    ...
    ideal_entries_per_table_word = jsl.IntField(
        required=True,
        description="The ideal number of entries per ideal table word width, based only on the match key bit width.",
    )
    ideal_table_word_bit_width = jsl.IntField(
        required=True,
        description="The ideal table word width that uses the most number of memory bits, assuming no packing constraints.",
    )
...
class StageMatchMemoryDetails(StageMemoryDetailsWithEntryWidthAndIdeal):
    ...
    entries_requested = jsl.IntField(
        required=True, description="The number of entries requested to go in this stage."
    )
    entries_allocated = jsl.IntField(
        required=True, description="The number of entries actually allocated in this stage."
    )
    imm_bit_width_in_overhead_requested = jsl.IntField(
        required=True,
        description="The immediate match overhead bit width expected, assuming ideal packing.",
    )
    imm_bit_width_in_overhead_allocated = jsl.IntField(
        required=True, description="The immediate match overhead bit width actually allocated."
    )
...
class MatchTables(jsl.Document):
    ...
    class StageDetails(jsl.Document):
        ...
        memories = jsl.ArrayField(
            items=jsl.OneOfField(
                [
                    jsl.DocumentField(
                        "StageMemoryDetails", as_ref=True
                    ),  # meter, selection, stateful, statistics
                    jsl.DocumentField("StageMatchMemoryDetails", as_ref=True),  # match
                    jsl.DocumentField(
                        "StageMemoryDetailsWithEntryWidthAndIdeal", as_ref=True
                    ),  # action data
                    jsl.DocumentField("StageMemoryDetailsWithEntryWidth", as_ref=True),
                ]
            ),  # tind
            required=True,
        ...
        # Needed?
        overhead_fields = jsl.ArrayField(
            items=jsl.DictField(
                properties={
                    "name": jsl.StringField(
                        required=True,
                    ...
                    "bit_width": jsl.IntField(
                        required=True,
                        description="The number of bits allocated for the overhead field.",
                    ),
                },
                required=True,
                description="",
            ...
            required=True,
            description="A list of the overhead fields that are used in each match entry, if any.",
        )

        match_format = jsl.DocumentField(
            "StageEntryFormat",
            as_ref=True,
            required=True,
        ...
        action_formats = jsl.ArrayField(
            items=jsl.DocumentField("StageActionFormat", as_ref=True),
            required=True,
            description="The action table's formatting and action bus usage on a per-action basis.",
        )
    ...
    lookup_types = jsl.ArrayField(
        items=jsl.StringField(enum=["exact", "ternary", "lpm", "range"]),
        required=True,
        description="The lookup types used by this table.",
    )

    entries_requested = jsl.IntField(
        required=True, description="The total number of table entries requested."
    )
    entries_allocated = jsl.IntField(
        required=True, description="The total number of table entries allocated."
    )

    # Needed?
    match_fields = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "name": jsl.StringField(
                    required=True, description="The name of a field in the match key."
                ),
                "start_bit": jsl.IntField(
                    required=True,
                ...
                "bit_width": jsl.IntField(
                    required=True, description="The number of bits used from the match key field."
                ),
                "lookup_type": jsl.StringField(
                    required=True,
        ...
        required=True,
        description="A list of the match fields that are used in each match entry, if any.",
    )

    action_parameters = jsl.ArrayField(
        items=jsl.DocumentField("ActionParameterInfo", as_ref=True),
        required=True,
        description="Action parameter information used per action.",
    )

    stage_allocation = jsl.ArrayField(
        items=jsl.DocumentField("StageDetails", as_ref=True, description=""),
        required=True,
        description="An array of the per-stage allocation information for this match table.",
    )
