...
class Source_infoJSONSchema(jsl.Document):
    ...
    symbols = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("SymbolDefinition", as_ref=True),
        description="An array of source symbols.",
    )
...
class SymbolDefinition(jsl.Document):
    ...
    symbol_type = jsl.StringField(
        required=False, description="The type of this symbol (e.g. p4 table, condition, etc.)"
    )
    symbol_name = jsl.StringField(required=True, description="Full name of this source symbol.")
    scope = jsl.ArrayField(
        required=False, items=jsl.StringField(), description="Symbol scope path as an array."
    )
    aliases = jsl.ArrayField(
        required=False, items=jsl.StringField(), description="Fully qualified symbol name aliases."
    )
    declaration = jsl.DocumentField(
        "SourceLocation", as_ref=True, description="Declaration of this symbol."
    )
    references = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("SourceLocation", as_ref=True),
        description="An array of references to this symbol.",
    )
...
class SourceLocation(jsl.Document):
    ...
    end_line = jsl.IntField(
        required=False, description="End line (inclusive) for multi-line references."
    )
    source_brief = jsl.StringField(
        required=False, description="Short summary of this source location."
    )
