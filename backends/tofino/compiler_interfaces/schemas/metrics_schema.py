...
class PhvTypeUsage(jsl.Document):
    ...
    containers_occupied = jsl.IntField(
        required=True, description="Total number of containers occupied."
    )
    ...
    bits_unusable = jsl.IntField(
        required=False,
        description="Total number of container bits that cannot be used due to allocation constraints.",
    )
...
class Phv(jsl.Document):
    ...
    normal = jsl.ArrayField(
        items=jsl.DocumentField("PhvTypeUsage", as_ref=True),
        required=True,
        description="Normal PHV containers.",
    )
    tagalong = jsl.ArrayField(
        items=jsl.DocumentField("PhvTypeUsage", as_ref=True),
        required=False,
        description="Tagalong PHV containers.",
    )
    mocha = jsl.ArrayField(
        items=jsl.DocumentField("PhvTypeUsage", as_ref=True),
        required=False,
        description="Mocha PHV containers.",
    )
    dark = jsl.ArrayField(
        items=jsl.DocumentField("PhvTypeUsage", as_ref=True),
        required=False,
        description="Dark PHV containers.",
    )
...
class Parser(jsl.Document):
    ...
    ingress = jsl.DocumentField(
        ...
        as_ref=True,
        required=True,
        description="Ingress parser resource information.",
    )
    egress = jsl.DocumentField(
        "GressParser", as_ref=True, required=True, description="Egress parser resource information."
    )
...
class Mau(jsl.Document):
    ...
    action_bus_bytes = jsl.IntField(
        required=True, description="Total number of action data bus bytes used."
    )
    exact_crossbar_bytes = jsl.IntField(
        required=True, description="Total number of exact match crossbar bytes used."
    )
    ternary_crossbar_bytes = jsl.IntField(
        required=True, description="Total number of ternary match crossbar bytes used."
    )
    latency = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "gress": jsl.StringField(
                    required=True,
                    enum=["ingress", "egress", "ghost"],
                    description="The gress this latency measurement applies to.",
                ),
                "cycles": jsl.IntField(
                    required=True, description="The total number of clock cycles in this gress."
                ),
            },
            required=True,
        ),
        required=True,
        description="MAU power information.",
    )

    power = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "gress": jsl.StringField(
                    required=True,
                    enum=["ingress", "egress", "ghost"],
                    description="The gress this power estimate applies to.",
                ),
                "estimate": jsl.StringField(
                    required=True,
                    description="The estimated worst case table control flow power, in Watts.",
                ),
            },
            required=True,
        ),
...
class GressDeparser(jsl.Document):
    ...
    fde_entries = jsl.IntField(
        required=True, description="Total number of field dictionary entries used."
    )
...
class Deparser(jsl.Document):
    ...
    ingress = jsl.DocumentField(
        ...
        as_ref=True,
        required=True,
        description="Ingress deparser resource information.",
    )
    egress = jsl.DocumentField(
        "GressDeparser",
        as_ref=True,
        required=True,
        description="Egress deparser resource information.",
    )
...
class GressClots(jsl.Document):
    ...
    all_allocated_bits = jsl.IntField(
        required=True,
        description="Total number of bits allocated to CLOTs, including those overwritten by PHVs and checksums (i.e., including bits used by MAU and checksum bits).",
    )
    allocated_bits = jsl.IntField(
        required=True,
        description="Number of unused (by MAU), non-checksum bits that are CLOT-allocated.",
    )
    unallocated_bits = jsl.IntField(
        required=True,
        description="Number of unused (by MAU), non-checksum bits in CLOT-eligible fields that are not CLOT-allocated.",
    )
    redundant_phv_bits = jsl.IntField(
        required=True,
        description="Number of unused (by MAU), non-checksum bits in CLOT-eligible fields that are double-allocated to CLOTs and PHVs.",
    )
...
class Clots(jsl.Document):
    ...
    ingress = jsl.DocumentField(
        "GressClots", as_ref=True, required=True, description="Ingress CLOT usage information."
    )
    egress = jsl.DocumentField(
        "GressClots", as_ref=True, required=True, description="Egress CLOT usage information."
    )
...
class MetricsJSONSchema(jsl.Document):
    ...
    compilation_time = jsl.StringField(
        required=True, description="The time, in seconds, it took to compile this program."
    )

    phv = jsl.DocumentField(
        "Phv", as_ref=True, required=True, description="Phv resource information."
    )
    parser = jsl.DocumentField(
        "Parser", as_ref=True, required=True, description="Parser resource information."
    )
    mau = jsl.DocumentField(
        "Mau", as_ref=True, required=True, description="MAU resource information."
    )
    deparser = jsl.DocumentField(
        "Deparser", as_ref=True, required=True, description="Deparser resource information."
    )
    clots = jsl.DocumentField(
        "Clots", as_ref=True, required=True, description="CLOT-usage information."
    )
