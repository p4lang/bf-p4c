...
class Slice(jsl.Document):
    ...
    lsb = jsl.IntField(
        required=True,
        description="Least significant bit of the field slice or position in the container.",
    )
    msb = jsl.IntField(
        required=True,
        description="Most significant bit of this field slice or position in the container.",
    )
...
class FieldSlice(jsl.Document):
    ...
    field_name = jsl.StringField(
        required=True, description="Name of the field to which this slice belongs to."
    )
    slice_info = jsl.DocumentField(
        "Slice", as_ref=True, required=True, description="A slice range for this field slice."
    )
...
class ContainerSlice(jsl.Document):
    ...
    slice_info = jsl.DocumentField(
        "Slice", as_ref=True, required=True, description="A slice range for this container slice."
    )
    field_slice = jsl.DocumentField(
        "FieldSlice",
        as_ref=True,
        required=True,
    ...
    # access patterns
    reads = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("Access", as_ref=True),
        description="A list of locations where this field is read.",
    )
    writes = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("Access", as_ref=True),
...
class Container(jsl.Document):
    ...
    gress = jsl.StringField(
        required=True, enum=["ingress", "egress", "ghost"], description="The packet thread."
    )
    slices = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("ContainerSlice", as_ref=True),
        min_items=1,
...
class Base_Constraint(jsl.Document):
    ...
    id = jsl.IntField(
        description="Id for the constraint, used to retrieve reason message from constraint_reasons",
        required=True,
    )
    source = jsl.DocumentField(
        "SourceLocation", as_ref=True, required=True, description="The source of the constraint."
    )
    pipeline_location = jsl.DocumentField(
        "Access",
        as_ref=True,
        required=False,
...
class BoolConstraint(Base_Constraint):
    ...
    inverted = jsl.BooleanField(
        required=True, description="If set to true, constraint enforces exact opposite behaviour"
    )
...
class IntConstraint(Base_Constraint):
    ...
    description = (
        """A constraint with a single integer parameter (offset, stage num, container size)."""
    )
...
class ListConstraint(Base_Constraint):
    ...
    lists = jsl.ArrayField(
        required=True,
        items=jsl.IntField(required=True, description="Index into field_items array"),
        min_items=1,
    )
...
class Constraint(jsl.Document):
    ...
    base_constraint = jsl.ArrayField(
        required=True,
        ...
        min_items=1,
        description="The reason(s) the constraint is attached to the field. A constraint may be imposed by a multitude of reasons"
...
class FieldGroupItem(jsl.Document):
    ...
    field_info = jsl.DocumentField(
        "FieldInfo", as_ref=True, required=True, description="Reference to first field in pair"
    )
    source = jsl.DocumentField(
        "SourceLocation",
        as_ref=True,
        required=True,
        description="Program points inducing the constraint.",
    )
    ...
    slice = jsl.DocumentField(
        "Slice",
        as_ref=True,
        required=False,
        description="Field slice. All slices of a field if not present",
    )
...
class Field(jsl.Document):
    ...
    field_info = jsl.DocumentField(
        "FieldInfo", as_ref=True, required=True, description="Field identification info."
    )

    field_slices = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("FieldSlice", as_ref=True),
        min_items=0,
    ...
    constraints = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("Constraint", as_ref=True),
        min_items=0,
    ...
    status = jsl.StringField(
        required=True,
        enum=["allocated", "unallocated", "partially allocated", "unreferenced", "eliminated"],
...
class PhvJSONSchema(jsl.Document):
    ...
    resources = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "group_type": jsl.StringField(
                    required=True,
                    enum=["mau", "tagalong", "deparser"],
                    description="The PHV group type.",
                ),
                "bit_width": jsl.IntField(
                    required=True, description="The bit width of containers in this group."
                ),
                "num_containers": jsl.IntField(
                    required=True, description="The number of PHV containers in this group."
                ),
                "addresses": jsl.ArrayField(
                    ...
                    required=True,
                    description="The container addresses in this group.",
                ),
            }
        ),
        required=True,
    ...
    structures = jsl.ArrayField(
        items=jsl.DocumentField("Structure", as_ref=True),
        required=True,
    ...
    fields = jsl.ArrayField(
        required=True,
        items=jsl.DocumentField("Field", as_ref=True),
    ...
    field_group_items = jsl.ArrayField(
        items=jsl.DocumentField("FieldGroupItem", required=True, as_ref=True),
        required=False,
        description="Concrete items pointed to by indices in field_groups",
    )

    # Groups of fields affected by same constraints
    field_groups = jsl.ArrayField(
        items=jsl.ArrayField(
            items=jsl.IntField(required=True, description="Index to field_group_items"),
            required=True,
            min_items=2,
            description="Group of fields under same constraint",
        ),
        required=False,
        description="Groups of fields affected by the same constraints",
    )

    # Constraint reasons. Maps generic string id to formatted reason string
    constraint_reasons = jsl.ArrayField(
        required=True,
        items=jsl.StringField(required=True),
        min_items=1,
    ...
    # POVs
    pov_structure = jsl.ArrayField(
        items=jsl.DictField(
            properties={
                "pov_bit_index": jsl.IntField(
                    required=True, description="Bit index in the packet occupancy vector."
                ),
                "pov_bit_name": jsl.StringField(
                    required=True, description="The name given to the POV bit."
                ),
                "gress": jsl.StringField(
                    required=True,
                    enum=["ingress", "egress", "ghost"],
                    description="The gress of the validity bit.",
                ),
            }
        ),
        required=True,
        description="A list of attributes describing the POV structure.",
    )

    # the allocation
    containers = jsl.ArrayField(
        items=jsl.DocumentField("Container", as_ref=True),
        required=True,
