...
const IR::Node *HashConverter::postorder(IR::MethodCallExpression *node) {
    ...
    BUG_CHECK(member->member == "get_hash", "unexpected Hash method %1%", member->member);
    auto method = new IR::Member(node->srcInfo, member->expr, "get"_cs);

    auto typeArgs = new IR::Vector<IR::Type>();
    auto args = new IR::Vector<IR::Argument>();
    auto args_size = node->arguments->size();
    ERROR_CHECK(args_size == 1 || args_size == 3,
...
const IR::Node *RandomConverter::postorder(IR::MethodCallExpression *node) {
    auto member = node->method->to<IR::Member>();
    BUG_CHECK(member->member == "read", "unexpected Random method %1%", member->member);
    auto method = new IR::Member(node->srcInfo, member->expr, "get"_cs);
    auto args = new IR::Vector<IR::Argument>();
...
const IR::Node *EgressParserConverter::postorder(IR::P4Parser *node) {
    ...
    auto result = new IR::BFN::TnaParser(parser->srcInfo, "egressParserImpl"_cs, parser_type,
                                         ...
                                         parser->states, tnaParams, EGRESS);
    return result;
}
...
const IR::Node *IngressControlConverter::postorder(IR::P4Control *node) {
    ...
    auto result = new IR::BFN::TnaControl(
        node->srcInfo, "ingress"_cs, controlType, node->constructorParams, *controlLocals,
...
const IR::Node *EgressControlConverter::postorder(IR::P4Control *node) {
    ...
    auto result = new IR::BFN::TnaControl(
        node->srcInfo, "egress"_cs, controlType, node->constructorParams, *controlLocals,
...
const IR::Node *TypeNameExpressionConverter::postorder(IR::TypeNameExpression *node) {
    ...
    if (mapped != enumsToTranslate.end()) {
        ...
        if (isSerEnum) {
            if (!structure->ser_enums.count(newName)) BUG("No translation for type %1%", node);
            auto newType = structure->ser_enums.at(newName);
            retval = new IR::TypeNameExpression(node->srcInfo, newType, new IR::Type_Name(newName));
        } else {
            if (!structure->enums.count(newName)) BUG("No translation for type %1%", node);
...
const IR::Node *TypeNameExpressionConverter::postorder(IR::Member *member) {
    ...
    if (!member->expr->type->is<IR::Type_SerEnum>()) return member;
...
const IR::Node *PathExpressionConverter::postorder(IR::Member *node) {
    ...
    if (auto t = expr->type->to<IR::Type_Struct>()) {
        ...
        if ((tName == PSA::IG_OUT_INTR_MD_TYPE) || (tName == PSA::EG_OUT_INTR_MD_TYPE))
            pathname = PSA::OUT_INTR_MD;
        else if ((tName == PSA::IG_INP_INTR_MD_TYPE) || (tName == PSA::EG_INP_INTR_MD_TYPE))
            pathname = PSA::INP_INTR_MD;
    }

    LOG5("Finding metadata field in block " << blockName << " for type " << expr->type->toString()
                                            << " : { " << pathname << ", " << membername << " }");
