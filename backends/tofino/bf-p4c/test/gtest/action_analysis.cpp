...
#else
bool valid_instruction_constant(unsigned value, int maxBits, int minBits, int C8Size,
                                int constant_size);
...
int C8Value(int value) { return value & C8Mask; }
}  // namespace
...
TEST(valid_instruction_constant, width5) {
    ...
    for (unsigned value = 0; value < (1U << width); ++value) {
        bool expected =
            (value == 0b01001 || value == 0b01101 || value == 0b10010 || value == 0b10110) ? false
                                                                                           : true;
...
TEST(valid_instruction_constant, Rotate000001) {
    ...
    for (unsigned ls = 0; ls < width; ++ls) {
        ...
        bool expected =
            (value == 0b000001 || value == 0b000010 || value == 0b000100) ? true : false;
        EXPECT_EQ(expected, valid_instruction_constant(value, maxBits, minBits, C8Size, -1));
        EXPECT_EQ(expected, valid_instruction_constant(value, maxBits, minBits2, C8Size, -1));
    }
}
...
TEST(valid_instruction_constant, Rotate010111111111111111111111111111) {
    ...
    for (unsigned ls = 0; ls < width; ++ls) {
        ...
        bool expected =
            (value == 0b101111111111111111111111111110 || value == 0b011111111111111111111111111101)
                ? false
                : true;
...
namespace {
bool run_cross_check_algorithm(int32_t val, int max_bits, int min_bits, int container_size,
                               int constant_size) {
...
TEST(valid_instruction_constant, CrossCheckAlgorithm8) {
    for (unsigned width = 1; width <= C8Size; ++width) {
        ...
        for (unsigned value = 0; value <= max; ++value) {
            ...
            BUG_CHECK(res1 == res2, "Mismatch %1% != %2%: %3%, %4%, %5%", res1, res2, value, C8Size,
                      width);
        }
    }
}
...
TEST(valid_instruction_constant, CrossCheckAlgorithm16) {  // ~4 msec to run
    for (unsigned width = 1; width <= C16Size; ++width) {
        ...
        for (unsigned value = 0; value <= max; ++value) {
            ...
            BUG_CHECK(res1 == res2, "Mismatch %1% != %2%: %3%, %4%, %5%", res1, res2, value,
                      C16Size, width);
        }
    }
}
...
TEST_F(ActionAnalysisTest, ParallelAction) {
    ...
    PassManager validate = {new CollectHeaderStackInfo,
                            // Instructions in actions are sequential.
                            new CollectPhvInfo(phv), new DoInstructionSelection(phv),
                            new CollectPhvInfo(phv), new BackendCopyPropagation(phv),
