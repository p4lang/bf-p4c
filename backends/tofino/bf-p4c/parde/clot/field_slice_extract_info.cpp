...
void FieldSliceExtractInfo::update(const IR::BFN::ParserState *state, unsigned stack_offset,
                                   unsigned state_bit_offset, int min_packet_offset,
...
const FieldSliceExtractInfo *FieldSliceExtractInfo::trim(int lo_idx, int bits) const {
    BUG_CHECK(lo_idx >= 0, "Attempted to trim an extract of %s to a negative start index: %d",
...
std::vector<const FieldSliceExtractInfo *> *FieldSliceExtractInfo::remove_conflicts(
    ...
    for (auto &extract_entry : state_bit_offsets()) {
        ...
        for (auto &candidate_entry : candidate->state_bit_offsets()) {
            ...
            for (auto it = shift_amounts->lower_bound(lower_bound);
                 it != shift_amounts->end() && *it <= upper_bound; ++it) {
                auto shift = *it;

                auto conflict_start =
                    std::max(0, shift + candidate_offset - extract_offset - preGapBits);
                auto conflict_end =
                    std::min(extract_size - 1, shift + candidate_offset - extract_offset +
...
const std::map<unsigned, StatePairSet> FieldSliceExtractInfo::byte_gaps(
    const CollectParserInfo &parserInfo, const FieldSliceExtractInfo *other) const {
    ...
    for (auto &entry : bit_gaps(parserInfo, other)) {
        ...
        BUG_CHECK(bit_gap % 8 == 0,
                  "Gap between %1% in state %2% and %3% in state %4% "
                  "can be %5% bits, which is not a whole-byte multiple",
                  slice_->shortString(), states.begin()->first, other->slice_->shortString(),
                  states.begin()->second, bit_gap);
...
const std::map<unsigned, StatePairSet> FieldSliceExtractInfo::bit_gaps(
    const CollectParserInfo &parserInfo, const FieldSliceExtractInfo *other) const {
    std::map<unsigned, StatePairSet> result;
