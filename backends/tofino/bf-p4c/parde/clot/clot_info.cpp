...
std::optional<nw_bitrange> ClotInfo::field_range(const IR::BFN::ParserState *state,
    ...
    BUG_CHECK(field_to_parser_states_.count(field), "Field %1% is never extracted from the packet",
              field->name);
    BUG_CHECK(field_to_parser_states_.at(field).count(state),
              "Parser state %1% does not extract %2%", state->name, field->name);
...
std::optional<unsigned> ClotInfo::offset(const IR::BFN::ParserState *state,
                                         const PHV::Field *field) const {
    if (auto range = field_range(state, field)) return range->lo;
...
cstring ClotInfo::sanitize_state_name(cstring state_name, gress_t gress) const {
    ...
    return (!state_name.startsWith(ss.str())  // if not prefixed with gress
                ? (ss.str() + "::")
                : "") +
           st.substr(0, pos)  // add the prefix, cut generated appendix
...
const Clot *ClotInfo::parser_state_to_clot(const IR::BFN::LoweredParserState *state,
    ...
    if (parser_state_to_clots.count(state_name)) {
        ...
        auto it = std::find_if(clots.begin(), clots.end(),
                               [&](const Clot *sclot) { return (sclot->tag == tag); });
    ...
    for (const auto &kv : clot->parser_state_to_slices()) {
        ...
        for (const auto *slice : slices) {
            ...
            if (slice_overwritten_by_phv(phv, clot, slice)) {
                field->foreach_alloc(
                    range, PHV::AllocContext::DEPARSER, nullptr, [&](const PHV::AllocSlice &alloc) {
                        auto container = alloc.container();
                        auto net_range = range.toOrder<Endian::Network>(field->size);
                        int field_in_clot_offset =
                            clot->bit_offset(parser_state, slice) - net_range.lo;

                        auto field_range =
                            alloc.field_slice().toOrder<Endian::Network>(field->size);

                        auto container_range = alloc.container_slice().toOrder<Endian::Network>(
                            alloc.container().size());

                        auto container_offset =
                            field_in_clot_offset - container_range.lo + field_range.lo;

                        BUG_CHECK(container_offset % 8 == 0,
                                  "CLOT %d container overwrite offset for %s is not byte-aligned",
                                  clot->tag, slice->shortString());
                        ...
                        if (containers.count(container_offset_in_byte)) {
                            ...
                            BUG_CHECK(container == other_container,
                                      ...
                                      "%s and %s",
                                      clot->tag, container_offset_in_byte, container,
                                      other_container);
                        } else {
...
void ClotInfo::add_field(const PHV::Field *f, const IR::BFN::ParserRVal *source,
    ...
    if (auto rval = source->to<IR::BFN::PacketRVal>()) field_range_[state][f] = rval->range;
}

void ClotInfo::add_stack(const IR::HeaderStack *hs, const IR::BFN::ParserState *state, int index) {
    ...
    if (LOGGING(4)) {
        ...
        for (auto &[state, field_byte_idx_map] : field_to_byte_idx) {
            ...
            for (auto &[field, indices] : field_byte_idx_map) {
                ...
                for (auto id : indices) std::clog << " " << id;
                std::clog << std::endl;
            }
        }
        ...
        for (auto &[state, byte_idx_field_map] : byte_idx_to_field) {
            ...
            for (auto &[byte, fields] : byte_idx_field_map) {
                ...
                for (auto *f : fields) std::clog << " " << f->name;
                std::clog << std::endl;
            }
        }
...
const ordered_set<const IR::BFN::ParserState *> *ClotInfo::find_full_states(
    ...
    if (num_clots_allocated(gress) < MAX_LIVE_CLOTS) {
        ...
        BUG_CHECK(largest->first <= MAX_LIVE_CLOTS,
                  "Packet has %d live CLOTs, when at most %d are allowed", largest->first,
                  MAX_LIVE_CLOTS);

        result = largest->first < MAX_LIVE_CLOTS ? new ordered_set<const IR::BFN::ParserState *>()
                                                 : largest->second;
    }
...
bool ClotInfo::is_used_in_multiple_checksum_update_sets(const PHV::Field *field) const {
    if (!field_to_checksum_updates_.count(field)) return false;
...
bool ClotInfo::is_extracted_in_multiple_non_mutex_states(const PHV::Field *f) const {
    ...
    for (auto state : Keys(states_to_sources)) {
        ...
        BUG_CHECK(parser == cur_parser, "Extracts of %1% don't all come from the same parser", f);
    }
...
bool ClotInfo::extracted_with_pov(const PHV::Field *field) const {
    if (!fields_to_pov_bits_.count(field)) return true;
...
bool ClotInfo::can_be_in_clot(const PHV::Field *field) const {
    if (do_not_use_clot_fields.count(field)) {
        LOG5("  Field " << field->name
                        << " can't be in a CLOT: " << "it is marked by @pragma do_not_use_clot");
        return false;
    }
    ...
    if (is_used_in_multiple_checksum_update_sets(field)) {
        LOG5("  Field " << field->name
                        << " can't be in a CLOT: it's involved in multiple "
    ...
    if (is_extracted_in_multiple_non_mutex_states(field)) {
        LOG5("  Field " << field->name
                        << " can't be in a CLOT: it's extracted in multiple parser "
                           "states");
        return false;
    }
    ...
    if (!extracts_full_width(field)) {
        LOG5("  Field " << field->name
                        << " can't be in a CLOT: its full width is not extracted "
    ...
    if (!has_consistent_bit_in_byte_offset(field)) {
        LOG5("  Field " << field->name
                        << " can't be in a CLOT: it has different bit-in-byte "
    ...
    if (!extracted_with_pov(field)) {
        LOG5(" Field "
             << field->name
             << " can't be in a CLOT: there is exists a path through "
...
bool ClotInfo::can_start_clot(const FieldSliceExtractInfo *extract_info) const {
    ...
    if (offset_from_byte != 0 && slice->size() < 9 - offset_from_byte) {
        LOG6("  Can't start CLOT with " << slice->shortString()
                                        << ": not byte-aligned, and "
    ...
    if (is_slice_below_min_offset(slice, extract_info->min_packet_bit_offset())) {
        LOG6("  Can't start CLOT with " << slice->field()->name
                                        << " : min offset"
...
bool ClotInfo::can_end_clot(const FieldSliceExtractInfo *extract_info) const {
    ...
    if (offset_from_byte != 0 && slice->size() < offset_from_byte + 1) {
        LOG6("  Can't end CLOT with " << slice->shortString()
                                      << ": not byte-aligned, and "
                                         "doesn't contain byte boundary");
        return false;
    }
...
bool ClotInfo::extracts_full_width(const PHV::Field *field) const {
    if (!field_to_parser_states_.count(field)) return false;
...
bool ClotInfo::is_checksum(const PHV::Field *field) const { return checksum_dests_.count(field); }
...
bool ClotInfo::is_unused(const PHV::FieldSlice *slice) const { return is_unused(slice->field()); }
...
bool ClotInfo::adjust(const PhvInfo &phv, Clot *clot) {
    ...
    for (const auto &[parser_state, slices] : clot->parser_state_to_slices()) {
        ...
        for (auto slice : slices) {
            ...
            LOG5("      overwrite_mask: "
                 << overwrite_mask << ", first_zero_idx: " << first_zero_idx
        ...
        if (num_start_bits_overwritten < clot->length_in_bytes(parser_state) * 8) {
            for (auto slice : boost::adaptors::reverse(slices)) {
                ...
                LOG5("      overwrite_mask: "
                     << overwrite_mask << ", last_zero_idx: " << last_zero_idx
                     << ", num_end_bits_overwritten: " << num_end_bits_overwritten);
                if (last_zero_idx < slice->size()) break;
            }
        }
        LOG5("  num_end_bits_overwritten:" << num_end_bits_overwritten << ", clot length_in_bytes: "
                                           << clot->length_in_bytes(parser_state));

        BUG_CHECK(num_end_bits_overwritten % 8 == 0,
                  "CLOT %d ends with %d overwritten bits, which are not byte-aligned", clot->tag,
                  num_end_bits_overwritten);
...
void ClotInfo::crop(Clot *clot, cstring parser_state, unsigned num_bits, bool from_start) {
    Log::TempIndent indent;
    LOG3("Cropping clot for parser state " << parser_state << ", num_bits: " << num_bits << indent);
    if (num_bits == 0) return;

    unsigned num_bits_skipped = 0;
    BUG_CHECK(clot->parser_state_to_slices_.count(parser_state),
              "Tried to crop %1% in a parser state it is not present in (%2%)", *clot,
              parser_state);
    ...
    for (auto slice : current_slices) {
        ...
        BUG_CHECK(!clot->is_checksum_field(field),
                  "Attempted to remove a slice of checksum field %s from CLOT %d", field->name,
                  clot->tag);
...
void ClotInfo::crop(Clot *clot, cstring parser_state, unsigned start_bits, unsigned end_bits) {
    BUG_CHECK(start_bits + end_bits <= clot->length_in_bytes(parser_state) * 8,
              "Cropping %d bits from CLOT %d, which is only %d bits long", start_bits + end_bits,
              clot->tag, clot->length_in_bytes(parser_state) * 8);

    crop(clot, parser_state, start_bits, true);
    crop(clot, parser_state, end_bits, false);
}

bool ClotInfo::slice_overwritten(const PhvInfo &phv, const Clot *clot,
                                 const PHV::FieldSlice *slice) const {
    return clot_covers_slice(clot, slice) && !bits_overwritten(phv, clot, slice).empty();
}

bool ClotInfo::slice_overwritten_by_phv(const PhvInfo &phv, const Clot *clot,
                                        const PHV::FieldSlice *slice) const {
    return clot_covers_slice(clot, slice) && !bits_overwritten_by_phv(phv, clot, slice).empty();
}

template <Endian Order>
bitvec ClotInfo::bits_overwritten(const PhvInfo &phv, const Clot *clot,
                                  const PHV::FieldSlice *slice) const {
...
template <Endian Order>
bitvec ClotInfo::bits_overwritten_by_phv(const PhvInfo &phv, const Clot *clot,
                                         const PHV::FieldSlice *slice) const {
    bitvec result;
    ...
    field->foreach_alloc(
        slice_range, PHV::AllocContext::DEPARSER, nullptr, [&](const PHV::AllocSlice &alloc) {
            ...
            //
            // We can ignore any slice allocated to the container if that allocated slice satisfies
            // any of the following conditions:
            //
            //   - The given slice and the allocated slice are from mutually exclusive fields.
            //   - The given slice and the allocated slice from different fields, and those fields
            //   are
            //     allocated to overlapping ranges in the container.
            //
            // Otherwise, we know one of the following is true, in which case, the container
            // overwrites the CLOT if the CLOT does not completely cover the allocated slice.
            //
            ...
            //     fields, and those fields are allocated to non-overlapping ranges in the
            //     container.
            if (!container_overwrites) {
                // Get the current container and figure out which bits of the container are occupied
                // by the field of the slice we were given.
                auto container = alloc.container();
                auto occupied_bits =
                ...
                for (const auto &alloc_slice :
                    ...
                    LOG7("  Container read slice: " << alloc_slice
                    ...
                    // Container overwrites the CLOT if the CLOT doesn't completely cover the
                    // allocated slice or if the other slice is modified.
                    const auto *other_slice =
                        new PHV::FieldSlice(other_field, alloc_slice.field_slice());
                    container_overwrites =
                        !clot_covers_slice(clot, other_slice) || is_modified(other_field);
                    LOG7("    other slice: "
                         << other_slice << ", container_overwrites: " << container_overwrites
...
bool ClotInfo::is_added_by_mau(cstring h) const { return headers_added_by_mau_.count(h); }

assoc::map<const PHV::FieldSlice *, Clot *, PHV::FieldSlice::Greater> *ClotInfo::slice_clots(
    const PHV::FieldSlice *slice) const {
    ...
    for (auto clot : clots_) {
        ...
        if (clot_slice->range().overlaps(slice->range())) (*result)[clot_slice] = clot;
    }
...
Clot *ClotInfo::whole_field_clot(const PHV::Field *field) const {
    for (auto clot : clots_)
        if (clot->has_slice(new PHV::FieldSlice(field))) return clot;
    return nullptr;
}
...
std::string ClotInfo::print(const PhvInfo *phvInfo) const {
    ...
    for (auto gress : {INGRESS, EGRESS}) {
        ...
        TablePrinter tp(out, {"CLOT", "Fields", "Bits", "Property"}, TablePrinter::Align::CENTER);
        ...
        for (auto entry : clot_to_parser_states()) {
            ...
            for (auto it = state_to_slices.begin(); it != state_to_slices.end(); ++it) {
                ...
                for (const auto *slice : slices) {
                    ...
                    bits << slice->size() << " [" << clot->bit_offset(parser_state, slice) << ".."
                    ...
                    bool is_phv_overwrite =
                        phvInfo && slice_overwritten_by_phv(*phvInfo, clot, slice);
                ...
                if (!last_row) tp.addBlank();
            }
            total_bits += bits_in_clot;
            if (bits_in_clot % 8 != 0) unaligned_clots.insert(clot->tag);
    ...
    TablePrinter field_tp(out, {"Field", "Bits", "CLOTs", "Property"}, TablePrinter::Align::CENTER);
    ...
    field_tp.addRow(
        {"Unused CLOT-allocated fields", std::to_string(total_unused_fields_in_clots), "", ""});
    field_tp.addRow(
        {"Unused CLOT-allocated bits", std::to_string(total_unused_bits_in_clots), "", ""});
...
std::pair<unsigned, ordered_set<const IR::BFN::ParserState *> *> *ClotInfo::find_largest_paths(
    ...
    auto result = new std::pair<unsigned, ordered_set<const IR::BFN::ParserState *> *>(
    ...
    if (state) {
        ...
        if (in_loopback) {
            if (const auto *hs = base->to<IR::HeaderStack>()) {
                int index = -1;
                if (const auto *c = hsir->index()->to<IR::Constant>()) index = c->asInt();
...
void CollectClotInfo::add_pseudoheader(
    ...
    if (LOGGING(6)) {
        ...
        LOG6(out.str());
        LOG6("  Fields:");
        for (auto field : fields) LOG6("    " << field->name);
    }
}
