...
class Clot final : public LiftCompare<Clot> {
 ...
 public:
    ...
    bool operator==(const Clot &clot) const { return tag == clot.tag && gress == clot.gress; }
 ...
 private:
    void set_slices(cstring parser_state, const std::vector<const PHV::FieldSlice *> &slices);
    ...
    void add_slice(cstring parser_state, FieldKind kind, const PHV::FieldSlice *slice);
    ...
    std::map<cstring, std::vector<const PHV::FieldSlice *>> parser_state_to_slices_;
 ...
 public:
    ...
    const std::set<const PHV::Field *> &checksum_fields() const { return checksum_fields_; }
