...
Clot::Clot(cstring name) {
    ...
    if (str.length() <= 5 || str.substr(0, 5) != "clot ") BUG("Invalid CLOT: '%s'", name);
    ...
    BUG_CHECK(sep_pos != std::string::npos && identifier.length() > sep_pos + 2 &&
    ...
    switch (kind) {
        ...
        case CHECKSUM:
            BUG_CHECK(slice->is_whole_field(),
                      "Attempted to allocate checksum slice to CLOT %d: %s", tag,
                      slice->shortString());
...
unsigned Clot::length_in_bytes(cstring parser_state) const {
    ...
    BUG_CHECK(parser_state_to_slices_.count(parser_state),
              "CLOT %d is not present in parser state %s", tag, parser_state);
    for (const auto *slice : parser_state_to_slices_.at(parser_state))
        length_in_bits += slice->range().size();

    BUG_CHECK(length_in_bits % 8 == 0, "CLOT %d has %d bits, which is not a whole number of bytes",
              tag, length_in_bits);

    return length_in_bits / 8;
}

unsigned Clot::bit_offset(cstring parser_state, const PHV::FieldSlice *slice) const {
    unsigned offset = 0;
    BUG_CHECK(parser_state_to_slices_.count(parser_state),
              "CLOT %d is not present in parser state %s", tag, parser_state);
    ...
    BUG("Field %s not in %s when issued by parser state %s", slice->shortString(), toString(),
        parser_state);
}

unsigned Clot::byte_offset(cstring parser_state, const PHV::FieldSlice *slice) const {
    return bit_offset(parser_state, slice) / 8;
}

bool Clot::is_phv_field(const PHV::Field *field) const { return phv_fields_.count(field); }
...
bool Clot::is_first_field_in_clot(const PHV::Field *field) const {
    for (const auto &kv : parser_state_to_slices_) {
        ...
        if (first_field == field) return true;
    }
    return false;
}
...
void Clot::set_slices(cstring parser_state, const std::vector<const PHV::FieldSlice *> &slices) {
    ...
    for (auto slice : slices) {
        ...
        BUG_CHECK(fields_to_slices_.count(field),
                  "Found a foreign field %s when setting slices for CLOT %d", field->name, tag);
