...
std::ostream &outputDebugInfo(std::ostream &out, indent_t indent, const BFN::DebugInfo &sourceDebug,
    ...
    for (auto &info : sourceDebug.info) {
        ...
        if (povDebug.info.size() == 1) out << " if " << povDebug.info.back();
    }

    if (povDebug.info.size() > 1) {
        out << std::endl << indent << "# pov:";
        for (auto &info : povDebug.info) out << std::endl << indent << "# - " << info;
    }
...
std::ostream &outputDebugInfo(std::ostream &out, indent_t indent,
    ...
    return povBitRef ? outputDebugInfo(out, indent, sourceRef->debug, povBitRef->debug)
...
struct OutputDictionary : public Inspector {
    ...
    bool preorder(const IR::BFN::LoweredEmitClot *emit) override { return preorder(emit->clot); }
    ...
    bool preorder(const IR::BFN::EmitClot *emit) override {
        ...
        if (range.size() != povField->size) out << "(" << range.lo << ")";
        out << std::endl;
...
struct OutputDigests : public Inspector {
    ...
    bool preorder(const IR::BFN::LoweredDigest *digest) override {
        ...
        if (digest->name == "learning") outputLearningContextJson(digest);
 ...
 private:
    void outputLearningContextJson(const IR::BFN::LoweredDigest *digest) {
        ...
        for (auto *digestEntry : digest->entries) {
            ...
            BUG_CHECK(entry,
                      "Digest table entry isn't a learning table "
                      "entry: %1%",
                      digestEntry);
