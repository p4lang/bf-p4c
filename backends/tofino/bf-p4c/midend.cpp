 ...
 * * BFN::TypeInference
 * * BFN::TypeChecking
 * * P4::ClearTypeMap - clears the P4::TypeMap if the program has changed or the 'force' flag is
 * set.
 *
 * There are also some best practices that are tied to using BFN::TypeChecking and
 * P4::ReferenceMap/P4::TypeMap:
 * * Each top level PassManager (that will in any way use P4::ReferenceMap or P4::TypeMap) should
 * start by invoking BFN::TypeChecking to ensure that it is working with the up-to-date data.
 * * BFN::TypeChecking should be also invoked in the middle of a PassManager anytime the IR changes
 * in a way that might change P4::ReferenceMap or P4::TypeMap (changing indentificators, changing
 * types, ...) and some of the subsequent passes need those structures to be up-to-date. There might
 * be some exceptions to this - for example if some of the subsequent passes still require the old
 * maps to update different parts of the IR correctly. In those cases it should still be invoked
 * after the last such pass is completed.
 * * P4::ClearTypeMap should be used as the last pass of each PassManager (or even before every
 * BFN::TypeChecking invocation) to force the need to use BFN::TypeChecking in case anything
 * changed.
 *
...
class EnumOn32Bits : public P4::ChooseEnumRepresentation {
    ...
    bool convert(const IR::Type_Enum *type) const override {
        ...
        if (reserved_enums.count(type->name)) return false;
        return true;
    }
...
bool skipRegisterActionOutput(const Visitor::Context *ctxt, const IR::Expression *) {
    ...
    if (!name->path->name.name.endsWith("Action")) return true;
    ...
    for (unsigned i = 1; i < params->size(); ++i)
        if (auto param = params->parameters.at(i)->to<IR::Parameter>())
            if (dest_path->path->name == param->name) return false;

    return true;
}

bool skipFlexibleHeader(const Visitor::Context *, const IR::Type_StructLike *e) {
    if (e->getAnnotation("flexible"_cs)) return false;
    return true;
}
...
MidEnd::MidEnd(BFN_Options &options) {
    ...
    addPasses({
        ...
        Device::currentDevice() != Device::TOFINO || options.disable_direct_exit
            ? new P4::RemoveExits(&typeMap, typeChecking)
            : nullptr,
        new P4::ConstantFolding(&typeMap, true, typeChecking),
        ...
        new P4::SynthesizeActions(&refMap, &typeMap,
                                  new ActionSynthesisPolicy(skip_controls, &refMap, &typeMap),
                                  typeChecking),
        new P4::MoveActionsToTables(&refMap, &typeMap, typeChecking),
        new CopyBlockPragmas(&refMap, &typeMap, typeChecking, {"stage"_cs}),
        (options.egress_intr_md_opt) ? new RewriteEgressIntrinsicMetadataHeader(&refMap, &typeMap)
                                     : nullptr,
        ...
        BackendOptions()
                .disable_parse_min_depth_limit &&BackendOptions()
                .disable_parse_max_depth_limit
            ? nullptr
