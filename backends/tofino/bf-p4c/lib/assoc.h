...
template<typename... Ts> struct _void { using type = void;};
...
class CondIterableAssocBase<Base, Itble, typename _void<typename Base::key_compare>::type>
 ...
 public:
    static constexpr bool iterable = Itble == Iterable::Yes
        || (Itble == Iterable::Auto
            &&  // either the comparator is different then the default (or a transparent version
                // thereof)
...
class CondIterableAssoc : public CondIterableAssocBase<Base, Itble> {
 ...
 public:
    ...
    iterator begin() noexcept { this->_ensure_iterable(); return Base::begin(); }
    const_iterator begin() const noexcept { this->_ensure_iterable(); return Base::begin(); }
    const_iterator cbegin() const noexcept { this->_ensure_iterable(); return Base::cbegin(); }
    ...
    reverse_iterator rbegin() noexcept {
        this->_ensure_iterable(); return Base::rbegin();
    }
    const_reverse_iterator rbegin() const noexcept {
        this->_ensure_iterable(); return Base::rbegin();
    }
    const_reverse_iterator crbegin() const noexcept {
        this->_ensure_iterable(); return Base::crbegin();
    }
...
class map : public detail::CondIterableAssoc<std::map<Key, T, Compare, Allocator>, Itble> {
 ...
 public:
    ...
    map(std::initializer_list<typename Base::value_type> init,
        ...
        const Allocator& alloc = Allocator()) : Base(init, comp, alloc) { }
...
class set : public detail::CondIterableAssoc<std::set<Key, Compare, Allocator>, Itble> {
 ...
 public:
    ...
    // the initializer_list constructor must be explicitly specified for GCC 6
    // to pick it up
    set(std::initializer_list<typename Base::value_type> init,
        const Compare& comp = Compare(),
        const Allocator& alloc = Allocator()) : Base(init, comp, alloc) { }
