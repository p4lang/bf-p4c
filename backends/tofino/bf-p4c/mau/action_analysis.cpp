...
bool setBitRangeFits(unsigned value, int numBits, int container_size) {
    ...
    if ((value & topBit) || (value & 1)) {  // <---bitsToCheck
        value =
            ((value >> rotate) | (value << (container_size - rotate))) & makeMask(container_size);
    }
...
EncodeConstant canRotateConstant(unsigned value, int max_bits, int min_bits, int container_size,
                                 int constant_size) {
    ...
    if (setBits <= max_bits) {  // Zero extend & check the set bit pattern.
        if (setBitRangeFits(value, max_bits, container_size)) return EncodeConstant::WithZeroExtend;
    }
...
EncodeConstant valid_instruction_constant(unsigned value, int max_bits, int min_bits,
    ...
    if (value < max_value) return EncodeConstant::WithZeroExtend;
    if (value > complement - min_value) return EncodeConstant::WithSignExtend;
...
bool valid_instruction_constant(unsigned value, int max_bits, int min_bits, int container_size,
                                int constant_size) {
    auto valid =
        ::valid_instruction_constant(value, max_bits, min_bits, container_size, constant_size);
...
const IR::Expression *ActionAnalysis::ActionParam::unsliced_expr() const {
    ...
    if (auto *sl = expr->to<IR::Slice>()) return sl->e0;
    return expr;
}
...
unsigned ActionAnalysis::ConstantInfo::build_constant() {
    unsigned rv = 0;
    BUG_CHECK(!positions.empty(),
              "Building a constant over a container when no constants are "
...
unsigned ActionAnalysis::ConstantInfo::build_shiftable_constant() {
    ...
    if (rv_bv.empty()) return rv;

    return alignment.bitrange_contiguous() ? rv_bv.getrange(alignment.write_bits().min().index(),
                                                            alignment.bitrange_contiguous_size())
                                           : rv;
}
...
ActionAnalysis::ActionParam::speciality_t ActionAnalysis::classify_attached_output(
    ...
    if (auto mtr = at->to<IR::MAU::Meter>()) {
        ...
        BUG("%s: Unrecognized implementation %s on meter %s", mtr->srcInfo, mtr->implementation,
            mtr->name);
        return ActionParam::NO_SPECIAL;
    } else if (at->is<IR::MAU::StatefulAlu>()) {
        return ActionParam::METER_ALU;
    }
    BUG("%s: Unrecognizable Attached Output %s being used in an ALU operation", at->srcInfo,
        at->name);
}
 ...
 */
const IR::Expression *ActionAnalysis::isActionParam(const IR::Expression *e, le_bitrange *bits_out,
                                                    ActionParam::type_t *type) {
...
bool ActionAnalysis::preorder(const IR::MAU::ActionArg *arg) {
    ...
    if (!findContext<IR::MAU::Instruction>()) return false;
...
bool ActionAnalysis::preorder(const IR::BFN::ReinterpretCast *) { return true; }
...
bool ActionAnalysis::preorder(const IR::MAU::StatefulAlu *) { return false; }

bool ActionAnalysis::preorder(const IR::MAU::StatefulCall *) { return false; }
...
void ActionAnalysis::postorder(const IR::MAU::Instruction *instr) {
    ...
    if (phv_alloc) {
        ...
        int split_count = 0;
        field->foreach_alloc(bits, tbl, &use, [&](const PHV::AllocSlice &) { split_count++; });
        ...
        field->foreach_alloc(bits, tbl, &use, [&](const PHV::AllocSlice &alloc) {
            ...
            if (!split) {
                ...
                auto *write_slice = MakeSliceDestination(
                    field_action.write.expr, alloc.field_slice().lo, alloc.field_slice().hi);
                ...
                for (auto &read : field_action.reads) {
                    auto read_slice =
                        MakeSliceSource(read.expr, alloc.field_slice().lo, alloc.field_slice().hi,
                ...
                LOG5("Adding field action split " << field_action_split << " to container "
                                                  << container);
            }
        });
    } else {
...
void ActionAnalysis::verify_conditional_set_without_phv(cstring action_name, FieldAction &fa) {
    ...
    BUG_CHECK(fa.reads.size() == 2 || fa.reads.size() == 3,
              "Conditional set instruction not "
              "calculated correctly");

    if (fa.reads.size() == 3) {
        auto param = fa.reads[op_idx];
        BUG_CHECK(param.type == ActionParam::PHV,
                  "Conditional write instruction PHV "
                  "field not built correctly");
        op_idx++;
    }

    auto arg_param = fa.reads[op_idx];
    BUG_CHECK(arg_param.type != ActionParam::PHV,
              "Conditional write action data field not built "
              "correctly");
    if (arg_param.speciality != ActionParam::NO_SPECIAL) {
        warning(
            "Conditional set instruction parameter %s in instruction %s in action %s "
            "must be from an action argument or constant",
            arg_param.to_string(), fa.to_string(), action_name);
        has_warning = true;
        ...
        if (sequential) {
            ...
            for (auto read : field_action.reads) {
                ...
                if (written_fields[field].intersects(read_bits)) {
                    if (error_verbose) {
                        warning(
                            "Action %s has a read of a field %s "
        ...
        if (field_action.name == "conditionally-set") {
            ...
            for (auto read : field_action.reads) {
                ...
                if (non_phv_count > 1) {
                    if (error_verbose) {
                        warning(
                            "In action %s, the following instruction has multiple "
...
bool ActionAnalysis::initialize_alignment(const ActionParam &write, const ActionParam &read,
                                          ...
                                          cstring action_name) {
    if (cont_action.is_shift() && read.type == ActionParam::CONSTANT) return true;

    error_message = "In the ALU operation over container " + container.toString() + " in action " +
                    action_name + ", ";
    ...
    bool initialized;
    if (read.is_conditional) return true;
    if (read.type == ActionParam::PHV) {
        initialized =
            init_phv_alignment(read, read_src, cont_action, write_bits, container, error_message);
    } else if (ad_alloc) {
        if (read.type == ActionParam::ACTIONDATA)
            initialized =
                init_ad_alloc_alignment(read, cont_action, write_bits, action_name, container);
        else
            initialized =
                init_constant_alignment(read, cont_action, write_bits, action_name, container);
    } else {
        initialized = init_simple_alignment(read, cont_action, write_bits);
    }

    if (!initialized) cont_action.set_mismatch(read.type);
...
bool ActionAnalysis::init_phv_alignment(const ActionParam &read, const op_type_t read_src,
                                        ...
                                        const PHV::Container container, cstring &error_message) {
    le_bitrange range;
    ...
    BUG_CHECK(field,
              "%1%: Operand %2% of instruction %3% operating on container %4% must be "
              "a PHV.",
              read.expr->srcInfo, read.expr, cont_action, container);
    ...
    if (container_reads.size() > MAX_PHV_SOURCES) {
        error_message +=
            "an individual read phv is contained within more than 2 containers, and "
        ...
        if (constant != nullptr && (constant->value == 0)) {
            if (auto type = constant->type->to<IR::Type_Bits>()) {
                ...
                field->foreach_alloc(range, tbl, &use, [&](const PHV::AllocSlice &alloc) {
                    ...
                    if (alloc.field_slice().hi != field->size - 1) return;
...
bool ActionAnalysis::init_special_alignment(const ActionParam &read, ContainerAction &cont_action,
    ...
    if (alu_pos == nullptr || alu_param == nullptr) return false;
...
bool ActionAnalysis::init_ad_alloc_alignment(const ActionParam &read, ContainerAction &cont_action,
    ...
    BUG_CHECK(action_arg != nullptr,
              "Action argument not converted correctly in the "
              "ActionAnalysis pass");
    ...
    for (auto slot_bits : slot_bits_brs) {
        ...
        if (cont_action.counts[ActionParam::ACTIONDATA] == 0) {
            ...
            if (alias == nullptr) alias = alu_param->param->name();
            adi.alignment.add_alignment(mini_write_bits, slot_bits);
...
void ActionAnalysis::initialize_constant(const ActionParam &read, ContainerAction &cont_action,
                                         le_bitrange write_bits,
    ...
    BUG_CHECK(constant->value >= INT_MIN && constant->value <= UINT_MAX,
              "%s: Constant "
              "value in an instruction not split correctly",
              constant->srcInfo);
    ...
    for (auto read_bits : read_bits_brs) {
        ...
        uint32_t mask =
            read_bits.size() == 32 ? 0xffffffff : ((1U << read_bits.size()) - 1) << shift;
...
bool ActionAnalysis::init_hash_constant_alignment(const ActionParam &read,
    ...
    if (alu_param == nullptr) return false;
 ...
 */
bool ActionAnalysis::init_constant_alignment(const ActionParam &read, ContainerAction &cont_action,
                                             le_bitrange write_bits, cstring action_name,
                                             PHV::Container container) {
    ...
    // for the purposes must fit within a 32 bit section
    // Constant can be from MINX_INT <= x <= MAX_UINT
    BUG_CHECK(constant->fitsUint() || constant->fitsInt(),
              "%s: Constant "
              "value in an instruction not split correctly",
              constant->srcInfo);
    ...
    if (alu_param == nullptr) return init_simple_alignment(read, cont_action, write_bits);
 ...
 */
bool ActionAnalysis::init_simple_alignment(const ActionParam &read, ContainerAction &cont_action,
                                           le_bitrange write_bits) {
    Log::TempIndent indent;
    LOG5("Init simple alignment for read param " << read << " and write bits " << write_bits
                                                 << indent);
    if (read.type == ActionParam::ACTIONDATA)
        BUG_CHECK(isActionParam(read.expr),
                  "Action Data parameter not configured properly "
                  "in ActionAnalysis pass");
    else if (read.type == ActionParam::CONSTANT)
        BUG_CHECK(read.expr->is<IR::Constant>(),
                  "Constant parameter not configured properly "
                  "in ActionAnalysis pass");
...
void ActionAnalysis::determine_unused_bits(PHV::Container container, ContainerAction &cont_action) {
    ...
    for (auto &field_action : cont_action.field_actions) {
        ...
        if (write_field == nullptr) BUG("Verify Overwritten: Action does not have a write?");
        fieldsWritten.insert(write_field);
    }

    PHV::FieldUse use(PHV::FieldUse::WRITE);
    bitvec container_occupancy =
        phv.bits_allocated(container, fieldsWritten, cont_action.table_context, &use);
...
void ActionAnalysis::verify_P4_action_with_phv(cstring action_name) {
    Log::TempIndent indent;
    if (verbose)
        LOG2("Verifying P4 action with phv for action " << action_name << " in table " << tbl->name
                                                        << indent);
    for (auto &container_action : *container_actions_map) {
        ...
        BUG_CHECK(cont_action.field_actions.size() > 0,
                  "Somehow a container action has no "
        ...
        for (auto &field_action : cont_action.field_actions) {
            if (field_action.name == "conditionally-set") to_bitmasked_set = true;
        }
        ...
        for (auto &field_action : cont_action.field_actions) {
            ...
            for (auto &read : field_action.reads) {
                LOG5("Initialize_alignment for field action: " << field_action
...
bitvec ActionAnalysis::TotalAlignment::df_src2_mask(PHV::Container container) const {
    ...
    if (all_write_bits == bitvec(0, container.size())) return direct_write_bits;
    int df_hole_start = 0;
    int df_hole_end = 0;
...
bool ActionAnalysis::TotalAlignment::contiguous() const {
    if (direct_write_bits.is_contiguous()) return true;
    if (direct_write_bits.empty()) return false;
...
bool ActionAnalysis::TotalAlignment::is_wrapped_shift(PHV::Container container, int *lo,
    ...
    if (read_bits().popcount() == static_cast<int>(container.size()) ||
        ...
        if (right_shift == 0) {
            ...
            if (lo) *lo = right_shift;
            if (hi) *hi = right_shift - 1;
            return true;
        }
    } else {
        ...
        if (curr_read_bits.is_contiguous()) return false;
        ...
        if (lo) *lo = reverse.max().index() + 1;
        if (hi) *hi = reverse.min().index() - 1;
        return true;
    }
}
...
bool ActionAnalysis::TotalAlignment::verify_individual_alignments(PHV::Container &container) {
    Log::TempIndent indent;
    LOG4("ActionAnalysis::TotalAlignment::verify_individual_alignments on container : " << container
                                                                                        << indent);
 ...
 * has been rotated
 */
bool ActionAnalysis::TotalAlignment::deposit_field_src1() const { return contiguous(); }
...
bool ActionAnalysis::TotalAlignment::is_byte_rotate_merge_src(PHV::Container container) const {
    if (container.is(PHV::Size::b8)) return false;
    for (size_t i = 0; i < container.size(); i += 8) {
        if (direct_write_bits.getslice(i, 8).empty()) continue;
        bitvec write_bits_per_byte = (direct_write_bits | unused_container_bits).getslice(i, 8);
        if (write_bits_per_byte.popcount() == 8) continue;
        return false;
    }
...
bitvec ActionAnalysis::TotalAlignment::brm_src_mask(PHV::Container container) const {
    ...
    for (size_t i = 0; i < container.size(); i += 8) {
        bitvec write_bits_per_byte = (direct_write_bits | unused_container_bits).getslice(i, 8);
        if (write_bits_per_byte.popcount() == 8) rv.setrange(i, 8);
    }
    return rv;
}
...
bitvec ActionAnalysis::TotalAlignment::byte_rotate_merge_byte_mask(PHV::Container container) const {
    ...
    for (size_t i = 0; i < container.size(); i += 8) {
        ...
        BUG_CHECK(write_bits_per_byte.empty() || write_bits_per_byte.popcount() == 8,
                  "Illegal "
                  "call of byte rotate merge src mask");
        if (write_bits_per_byte.empty()) continue;
        rv.setbit(i / 8);
    }
    return rv;
}
...
bool ActionAnalysis::ContainerAction::is_byte_rotate_merge(PHV::Container container,
    ...
    if (container.is(PHV::Size::b8)) return false;
    if (name == "to-bitmasked-set") return false;
    if (ad_sources()) {
        if (!ad_alignment.is_byte_rotate_merge_src(container)) return false;
    }

    for (auto phv_ta : Values(phv_alignment)) {
        if (!phv_ta.is_byte_rotate_merge_src(container)) return false;
    }

    convert_instr_to_byte_rotate_merge = true;
    if (read_sources() == 1) implicit_src2 = true;
    return true;
}
...
bool ActionAnalysis::ContainerAction::verify_deposit_field_variant(PHV::Container container,
    ...
    if (phv_non_aligned > max_phv_non_aligned) return false;
    if (phv_non_contiguous > max_phv_non_contiguous) return false;
...
bool ActionAnalysis::ContainerAction::verify_set_alignment(PHV::Container container,
    ...
    LOG5("Non aligned non contiguous sources: "
         << non_aligned_and_non_contiguous_sources << ", non aligned sources: "
         << non_aligned_sources << ", non aligned phv sources : " << non_aligned_phv_sources
         << ", ad sources: " << ad_sources() << ", ad alignment is contiguous: "
         << ad_alignment.contiguous() << ", non contiguous sources: " << non_contiguous_sources);
    ...
    if (non_aligned_sources == 2 || non_contiguous_sources == 2)
        if (is_byte_rotate_merge(container, ad_alignment)) return true;
...
void ActionAnalysis::ContainerAction::determine_src1() {
    ...
    if (!src1_assigned) {
        for (auto &tot_align_info : phv_alignment) {
            ...
            if (!tot_alignment.aligned()) {
                ...
                src1_assigned = true;
                LOG5("SRC1 Assigned (Aligned): " << tot_align_info.first << ":" << tot_alignment);
            }
        }
    }
    ...
    if (!src1_assigned) {
        for (auto &tot_align_info : phv_alignment) {
            ...
            tot_alignment.is_src1 = true;
            src1_assigned = true;
            LOG5("SRC1 Assigned (Contiguous): " << tot_align_info.first << ":" << tot_alignment);
        }
    }
    ...
    if (!src1_assigned) {
        for (auto &tot_align_info : phv_alignment) {
            ...
            tot_alignment.is_src1 = true;
            src1_assigned = true;
            LOG5("SRC1 Assigned: " << tot_align_info.first << ":" << tot_alignment);
            break;
        }
    }
}
...
void ActionAnalysis::TotalAlignment::set_implicit_bits_from_mask(bitvec mask,
    ...
    LOG5("  Setting implicit_read_bits : 0x" << std::hex << implicit_read_bits
                                             << ", implicit_write_bits : 0x" << implicit_write_bits
                                             << std::dec);
}
...
void ActionAnalysis::ContainerAction::determine_implicit_bits(PHV::Container container,
    ...
    if (is_deposit_field_variant) {
        ...
        if (adi.initialized || ci.initialized) ad_alignment.determine_df_implicit_bits(container);
        ...
        BUG_CHECK((src1_mask & src2_mask).empty() &&
                      ...
                      (src1_mask | src2_mask).is_contiguous(),
                  "Byte rotate merge implicit bits "
                  "incorrectly done");
    } else {
        for (auto &ta : phv_alignment) {
...
bool ActionAnalysis::ContainerAction::verify_alignment(PHV::Container &container) {
    ...
    if (adi.initialized) adi.alignment.right_shift = ad_alignment.right_shift;
    if (ci.initialized) ci.alignment.right_shift = ad_alignment.right_shift;
...
bitvec ActionAnalysis::ContainerAction::specialities() const {
    ...
    for (auto &fa : field_actions) {
        for (auto &read : fa.reads) {
            if (read.type != ActionParam::ACTIONDATA) continue;
...
bool ActionAnalysis::ContainerAction::verify_overwritten(const PHV::Container container,
    ...
    for (auto &field_action : field_actions) {
        ...
        if (write_field == nullptr) BUG("Verify Overwritten: Action does not have a write?");
        fieldsWritten.insert(write_field);
        LOG4("Adding written field " << write_field->name << " for expr "
                                     << field_action.write.expr);
    }
    ...
    if (name == "invalidate") total_write_bits |= invalidate_write_bits;
...
bool ActionAnalysis::ContainerAction::verify_only_read(const PhvInfo &phv, int num_source) {
    ...
    BUG_CHECK(src_cnt == num_source,
              "Shift operation with improper number of sources, expected "
              "%d but got %d",
              num_source, src_cnt);
...
void ActionAnalysis::check_single_ad_params(ContainerAction &cont_action) {
    if (cont_action.field_actions.size() != 1) return;
    const IR::MAU::ActionArg *aa = nullptr;
    for (auto &param : cont_action.field_actions[0].reads) {
        ...
        LOG5("Multiple AD Params Count : " << ad_params << ", aa: " << aa->name
                                           << ", range: " << aa_range);
        if (ad_params > 0) {
...
void ActionAnalysis::check_constant_to_actiondata(ContainerAction &cont_action,
    ...
    if (counts[ActionParam::CONSTANT] == 0) return;

    if (!cont_action.ci.initialized) BUG("Constant not setup by the program correctly");
 ...
 */
bool ActionAnalysis::ContainerAction::verify_source_to_bit(int operands, PHV::Container container) {
     ...
     */
    if (name == "to-bitmasked-set") return true;
    ...
    for (size_t i = 0; i < container.size(); i++) {
        if (!(bit_uses[i] == operands || bit_uses[i] == 0)) return false;
    }
...
bool ActionAnalysis::ContainerAction::verify_phv_mau_group(PHV::Container container) {
    ...
    for (auto phv_ta : phv_alignment) {
        ...
        if (write_group != read_group) return false;
    }
    return true;
}
...
bool ActionAnalysis::ContainerAction::verify_speciality(cstring &error_message,
    ...
    if (specialities().empty()) return true;
    ...
    if (speciality_read && speciality_read->speciality == ActionParam::METER_ALU) {
        ...
        if (lo / container.size() != hi / container.size()) {
            error_message +=
                "the alignment of the output of the meter ALU forces the required "
...
bool ActionAnalysis::ContainerAction::verify_shift(cstring &error_message, PHV::Container container,
                                                   const PhvInfo &phv) {
    ...
    if (!total_overwrite_possible) {
        ...
        error_message +=
            "either a read or write in a shift operation is not the only field "
            "within a container";
    }
    return true;
}
...
bool ActionAnalysis::ContainerAction::verify_mocha_and_dark(cstring &error_message,
                                                            PHV::Container container) {
    if (container.is(PHV::Kind::normal)) return true;
    ...
    if (!(sources_needed == 1 && name == "set")) {
        ...
        error_message += cont_type_name +
                         " containers can only perform assignment operations "
...
bool ActionAnalysis::ContainerAction::verify_possible(cstring &error_message,
    ...
    LOG3("Sources needed: " << sources_needed << "[ PHV: " << counts[ActionParam::PHV]
                            << ", AD: " << actual_ad << " ]");
    ...
    if (sources_needed > 2) {
        if (actual_ad) {  // If action data as a source
            ...
            error_message += "Action " + action_name +
                             " writes fields using the same assignment "
                             "type but different source operands (both action parameter and phv)";
        } else {  // no action data sources
            error_code |= TOO_MANY_PHV_SOURCES;
            LOG4("Error Code Update: TOO_MANY_PHV_SOURCES");
            error_message +=
                "over 2 PHV sources for the ALU operation are required, thus rendering"
                " the action impossible";
        }
        return false;
    }

    if (!verify_mocha_and_dark(error_message, container)) return false;
    ...
    if (!is_from_set() && sources_needed != operands()) {
        ...
        error_message +=
            ("the number of operands(" + std::to_string(operands()) +
    ...
    if (!aligned) {
        ...
        error_message +=
            "the alignment of fields within the container renders the action "
            "impossible";
        return false;
    }
    ...
    if (check_overwrite) {
        ...
        if (!total_overwrite_possible) {
            ...
            LOG4("Error Code Update: ILLEGAL_OVERWRITE");
            error_message +=
                "the container is not completely overwritten when the operand is "
...
std::ostream &operator<<(std::ostream &out, const ActionAnalysis::ActionParam &ap) {
    out << "ActionParam: " << ap.expr << ", type: " << ap.get_type_string()
        << ", speciality: " << ap.get_speciality_string() << ", is cond: " << ap.is_conditional;
    return out;
}
...
std::ostream &operator<<(std::ostream &out, const ActionAnalysis::FieldAction &fa) {
    out << fa.name << " ";
    out << fa.write;
    for (auto &read : fa.reads) out << ", " << read;
    return out;
}
...
std::ostream &operator<<(std::ostream &out, const ActionAnalysis::ContainerAction &ca) {
    ...
    out << "Convert to [ Bitmasked Set : " << ca.convert_instr_to_bitmasked_set
        ...
        << " Byte Rotate Merge : " << ca.convert_instr_to_byte_rotate_merge << " ]" << Log::endl;
...
std::ostream &operator<<(std::ostream &out, const ActionAnalysis::TotalAlignment &ta) {
    out << "Total Alignment : [" << " DRB : " << ta.direct_write_bits
        << " DWB : " << ta.direct_write_bits << " IRB : " << ta.implicit_read_bits
        << " IWB : " << ta.implicit_write_bits << " UCB : " << ta.unused_container_bits << " ]";
    ...
    for (int i = 0; i < (int)ta.indiv_alignments.size(); i++) {
        ...
        if (i < ((int)ta.indiv_alignments.size() - 1)) out << Log::endl;
    }
    return out;
}
...
std::ostream &operator<<(std::ostream &out, const ActionAnalysis &aa) {
    Log::TempIndent indent;
    out << "Action Analysis: " << " table: " << aa.get_table()->name << indent << Log::endl;
    if (auto *cam = aa.get_container_actions_map()) {
        out << "[ ";
        for (auto [cont, cont_act] : *cam) out << cont << ":" << cont_act;
        out << " ]";
    }
