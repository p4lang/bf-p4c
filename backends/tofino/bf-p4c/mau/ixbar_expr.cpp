...
void P4HashFunction::slice(le_bitrange hash_slice) {
    ...
    BUG_CHECK(hash_bits.contains(shifted_hash_bits),
              "Slice over hash function is not able "
              "to be resolved");
...
bool P4HashFunction::equiv_inputs_alg(const P4HashFunction *func) const {
    if (algorithm != func->algorithm) return false;
    if (inputs.size() != func->inputs.size()) return false;
    for (size_t i = 0; i < inputs.size(); i++) {
        if (!inputs[i]->equiv(*func->inputs[i])) return false;
    }

    if (symmetrically_hashed_inputs != func->symmetrically_hashed_inputs) return false;
    return true;
}

bool P4HashFunction::equiv(const P4HashFunction *func) const {
    if (!equiv_dyn(func)) return false;
    if (!equiv_inputs_alg(func)) return false;
    if (hash_bits != func->hash_bits) return false;
    return true;
}

bool P4HashFunction::is_next_bit_of_hash(const P4HashFunction *func) const {
    if (size() != 1) return false;
    if (hash_bits.lo != func->hash_bits.hi + 1) return false;
...
bool P4HashFunction::overlap(const P4HashFunction *func, le_bitrange *my_overlap,
    ...
    bool equal_inputs =
        is_dynamic() || func->is_dynamic() ? equiv_dyn(func) : equiv_inputs_alg(func);
    if (!equal_inputs) return false;

    auto boost_sl =
        toClosedRange<RangeUnit::Bit, Endian::Little>(hash_bits.intersectWith(func->hash_bits));
    ...
    if (my_overlap) {
        auto ov_boost_sl =
            toClosedRange<RangeUnit::Bit, Endian::Little>(hash_bits.intersectWith(overlap));
    ...
    if (hash_overlap) {
        auto ov_boost_sl =
            toClosedRange<RangeUnit::Bit, Endian::Little>(func->hash_bits.intersectWith(overlap));
...
bool verifySymmetricHashPairs(const PhvInfo &phv, safe_vector<const IR::Expression *> &field_list,
    ...
    for (auto annot : annotations->annotations) {
        ...
        if (field0 == nullptr || field1 == nullptr) {
            error(
                "%1%: The key %2% in the symmetric annotation is not recognized as a PHV "
                "field",
                annot->srcInfo, ((field0 == nullptr) ? sl0->value : sl1->value));
            continue;
        }
        ...
        if (key_pos0 == field_list.end() || key_pos1 == field_list.end()) {
            error(
                "%1%: The key %2% in the symmetric annotation does not appear within the "
                "field list",
                annot->srcInfo, ((key_pos0 == field_list.end()) ? sl0->value : sl1->value));
            continue;
        }

        if (!(symmetric_fields.count(fs0) == 0 && symmetric_fields.count(fs1) == 0)) {
            error(
                "%1%: The key %2% in the symmetric annotation has already been declared "
                "symmetric with another field.  Please use it only once",
                annot->srcInfo, (symmetric_fields.count(fs0) == 0) ? sl0->value : sl1->value);
            continue;
        }
        ...
        if (sym_pairs) (*sym_pairs)[std::max(index0, index1)][std::min(index0, index1)] = 1;
    }
...
bool BuildP4HashFunction::InsideHashGenExpr::preorder(const IR::MAU::ActionArg *aa) {
    error(
        "%s: Action Data Argument %s cannot be used in a hash generation expression.  "
        "Data plane values and constants only",
        aa->srcInfo, aa->name);
    return false;
}
...
bool BuildP4HashFunction::InsideHashGenExpr::preorder(const IR::Cast *) { return true; }

bool BuildP4HashFunction::InsideHashGenExpr::preorder(const IR::Concat *) { return true; }
...
bool BuildP4HashFunction::InsideHashGenExpr::preorder(const IR::StructExpression *) { return true; }
...
void BuildP4HashFunction::InsideHashGenExpr::postorder(const IR::BFN::SignExtend *se) {
    ...
    auto expr = se->expr;
    while (auto c = expr->to<IR::Cast>()) expr = c->expr;
...
void BuildP4HashFunction::InsideHashGenExpr::postorder(const IR::MAU::HashGenExpression *hge) {
    ...
    if (hge->dynamic) self._func->dyn_hash_name = hge->id.name;
}
...
void BuildP4HashFunction::end_apply() {
    bool repeats_allowed =
        !_func->is_dynamic() && _func->algorithm.type != IR::MAU::HashFunction::CRC;
    if (repeats_allowed) return;
...
bool AdjustIXBarExpression::preorder(IR::MAU::IXBarExpression *e) {
    ...
    if (findContext<IR::MAU::HashDist>()) return false;
