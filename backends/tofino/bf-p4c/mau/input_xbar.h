...
struct IXBar : public IHasDbPrint {
    ...
    struct Loc {
        ...
        bool operator==(const Loc &loc) const { return group == loc.group && byte == loc.byte; }
    ...
    struct FieldInfo {
        ...
        bool operator!=(const FieldInfo &fi) const { return !((*this) == fi); }
        ...
        bitvec cont_loc() const { return bitvec(cont_lo, width()); }
    ...
    struct Use {
        ...
        struct Byte {
            ...
            void set_spec(byte_speciality_t bs) { specialities |= (1 << bs); }

            void clear_spec(byte_speciality_t bs) { specialities &= ~(1 << bs); }

            bool is_spec(byte_speciality_t bs) const { return specialities & (1 << bs); }
            ...
            bool operator==(const Byte &b) const { return container == b.container && lo == b.lo; }
            ...
            bool has_field_slice(const PHV::FieldSlice &sl) const {
                for (auto &fi : field_bytes) {
                    if (fi.field == sl.field()->name && sl.range().overlaps(fi.lo, fi.hi))
                        return true;
                }
                return false;
            }
        ...
        struct TotalInfo {
            ...
            TotalInfo(int hg, safe_vector<GroupInfo> agi) : hash_group(hg), all_group_info(agi) {}
        };
 ...
 protected:
    ...
    static void add_use(ContByteConversion &map_alloc, const PHV::Field *field, const PhvInfo &phv,
                        const IR::MAU::Table *ctxt, std::optional<cstring> aliasSourceName,
                        const le_bitrange *bits = nullptr, int flags = 0,
                        byte_type_t byte_type = NO_BYTE_TYPE, unsigned extra_align = 0,
                        int range_index = 0, int pragma_forced_ixbar_group = -1);
