...
std::ostream &operator<<(std::ostream &out, const DependencyGraph &dg) {
    ...
    std::map<int, safe_vector<std::pair<const IR::MAU::Table *, DependencyGraph::StageInfo>>>
        min_stage_info;
    ...
    for (auto &msi : min_stage_info) {
        ...
        for (auto val : msi.second) {
            ...
            if (!dg.display_min_edges) continue;
            if (dg.min_stage_edges.count(val.first) == 0) continue;
            for (auto edge : dg.min_stage_edges.at(val.first)) {
                out << "\t- Edge " << dep_types(edge.second) << " " << edge.first->name << " ( "
                    << edge.first->externalName() << " )" << std::endl;
            }
        }
    }
...
void DependencyGraph::dump_viz(std::ostream &out, const DependencyGraph &dg) {
    static ordered_map<DependencyGraph::dependencies_t, std::pair<cstring, cstring>> dep_color = {
        {DependencyGraph::IXBAR_READ, std::make_pair("ixbar_read"_cs, "gold"_cs)},
        {DependencyGraph::ACTION_READ, std::make_pair("action_read"_cs, "blue"_cs)},
        {DependencyGraph::OUTPUT, std::make_pair("output"_cs, "red"_cs)},
        {DependencyGraph::CONT_CONFLICT, std::make_pair("cont_conflict"_cs, "navy"_cs)},
        {DependencyGraph::REDUCTION_OR_READ, std::make_pair("reduction_or_read"_cs, "cyan"_cs)},
        {DependencyGraph::REDUCTION_OR_OUTPUT, std::make_pair("reduction_or_output"_cs, "cyan"_cs)},
        {DependencyGraph::ANTI_TABLE_READ, std::make_pair("anti_table_read"_cs, "pink"_cs)},
        {DependencyGraph::ANTI_ACTION_READ, std::make_pair("anti_next_action_read"_cs, "pink"_cs)},
        ...
        {DependencyGraph::ANTI_EXIT, std::make_pair("exit"_cs, "black"_cs)},
        {DependencyGraph::CONTROL_ACTION, std::make_pair("control_action"_cs, "green"_cs)},
        ...
        {DependencyGraph::CONTROL_TABLE_HIT, std::make_pair("control_table_hit"_cs, "green"_cs)},
        {DependencyGraph::CONTROL_TABLE_MISS, std::make_pair("control_table_miss"_cs, "green"_cs)},
        {DependencyGraph::CONTROL_DEFAULT_NEXT_TABLE,
         std::make_pair("control_default_next_table"_cs, "green"_cs)},
        {DependencyGraph::CONTROL_EXIT, std::make_pair("control_exit"_cs, "green"_cs)}};
    ...
    for (auto ts : dg.stage_info) tables_per_stage[ts.second.min_stage].insert(tableName(ts.first));
    ...
    for (boost::tie(edges, edges_end) = boost::edges(dg.g); edges != edges_end; ++edges) {
        ...
        out << "   \"" << src_name.c_str() << "\" -> \"" << dst_name.c_str() << "\" [ "
    ...
    for (auto &kv : name_pairs) {
        ...
        for (auto &edge : edges) {
            if ((edge == "ANTI_EXIT") || (edge == "ANTI_TABLE_READ") ||
                (edge == "ANTI_ACTION_READ") || (edge == "ANTI_NEXT_TABLE_DATA") ||
                (edge == "ANTI_NEXT_TABLE_CONTROL") || (edge == "ANTI_NEXT_TABLE_METADATA")) {
                continue;
            ...
            } else if ((edge == "CONTROL_ACTION") || (edge == "CONTROL_COND_TRUE") ||
                       (edge == "CONTROL_COND_FALSE") || (edge == "CONTROL_TABLE_HIT") ||
                       (edge == "CONTROL_TABLE_MISS") || (edge == "CONTROL_DEFAULT_NEXT_TABLE") ||
                       (edge == "CONTROL_EXIT")) {
...
std::pair<typename DependencyGraph::Graph::edge_descriptor *, bool> DependencyGraph::add_edge(
    const IR::MAU::Table *src, const IR::MAU::Table *dst,
    ...
    if (has_back_edge(src, dst)) {
        LOG7("Cannot add edge " << src->name << " --> " << dst->name << " as back-edge is present");
        return {e, false};
    }
...
void DependencyGraph::to_json(Util::JsonObject *dgsJson, const FlowGraph &fg, cstring passContext,
                              bool placed) {
    if (!dgsJson) return;
    ...
    if (!fg.is_empty()) {
        ...
        for (boost::tie(fedges, fedges_end) = boost::edges(fg.g); fedges != fedges_end; ++fedges) {
            ...
            if (src_gress != tgt_gress) {
                ...
                warning(
                    " Invalid gress on flow graph edges while creating"
    ...
    auto check_and_add_graph = [&](Util::JsonArray *gJson) {
        for (auto g : {INGRESS, EGRESS, GHOST}) {
            if (dgJson[g]->size() > 0) {
                ...
                if (nodes->size() > 0 && edges->size() > 0) gJson->append(dgJson[g]);
            }
        }
    };
...
class FindDataDependencyGraph::AddDependencies : public MauInspector, TofinoWriteContext {
 ...
 public:
    AddDependencies(FindDataDependencyGraph &self, const IR::MAU::Table *t)
        : self(self), table(t) {}
 ...
 private:
    ...
    void addDeps(ordered_set<std::pair<const IR::MAU::Table *, const IR::MAU::Action *>> tables,
        ...
        for (auto upstream_t_pair : tables) {
            ...
            if (dep == DependencyGraph::ANTI_TABLE_READ) {
                if (!upstream_t_pair.second) new_dep = DependencyGraph::ANTI_ACTION_READ;
            }

            if (self.dg.has_back_edge(upstream_t, table)) {
                LOG7("\tCannot add edge between " << upstream_t->name << " and " << table->name
    ...
    void addContDeps(const cont_write_t &tables, const ordered_set<write_op_t> &writes,
        ...
        if (LOGGING(7)) {
            ...
            for (auto f : writes) LOG7("\t\t" << f.first << " , " << f.second);
            LOG7("\tAll Table Writes: ");
            for (auto t : tables) {
                LOG7("\t\tTable: " << t.first->name);
                for (auto f : t.second) LOG7("\t\t\t" << f.first << " , " << f.second);
            }
                ...
                for (auto t : tables) {
                    for (auto f2 : t.second) {
                        ...
                        LOG7("\t\t( " << f1F << " , " << f2F << " ) = FeldMutex "
                                      << self.phv.isFieldMutex(f1F, f2F) << ", MetdataMutex "
                                      << self.phv.isMetadataMutex(f1F, f2F));
                    }
                }
            }
            LOG7("-------------------------------------------------------------");
        ...
        for (auto upstream_t : tables) {
            ...
            if (self.ignore.ignore_deps(table, upstream_t.first)) {
                WARN_CHECK(upstream_range == range, BFN::ErrorType::WARN_PRAGMA_USE,
                           ...
                           "%3%, which may not have been the desired outcome",
                           table, upstream_t.first->name, container.toString());
                continue;
            }
                ...
                for (auto w : writes) {
                    ...
                    if (self.phv.isFieldMutex(uf, f)) {
                        LOG1("\t Skipping container conflict (container "
                             << container << ") between tables as ( " << table->name << " and "
                             << upstream_t.first->name << " ) as fields ( " << f << " and " << uf
                        ...
                        LOG1("\tAdd container conflict between table "
                             << upstream_t.first->name << " and table " << table->name
                             << " because of container " << container);
            ...
            if (any_unavoidable()) {
                LOG1("\tAdd unavoidable container conflict between table "
                     << upstream_t.first->name << " and table " << table->name
                     << " because of container " << container);
        ...
        for (const PHV::Field *field : candidateFields) {
            ...
            for (const cstring &field_name : field_slice_set) {
                if (self.access.count(field_name)) {
                    ...
                    if (isWrite()) {
                        if (table->is_always_run_action() || (table->is_detached_attached_tbl)) {
                            ...
                            if (is_red_or) {
                                ...
                                if (pos != self.red_or_use.end()) {
                                    ERROR_CHECK(pos->second == red_or_key,
                                                "%s: The reduction_or "
                                                "groups collide on field %s, over group %s and "
                                                "group %s",
                                                table->srcInfo, field_name, red_or_key,
                                                pos->second);
                        ...
                        if (isIxbarRead()) {
                            addDeps(self.access[field_name].write, DependencyGraph::IXBAR_READ,
                                    field);
                        } else if (is_red_or) {
                            // If reduction_or, and the previous write was a reduction_or
                            auto pos = self.red_or_use.find(field_name);
                            if (pos != self.red_or_use.end()) {
                                ERROR_CHECK(pos->second == red_or_key,
                                            "%s: "
                                            "The reduction_or groups collide on field %s, over "
                                            "group %s and "
                                            "group %s",
                                            table->srcInfo, field_name, red_or_key, pos->second);
                                addDeps(self.access[field_name].reduction_or_write,
            ...
            if (isWrite() && !non_first_write_red_or && self.phv.alloc_done()) {
                ...
                field->foreach_alloc(range, table, &WRITE, [&](const PHV::AllocSlice &sl) {
                    ...
                    LOG5("Adding container write: "
                         << sl.container() << " = Field: " << sl.field()->name
                         << ", slice: " << sl.field_slice() << ", range: " << cont_range);
            ...
            for (const cstring &field_name : field_slice_set) {
                ...
                if (isWrite()) {
                    ...
                    bool is_red_or = self.dg.red_info.is_reduction_or(
                        findContext<IR::MAU::Instruction>(), table, red_or_key);
                    if (is_red_or) {
...
bool FindDataDependencyGraph::preorder(const IR::MAU::Table *t) {
    ...
    for (auto ixbar_read : t->match_key) ixbar_read->apply(AddDependencies(*this, t));
    for (auto &action : Values(t->actions)) action->apply(AddDependencies(*this, t));
    for (auto &at : t->attached) at->apply(AddDependencies(*this, t));
    ...
    for (auto &at : t->attached) at->apply(UpdateAccess(*this, t));
...
void DependencyGraph::fill_dep_stages_from_topo(
    ...
    for (int i = int(topo.size()) - 1; i >= 0; --i) {
        for (const auto &vertex : topo[i]) {
            ...
            int tbl_dep_stages = std::accumulate(
                happens_later.begin(), happens_later.end(), 0,
                [&](int sz, const IR::MAU::Table *later) {
                    ...
                    if (dep_type_map.count(table) && dep_type_map.at(table).count(later) &&
                        ...
                        if (!stage_addition) stage_addition = 1;
                    }
                    if (include_stages)
...
bool DependencyGraph::is_anti_edge(DependencyGraph::dependencies_t dep) const {
    return (dep == DependencyGraph::ANTI_EXIT || dep == DependencyGraph::ANTI_TABLE_READ ||
...
bool DependencyGraph::is_ctrl_edge(DependencyGraph::dependencies_t dep) const {
    return (
        ...
        dep == DependencyGraph::CONTROL_COND_FALSE || dep == DependencyGraph::CONTROL_TABLE_HIT ||
        dep == DependencyGraph::CONTROL_TABLE_MISS ||
        dep == DependencyGraph::CONTROL_DEFAULT_NEXT_TABLE || dep == DependencyGraph::CONTROL_EXIT);
}

bool DependencyGraph::is_non_directional_edge(DependencyGraph::dependencies_t dep) const {
    return (dep == DependencyGraph::CONT_CONFLICT || dep == DependencyGraph::REDUCTION_OR_READ ||
...
FindDependencyGraph::calc_topological_stage(unsigned dep_flags, DependencyGraph *local_dg) {
    ...
    ordered_map<DependencyGraph::Graph::vertex_descriptor,
                std::set<DependencyGraph::Graph::edge_descriptor>>
        n_depending_on_with_edges;
    ...
    if (curr_dg.has_cycle()) LOG7("The graph has a cycle");
    ...
    for (boost::tie(out, out_end) = boost::edges(dep_graph); out != out_end; ++out) {
        ...
        if ((include_anti || !(dg.is_anti_edge(dep))) &&
            ...
            dep != DependencyGraph::CONT_CONFLICT && dep != DependencyGraph::REDUCTION_OR_OUTPUT &&
    ...
    while (processed.size() < num_vertices(dep_graph)) {
        ...
        for (auto &kv : n_depending_on) {
            if (!processed.count(kv.first) && kv.second == 0) {
                ...
                LOG5("\t\t Non processed vertex: " << curr_dg.get_vertex(kv.first)->name << " with "
                                                   << kv.second << " incoming deps");
            } else if (!processed.count(kv.first) && kv.second) {
                LOG5("\t\t Non processed vertex: " << curr_dg.get_vertex(kv.first)->name << " with "
                                                   << kv.second << " incoming deps");
            } else {
                LOG5("\t\t Processed vertex: " << curr_dg.get_vertex(kv.first)->name << " with "
                                               << kv.second << " incoming deps");
            }
        }
        ...
        if (this_generation.size() == 0) {
            for (boost::tie(v, v_end) = boost::vertices(dep_graph); v != v_end; ++v) {
                if (processed.count(*v) == 0) {
                    LOG5("Unprocessed vertex " << curr_dg.get_vertex(*v)->name << " with "
                                               << n_depending_on[*v] << " incoming deps");
        ...
        for (auto &v : this_generation) {
            ...
            for (; out != out_end; ++out) {
                ...
                if ((include_anti || !(dg.is_anti_edge(dep))) &&
                    ...
                    auto rm_edge =
                        std::find(vertex_later_edges.begin(), vertex_later_edges.end(), *out);
...
ControlPathwaysToTable::Path ControlPathwaysToTable::common_reverse_path(
    const Path &a, const Path &b, bool check_diff_is_seq) const {
    Path rv;
    auto a_it = a.rbegin();
    auto b_it = b.rbegin();
    while (a_it != a.rend() && b_it != b.rend()) {
        ...
        if (a_seq && b_seq && equiv_seqs(a_seq, b_seq)) {
            ...
            if (check_diff_is_seq)
                BUG_CHECK(a_seq && b_seq,
                          "The dominator pass is broken as these node should be "
                          "sequences");
            break;
        }
        a_it++;
...
bool ControlPathwaysToTable::preorder(const IR::MAU::Table *tbl) {
    ...
    for (auto &path : table_pathways[tbl]) {
        if (equiv(path, pathway)) return true;
    }
 ...
 */
ControlPathwaysToTable::InjectPoints ControlPathwaysToTable::get_inject_points(
    const IR::MAU::Table *a, const IR::MAU::Table *b, bool tbls_only) const {
    InjectPoints rv;
    ...
    for (safe_vector<const IR::Node *> a_path : table_pathways.at(a)) {
        for (safe_vector<const IR::Node *> b_path : table_pathways.at(b)) {
            ...
            if (a_first_div->is<IR::MAU::TableSeq>()) {
                BUG_CHECK(b_first_div->is<IR::MAU::TableSeq>(),
                          "The first divergence is not "
                          "the same IR node, thus the IR tree is inconsistent");
                if (!tbls_only) rv.emplace_back(a_first_div, b_first_div);
                continue;
            }
            ...
            if (a_dom != nullptr) {
                ...
                BUG_CHECK(b_dom != nullptr,
                          "The first divergence is not the same IR Node, thus "
...
void DepStagesThruDomFrontier::postorder(const IR::MAU::Table *tbl) {
    ...
    ordered_map<const IR::MAU::Table *, DependencyGraph::Graph::vertex_descriptor>
        local_labelToVertex;
    ...
    for (auto cd_tbl : dom_frontier) {
        ...
        for (; out != out_end; ++out) {
            ...
            if (dom_frontier.count(second_tbl) == 0) continue;

            if (local_dg.has_back_edge(cd_tbl, second_tbl)) {
                LOG5("\tCannot add edge between " << cd_tbl->name << " and " << second_tbl->name
                                                  << ": " << dg.g[*out]
                                                  << " as a backedge already exists");
                continue;
            }
            LOG5("\tAdding edge between " << cd_tbl->name << " and " << second_tbl->name << " : "
                                          << dg.g[*out]);
...
void FindDependencyGraph::add_logical_deps_from_control_deps(void) {
    ...
    std::vector<ordered_set<DependencyGraph::Graph::vertex_descriptor>> topo_rst =
        calc_topological_stage(DependencyGraph::ANTI);
...
void FindDependencyGraph::finalize_dependence_graph(void) {
    ...
    std::vector<ordered_set<DependencyGraph::Graph::vertex_descriptor>> topo_rst_control =
        calc_topological_stage(DependencyGraph::CONTROL);
    for (int i = int(topo_rst_control.size()) - 1; i >= 0; --i) {
        for (const auto &vertex : topo_rst_control[i]) {
            const IR::MAU::Table *table = dg.get_vertex(vertex);
            auto &happens_later = dg.happens_before_work_map[table];
            dg.stage_info[table].dep_stages_control = std::accumulate(
                happens_later.begin(), happens_later.end(), 0,
                [this, table](int sz, const IR::MAU::Table *later) {
                    int stage_addition = 0;
                    if (dg.dep_type_map.count(table) && dg.dep_type_map.at(table).count(later) &&
                        !(dg.is_ctrl_edge(dg.dep_type_map.at(table).at(later))) &&
                        dg.dep_type_map.at(table).at(later) != DependencyGraph::CONT_CONFLICT &&
                        dg.dep_type_map.at(table).at(later) != DependencyGraph::REDUCTION_OR_READ &&
            ...
            LOG4("Dep stages of " << dg.stage_info[table].dep_stages << " for table "
                                  << table->name);
    ...
    for (auto kv : dg.happens_before_work_map) {
        ...
        for (auto *tbl : kv.second) map.insert(tbl);
    }
    ...
    if (LOGGING(4)) {
        ...
        for (auto &kv : dg.happens_logi_before_map) {
            ss << "Table " << kv.first->name << " has priors of ";
            for (auto &tbl : kv.second) ss << tbl->name << ", ";
            ss << std::endl;
        }
        LOG4(ss.str());
    }
    ...
    for (size_t i = 1; i < topo_rst_logical.size(); i++) {
        for (const auto &vertex : topo_rst_logical[i]) {
            ...
            for (auto edge = in_edges.first; edge != in_edges.second; edge++) {
                ...
                } else if (dg.is_ctrl_edge(dep) || dep == DependencyGraph::CONT_CONFLICT ||
                           dep == DependencyGraph::REDUCTION_OR_READ ||
                           dep == DependencyGraph::REDUCTION_OR_OUTPUT || dg.is_anti_edge(dep)) {
    ...
    if (LOGGING(3)) dg.display_min_edges = true;
    ...
    for (auto kv : dg.happens_after_work_map) {
        for (auto after_tbl : kv.second) {
            dg.happens_phys_after_map[kv.first].insert(dg.happens_logi_after_map[after_tbl].begin(),
    ...
    if (LOGGING(4)) DependencyGraph::dump_viz(std::cout, dg);
    calc_max_min_stage();
    dg.finalized = true;
}
...
void FindDependencyGraph::calc_max_min_stage() {
    ...
    dg.max_min_stage = (dg.max_min_stage_per_gress[0] > dg.max_min_stage_per_gress[1])
                           ? dg.max_min_stage_per_gress[0]
                           : dg.max_min_stage_per_gress[1];
    dg.max_min_stage = (dg.max_min_stage > dg.max_min_stage_per_gress[2])
                           ? dg.max_min_stage
...
FindDependencyGraph::FindDependencyGraph(const PhvInfo &phv, DependencyGraph &out,
                                         const BFN_Options *o, cstring dotFileName,
                                         cstring passCont, const TableSummary *s)
    : Logging::PassManager("table_dependency_graph"_cs, Logging::Mode::AUTO),
      ...
      summary(s) {
    addPasses({new NameToTableMapBuilder(dg), &mutex, &ntp, &con_paths, &ignore,
               new GatherReductionOrReqs(dg.red_info), new PrintPipe,
...
std::ostream &operator<<(std::ostream &out, DependencyGraph::dependencies_t deps) {
    static const char *bit_names[] = {"NONE",
                                      "CONTROL_ACTION",
                                      "CONTROL_COND_TRUE",
                                      "CONTROL_COND_FALSE",
                                      "CONTROL_TABLE_HIT",
...
bool PrintDependencyGraph::preorder(const IR::BFN::Pipe *p) {
    ...
    for (boost::tie(vertices, vertices_end) = boost::vertices(dg.g); vertices != vertices_end;
         ++vertices) {
...
std::stringstream PrintDependencyGraph::print_graph(const DependencyGraph &dg) {
    ...
    for (boost::tie(vertices_it, vertices_end) = boost::vertices(dg.g); vertices_it != vertices_end;
         ++vertices_it) {
    ...
    std::sort(vertices.begin(), vertices.end(), [&](auto a, auto b) {
        ...
        if (a_vertex->stage() != b_vertex->stage()) return a_vertex->stage() < b_vertex->stage();
    ...
    for (auto &src_v : vertices) {
        ...
        ss << " " << vertex_names.at(src_v) << "(" << dg.min_stage(t) << "," << dg.max_stage(t)
           << ") : " << t->gress;
        ss << std::endl;
    }
...
void PrintDependencyGraph::reverse_dfs(
    const IR::MAU::Table *tbl, std::stack<const IR::MAU::Table *> &chain,
    std::vector<std::vector<const IR::MAU::Table *>> &crit_chains,
    std::map<const IR::MAU::Table *, bool> &visited_tbls, int last_stg) {
    chain.push(tbl);
...
void PrintDependencyGraph::print_critical_chains(const DependencyGraph &dg) {
    ...
    LOG_FEATURE("table_dependency_summary", 3,
                Log::unindent << "*** Table dependency chains (of minimum length " << min_path_len
                              << ") ***");
    int c_num = 1;
