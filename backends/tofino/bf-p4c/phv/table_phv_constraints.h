...
class CollectForceImmediateFields : public MauInspector {
 ...
 public:
    explicit CollectForceImmediateFields(PhvInfo &p, const ActionPhvConstraints &a,
                                         PackConflicts &c)
...
class TablePhvConstraints : public PassManager {
 public:
    explicit TablePhvConstraints(PhvInfo &p, const ActionPhvConstraints &a, PackConflicts &c) {
        addPasses({new TernaryMatchKeyConstraints(p), new CollectForceImmediateFields(p, a, c)});
    }
};
