...
struct SliceListGroupProp {
    ...
    std::optional<std::pair<solver::ContainerID, int>> find_allocation(
        const FieldSlice &fs, const SliceListAssignedStarts &sl_assigned_start = {}) const {
...
bitvec compute_single_source_bytes_bv(const SliceListGroupProp &prop, const IR::MAU::Action *action,
                                      const SuperCluster::SliceList *sl, const int dest_offset,
...
Result validate_slicelist_for_action(const PhvUse &uses, const SliceListGroupProp &prop,
                                     const solver::ContainerID dest, const int dest_cont_size,
                                     const int dest_offset, const SuperCluster::SliceList *sl,
                                     const IR::MAU::Action *action, const bool loose_mode,
                                     const SliceListAssignedStarts &sl_assigned_start = {}) {
    // setup solver
...
std::optional<const SuperCluster::SliceList *> find_the_only_floating_src_sl(
    const SliceListGroupProp &prop, const SuperCluster::SliceList *sl,
...
Result validate_slicelist_for_action_allow_one_floating_src(
    ...
    if (!the_only_floating_src_sl) {
        return validate_slicelist_for_action(uses, prop, dest, dest_cont_size, dest_offset, sl,
                                             action, loose_mode);
    }
...
Result validate_one_byte_slice_list(const PhvUse &uses, const SliceListGroupProp &prop,
    ...
    for (const auto *action : prop.sl_actions.at(byte_list)) {
        ...
        for (int offset = 0; offset <= *cont_size - byte_list_sz; offset += step) {
            ...
            rst = validate_slicelist_for_action(uses, prop, "byte_dest"_cs, *cont_size, offset,
                                                byte_list, action, false);
            if (rst.code == Code::OK || rst.code == Code::UNKNOWN) {
        ...
        for (const auto *action : prop.sl_actions.at(sl)) {
            ...
            for (int offset = 0; offset <= floating_range; offset += step) {
                ...
                rst = validate_slicelist_for_action_allow_one_floating_src(
                    uses_i, prop, *prop.container(sl), *prop.container_size(sl), offset, sl, action,
                    loose_mode);
                if (rst.code == Code::OK || rst.code == Code::UNKNOWN) {
                    break;
                }
