...
Visitor::profile_t DetermineCandidateHeaders::init_apply(const IR::Node *root) {
    ...
    for (const auto &f : phv) allHeaders.insert(f.header());
    ;
...
void DetermineCandidateHeaders::end_apply() {
    ...
    for (auto &kv : headers) {
        std::stringstream ss;
        ss << "\t  " << kv.first << " : ";
        for (const auto *action : kv.second) ss << action->name << " ";
        LOG3(ss.str());
    }
}
...
bool DetermineCandidateFields::multipleSourcesFound(const PHV::Field *dest,
    ...
    for (auto &kv : candidateSources)
        if (kv.first != src) newSourceFound = true;
...
// :)
bool DetermineCandidateFields::incompatibleConstraints(const PHV::Field *dest,
                                                       const PHV::Field *src) const {
    ...
    if (src->size % 8 != 0) {
        if (d2i.dest_to_inst.count(src)) {
            for (auto inst : d2i.dest_to_inst.at(src)) {
                if (inst->name == "add" || inst->name == "sub") return true;
            }
        }
    }
    ...
    for (auto &candidate : candidateSources) {
        ...
        if (candidate.second.count(destField)) {
            ...
            LOG3("\tDrop candidate header : " << candidate.first->name
                                              << " because it's source : " << destField->name
