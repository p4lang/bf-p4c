...
Type::Type(const char *name, bool abort_if_invalid) {
    ...
    switch (*n++) {
        ...
        default:
            size_ = Size::null;
            if (abort_if_invalid) BUG("Invalid PHV type '%s'", name);
    }

    if (*n && abort_if_invalid) BUG("Invalid PHV type '%s'", name);
}
...
unsigned Type::log2sz() const {
    switch (size_) {
        ...
        case Size::b32:
            return 2;
        default:
            BUG("Called log2sz() on an invalid container");
    }
}
...
Container::Container(const char *name, bool abort_if_invalid) {
    ...
    if (end == n || *end || index_ != static_cast<unsigned long>(v)) {
        type_ = Type();
        if (abort_if_invalid) BUG("Invalid register '%s'", name);
    }
}
...
std::ostream &operator<<(std::ostream &out, PHV::Type t) { return out << t.kind() << t.size(); }
...
std::ostream &operator<<(std::ostream &out, const PHV::FieldUse u) { return out << u.toString(); }
