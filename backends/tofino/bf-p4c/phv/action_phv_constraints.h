...
using CanPackReturnType =
    std::tuple<CanPackErrorCode, std::optional<PHV::Allocation::ConditionalConstraints>>;
...
struct CanPackErrorV2 {
    ...
    CanPackErrorV2(CanPackErrorCode code) : code(code) {}  // NOLINT
    CanPackErrorV2(CanPackErrorCode code, bool clear_align)
        : code(code), remove_align_req(clear_align) {}
...
class ActionPhvConstraints : public Inspector {
 private:
    ...
    typedef enum action_data_uses_t {
        SOME_AD_CONSTANT = 0,
        ALL_AD_CONSTANT = 1,
    ...
    struct OperandInfo {
        ...
        bool operator==(OperandInfo other) const {
            return phv_used == other.phv_used && ad == other.ad && constant == other.constant &&
    ...
    friend std::ostream &operator<<(std::ostream &out,
                                    const ActionPhvConstraints::OperandInfo &info);
    ...
    class ConstraintTracker {
        ...
        ordered_map<const PHV::Field *,
                    ordered_map<le_bitrange, ordered_set<const IR::MAU::Action *>>>
        ...
        ordered_map<const PHV::Field *,
                    ordered_map<const IR::MAU::Action *,
                                ordered_map<le_bitrange, std::vector<OperandInfo>>>>
            write_to_reads_per_action;

        ordered_map<const PHV::Field *,
                    ordered_map<const IR::MAU::Action *,
                                ordered_map<le_bitrange, ordered_set<PHV::FieldSlice>>>>
            read_to_writes_per_action;
        ...
        ordered_map<const PHV::Field *,
                    ordered_map<le_bitrange, ordered_map<const IR::MAU::Action *,
                                                         ordered_set<std::pair<int, int>>>>>
            statefulWrites;
     ...
     public:
        ConstraintTracker(const PhvInfo &phv, ActionPhvConstraints &self) : phv(phv), self(self) {}

        /// Clear internal state.
        void clear();

        /// Add constraints induced by @p act.
        void add_action(const IR::MAU::Action *act,
        ...
        std::optional<bool> hasPHVSource(const PHV::Field *field, const IR::MAU::Action *act) const;
        ...
        /// @returns std::nullopt if the field is not written in this action.
        std::optional<bool> hasActionDataOrConstantSource(const PHV::Field *field,
         ...
         */
        std::vector<OperandInfo> sources(const PHV::FieldSlice &dst,
                                         const IR::MAU::Action *act) const;

        /// Convenience method that translates @p dst to a FieldSlice and passes
        /// it to `sources` above.
        std::vector<OperandInfo> sources(const PHV::AllocSlice &dst,
                                         const IR::MAU::Action *act) const {
         ...
         * read in @p act.
         */
        ordered_set<PHV::FieldSlice> destinations(PHV::FieldSlice src,
                                                  const IR::MAU::Action *act) const;

        /// Convenience method that translates @p src to a FieldSlice and passes
        /// it to `destinations` above.
        ordered_set<PHV::FieldSlice> destinations(PHV::AllocSlice src,
                                                  const IR::MAU::Action *act) const {
        ...
        std::optional<OperandInfo> is_written(PHV::FieldSlice slice,
                                              const IR::MAU::Action *act) const;
        ...
        /// TODO: The OperandInfo returned in this function is NOT COMPLETE!
        std::optional<OperandInfo> is_written(PHV::AllocSlice slice,
                                              const IR::MAU::Action *act) const {
        ...
        std::optional<int> can_be_both_sources(const std::vector<PHV::AllocSlice> &slices,
                                               ordered_set<PHV::FieldSlice> &packing_slices,
     ...
     */
    std::optional<PHV::AllocSlice> getSourcePHVSlice(const PHV::Allocation &alloc,
    ...
    ActionDataUses all_or_none_constant_sources(
        const PHV::Allocation::MutuallyLiveSlices &slices, const IR::MAU::Action *act,
        const PHV::Allocation::LiveRangeShrinkingMap &initActions) const;
     ...
     */
    bool valid_container_operation_type(const IR::MAU::Action *act,
     ...
     */
    unsigned container_operation_type(const IR::MAU::Action *,
     ...
     */
    bool pack_slices_together(const PHV::Allocation &alloc,
                              const PHV::Allocation::MutuallyLiveSlices &container_state,
                              PackingConstraints *packing_constraints,
                              const IR::MAU::Action *action, bool pack_unallocated_only,
     ...
     * allocated sources.
     */
    bool is_aligned(const PHV::Allocation::MutuallyLiveSlices &container_state,
                    const IR::MAU::Action *act, const PHV::Allocation &alloc) const;
     ...
     */
    bool masks_valid(const PHV::Allocation::MutuallyLiveSlices &container_state,
                     const IR::MAU::Action *action, bool actionDataOnly,
                     const PHV::Allocation::LiveRangeShrinkingMap &initActions) const;
    ...
    bool check_and_generate_constraints_for_move_with_unallocated_sources(
        ...
        ActionContainerProperty *action_props, PackingConstraints *copacking_constraints) const;
 ...
 public:
    ...
    explicit ActionPhvConstraints(const PhvInfo &p, const PhvUse &u, const PackConflicts &c,
                                  const MapTablesToActions &m, const DependencyGraph &d)
        : phv(p),
          uses(u),
          conflicts(c),
          tableActionsMap(m),
    ...
    bool hasSpecialityReads(const PHV::Field *f) const { return special_no_pack.count(f) > 0; }

    /// @returns all the fields that are written using meter color fields.
    ordered_set<const PHV::Field *> &meter_color_dests() { return meter_color_destinations; }
    ...
    ordered_set<const IR::MAU::Action *> actions_writing_fields(
    ...
    std::optional<const PHV::Field *> field_destination(const PHV::Field *f,
                                                        const IR::MAU::Action *action) const;
    ...
    bool pack_conflicts_present(const PHV::Allocation::MutuallyLiveSlices &container_state,
     ...
     */
    CanPackReturnType can_pack(const PHV::Allocation &alloc,
                               const std::vector<PHV::AllocSlice> &slices,
    ...
    CanPackErrorV2 can_pack_v2(const PHV::Allocation &alloc,
                               const std::vector<PHV::AllocSlice> &slices) const;

    /** @returns true if this packing would create container conflicts because of metadata
     * initialization issues.
     */
    bool creates_container_conflicts(const PHV::Allocation::MutuallyLiveSlices &container_state,
                                     const PHV::Allocation::LiveRangeShrinkingMap &initActions,
    ...
    bool is_bitmasked_set(const std::vector<PHV::AllocSlice> &container_state,
    ...
    const ordered_set<PHV::FieldSlice> actionWritesSlices(const IR::MAU::Action *act) const {
        ...
        for (auto &info : writtenSlices)
            if (info.phv_used != std::nullopt) rs.insert(*(info.phv_used));
        return rs;
    }
     ...
     * at action @p action.
     */
    bool cannot_initialize(const PHV::Container &c, const IR::MAU::Action *action,
     ...
     */
    bool diagnoseSuperCluster(const ordered_set<const PHV::SuperCluster::SliceList *> &sc,
                              const ordered_map<PHV::FieldSlice, unsigned> &fieldAlignments,
     ...
     */
    ActionSources getActionSources(const IR::MAU::Action *act, const PHV::Container &c,
