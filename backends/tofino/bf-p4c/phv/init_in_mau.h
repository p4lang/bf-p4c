...
class AddInitTable : public Transform {
 private:
            ...
            if (const auto *fld1 = phv.field(mau_init.field_name)) {
                ...
                for (auto &f_range : mau_init.conflicts) {
                    ...
                    if (fld2 && same_container(PHV::FieldSlice(fld1, mau_init.range),
                        ...
                        auto slice = new IR::Slice(fieldToExpr.getExpr(fld1), mau_init.range.hi,
                                                   mau_init.range.lo);
    ...
    IR::MAU::TableSeq *preorder(IR::MAU::TableSeq *tSeq) override {
        ...
        if (((seq_gress == INGRESS) && !ingress_inits) ||
            ...
            LOG3("AddInitTable: Table Sequence of gress "
                 << seq_gress << " without fields of that gress requiring init - nothing to do");
            return tSeq;
        }
 ...
 public:
    explicit AddInitTable(PhvInfo &p, MapFieldToExpr &f2exp,
                          std::set<PHV::FieldRange> &to_be_inited, bool container_info)
        : phv(p),
          fieldToExpr(f2exp),
          mauInitFields(to_be_inited),
