...
class PragmaContainerSize : public Inspector {
 ...
 public:
    ...
    const ordered_map<const PHV::Field *, std::vector<PHV::Size>> &field_to_sizes() const {
        return pa_container_sizes_i;
    }
    ...
    const ordered_map<const PHV::Field *, std::vector<int>> &field_to_layout() const {
        return field_layouts_i;
    }
