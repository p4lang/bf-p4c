...
class MapFieldToExpr : public Inspector {
 ...
 public:
    ...
    const IR::Expression *getExpr(const PHV::Field *field) const {
        BUG_CHECK(fieldExpressions.count(field->id), "Missing IR::Expression mapping of %1%",
                  field->name);
    ...
    const IR::MAU::Instruction *generateInitInstruction(const PHV::AllocSlice &dest,
...
class ComputeFieldsRequiringInit : public Visitor {
 ...
 public:
    ...
    const ordered_map<const IR::MAU::Action *, std::vector<PHV::AllocSlice>> &getAllActionInits()
        const {
        return actionInits;
    }

    // @returns the set of fields that must be initialized at action @act.
    const std::vector<PHV::AllocSlice> getInitsForAction(const IR::MAU::Action *act) const {
        std::vector<PHV::AllocSlice> empty;
        if (!actionInits.count(act)) return empty;
...
class ComputeDarkInitialization : public Inspector {
 ...
 public:
    ...
    explicit ComputeDarkInitialization(PhvInfo &p, const MapTablesToActions &m,
...
class ComputeDependencies : public Inspector {
 private:
    ...
    void addDepsForSetsOfAllocSlices(const std::vector<PHV::AllocSlice> &alloc_slices,
                                     const StageFieldUse &fieldWrites,
                                     const StageFieldUse &fieldReads, bool checkBitsOverlap = true);
    ...
    // range.
    void accountUses(int min_stage, int max_stage, const PHV::AllocSlice &alloc,
                     const StageFieldUse &uses, ordered_set<const IR::MAU::Table *> &tables) const;
    void summarizeDarkInits(StageFieldUse &fieldWrites, StageFieldUse &fieldReads);

 public:
    ComputeDependencies(PhvInfo &p, const DependencyGraph &g, const MapTablesToActions &a,
                        const ComputeFieldsRequiringInit &i, const FieldDefUse &d,
                        const MetadataLiveRange &r, const TablesMutuallyExclusive &m)
        : phv(p), dg(g), actionsMap(a), fieldsForInit(i), defuse(d), liverange(r), tableMutex(m) {}

    bool isInitTable(const IR::MAU::Table *tbl) const { return initTableNames.count(tbl->name); }
};
...
class AddSliceInitialization : public PassManager {
 ...
 public:
    explicit AddSliceInitialization(PhvInfo &p, FieldDefUse &d, const DependencyGraph &g,
