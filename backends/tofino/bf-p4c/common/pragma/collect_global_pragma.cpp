...
bool CollectGlobalPragma::preorder(const IR::StructField *s) {
    ...
    for (auto ann : s->annotations->annotations) {
        ...
        if (!notParsedDeparsedAnnotation) continue;
        ...
        if (!PHV::Pragmas::checkNumberArgs(ann, required_arguments, min_required_arguments, true,
                                           ann->name.name, "`gress'"_cs)) {
            continue;
        }
        ...
        IR::Annotation *newAnnotation =
            new IR::Annotation(IR::ID(ann->name.name), {pipe_arg, gress_arg, name});
...
bool CollectGlobalPragma::preorder(const IR::Annotation *annotation) {
    ...
    bool is_global_pragma = (std::find(g_global_pragma_names->begin(), g_global_pragma_names->end(),
                                       pragma_name) != g_global_pragma_names->end());
    if (is_global_pragma) {
