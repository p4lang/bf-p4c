...
class HasTableApply : public Inspector {
 ...
 public:
    ...
    bool preorder(const IR::Member *member) override {
        if (member->member == "hit" || member->member == "miss") return true;
        return false;
    }
};
...
class SplitAlpm : public Transform {
    ...
    const IR::MAU::Table *create_pre_classifier_tcam(IR::MAU::Table *tbl, IR::TempVar *tv,
                                                     IR::TempVar *tk, unsigned partition_index_bits,
    ...
    bool pragma_exclude_msbs(const IR::P4Table *tbl, ordered_map<cstring, int> &fields_to_exclude);
    ...
    const IR::IndexedVector<IR::Declaration> *create_temp_var(const IR::P4Table *, unsigned,
                                                              unsigned, unsigned, unsigned,
                                                              const IR::Expression *);
    const IR::IndexedVector<IR::Declaration> *create_preclassifer_actions(const IR::P4Table *,
                                                                          unsigned, unsigned,
                                                                          unsigned, unsigned,
                                                                          unsigned,
                                                                          const IR::Expression *);
    const IR::P4Table *create_preclassifier_table(const IR::P4Table *, unsigned, unsigned, unsigned,
                                                  unsigned);
    const IR::P4Table *create_atcam_table(const IR::P4Table *, unsigned, unsigned, unsigned, int,
                                          int, ordered_map<cstring, int> &fields_to_exclude,
                                          const IR::Expression *);
