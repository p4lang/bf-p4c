...
const IR::Node *EliminateWidthCasts::preorder(IR::Cast *expression) {
    ...
    if (srcType->is<IR::Type_Bits>() && dstType->is<IR::Type_Bits>()) {
        ...
        if (srcType->width_bits() > dstType->width_bits()) {
            ...
            return new IR::Slice(expression->srcInfo, expression->destType, expression->expr,
            ...
            if ((expression->expr->is<IR::Shl>()) && (dstType->width_bits() > MAX_CONTAINER_SIZE) &&
                ...
                if (value->asInt() >= (2 * MAX_CONTAINER_SIZE)) {
                    error(
                        "%s: shifting of more than 63 bits with casting is too complex to "
                        "handle, consider simplifying the expression.",
                        expression->srcInfo);
                    return expression;
                }
                if (value->asInt() >= MAX_CONTAINER_SIZE) {
                    ...
                    return new IR::Concat(
                        new IR::Shl(expression->srcInfo, shift->left, shift_adj),
                        new IR::Constant(IR::Type_Bits::get(MAX_CONTAINER_SIZE), 0));
                } else {
...
const IR::Node *RewriteCastToReinterpretCast::preorder(IR::Cast *expression) {
    ...
    if (srcType->is<IR::Type_Boolean>() || dstType->is<IR::Type_Boolean>()) {
        auto retval = new IR::BFN::ReinterpretCast(expression->srcInfo, expression->destType,
                                                   expression->expr);
        typeMap->setType(retval, expression->destType);
        return retval;
    } else if (srcType->is<IR::Type_Bits>() && dstType->is<IR::Type_Bits>()) {
        if (srcType->width_bits() != dstType->width_bits()) return expression;
        if (srcType->to<IR::Type_Bits>()->isSigned != dstType->to<IR::Type_Bits>()->isSigned) {
            auto retval = new IR::BFN::ReinterpretCast(expression->srcInfo, expression->destType,
                                                       expression->expr);
            typeMap->setType(retval, expression->destType);
...
const IR::Node *SimplifyRedundantCasts::preorder(IR::Cast *expression) {
    ...
    if (!srcType->is<IR::Type_Bits>() || !dstType->is<IR::Type_Bits>()) return expression;
    ...
    if (expression->expr->is<IR::Cast>()) {
        ...
        if (srcType->to<IR::Type_Bits>()->isSigned == dstType->to<IR::Type_Bits>()->isSigned)
            return expression;
        if (dstType->width_bits() == innerCast->expr->type->width_bits()) return innerCast->expr;
    } else {
        if (srcType->to<IR::Type_Bits>()->isSigned != dstType->to<IR::Type_Bits>()->isSigned)
            return expression;
        if (dstType->width_bits() == srcType->width_bits()) return expression->expr;
    }
...
const IR::Node *SimplifyNestedCasts::preorder(IR::Cast *expression) {
    if (!expression->expr->is<IR::Cast>()) return expression;

    const IR::Type *dstType = expression->destType;
    auto secondCast = expression->expr->to<IR::Cast>();

    // stop if not a nested cast
    if (!secondCast) return expression;

    // stop if more than two levels of nested casts.
    if (secondCast->expr->is<IR::Cast>()) {
        error(
            "Expression %1% is too complex to handle, "
            "consider simplifying the nested casts.",
            expression);
        return expression;
    }
...
const IR::Node *SimplifyOperationBinary::preorder(IR::Cast *expression) {
    if (!expression->expr->is<IR::Operation_Binary>()) return expression;

    // cannot cast lhs and rhs to boolean on
    // a bitwise binary operation.
    if (expression->destType->is<IR::Type_Boolean>()) return expression;
    ...
    if (expression->type->width_bits() > binop->type->width_bits()) {
        ...
        // narrowing cast
        if (binop->is<IR::AddSat>() || binop->is<IR::SubSat>() || binop->is<IR::Div>() ||
            binop->is<IR::Mod>()) {
    ...
    if (auto op = expression->expr->to<IR::Add>()) return new IR::Add(op->srcInfo, left, right);
    if (auto op = expression->expr->to<IR::Sub>()) return new IR::Sub(op->srcInfo, left, right);
    ...
    if (auto op = expression->expr->to<IR::BAnd>()) return new IR::BAnd(op->srcInfo, left, right);
    if (auto op = expression->expr->to<IR::BOr>()) return new IR::BOr(op->srcInfo, left, right);
    if (auto op = expression->expr->to<IR::BXor>()) return new IR::BXor(op->srcInfo, left, right);
    return expression;
}
...
const IR::Node *create_block(Statements &stmts, IR::Statement *stmt = nullptr) {
    ...
    for (const auto &decl : stmts.decl) block->components.push_back(decl);
    for (const auto &wr_tmp : stmts.wr_tmp) block->components.push_back(wr_tmp);
    for (const auto &rd_tmp : stmts.rd_tmp) block->components.push_back(rd_tmp);
    if (stmt) block->components.push_back(stmt);
    return block;
}
...
bool nonsliceable(const IR::Operation_Binary *binOp) {
    ...
    bool sliceable = binOp->is<IR::Concat>() || binOp->is<IR::Constant>() ||
                     binOp->is<IR::BAnd>() || binOp->is<IR::BOr>() || binOp->is<IR::BXor>() ||
...
bool nested_nonsliceable(const IR::Expression *expr) {
    if (auto sliceOp = expr->to<IR::Slice>()) return nested_nonsliceable(sliceOp->e0);
    if (auto binOp = expr->to<IR::Operation_Binary>()) {
        if (nonsliceable(binOp)) return true;
...
const IR::Concat *find_nested_concat(const IR::Expression *expr) {
    ...
    if (auto binOp = expr->to<IR::Operation_Binary>()) {
        if (nonsliceable(binOp)) return nullptr;  // nonsliceable clean.
        if (auto concatOp = expr->to<IR::Concat>()) return concatOp;
        if (auto concatOp = find_nested_concat(binOp->left)) return concatOp;
        if (auto concatOp = find_nested_concat(binOp->right)) return concatOp;
    }
    return nullptr;
}

const IR::Expression *remove_concat_from_nonsliceable(const IR::Expression *expr, Statements &stmts,
...
const IR::Expression *remove_concat_from_nonsliceable(const IR::Operation_Binary *binOp,
                                                      Statements &stmts, bool in_nonsliceable) {
    if (in_nonsliceable) {
    ...
    if (newleft || newright) {
        ...
        if (newleft) clone->left = newleft;
        if (newright) clone->right = newright;
        return clone;
    }
    return nullptr;
}
...
const IR::Expression *remove_concat_from_nonsliceable(const IR::Operation_Ternary *ternOp,
    ...
    if (newe0 || newe1 || newe2) {
        ...
        if (newe0) clone->e0 = newe0;
        if (newe1) clone->e1 = newe1;
        if (newe2) clone->e2 = newe2;
        return clone;
    }
    return nullptr;
}
...
const IR::Expression *remove_nonsliceable_from_sliceable(const IR::Expression *expr,
    ...
    if (auto binOp = expr->to<IR::Operation_Binary>()) {
        if (nonsliceable(binOp)) return assign_expr_to_temp(binOp, stmts);
...
const IR::Operation_Binary *replaceConcatComp(const IR::Operation_Binary *expr, Search search,
                                              Comp comp) {
    if (auto concatOp = find_nested_concat(search == Search::Left ? expr->left : expr->right)) {
        ...
        if (comp == Comp::Equ) {
            return new IR::LAnd(expr->srcInfo, new IR::Equ(expr->srcInfo, lSlice.hi, rSlice.hi),
                                new IR::Equ(expr->srcInfo, lSlice.lo, rSlice.lo));
        } else {
            return new IR::LOr(expr->srcInfo, new IR::Neq(expr->srcInfo, lSlice.hi, rSlice.hi),
...
const IR::Node *RewriteConcatToSlices::preorder(IR::Equ *equ) {
    // The returned node will be re-visited.
    if (auto newexpr = replaceConcatComp(equ, Search::Left, Comp::Equ)) return newexpr;
    if (auto newexpr = replaceConcatComp(equ, Search::Right, Comp::Equ)) return newexpr;
    return equ;
}

const IR::Node *RewriteConcatToSlices::preorder(IR::Neq *neq) {
    // The returned node will be re-visited.
    if (auto newexpr = replaceConcatComp(neq, Search::Left, Comp::Neq)) return newexpr;
    if (auto newexpr = replaceConcatComp(neq, Search::Right, Comp::Neq)) return newexpr;
    return neq;
}
