...
class PingPongGeneration::DeclarationChanger : public Transform {
 public:
    ...
    IR::Node *postorder(IR::StringLiteral *sl) {
        ...
        if (!annot || annot->name != "name") return sl;
        ...
        size_t pos = 0;
        while ((pos = local_name.find(".")) != std::string::npos) local_name.erase(0, pos + 1);
        if (!names_to_change.count(local_name)) return sl;
        // Update and return new literal
        sl->value = sl->value + PingPongGeneration::ID_PING_PONG_SUFFIX;
        return sl;
    }

    explicit DeclarationChanger(PingPongGeneration &self, std::set<cstring> &names_to_change)
...
class PingPongGeneration::RegActionChanger : public PingPongGeneration::DeclarationChanger {
 ...
 public:
    explicit RegActionChanger(PingPongGeneration &self, std::set<cstring> &names_to_change)
...
class PingPongGeneration::P4ActionChanger : public PingPongGeneration::DeclarationChanger {
    ...
    IR::Node *postorder(IR::Path *path) {
        ...
        if (!member || (member->member != "execute" && member->member != "count")) return path;
        // Clone and update new path
        return self.appendPingPongSuffix(path, names_to_change);
    }
...
class PingPongGeneration::P4TableChanger : public PingPongGeneration::DeclarationChanger {
    ...
    IR::Node *postorder(IR::Path *path) {
        ...
        if (!prop || (prop->name != "default_action" && prop->name != "counters" &&
...
class PingPongGeneration::ApplyMCSChanger : public Transform {
    ...
    IR::Node *preorder(IR::P4Table *tab) {
        ...
        if (tab->name != orig_table_name) return tab;
    ...
    IR::Node *postorder(IR::Type_Struct *ts) {
        // Check if this is one of the identifiers that are being worked on
        if (ts->name != orig_table_name) return ts;
        // Create a duplicate and update it
        return new IR::Type_Struct(IR::ID(ts->name + ID_PING_PONG_SUFFIX), ts->annotations,
                                   ts->fields);
    }
    /**
    ...
    IR::Node *postorder(IR::Path *path) {
        std::set<cstring> names_to_change;
        names_to_change.insert(orig_action_names.begin(), orig_action_names.end());
 ...
 public:
    explicit ApplyMCSChanger(PingPongGeneration &self, const cstring &orig_table_name,
                             const IR::P4Table *dupl_table, std::set<cstring> &orig_action_names)
        : self(self),
          orig_table_name(orig_table_name),
          dupl_table(dupl_table),
          orig_action_names(orig_action_names) {}
};
...
class PingPongGeneration::PingPongFieldFinder : public Inspector {
    bool preorder(const IR::Member *member) override {
        ...
        if (member->member != PING_PONG_FIELD_NAME) return false;
        // Get PathExpression
        auto path_exp = member->expr->to<IR::PathExpression>();
        if (!path_exp) return false;
        // Check if this is part of metadata
        if (path_exp->type->is<IR::Type_Header>() || path_exp->type->is<IR::Type_Struct>()) {
            found = true;
        }
        return false;
    }
 ...
 public:
    bool found;
    PingPongFieldFinder() : found(false) {}
};
...
inline IR::Path *PingPongGeneration::appendPingPongSuffix(IR::Path *path,
                                                          std::set<cstring> &names_to_change) {
    // Check if this is one of the identifiers that are being worked on
    if (!names_to_change.count(path->name)) return path;
    // Create a duplicate and update it
...
inline void PingPongGeneration::duplicateNodeDeclaration(const IR::Declaration *node,
    ...
    if (node->is<IR::Declaration_Instance>()) {
        ...
        auto new_node = new IR::Declaration_Instance(new_name, DI_node->annotations, DI_node->type,
                                                     DI_node->arguments, DI_node->initializer);
        ...
        auto new_node =
            new IR::P4Action(new_name, P4A_node->annotations, P4A_node->parameters, P4A_node->body);
        ...
        auto new_node = new IR::P4Table(new_name, P4T_node->annotations, P4T_node->properties);
...
std::set<const IR::Declaration_Instance *> PingPongGeneration::allRegisters() const {
    ...
    for (auto &kv : controlToRegister)
        for (auto reg : kv.second) all.insert(reg);
...
bool PingPongGeneration::shouldTransform() {
    ...
    for (auto &p1 : pipelines) {
        if (!p1.ghost || validInPipe[&p1].empty()) continue;
...
bool PingPongGeneration::isPingPongValid(const IR::Declaration_Instance *reg) {
    for (auto &pipe : pipelines)
        if (isPingPongValid(pipe, reg)) return true;
    return false;
}
...
bool PingPongGeneration::isPingPongValid(const IR::BFN::TnaControl *gress,
    ...
    for (auto &pipe : pipelines.get(gress))
        if (isPingPongValid(pipe, reg, calculatingCovered)) return true;
    return false;
}
...
bool PingPongGeneration::isPingPongValid(const CollectPipelines::Pipe &pipe,
    ...
    if (!pipe.ghost || controlToRegister[pipe.egress.control].count(reg)) return false;
        ...
        if (!calculatingCovered && (coveredIn || coveredGhost)) {
            if (!(coveredIn && coveredGhost)) {
                warning(ErrorType::WARN_MISMATCH,
                        "Register %1% is covered by ping-pong 'if' in "
...
bool PingPongGeneration::GetAllRegisters::preorder(const IR::MethodCallExpression *mce) {
    ...
    if (!em || !em->originalExternType->name.name.startsWith("RegisterAction")) return false;
    ...
    if (other_reg_it != self.p4ActionToRegister.end()) {
        ...
        LOG4("[PPG::GAR] adding register with declaration: "
             << r_decl->name << "; register action declaration: " << ra_decl->name
...
bool PingPongGeneration::AddAllTables::preorder(const IR::ActionListElement *ale) {
    ...
    if (regIt == self.p4ActionToRegister.end() || !regIt->second) return false;
    ...
    if (self.p4ActionToP4Table.count({gress, p4_action}) ||
        ...
        LOG3("[PPG::AAT] Register "
             << reg->name << " does not have 1-1 mapping to P4 tables => not a ping-pong");
...
bool PingPongGeneration::CheckPingPongTables::preorder(const IR::Type_Header *th) {
    ...
    if (th->name != GMD_STRUCTURE_NAME) return false;
    // Store them
...
bool PingPongGeneration::CheckPingPongTables::preorder(const IR::PathExpression *path_exp) {
    ...
    if (!path_exp->type->is<IR::Type_Table>()) return false;

    // Check if we are inside apply
    auto member = findContext<IR::Member>();
    if (!member || member->member != "apply") return false;
    ...
    if (!self.p4TableToRegister.count({gress, p4_table})) return false;
    auto reg = self.p4TableToRegister[{gress, p4_table}];

    // Check if this is still valid ping-pong candidate
    if (!self.isPingPongValid(gress, reg, true)) return false;
    ...
    if (!if_stmt || !if_stmt->condition) return false;
    ...
    if (findPingPongField.found) {
        LOG3("[PPG::CPPT] Found ping pong mechanism for: "
             << reg->name << " in gress " << gress->getName() << " => invalidating.");
    ...
    for (auto const &reg : self.allRegisters()) {
        ...
        if (self.isPingPongValid(reg)) {
            ...
            auto new_reg = new IR::Declaration_Instance(
                IR::ID(reg->name + PingPongGeneration::ID_PING_PONG_SUFFIX), reg->annotations,
                reg->type, reg->arguments, reg->initializer);
...
IR::Node *PingPongGeneration::GeneratePingPongMechanismDeclarations::preorder(
    ...
    if (gress->thread == EGRESS) return gress;
            ...
            if (auto props = p4_table->properties) {
                for (auto prop : props->properties) {
                    if (prop->name == "counters" || prop->name == "registers" ||
                        ...
                        LOG4("[PPG::GPPMD] found direct resource "
                             << res_decl->name << " for p4 table: " << p4_table->name);
                    }
                }
            }
            // Create a vector of names that are worked on
            std::set<cstring> names_to_change = {reg->name, p4_table->name, reg->name.originalName,
...
IR::Node *PingPongGeneration::GeneratePingPongMechanism::postorder(IR::MethodCallStatement *mcs) {
    ...
    if (!gress) return mcs;
    // We dont care about egress
    if (gress->thread == EGRESS) return mcs;
    ...
    if (!am || !am->isTableApply()) return mcs;
    auto p4_table = mi->object->to<IR::P4Table>();
    if (!p4_table) return mcs;
    // Get the approriate register
    if (!self.p4TableToRegister.count({gress, p4_table})) return mcs;
    auto reg = self.p4TableToRegister[{gress, p4_table}];
    // Check if this is valid ping-pong candidate
    if (!self.isPingPongValid(gress, reg)) return mcs;
    // Get the duplicate of this table
    if (!self.p4TableToDuplicateTable.count(p4_table)) return mcs;
    auto duplicate_table = self.p4TableToDuplicateTable[p4_table];
    if (!duplicate_table) return mcs;
    // Get the action for this table
    if (!self.registerToP4Action.count({gress, reg})) return mcs;
    std::set<cstring> p4_actions_names;
    auto p4_actions = self.registerToP4Action[{gress, reg}];
    for (auto p4_action : p4_actions) p4_actions_names.insert(p4_action->name);

    // Update the mcs
    auto new_mcs =
        mcs->apply(ApplyMCSChanger(self, p4_table->name, duplicate_table, p4_actions_names))
            ->to<IR::MethodCallStatement>();

    // Check existence of ghost metadata
    if (!self.ghost_meta_struct) return mcs;
    ...
    auto left = new IR::Member(
        IR::Type::Bits::get(1),
        new IR::PathExpression(self.ghost_meta_struct,
                               new IR::Path(IR::ID(self.ghost_meta_name[gress]), false)),
