...
class DoCopyHeaders : public Transform {
 ...
 public:
    ...
    const IR::Node *preorder(IR::ParserState* scope) override {
        prev_assignments.clear(); return scope; }
...
bool isMethod_xValid(IR::ID member) {
    return member == IR::Type_Header::setValid ||
...
void bugcheck_xValid(const IR::MethodCallExpression* call, const IR::Expression* hdr) {
    ...
    BUG_CHECK(hdr->type->is<IR::Type_Header>() ||
              hdr->type->is<IR::Type_HeaderUnion>() ||
...
const IR::Node* DoCopyHeaders::replaceValidMethod(const IR::Member* mem, int value,
    ...
    for (auto m : boost::adaptors::reverse(prev_assignments)) {
        if (mem->expr->equiv(*m->expr)) {
            ...
            if (mem->member == m->member)
                return nullptr;  // Resetting with same value - redundant.
...
const IR::Node* DoCopyHeaders::postorder(IR::MethodCallStatement *mc) {
    ...
    if (!mem || !isMethod_xValid(mem->member))
        return mc;

    bugcheck_xValid(mc->methodCall, mem->expr);
    if (mem->member == IR::Type_Header::setValid)
        return replaceValidMethod(mem, 1, mc->srcInfo);
    if (mem->member == IR::Type_Header::setInvalid)
        return replaceValidMethod(mem, 0, mc->srcInfo);
...
const IR::Node* DoCopyHeaders::postorder(IR::MethodCallExpression* mc) {
    ...
    if (!mem || mem->member != IR::Type_Header::isValid)
        return mc;
    ...
    if (findContext<IR::IfStatement>() ||
        findContext<IR::AssignmentStatement>()) {
...
const IR::Node* DoCopyHeaders::preorder(IR::P4Table* tbl) {
    ...
    if (!keys || keys->keyElements.empty() ||
        !entryList || entryList->entries.empty())
        return tbl;
...
BFN::CopyHeaders::CopyHeaders(P4::ReferenceMap* refMap, P4::TypeMap* typeMap,
    ...
    CHECK_NULL(refMap); CHECK_NULL(typeMap); CHECK_NULL(typeChecking);
    setName("CopyHeaders");
