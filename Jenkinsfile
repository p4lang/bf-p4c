...
node ('compiler-travis') {
    ...
    parallel (
        ...
        travis_tofino2_part1: {
            ansiColor('xterm') {
                timestamps {
                    sh "echo 'Running tofino2 tests'"
                    sh "docker run --privileged -w /bfn/bf-p4c-compilers/build -e CTEST_PARALLEL_LEVEL=4 -e CTEST_OUTPUT_ON_FAILURE='true' barefootnetworks/bf-p4c-compilers:${image_tag} ctest -R '^tofino2|cpplint|gtest' -L 'JENKINS_PART1|cpplint|gtest' -E 'ignore_test_|smoketest|p4_16_programs_tna_exact_match|p4_16_programs_tna_meter_lpf_wred|/p4_16/customer/extreme/p4c-1([^3]|3[^1]).*|/dkm/|entry_read_from_hw|/p4_14/stf/decaf_9.*|ptf/digest.p4|npb-master-ptf'"
                }
            }
        },
        travis_tofino2_part2: {
            ansiColor('xterm') {
                timestamps {
                    sh "echo 'Running tofino2 tests'"
                    sh "docker run --privileged -w /bfn/bf-p4c-compilers/build/p4c -e CTEST_PARALLEL_LEVEL=4 -e CTEST_OUTPUT_ON_FAILURE='true' barefootnetworks/bf-p4c-compilers:${image_tag} ctest -R '^tofino2' -L 'JENKINS_PART2' -E 'ignore_test_|smoketest|p4_16_programs_tna_exact_match|p4_16_programs_tna_meter_lpf_wred|/p4_16/customer/extreme/p4c-1([^3]|3[^1]).*|/dkm/|entry_read_from_hw|/p4_14/stf/decaf_9.*|ptf/digest.p4|3174'"
                }
            }
        },
        travis_tofino3: {
