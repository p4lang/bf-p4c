...
class TableCommon(NamedObject):
    ...
    depends_on = jsl.ArrayField(
        required=True, items=jsl.IntField(),
...
class TableIndirectMatch(TableCommon):
    ...
    table_type = jsl.StringField(
        required=True,
        enum=["MatchAction_Indirect", "MatchAction_Indirect_Selector"])
    has_const_default_action = jsl.BooleanField(required=True)
...
class TableGeneric(TableCommon):
    ...
    table_type = jsl.StringField(
        required=True,
        enum=["Selector", "SelectorGetMember",
              "Counter", "Meter", "Register", "RegisterParam", "Lpf", "Wred",
              "PortMetadata",
              "ParserValueSet",
              "SnapshotTrigger", "SnapshotData", "SnapshotCfg", "SnapshotLiveness",
              "DynHashConfigure", "DynHashCompute",
              "TblDbgCnt",
