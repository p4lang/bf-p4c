...
class TableGraphsSchema(JsonGraphFormatSchema):
    ...
    graphs = jsl.ArrayField(required=True, items=jsl.DocumentField("TableGraph", as_ref=True, required=True), description="An array of table graph data.")

class TableGraph(Graph):
    title = "TableGraph"
    description = "Table graph data."

    nodes = jsl.ArrayField(jsl.DocumentField("TableGraphNode", as_ref=True), required=True, description="An array containing all graph nodes")
    edges = jsl.ArrayField(jsl.DocumentField("TableGraphEdge", as_ref=True), required=True, description="An array containing all graph connections")
    metadata = jsl.DictField(properties={
      "gress": jsl.StringField(required=True, enum=["ingress", "egress", "ghost"], description="Graph for ingress or egress pipeline."),
      "placement_complete": jsl.BooleanField(required=True, description="A flag indicating whether tables have been placed."),
      "compile_iteration": jsl.StringField(required=False, description="The name of this compilation pass."),
      "description": jsl.StringField(required=True, description="A description for this graph."),
    }, required=True, description="Table graph metadata.", additional_properties=False)

# Graph Nodes

class TableGraphNode(GraphNode):
    title = "TableGraphNode"
    metadata = jsl.DocumentField("NodeMetadata", as_ref=True, required=True, description="Table graph node metadata.")
...
class NodeMetadata(jsl.Document):
    ...
    tables = jsl.ArrayField(required=True, items=jsl.OneOfField([
    ...
    placement = jsl.DictField(required=False, properties={
      "logical_table_id": jsl.NumberField(required=True, description="Unique identifier for this logical table within this stage."),
      "stage_number": jsl.NumberField(required=True, description="Number identifying the stage containing this stage table."),
    }, description="Placement information for this graph node (can be undefined if graph.placement_complete is False).", additional_properties=False)
    min_stage = jsl.IntField(required=True, description="The minimum stage this table can be placed.")
    dep_chain = jsl.IntField(required=True, description="The number of nodes following this node in the program flow.")
...
class AttachedTable(jsl.Document):
    ...
    name = jsl.StringField(required=True, description="Table name (format must match other JSON artifacts like context.json).")
    table_type = jsl.StringField(required=True, enum=[
      "action",
      "meter",
      "selection",
      "stateful",
...
class MatchTable(jsl.Document):
    ...
    name = jsl.StringField(required=True, description="Table name (format must match other JSON artifacts like context.json).")
    table_type = jsl.StringField(required=True, enum=["match"], description="Discriminator tag for this table type.")
    match_type = jsl.StringField(required=True, enum=[
        "exact",
        "ternary",
        "proxy_hash",
        "match_with_no_key",
...
class ConditionTable(jsl.Document):
    ...
    name = jsl.StringField(required=True, description="Table name (format must match other JSON artifacts like context.json).")
    table_type = jsl.StringField(required=True, enum=["condition"], description="Discriminator tag for this table type.")
    condition = jsl.StringField(required=True, description="Condition string of this condition table.")
...
class TableGraphEdge(GraphEdge):
    ...
    ], required=True, description="Table graph edge metadata.")
...
class EdgeMetadataBase(jsl.Document):
    ...
    is_critical = jsl.BooleanField(required=False, description="A flag indicating if edge lies on a critical path.")
    tags = jsl.ArrayField(required=False, items=jsl.StringField(), description="An optional array of tags which can be used for grouping and filtering.")
...
class EdgeMetadataDefaultNextTable(EdgeMetadataControlBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["default_next_table"], description="Type of control edge.")
...
class EdgeMetadataControlExit(EdgeMetadataControlBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["control_exit"], description="Type of control edge.")

class EdgeMetadataCondition(EdgeMetadataControlBase):
    title = "EdgeMetadataCondition"
    description = "Condition next-table pointer."

    sub_type = jsl.StringField(required=True, enum=["condition"], description="Type of control edge.")
...
class EdgeMetadataTableHitMiss(EdgeMetadataControlBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["table_hit", "table_miss"], description="Type of control edge.")

class EdgeMetadataAnti(EdgeMetadataControlBase):
    title = "EdgeMetadataAnti"
    description = "Write after read data dependency."

    sub_type = jsl.StringField(required=True, enum=["anti"], description="Type of control dependency.")
    anti_type = jsl.StringField(
      required=True,
      enum=["table_read", "action_read", "next_table_data", "next_table_control", "table_metadata"],
      description="""
...
""")
    dep_fields = jsl.ArrayField(required=True, items=jsl.DocumentField("FieldSlice", as_ref=True), description="Dependency fields.")
...
class EdgeMetadataExit(EdgeMetadataControlBase):
    ...
    action_name = jsl.StringField(required=True, description="The name of the action which has the exit command.")
...
class EdgeMetadataIxbarRead(EdgeMetadataMatchBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["ixbar_read"], description="Type of match dependency.")
    dep_fields = jsl.ArrayField(required=True, items=jsl.DocumentField("FieldSlice", as_ref=True), description="Dependency fields.")
...
class EdgeMetadataActionRead(EdgeMetadataActionBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["action_read"], description="Type of action dependency.")
...
class EdgeMetadataOutput(EdgeMetadataActionBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["output"], description="Type of action dependency.")
    dep_fields = jsl.ArrayField(required=True, items=jsl.DocumentField("FieldSlice", as_ref=True), description="Dependency fields.")
...
class EdgeMetadataContainerConflict(EdgeMetadataActionBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["cont_conflict"], description="Type of action dependency.")
...
class EdgeMetadataReductionOr(EdgeMetadataBase):
    ...
    sub_type = jsl.StringField(required=True, enum=["reduction_or_read", "reduction_or_output"], description="Type of data dependency.")
    dep_fields = jsl.ArrayField(required=True, items=jsl.DocumentField("FieldSlice", as_ref=True), description="Dependency fields.")
...
class FieldSlice(jsl.Document):
    ...
    gress = jsl.StringField(required=True, enum=["ingress", "egress", "ghost"], description="Gress of this field.")
    field_name = jsl.StringField(required=True, description="Name of this field (should be in the same format as context.json)")
