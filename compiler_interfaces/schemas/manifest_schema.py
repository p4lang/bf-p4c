...
class BFNCompilerArchive(jsl.Document):
    ...
    compilation_succeeded = jsl.BooleanField(required=True,
                                             description="A Boolean indicating if the compilation succeeded.")
    compilation_time = jsl.StringField(required=True,
                                       description="The time (sec) it took to compile the program.")
    run_id = jsl.StringField(required=True, description="Unique ID for this compile run.")
    target_data = jsl.DocumentField("TargetData", as_ref=True, required=True, description="Target dependant data")
...
class TargetData(jsl.Document):
    ...
    conf_file = jsl.StringField(required=False, description="Path to the compiler generated driver conf file")
...
class TofinoArchitecture(jsl.Document):
    ...
    pipes = jsl.ArrayField(required=True, description="list of pipelines in the program",
                           items=jsl.DictField(
                               properties = {
                                   "pipe": jsl.IntField(required=True, description="logical pipe number"),
                                   "ingress": jsl.DocumentField("PipelineConfig",
                                                                description="ingress configuration",
                                                                required=True),
                                   "egress": jsl.DocumentField("PipelineConfig",
...
class PipelineConfig(jsl.Document):
    ...
    nextControl = jsl.ArrayField(required=True, description="list of possible next controls",
                                 items=jsl.DictField(
                                     properties = {
                                         "pipe": jsl.IntField(required=True,
                                                              description="logical next pipe number"),
                                         "pipeName": jsl.StringField(required=True,
                                                                     description="next control name")
                                     }, required=True, description="", additional_properties=False))
...
class InputFiles(jsl.Document):
    ...
    includes = jsl.ArrayField(jsl.StringField(), required=False,
                              description="List of include directories")
    defines = jsl.ArrayField(jsl.StringField(),required=False,
                             description="List of pre-processor defines")
...
class TargetOutputFiles(jsl.Document):
    ...
    resources = jsl.ArrayField(required=False,
    ...
    binary = jsl.DictField(required=False,
            properties = {
    ...
    graphs =  jsl.ArrayField(required=False, description="List of graph files",
            ...
            properties = {
                ...
                "gress": jsl.StringField(required=True, description="Pipeline gress",
    ...
    metrics = jsl.DictField(required=False, description="Metrics file info",
            properties = {
                "path": jsl.StringField(required=True, description="Path to the metrics file")
            })
...
class CompiledProgram(jsl.Document):
    ...
    frontend_ir_log_file = jsl.StringField(required=False, description="Path to frontend IR log file")
    source_files = jsl.DocumentField("InputFiles", as_ref=True, required=True)
    # TARGET DEPENDENT
    pipes = jsl.ArrayField(required=True, description = "An array of output files for each pipe",
                           min_items = 1, items = jsl.DictField(required=True,
            properties = {
                "pipe_id": jsl.IntField(required=True, description="Logical id of the control flow"),
                ...
                "compilation_succeeded" : jsl.BooleanField(required=False,
